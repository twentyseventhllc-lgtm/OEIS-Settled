...
cd "$(dirname "$0")/.."
FAMS="${FAMS:-fam_neighbour fam_avoid fam_marked fam_stress fam_perimeter fam_countval fam_digits fam_linesum fam_monotone fam_unimodal fam_idem fam_avoidrc fam_word1d fam_divrc fam_lexsub fam_commute fam_offsets fam_repeated fam_totalling fam_nopattern fam_indexchange fam_majority fam_edgediff fam_subblock fam_cellcount fam_triple fam_exists fam_mistakes}"
i=0
for f in $FAMS; do
