...
\title{Proofs of the conjectured column and row recurrences for OEIS A235087}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A235087}{A235087} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
\begin{quote}\ttfamily Empirical for column k (k=4..7 recurrence works for k=1..3 also; apparently all rows and columns satisfy the same order 16 recurrence):\\
k=1: a(n) = 2*a(n-1) +6*a(n-2) -10*a(n-3) -5*a(n-4).\\k=2: [order 10].\\k=3: [order 15].\end{quote}
...
\begin{theorem} For column $k = 1$ of A235087, the recurrence $a(n) = 2\,a(n-1) + 6\,a(n-2) - 10\,a(n-3) - 5\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $25$ states, $20$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A235087, the recurrence $a(n) = 0$ holds for every $n \ge 11$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $125$ states, $80$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A235087, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $625$ states, $320$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A235087}{A235087}, revision 6, last modified Jun 20 2022.
