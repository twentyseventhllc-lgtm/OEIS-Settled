...
\title{Proofs of the conjectured column and row recurrences for OEIS A297993}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297993}{A297993} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 3 or 6 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) +2*a(n-3) -2*a(n-4) -4*a(n-5) for n>6\\k=3: [order 11] for n>13\\k=4: [order 24] for n>27\\k=5: [order 99] for n>104\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 10 2018). As of revision 4, last modified Jan 10 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A297993, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297993, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 2\,a(n-3) - 2\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297993, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $58$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 4$ of A297993, the recurrence $a(n) = 0$ holds for every $n > 27$, and fails at $n = 27$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $158$ after trimming, and $S = 38$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 27$; after that point more than $S = 38$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 27$; the proved threshold is $n > 27$.

\begin{theorem} For column $k = 5$ of A297993, the recurrence $a(n) = 0$ holds for every $n > 104$, and fails at $n = 104$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $655$ after trimming, and $S = 124$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 104$; after that point more than $S = 124$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 104$; the proved threshold is $n > 104$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297993}{A297993}, revision 4, last modified Jan 10 2018.
