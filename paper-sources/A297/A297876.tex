...
\title{Proofs of the conjectured column and row recurrences for OEIS A297876}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297876}{A297876} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 2, 3 or 5 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -4*a(n-4)\\k=3: [order 17] for n>18\\k=4: [order 52] for n>53\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 07 2018). As of revision 4, last modified Jan 07 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A297876, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297876, the recurrence $a(n) = a(n-1) + 3\,a(n-2) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297876, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A297876, the recurrence $a(n) = 0$ holds for every $n > 53$, and fails at $n = 53$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $201$ after trimming, and $S = 54$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 53$; after that point more than $S = 54$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297876}{A297876}, revision 4, last modified Jan 07 2018.
