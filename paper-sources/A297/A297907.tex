...
\title{Proofs of the conjectured column and row recurrences for OEIS A297907}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297907}{A297907} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 3 or 4 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +3*a(n-2) +8*a(n-3) -4*a(n-4) -16*a(n-5) for n>6\\k=3: [order 12] for n>13\\k=4: [order 48] for n>51\end{quote}
...
\begin{theorem} For column $k = 1$ of A297907, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297907, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 8\,a(n-3) - 4\,a(n-4) - 16\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297907, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $42$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 4$ of A297907, the recurrence $a(n) = 0$ holds for every $n > 51$, and fails at $n = 51$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $199$ after trimming, and $S = 56$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 51$; after that point more than $S = 56$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297907}{A297907}, revision 4, last modified Jan 08 2018.
