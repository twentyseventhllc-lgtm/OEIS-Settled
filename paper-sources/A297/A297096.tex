...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297096}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297096}{A297096} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with no 1 adjacent to 4 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 2*a(n-2) + 6*a(n-3) - 8*a(n-4) - 4*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(2 + x\textasciicircum{}2 - 4*x\textasciicircum{}3 - 2*x\textasciicircum{}4 - 2*x\textasciicircum{}5) / (1 - 4*x + 2*x\textasciicircum{}2 - 6*x\textasciicircum{}3 + 8*x\textasciicircum{}4 + 4*x\textasciicircum{}6). - Colin Barker, Feb 26 2019\end{quote}
...
\[a(n) = 4\,a(n-1) - 2\,a(n-2) + 6\,a(n-3) - 8\,a(n-4) - 4\,a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x + 2x^{3} - 8x^{4} - 4x^{5} - 4x^{6}}{1 - 4x + 2x^{2} - 6x^{3} + 8x^{4} + 4x^{6}}.\]
...
\begin{theorem} For A297096, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 6\,a(n-3) - 8\,a(n-4) - 4\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297096, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x + 2x^{3} - 8x^{4} - 4x^{5} - 4x^{6}}{1 - 4x + 2x^{2} - 6x^{3} + 8x^{4} + 4x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 16, 58, 216, 808, 3016, 11264, 42080,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297096}{A297096}, revision 8, last modified Feb 26 2019.
