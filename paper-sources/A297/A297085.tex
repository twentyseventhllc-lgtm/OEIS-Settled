...
\title{Proofs of the conjectured column and row recurrences for OEIS A297085}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297085}{A297085} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no 1 adjacent to 2 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2) +a(n-3)\\k=2: a(n) = 2*a(n-1) +4*a(n-2) +4*a(n-3) -14*a(n-4) -14*a(n-5) -4*a(n-6)\\k=3: [order 17]\\k=4: [order 34]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 25 2017). As of revision 4, last modified Dec 25 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A297085, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297085, the recurrence $a(n) = 2\,a(n-1) + 4\,a(n-2) + 4\,a(n-3) - 14\,a(n-4) - 14\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297085, the recurrence $a(n) = 0$ holds for every $n \ge 18$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A297085, the recurrence $a(n) = 0$ holds for every $n \ge 35$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 41$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 41$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297085}{A297085}, revision 4, last modified Dec 25 2017.
