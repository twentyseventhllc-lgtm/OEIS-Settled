...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297953}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297953}{A297953} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with every element equal to 1, 2, 4 or 5 king-move adjacent elements, with upper left element zero\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: (1 + x)*(1 + 2*x\textasciicircum{}2 - 4*x\textasciicircum{}3) / ((1 - x)*(1 - 2*x)*(1 + x + 2*x\textasciicircum{}2)). - Colin Barker, Feb 19 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^{n-1} \;=\; \frac{1 + x + 2x^{2} - 2x^{3} - 4x^{4}}{1 - 2x + x^{2} - 4x^{3} + 4x^{4}}.\]
...
\begin{theorem} For A297953, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) - 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,48$. The last nonvanishing one is at $m = 1$, that is at $n = 5$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 5$. The theorem is at least as strong.

\begin{theorem} For A297953, the generating function is exactly $\sum_{n\ge 1} a(n)x^{n-1} = \frac{1 + x + 2x^{2} - 2x^{3} - 4x^{4}}{1 - 2x + x^{2} - 4x^{3} + 4x^{4}}$.\end{theorem}

{\itshape Proof.} Let $N(x)$ and $D(x)$ be the numerator and denominator above, $D(0)=1$. The residual test applied to the recurrence read off $D$ --- namely $a(n) = \sum_i c_i a(n-i)$ with $c_i$ the negated coefficients of $D$ --- gives vanishing residuals for every $n > 5$, so the power series $F(x)=\sum_{n\ge 1}a(n)x^n$ satisfies $[x^k]\bigl(F(x)D(x)\bigr) = 0$ for every $k > 5$. The remaining coefficients of $F(x)D(x)$, for $k \le 12$, were computed from the walk count and agree with $N(x)$ term by term. Hence $F(x)D(x) = N(x)$ identically, which is the assertion. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 7, 13, 23, 49, 99, 189,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297953}{A297953}, revision 7, last modified Feb 19 2018.
