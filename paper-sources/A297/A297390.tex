...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297390}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297390}{A297390} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 1 neighboring 1\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 4*a(n-2) + 2*a(n-3) - 4*a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 2*x)*(3 - 2*x\textasciicircum{}2) / (1 - x - 4*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + 4*x\textasciicircum{}4). - Colin Barker, Feb 28 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{3x + 6x^{2} - 2x^{3} - 4x^{4}}{1 - x - 4x^{2} - 2x^{3} + 4x^{4}}.\]
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $26$; neither removal changes any count.
...
\begin{theorem} For A297390, the recurrence $a(n) = a(n-1) + 4\,a(n-2) + 2\,a(n-3) - 4\,a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297390, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{3x + 6x^{2} - 2x^{3} - 4x^{4}}{1 - x - 4x^{2} - 2x^{3} + 4x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 9, 19, 57, 139, 369, 963, 2489,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297390}{A297390}, revision 8, last modified Feb 28 2019.
