...
\title{A proof of the conjectured linear recurrence for OEIS A297515}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $221$; neither removal changes any count.
...
\begin{theorem} For A297515, the recurrence of order $32$ displayed in Section 1 holds for every $n \ge 33$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,91$. Every one of them vanished. After it, more than $S = 32$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 33, 128, 624, 3383, 16173, 80436, 406779,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297515}{A297515}, revision 4, last modified Dec 31 2017.
