...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297370}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297370}{A297370} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every 1 horizontally or antidiagonally adjacent to 0, 1 or 2 neighboring 1's\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 13*a(n-1) - 4*a(n-2) - 3*a(n-3) - 19*a(n-4) + 15*a(n-5) - a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(16 - 30*x\textasciicircum{}2 + 4*x\textasciicircum{}3 + 10*x\textasciicircum{}4 - x\textasciicircum{}5) / ((1 - x)*(1 - 12*x - 8*x\textasciicircum{}2 - 5*x\textasciicircum{}3 + 14*x\textasciicircum{}4 - x\textasciicircum{}5)). - Colin Barker, Feb 27 2019\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Jun 08 2021, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 13\,a(n-1) - 4\,a(n-2) - 3\,a(n-3) - 19\,a(n-4) + 15\,a(n-5) - a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x - 30x^{3} + 4x^{4} + 10x^{5} - x^{6}}{1 - 13x + 4x^{2} + 3x^{3} + 19x^{4} - 15x^{5} + x^{6}}.\]
...
\begin{theorem} For A297370, the recurrence $a(n) = 13\,a(n-1) - 4\,a(n-2) - 3\,a(n-3) - 19\,a(n-4) + 15\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297370, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x - 30x^{3} + 4x^{4} + 10x^{5} - x^{6}}{1 - 13x + 4x^{2} + 3x^{3} + 19x^{4} - 15x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 16 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 208, 2610, 33054, 418344, 5294713, 67012245, 848136217,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297370}{A297370}, revision 10, last modified Jun 08 2021.
