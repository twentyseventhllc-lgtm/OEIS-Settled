...
\title{Proofs of the conjectured column and row recurrences for OEIS A297102}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297102}{A297102} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no 1 adjacent to 4 king-move neighboring 1s\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +6*a(n-3) -8*a(n-4) -4*a(n-6)\\k=3: [order 20]\\k=4: [order 47]\end{quote}
...
\begin{theorem} For column $k = 1$ of A297102, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297102, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 6\,a(n-3) - 8\,a(n-4) - 4\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297102, the recurrence $a(n) = 0$ holds for every $n \ge 21$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A297102, the recurrence $a(n) = 0$ holds for every $n \ge 48$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 51$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 51$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297102}{A297102}, revision 4, last modified Dec 25 2017.
