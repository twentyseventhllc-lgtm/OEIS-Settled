...
\title{Proofs of the conjectured column and row recurrences for OEIS A297978}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297978}{A297978} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 2, 3, 4 or 5 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) +2*a(n-3)\\k=3: [order 13] for n>15\\k=4: [order 46] for n>49\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 10 2018). As of revision 4, last modified Jan 10 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A297978, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297978, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297978, the recurrence $a(n) = 0$ holds for every $n > 15$, and fails at $n = 15$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A297978, the recurrence $a(n) = 0$ holds for every $n > 49$, and fails at $n = 49$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $260$ after trimming, and $S = 55$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 49$; after that point more than $S = 55$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297978}{A297978}, revision 4, last modified Jan 10 2018.
