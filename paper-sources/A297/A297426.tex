...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297426}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297426}{A297426} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 2 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 7*a(n-3) + a(n-5) - 4*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 6*x + 7*x\textasciicircum{}2 + x\textasciicircum{}3 - 3*x\textasciicircum{}4 - 4*x\textasciicircum{}5) / (1 - x - 7*x\textasciicircum{}3 - x\textasciicircum{}5 + 4*x\textasciicircum{}6). - Colin Barker, Feb 28 2019\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 12, last modified Aug 21 2025, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 6x^{2} + 7x^{3} + x^{4} - 3x^{5} - 4x^{6}}{1 - x - 7x^{3} - x^{5} + 4x^{6}}.\]
...
\begin{theorem} For A297426, the recurrence $a(n) = a(n-1) + 7\,a(n-3) + a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297426, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 6x^{2} + 7x^{3} + x^{4} - 3x^{5} - 4x^{6}}{1 - x - 7x^{3} - x^{5} + 4x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 7, 14, 22, 68, 163, 320, 782,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297426}{A297426}, revision 12, last modified Aug 21 2025.
