...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297067}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297067}{A297067} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with no 1 adjacent to 1 king-move neighboring 1\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - a(n-2) - a(n-3) - 4*a(n-4) - 8*a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(3 - 2*x - 5*x\textasciicircum{}2 - 12*x\textasciicircum{}3 - 8*x\textasciicircum{}4) / (1 - 4*x + x\textasciicircum{}2 + x\textasciicircum{}3 + 4*x\textasciicircum{}4 + 8*x\textasciicircum{}5). - Colin Barker, Feb 25 2019\end{quote}
...
\[a(n) = 4\,a(n-1) - a(n-2) - a(n-3) - 4\,a(n-4) - 8\,a(n-5).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{3x - 2x^{2} - 5x^{3} - 12x^{4} - 8x^{5}}{1 - 4x + x^{2} + x^{3} + 4x^{4} + 8x^{5}}.\]
...
\begin{theorem} For A297067, the recurrence $a(n) = 4\,a(n-1) - a(n-2) - a(n-3) - 4\,a(n-4) - 8\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297067, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{3x - 2x^{2} - 5x^{3} - 12x^{4} - 8x^{5}}{1 - 4x + x^{2} + x^{3} + 4x^{4} + 8x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 10, 32, 103, 350, 1201, 4143, 14353,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297067}{A297067}, revision 7, last modified Feb 25 2019.
