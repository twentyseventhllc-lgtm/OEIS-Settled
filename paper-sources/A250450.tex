...
\title{A proof of the conjectured order of the recurrence for OEIS A250450}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250450}{A250450} as

\begin{quote}\itshape Number of (n+1)X(4+1) 0..2 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}
...
For scale: at $n = 8$ there are $3^{40}$ arrays, about $10^{19}$, against $52$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A250450, the least order of a linear recurrence that $a$ eventually satisfies is exactly $50$, and that recurrence holds for every $n > 50$, the entry's own figure.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 14 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4535, 141857, 3865967, 104578590, 2750712613, 71984404630, 1868402502223, 48446319776685,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250450}{A250450}, revision 6, last modified Jul 23 2025.
