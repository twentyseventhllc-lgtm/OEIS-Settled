...
\title{Proofs of the conjectured column and row recurrences for OEIS A224550}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224550}{A224550} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..1 matrices with each 2X2 subblock idempotent\end{quote}
...
The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 10 2013). As of revision 8, last modified Oct 03 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A224550, the recurrence $a(n) = 2\,a(n-1) - a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A224550, the recurrence $a(n) = 4\,a(n-1) - 5\,a(n-2) + a(n-3) + 2\,a(n-4) - a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $8$ states, $8$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224550}{A224550}, revision 8, last modified Oct 03 2025.
