...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207700}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207700}{A207700} as

\begin{quote}\itshape Number of n X 5 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}
...
\begin{quote}\ttfamily G.f.: x*(15 + 135*x - 54*x\textasciicircum{}2 - 30*x\textasciicircum{}3 + 15*x\textasciicircum{}4 - x\textasciicircum{}5) / (1 - x)\textasciicircum{}6.\end{quote}

\begin{quote}\ttfamily a(n) = (3 - 10*n - 15*n\textasciicircum{}2 + 44*n\textasciicircum{}3 + 21*n\textasciicircum{}4 + 2*n\textasciicircum{}5) / 3.\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 19 2012).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 12, last modified Jun 25 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{15x + 135x^{2} - 54x^{3} - 30x^{4} + 15x^{5} - x^{6}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}.\]

Written out, the claim is

\[a(n) = 1 - \frac{10}{3}n - 5n^{2} + \frac{44}{3}n^{3} + 7n^{4} + \frac{2}{3}n^{5}.\]
...
\begin{theorem} For A207700, the recurrence $a(n) = 6\,a(n-1) - 15\,a(n-2) + 20\,a(n-3) - 15\,a(n-4) + 6\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. Every one of them vanished. After it, more than $S = 14$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A207700, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{15x + 135x^{2} - 54x^{3} - 30x^{4} + 15x^{5} - x^{6}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}$.\end{theorem}
...
\begin{theorem} For A207700, $a(n)$ is the polynomial $1 - \frac{10}{3}n - 5n^{2} + \frac{44}{3}n^{3} + 7n^{4} + \frac{2}{3}n^{5}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $5$ is annihilated by the difference operator $(E-1)^{6}$, whose characteristic polynomial is $(t-1)^{6}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 6 = 20$. Its values were computed exactly; all of them vanish, and more than $20$ consecutive vanishing values follow, so every later one vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 15, 225, 1071, 3321, 8151, 17225, 32775, 57681,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207700}{A207700}, revision 12, last modified Jun 25 2018.
