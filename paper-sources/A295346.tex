...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A295346}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295346}{A295346} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 horizontally or vertically adjacent to 0, 2 or 3 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 3*a(n-2) - 4*a(n-4) - a(n-5) + a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(3 - 4*x - 3*x\textasciicircum{}2 - 5*x\textasciicircum{}3 + x\textasciicircum{}5) / ((1 - 3*x + x\textasciicircum{}2)*(1 - x - x\textasciicircum{}2 - 2*x\textasciicircum{}3 - x\textasciicircum{}4)). - Colin Barker, Feb 26 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{3x - 4x^{2} - 3x^{3} - 5x^{4} + x^{6}}{1 - 4x + 3x^{2} + 4x^{4} + x^{5} - x^{6}}.\]
...
\begin{theorem} For A295346, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2) - 4\,a(n-4) - a(n-5) + a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A295346, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{3x - 4x^{2} - 3x^{3} - 5x^{4} + x^{6}}{1 - 4x + 3x^{2} + 4x^{4} + x^{5} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 8, 20, 51, 132, 341, 883, 2293,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295346}{A295346}, revision 8, last modified Feb 26 2018.
