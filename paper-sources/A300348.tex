...
\title{A proof of the conjectured linear recurrence for OEIS A300348}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300348}{A300348} as

\begin{quote}\itshape Number of nX6 0..1 arrays with every element equal to 1 or 2 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) -8*a(n-2) +4*a(n-3) +a(n-7) -3*a(n-8) +4*a(n-9) -2*a(n-10) +a(n-11) -a(n-12) -a(n-16) +a(n-17) for n>22\end{quote}
...
\begin{multline*} a(n) = 5\,a(n-1) - 8\,a(n-2) + 4\,a(n-3) + a(n-7) - 3\,a(n-8) + 4\,a(n-9) - \\ 2\,a(n-10) + a(n-11) - a(n-12) - a(n-16) + a(n-17). \end{multline*}
...
The construction gives $4160$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $430$; neither removal changes any count.
...
That refinement leaves $101$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 47$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A300348, the recurrence $a(n) = 5\,a(n-1) - 8\,a(n-2) + 4\,a(n-3) + a(n-7) - 3\,a(n-8) + 4\,a(n-9) - 2\,a(n-10) + a(n-11) - a(n-12) - a(n-16) + a(n-17)$ holds for every $n > 22$, and fails at $n = 22$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,98$. The last nonvanishing one is at $m = 5$, that is at $n = 22$. After it, more than $S = 47$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 90, 133, 325, 729, 1371, 3092, 6068,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300348}{A300348}, revision 4, last modified Mar 03 2018.
