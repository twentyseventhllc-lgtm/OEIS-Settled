...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296552}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296552}{A296552} as

\begin{quote}\itshape Number of n X 6 0..1 arrays with each 1 horizontally, vertically or antidiagonally adjacent to 3 or 6 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 4*a(n-4) + a(n-5) + 3*a(n-6) + 6*a(n-7) + 2*a(n-9) + 4*a(n-10) + a(n-13).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 4*x\textasciicircum{}2 + 5*x\textasciicircum{}3 + 4*x\textasciicircum{}4 + 9*x\textasciicircum{}5 + 6*x\textasciicircum{}6 + 2*x\textasciicircum{}7 + 6*x\textasciicircum{}8 + 4*x\textasciicircum{}9 + x\textasciicircum{}11 + x\textasciicircum{}12) / (1 - x - 4*x\textasciicircum{}4 - x\textasciicircum{}5 - 3*x\textasciicircum{}6 - 6*x\textasciicircum{}7 - 2*x\textasciicircum{}9 - 4*x\textasciicircum{}10 - x\textasciicircum{}13). - Colin Barker, Feb 23 2019\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 4\,a(n-4) + a(n-5) + 3\,a(n-6) + 6\,a(n-7) + 2\,a(n-9) + \\ 4\,a(n-10) + a(n-13). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 4x^{3} + 5x^{4} + 4x^{5} + 9x^{6} + 6x^{7} + 2x^{8} + 6x^{9} + 4x^{10} + x^{12} + x^{13}}{1 - x - 4x^{4} - x^{5} - 3x^{6} - 6x^{7} - 2x^{9} - 4x^{10} - x^{13}}.\]
...
The construction gives $4160$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $37$; neither removal changes any count.
...
That refinement leaves $13$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 13$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A296552, the recurrence $a(n) = a(n-1) + 4\,a(n-4) + a(n-5) + 3\,a(n-6) + 6\,a(n-7) + 2\,a(n-9) + 4\,a(n-10) + a(n-13)$ holds for every $n \ge 14$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,65$. Every one of them vanished. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A296552, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 4x^{3} + 5x^{4} + 4x^{5} + 9x^{6} + 6x^{7} + 2x^{8} + 6x^{9} + 4x^{10} + x^{12} + x^{13}}{1 - x - 4x^{4} - x^{5} - 3x^{6} - 6x^{7} - 2x^{9} - 4x^{10} - x^{13}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 33 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 1, 5, 10, 18, 32, 62, 118,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296552}{A296552}, revision 8, last modified Feb 23 2019.
