...
\title{Proofs of the conjectured column and row recurrences for OEIS A230250}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A230250}{A230250} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 4, and upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 4*a(n-1) +a(n-2) +4*a(n-3)\\k=3: [order 13]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 13 2013). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} In A230250, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A230250, the recurrence $a(n) = 4\,a(n-1) + a(n-2) + 4\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $189$ states, $147$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A230250, the recurrence $a(n) = 0$ holds for every $n \ge 14$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $3681$ states, $3373$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A230250}{A230250}, revision 6, last modified Jul 23 2025.
