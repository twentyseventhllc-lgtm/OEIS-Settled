...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A259956}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A259956}{A259956} as

\begin{quote}\itshape Number of (n+2) X (2+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00000000 00000001 or 00001001\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-2) + 6*a(n-3) + 5*a(n-4) + a(n-5) - 2*a(n-6) - 2*a(n-7) - a(n-8) for n>9.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(67 + 79*x + 159*x\textasciicircum{}2 + 54*x\textasciicircum{}3 - 49*x\textasciicircum{}4 - 68*x\textasciicircum{}5 - 28*x\textasciicircum{}6 - 5*x\textasciicircum{}7 + 7*x\textasciicircum{}8) / (1 - x - 2*x\textasciicircum{}2 - 6*x\textasciicircum{}3 - 5*x\textasciicircum{}4 - x\textasciicircum{}5 + 2*x\textasciicircum{}6 + 2*x\textasciicircum{}7 + x\textasciicircum{}8). - Colin Barker, Dec 27 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Jul 10 2015).
...
\begin{multline*} a(n) = a(n-1) + 2\,a(n-2) + 6\,a(n-3) + 5\,a(n-4) + a(n-5) - 2\,a(n-6) - \\ 2\,a(n-7) - a(n-8). \end{multline*}

stated for $n > 9$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{67x + 79x^{2} + 159x^{3} + 54x^{4} - 49x^{5} - 68x^{6} - 28x^{7} - 5x^{8} + 7x^{9}}{1 - x - 2x^{2} - 6x^{3} - 5x^{4} - x^{5} + 2x^{6} + 2x^{7} + x^{8}}.\]
...
The condition is that $w(i,j)$ is a cyclic rotation of one of \texttt{00000000}, \texttt{00000001}, \texttt{00001001}.
...
\begin{theorem} For A259956, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + 6\,a(n-3) + 5\,a(n-4) + a(n-5) - 2\,a(n-6) - 2\,a(n-7) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. The last nonvanishing one is at $m = 1$, that is at $n = 9$. After it, more than $S = 17$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A259956, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{67x + 79x^{2} + 159x^{3} + 54x^{4} - 49x^{5} - 68x^{6} - 28x^{7} - 5x^{8} + 7x^{9}}{1 - x - 2x^{2} - 6x^{3} - 5x^{4} - x^{5} + 2x^{6} + 2x^{7} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 67, 146, 439, 1187, 3227, 8964, 24719, 67952,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A259956}{A259956}, revision 8, last modified Dec 27 2018.
