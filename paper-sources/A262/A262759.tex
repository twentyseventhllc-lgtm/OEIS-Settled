...
\title{Proofs of the conjectured column and row recurrences for OEIS A262759}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A262759}{A262759} as

\begin{quote}\itshape T(n,k) = Number of (n+2) X (k+2) 0..1 arrays with each row divisible by 5 and each column divisible by 7, read as a binary number with top and left being the most significant bits\end{quote}
...
k=1: a(n) = 2*a(n-1) +a(n-3) -2*a(n-4)\\k=2: a(n) = 4*a(n-1) +9*a(n-3) -36*a(n-4) -8*a(n-6) +32*a(n-7)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Sep 30 2015). As of revision 6, last modified Aug 19 2022, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A262759, the recurrence $a(n) = 2\,a(n-1) + a(n-3) - 2\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A262759, the recurrence $a(n) = 4\,a(n-1) + 9\,a(n-3) - 36\,a(n-4) - 8\,a(n-6) + 32\,a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $49$ states, $49$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A262759}{A262759}, revision 6, last modified Aug 19 2022.
