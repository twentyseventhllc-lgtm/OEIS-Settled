...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207303}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207303}{A207303} as

\begin{quote}\itshape Number of n X 6 0..1 arrays avoiding 0 0 0 and 1 0 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (4/15)*n\textasciicircum{}5 + (45/4)*n\textasciicircum{}4 + (199/6)*n\textasciicircum{}3 - (73/4)*n\textasciicircum{}2 - (373/30)*n + 5.\end{quote}

\begin{quote}\ttfamily G.f.: x*(19 + 247*x - 206*x\textasciicircum{}2 - 76*x\textasciicircum{}3 + 53*x\textasciicircum{}4 - 5*x\textasciicircum{}5) / (1 - x)\textasciicircum{}6.\end{quote}

\begin{quote}\ttfamily a(n) = 6*a(n-1) - 15*a(n-2) + 20*a(n-3) - 15*a(n-4) + 6*a(n-5) - a(n-6) for n>6. (End)\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 16 2012).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 12, last modified Oct 26 2024, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 5 - \frac{373}{30}n - \frac{73}{4}n^{2} + \frac{199}{6}n^{3} + \frac{45}{4}n^{4} + \frac{4}{15}n^{5}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{19x + 247x^{2} - 206x^{3} - 76x^{4} + 53x^{5} - 5x^{6}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}.\]
...
That refinement leaves $35$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 24$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A207303, $a(n)$ is the polynomial $5 - \frac{373}{30}n - \frac{73}{4}n^{2} + \frac{199}{6}n^{3} + \frac{45}{4}n^{4} + \frac{4}{15}n^{5}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $5$ is annihilated by the difference operator $(E-1)^{6}$, whose characteristic polynomial is $(t-1)^{6}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 6 = 30$. Its values were computed exactly; all of them vanish, and more than $30$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A207303, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{19x + 247x^{2} - 206x^{3} - 76x^{4} + 53x^{5} - 5x^{6}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}$.\end{theorem}
...
\begin{theorem} For A207303, the recurrence $a(n) = 6\,a(n-1) - 15\,a(n-2) + 20\,a(n-3) - 15\,a(n-4) + 6\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,65$. Every one of them vanished. After it, more than $S = 24$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 19, 361, 1675, 4939, 11497, 23091, 41893, 70537,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207303}{A207303}, revision 12, last modified Oct 26 2024.
