...
\title{Proofs of the conjectured column and row recurrences for OEIS A300966}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300966}{A300966} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 2, 3, 4 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +4*a(n-2) +2*a(n-3) -a(n-6)\\k=3: [order 22] for n>24\\k=4: [order 75] for n>78\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 17 2018). As of revision 4, last modified Mar 17 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A300966, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300966, the recurrence $a(n) = a(n-1) + 4\,a(n-2) + 2\,a(n-3) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300966, the recurrence $a(n) = 0$ holds for every $n > 24$, and fails at $n = 24$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $66$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 24$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 24$; the proved threshold is $n > 24$.

\begin{theorem} For column $k = 4$ of A300966, the recurrence $a(n) = 0$ holds for every $n > 78$, and fails at $n = 78$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $262$ after trimming, and $S = 93$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 78$; after that point more than $S = 93$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 78$; the proved threshold is $n > 78$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300966}{A300966}, revision 4, last modified Mar 17 2018.
