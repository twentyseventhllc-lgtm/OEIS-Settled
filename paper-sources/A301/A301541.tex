...
\title{Proofs of the conjectured column and row recurrences for OEIS A301541}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301541}{A301541} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1 or 3 horizontally or vertically adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 2*a(n-1) +a(n-2) -a(n-3) -2*a(n-4) +a(n-5)\\k=3: [order 20]\\k=4: [order 62]\end{quote}
...
\begin{theorem} For column $k = 1$ of A301541, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A301541, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - a(n-3) - 2\,a(n-4) + a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A301541, the recurrence $a(n) = 0$ holds for every $n \ge 21$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A301541, the recurrence $a(n) = 0$ holds for every $n \ge 63$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $262$ after trimming, and $S = 67$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 67$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301541}{A301541}, revision 4, last modified Mar 23 2018.
