...
\title{Proofs of the conjectured column and row recurrences for OEIS A301323}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301323}{A301323} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 5 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -a(n-2) +a(n-3) +a(n-4) -2*a(n-5) -a(n-6)\\k=3: a(n) = 4*a(n-1) -3*a(n-2) -3*a(n-3) +2*a(n-4) +3*a(n-5) -2*a(n-6) for n>8\\k=4: a(n) = 4*a(n-1) -4*a(n-2) +2*a(n-5) -3*a(n-7) +2*a(n-8) +a(n-9) -a(n-10) for n>13\\k=5: [order 14] for n>16\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 18 2018). As of revision 4, last modified Mar 18 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A301323, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A301323, the recurrence $a(n) = 3\,a(n-1) - a(n-2) + a(n-3) + a(n-4) - 2\,a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $20$ states, $16$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A301323, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2) - 3\,a(n-3) + 2\,a(n-4) + 3\,a(n-5) - 2\,a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $39$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 8$; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 8$; the proved threshold is $n > 8$.

\begin{theorem} For column $k = 4$ of A301323, the recurrence $a(n) = 4\,a(n-1) - 4\,a(n-2) + 2\,a(n-5) - 3\,a(n-7) + 2\,a(n-8) + a(n-9) - a(n-10)$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $101$ after trimming, and $S = 22$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 22$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 5$ of A301323, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $274$ after trimming, and $S = 35$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 16$; after that point more than $S = 35$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301323}{A301323}, revision 4, last modified Mar 18 2018.
