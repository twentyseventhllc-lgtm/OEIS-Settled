...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +2*a(n-2) +a(n-3)\\k=3: a(n) = 2*a(n-1) +3*a(n-2) -2*a(n-4)\\k=4: a(n) = 2*a(n-1) +7*a(n-2) +6*a(n-3) -3*a(n-4) -4*a(n-5) +a(n-6)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 29 2017). As of revision 21, last modified Sep 16 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
