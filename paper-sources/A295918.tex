...
\title{Proofs of the conjectured column and row recurrences for OEIS A295918}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295918}{A295918} as

\begin{quote}\itshape T(n,k)=Number of n X k 0..1 arrays with each 1 adjacent to 0 or 3 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +2*a(n-2) +a(n-3)\\k=3: a(n) = 2*a(n-1) +3*a(n-2) -2*a(n-4)\\k=4: a(n) = 2*a(n-1) +7*a(n-2) +6*a(n-3) -3*a(n-4) -4*a(n-5) +a(n-6)\\k=5: [order 38]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 29 2017). As of revision 21, last modified Sep 16 2025, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A295918, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A295918, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A295918, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 2\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $24$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A295918, the recurrence $a(n) = 2\,a(n-1) + 7\,a(n-2) + 6\,a(n-3) - 3\,a(n-4) - 4\,a(n-5) + a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $55$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 5$ of A295918, the recurrence $a(n) = 0$ holds for every $n \ge 39$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $387$ after trimming, and $S = 46$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 46$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295918}{A295918}, revision 21, last modified Sep 16 2025.
