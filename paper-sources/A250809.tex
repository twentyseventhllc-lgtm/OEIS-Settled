...
\title{A proof of the conjectured generating function for OEIS A250809}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250809}{A250809} as
...
\begin{quote}\ttfamily Empirical g.f.: x*(784 - 1607*x + 1158*x\textasciicircum{}2) / ((1 - x)*(1 - 2*x)*(1 - 3*x)). - Colin Barker, Nov 20 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{784x - 1607x^{2} + 1158x^{3}}{1 - 6x + 11x^{2} - 6x^{3}}.\]
...
That refinement leaves $19$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 13$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $3^{48}$ arrays, about $10^{23}$, against $13$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A250809, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{784x - 1607x^{2} + 1158x^{3}}{1 - 6x + 11x^{2} - 6x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 784, 3097, 11116, 37333, 120304, 377857, 1167796, 3572173,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250809}{A250809}, revision 8, last modified Nov 20 2018.
