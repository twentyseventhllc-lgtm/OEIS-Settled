...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A234883}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234883}{A234883} as

\begin{quote}\itshape Number of (n+1) X (1+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 14*a(n-2) - 37*a(n-4) + 21*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 8*x*(10 + 34*x - 33*x\textasciicircum{}2 - 114*x\textasciicircum{}3 + 21*x\textasciicircum{}4 + 73*x\textasciicircum{}5) / (1 - 14*x\textasciicircum{}2 + 37*x\textasciicircum{}4 - 21*x\textasciicircum{}6). - Colin Barker, Oct 16 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{80x + 272x^{2} - 264x^{3} - 912x^{4} + 168x^{5} + 584x^{6}}{1 - 14x^{2} + 37x^{4} - 21x^{6}}.\]
...
\begin{theorem} For A234883, the recurrence $a(n) = 14\,a(n-2) - 37\,a(n-4) + 21\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A234883, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{80x + 272x^{2} - 264x^{3} - 912x^{4} + 168x^{5} + 584x^{6}}{1 - 14x^{2} + 37x^{4} - 21x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 80, 272, 856, 2896, 9192, 31064, 98696, 333456,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234883}{A234883}, revision 11, last modified Jun 19 2022.
