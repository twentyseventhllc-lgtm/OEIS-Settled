...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196584}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196584}{A196584} as

\begin{quote}\itshape Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,2,0,1,4 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +6*a(n-2) +4*a(n-3) -3*a(n-4) -a(n-5) -2*a(n-6) -a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 6*x + 7*x\textasciicircum{}2 - x\textasciicircum{}3 - 7*x\textasciicircum{}4 - 3*x\textasciicircum{}5) / (1 - x - 6*x\textasciicircum{}2 - 4*x\textasciicircum{}3 + 3*x\textasciicircum{}4 + x\textasciicircum{}5 + 2*x\textasciicircum{}6 + x\textasciicircum{}7). - Colin Barker, Feb 22 2018\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 6\,a(n-2) + 4\,a(n-3) - 3\,a(n-4) - a(n-5) - 2\,a(n-6) - \\ a(n-7). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 6x^{2} + 7x^{3} - x^{4} - 7x^{5} - 3x^{6}}{1 - x - 6x^{2} - 4x^{3} + 3x^{4} + x^{5} + 2x^{6} + x^{7}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 3, \tau(1) = 2, \tau(2) = 0, \tau(3) = 1, \tau(4) = 4. The condition is that for every cell,
...
\begin{theorem} For A196584, the recurrence $a(n) = a(n-1) + 6\,a(n-2) + 4\,a(n-3) - 3\,a(n-4) - a(n-5) - 2\,a(n-6) - a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196584, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 6x^{2} + 7x^{3} - x^{4} - 7x^{5} - 3x^{6}}{1 - x - 6x^{2} - 4x^{3} + 3x^{4} + x^{5} + 2x^{6} + x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 7, 20, 65, 203, 648, 2057, 6527,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196584}{A196584}, revision 8, last modified Feb 22 2018.
