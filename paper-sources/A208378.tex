...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A208378}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208378}{A208378} as

\begin{quote}\itshape Number of n X 7 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 84*n\textasciicircum{}3 + 840*n\textasciicircum{}2 - 1344*n + 420 for n>1.\end{quote}

\begin{quote}\ttfamily G.f.: 42*x*(1 + 38*x - 14*x\textasciicircum{}2 - 14*x\textasciicircum{}3 + x\textasciicircum{}4) / (1 - x)\textasciicircum{}4.\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Jul 02 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 420 - 1344n + 840n^{2} + 84n^{3}.\]

stated for $n > 1$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{42x + 1596x^{2} - 588x^{3} - 588x^{4} + 42x^{5}}{1 - 4x + 6x^{2} - 4x^{3} + x^{4}}.\]
...
\begin{theorem} For A208378, $a(n)$ is the polynomial $420 - 1344n + 840n^{2} + 84n^{3}$ for every $n > 1$, and not at $n = 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $3$ is annihilated by the difference operator $(E-1)^{4}$, whose characteristic polynomial is $(t-1)^{4}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 4 = 12$. Its values were computed exactly; the last nonvanishing one is at $n = 1$, and more than $12$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A208378, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{42x + 1596x^{2} - 588x^{3} - 588x^{4} + 42x^{5}}{1 - 4x + 6x^{2} - 4x^{3} + x^{4}}$.\end{theorem}
...
\begin{theorem} For A208378, the recurrence $a(n) = 4\,a(n-1) - 6\,a(n-2) + 4\,a(n-3) - a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,49$. The last nonvanishing one is at $m = 1$, that is at $n = 5$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 42, 1764, 6216, 13860, 25200, 40740, 60984, 86436,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208378}{A208378}, revision 8, last modified Jul 02 2018.
