...
k=1: a(n) = a(n-1) +3*a(n-3) +a(n-4) for n>5\\k=2: a(n) = a(n-1) +a(n-2) +5*a(n-3) +a(n-4) -3*a(n-5) -3*a(n-6) for n>7\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 17 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
