...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208029}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208029}{A208029} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 11*a(n-2) + 2*a(n-3) - 10*a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 9*x*(1 - x)*(1 + 8*x + 10*x\textasciicircum{}2) / (1 - 2*x - 11*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + 10*x\textasciicircum{}4). - Colin Barker, Jun 26 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 63x^{2} + 18x^{3} - 90x^{4}}{1 - 2x - 11x^{2} - 2x^{3} + 10x^{4}}.\]
...
\begin{theorem} For A208029, the recurrence $a(n) = 2\,a(n-1) + 11\,a(n-2) + 2\,a(n-3) - 10\,a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208029, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 63x^{2} + 18x^{3} - 90x^{4}}{1 - 2x - 11x^{2} - 2x^{3} + 10x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 279, 1377, 5895, 26685, 118179, 527913,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208029}{A208029}, revision 8, last modified Jun 26 2018.
