...
\title{Proofs of the conjectured column and row recurrences for OEIS A298547}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298547}{A298547} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 3, 4 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) +4*a(n-3) +4*a(n-4) for n>5\\k=3: [order 12] for n>13\\k=4: [order 36] for n>39\end{quote}
...
\begin{theorem} For column $k = 1$ of A298547, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298547, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 4\,a(n-3) + 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298547, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 4$ of A298547, the recurrence $a(n) = 0$ holds for every $n > 39$, and fails at $n = 39$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $256$ after trimming, and $S = 46$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 39$; after that point more than $S = 46$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298547}{A298547}, revision 4, last modified Jan 21 2018.
