...
\title{Proofs of the conjectured column and row recurrences for OEIS A229672}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A229672}{A229672} as

\begin{quote}\itshape T(n,k) = number of defective 4-colorings of an n X k 0..3 array connected horizontally, antidiagonally and vertically with exactly two mistakes, and colors introduced in row-major 0..3 order\end{quote}
...
k=1: a(n) = 12*a(n-1) - 57*a(n-2) + 136*a(n-3) - 171*a(n-4) + 108*a(n-5) - 27*a(n-6) for n > 9.\\k=2: a(n) = 12*a(n-1) - 48*a(n-2) + 64*a(n-3) for n > 6.\\k=3: [order 6] for n > 7.\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Sep 27 2013). As of revision 9, last modified Apr 28 2021, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A229672, the recurrence $a(n) = 12\,a(n-1) - 57\,a(n-2) + 136\,a(n-3) - 171\,a(n-4) + 108\,a(n-5) - 27\,a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A229672, the recurrence $a(n) = 12\,a(n-1) - 48\,a(n-2) + 64\,a(n-3)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $102$ states, $73$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 3$ of A229672, the recurrence $a(n) = 0$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $307$ states, $210$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A229672}{A229672}, revision 9, last modified Apr 28 2021.
