...
\title{A proof of the conjectured linear recurrence for OEIS A196213}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196213}{A196213} as

\begin{quote}\itshape Number of nX3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,0,1,2,4 for x=0,1,2,3,4\end{quote}
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $314$; neither removal changes any count.
...
\begin{theorem} For A196213, the recurrence of order $36$ displayed in Section 1 holds for every $n \ge 37$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,104$. Every one of them vanished. After it, more than $S = 40$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 20, 58, 290, 1217, 5110, 21969, 93666,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196213}{A196213}, revision 9, last modified Oct 03 2025.
