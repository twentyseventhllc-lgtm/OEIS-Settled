...
\title{A proof of the conjectured linear recurrence for OEIS A304128}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304128}{A304128} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 3 or 6 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +2*a(n-3) for n>7\end{quote}
...
\begin{theorem} For A304128, the recurrence $a(n) = a(n-1) + 2\,a(n-3)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,51$. The last nonvanishing one is at $m = 4$, that is at $n = 7$. After it, more than $S = 7$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 35 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 7, 9, 19, 33, 53, 89, 155,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304128}{A304128}, revision 4, last modified May 07 2018.
