...
\title{A proof of the conjectured linear recurrence for OEIS A305583}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305583}{A305583} as

\begin{quote}\itshape Number of nX5 0..1 arrays with every element unequal to 0, 1, 2, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-2) +12*a(n-3) -9*a(n-5) -27*a(n-6) for n>10\end{quote}
...
The construction gives $1056$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $82$; neither removal changes any count.
...
\begin{theorem} For A305583, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 12\,a(n-3) - 9\,a(n-5) - 27\,a(n-6)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,52$. The last nonvanishing one is at $m = 4$, that is at $n = 10$. After it, more than $S = 14$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 8, 19, 38, 76, 327, 967, 2641,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305583}{A305583}, revision 4, last modified Jun 05 2018.
