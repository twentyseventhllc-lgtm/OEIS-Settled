...
\title{A proof of the conjectured linear recurrence for OEIS A197800}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A197800}{A197800} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 4,2,1,0,1 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) +6*a(n-2) +55*a(n-3) +93*a(n-4) +335*a(n-5) +452*a(n-6) +139*a(n-7) -1793*a(n-8) -3659*a(n-9) -6365*a(n-10) -7501*a(n-11) -3080*a(n-12) +14194*a(n-13) +48711*a(n-14) +46798*a(n-15) +16724*a(n-16) -51146*a(n-17) -68258*a(n-18) -58699*a(n-19) -12222*a(n-20) +5431*a(n-21) +7158*a(n-22) +52233*a(n-23) +9954*a(n-24) +5396*a(n-25) +905*a(n-26) -3014*a(n-27) +1824*a(n-28) +1787*a(n-29) -198*a(n-30) -430*a(n-31) -72*a(n-32).\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Oct 18 2011).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Jun 10 2014, the entry records no proof and links to none, so the statement is open. This note settles it.
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 4, \tau(1) = 2, \tau(2) = 1, \tau(3) = 0, \tau(4) = 1. The condition is that for every cell,
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $452$; neither removal changes any count.
...
That refinement leaves $40$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 38$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A197800, the recurrence of order $32$ displayed in Section 1 holds for every $n \ge 33$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,96$. Every one of them vanished. After it, more than $S = 38$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 20 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 16, 102, 596, 3713, 22742, 139212, 853186,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A197800}{A197800}, revision 9, last modified Jun 10 2014.
