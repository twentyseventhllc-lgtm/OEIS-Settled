...
\title{A proof of the conjectured recurrence and generating function for OEIS A299594}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299594}{A299594} as

\begin{quote}\itshape Number of n X 7 0..1 arrays with every element unequal to 0, 1 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: x*(21 + 5*x + 18*x\textasciicircum{}2 + 96*x\textasciicircum{}3 + 127*x\textasciicircum{}4 + 234*x\textasciicircum{}5 + 49*x\textasciicircum{}6 - 102*x\textasciicircum{}7 - 73*x\textasciicircum{}8) / ((1 + x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - x - 2*x\textasciicircum{}2 - x\textasciicircum{}3)). - Colin Barker, Feb 15 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Feb 15 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{21x + 5x^{2} + 18x^{3} + 96x^{4} + 127x^{5} + 234x^{6} + 49x^{7} - 102x^{8} - 73x^{9}}{1 - x - x^{2} - 3x^{3} - x^{4} + x^{5} + x^{6}}.\]
...
The construction gives $16512$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $83$; neither removal changes any count.
...
Here the refinement leaves $S = 32$ blocks. Writing $L$ for the $S\times S$ quotient matrix, $\tilde w$ for the start weights summed over each block and $\tilde f$ for the acceptance indicator,
...
\begin{theorem} For A299594, the recurrence $a(n) = a(n-1) + a(n-2) + 3\,a(n-3) + a(n-4) - a(n-5) - a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,72$. The last nonvanishing one is at $m = 3$, that is at $n = 9$. After it, more than $S = 32$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A299594, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{21x + 5x^{2} + 18x^{3} + 96x^{4} + 127x^{5} + 234x^{6} + 49x^{7} - 102x^{8} - 73x^{9}}{1 - x - x^{2} - 3x^{3} - x^{4} + x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 21, 26, 65, 250, 541, 1225, 2583, 5488,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299594}{A299594}, revision 8, last modified Feb 15 2018.
