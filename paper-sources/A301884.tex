...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -a(n-2)\\k=3: a(n) = a(n-1) +6*a(n-2) +2*a(n-3)\\k=4: a(n) = a(n-1) +12*a(n-2) -2*a(n-3) -38*a(n-4) +a(n-5) +12*a(n-6) -a(n-8)\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = a(n-1) +a(n-2)\\n=2: a(n) = 2*a(n-1) +a(n-3) -a(n-4) -2*a(n-6) +a(n-7)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 28 2018). As of revision 4, last modified Mar 28 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences and the 2 row recurrences listed above.
