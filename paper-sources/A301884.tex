...
\title{Proofs of the conjectured column and row recurrences for OEIS A301884}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301884}{A301884} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1 or 3 horizontally or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -a(n-2)\\k=3: a(n) = a(n-1) +6*a(n-2) +2*a(n-3)\\k=4: a(n) = a(n-1) +12*a(n-2) -2*a(n-3) -38*a(n-4) +a(n-5) +12*a(n-6) -a(n-8)\\k=5: [order 7] for n>9\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = a(n-1) +a(n-2)\\n=2: a(n) = 2*a(n-1) +a(n-3) -a(n-4) -2*a(n-6) +a(n-7)\\n=3: [order 24] for n>25\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 28 2018). As of revision 4, last modified Mar 28 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences and the 3 row recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A301884, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A301884, the recurrence $a(n) = 3\,a(n-1) - a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A301884, the recurrence $a(n) = a(n-1) + 6\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $44$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A301884, the recurrence $a(n) = a(n-1) + 12\,a(n-2) - 2\,a(n-3) - 38\,a(n-4) + a(n-5) + 12\,a(n-6) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $109$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 5$ of A301884, the recurrence $a(n) = 0$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $331$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For row $n = 1$ of A301884, the recurrence $a(k) = a(n-1) + a(n-2)$ holds for every $k \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For row $n = 2$ of A301884, the recurrence $a(k) = 2\,a(n-1) + a(n-3) - a(n-4) - 2\,a(n-6) + a(n-7)$ holds for every $k \ge 8$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For row $n = 3$ of A301884, the recurrence $a(k) = 0$ holds for every $k > 25$, and fails at $k = 25$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 33$ blocks after refinement. The model reproduces every published value of row $n = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 25$; after that point more than $S = 33$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301884}{A301884}, revision 4, last modified Mar 28 2018.
