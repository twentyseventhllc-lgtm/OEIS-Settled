...
\title{A proof of the conjectured linear recurrence for OEIS A259739}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A259739}{A259739} as

\begin{quote}\itshape Number of (n+2) X (5+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00010101 or 01010101\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-2) +24*a(n-4) +17*a(n-6) -53*a(n-8) +26*a(n-10) +9*a(n-12) -13*a(n-14) +a(n-16) +a(n-18) for n>19.\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Jul 04 2015).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 6, last modified Aug 08 2023, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 3\,a(n-2) + 24\,a(n-4) + 17\,a(n-6) - 53\,a(n-8) + 26\,a(n-10) + 9\,a(n-12) - \\ 13\,a(n-14) + a(n-16) + a(n-18). \end{multline*}
...
That refinement leaves $33$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 24$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $2^{56}$ arrays, about $10^{17}$, against $24$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A259739, the recurrence $a(n) = 3\,a(n-2) + 24\,a(n-4) + 17\,a(n-6) - 53\,a(n-8) + 26\,a(n-10) + 9\,a(n-12) - 13\,a(n-14) + a(n-16) + a(n-18)$ holds for every $n > 19$, and fails at $n = 19$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,72$. The last nonvanishing one is at $m = 1$, that is at $n = 19$. After it, more than $S = 24$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 101, 230, 792, 1882, 4904, 12086, 35036, 82492,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A259739}{A259739}, revision 6, last modified Aug 08 2023.
