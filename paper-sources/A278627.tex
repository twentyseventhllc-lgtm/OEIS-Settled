...
\title{Proofs of the conjectured column and row recurrences for OEIS A278627}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A278627}{A278627} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with rows in nondecreasing lexicographic order and columns in nonincreasing lexicographic order, but with exactly one mistake\end{quote}
...
k=1: a(n) = (1/120)*n\textasciicircum{}5 + (1/8)*n\textasciicircum{}4 + (5/24)*n\textasciicircum{}3 - (1/8)*n\textasciicircum{}2 - (13/60)*n\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 24 2016). As of revision 4, last modified Nov 24 2016, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A278627, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $10$ states, $7$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A278627}{A278627}, revision 4, last modified Nov 24 2016.
