...
\title{A proof of the conjectured linear recurrence for OEIS A302675}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302675}{A302675} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element equal to 0, 1, 4 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) for n>5\end{quote}
...
\begin{theorem} For A302675, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,52$. The last nonvanishing one is at $m = 3$, that is at $n = 5$. After it, more than $S = 7$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 3, 8, 7, 11, 18, 29, 47,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302675}{A302675}, revision 4, last modified Apr 11 2018.
