...
\title{Proofs of the conjectured column and row recurrences for OEIS A318091}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318091}{A318091} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 2, 3 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) +a(n-3) -a(n-4) -a(n-5) -a(n-6)\\k=3: [order 24]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 16 2018). As of revision 4, last modified Aug 16 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A318091, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318091, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + a(n-3) - a(n-4) - a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A318091, the recurrence $a(n) = 0$ holds for every $n \ge 25$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $59$ after trimming, and $S = 29$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 29$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318091}{A318091}, revision 4, last modified Aug 16 2018.
