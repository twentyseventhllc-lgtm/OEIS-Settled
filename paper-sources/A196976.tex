...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196976}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196976}{A196976} as

\begin{quote}\itshape Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,2,1,0,4 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + a(n-2) + 4*a(n-3) - 2*a(n-4) - a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x)\textasciicircum{}2*(1 + x - x\textasciicircum{}3) / (1 - 2*x - x\textasciicircum{}2 - 4*x\textasciicircum{}3 + 2*x\textasciicircum{}4 + x\textasciicircum{}6). - Colin Barker, Mar 01 2018\end{quote}
...
\[a(n) = 2\,a(n-1) + a(n-2) + 4\,a(n-3) - 2\,a(n-4) - a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 3x^{2} + 3x^{3} - 2x^{5} - x^{6}}{1 - 2x - x^{2} - 4x^{3} + 2x^{4} + x^{6}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 3, \tau(1) = 2, \tau(2) = 1, \tau(3) = 0, \tau(4) = 4. The condition is that for every cell,
...
\begin{theorem} For A196976, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + 4\,a(n-3) - 2\,a(n-4) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196976, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 3x^{2} + 3x^{3} - 2x^{5} - x^{6}}{1 - 2x - x^{2} - 4x^{3} + 2x^{4} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 5, 14, 37, 104, 290, 803, 2233,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196976}{A196976}, revision 8, last modified Mar 01 2018.
