...
\title{Proofs of the conjectured column and row recurrences for OEIS A253044}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A253044}{A253044} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every consecutive three elements in every row, column, diagonal and antidiagonal having exactly two distinct values, and new values 0 upwards introduced in row major order\end{quote}
...
k=1: [linear recurrence of order 34] for n>35\\k=2: [order 57] for n>61\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 26 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A253044, the recurrence $a(n) = 0$ holds for every $n > 35$, and fails at $n = 35$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $1477$ states, $1477$ after trimming, and $S = 42$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 35$; after that point more than $S = 42$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 35$; the proved threshold is $n > 35$.

\begin{theorem} For column $k = 2$ of A253044, the recurrence $a(n) = 0$ holds for every $n > 61$, and fails at $n = 61$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $3742$ states, $3742$ after trimming, and $S = 93$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 61$; after that point more than $S = 93$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A253044}{A253044}, revision 6, last modified Jul 23 2025.
