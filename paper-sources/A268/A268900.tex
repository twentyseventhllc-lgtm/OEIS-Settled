...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A268900}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A268900}{A268900} as

\begin{quote}\itshape Number of n X 4 0..2 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling two exactly once\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 18*a(n-1) - 81*a(n-2) for n>3.\end{quote}

\begin{quote}\ttfamily G.f.: 12*x*(3 + 4*x + 9*x\textasciicircum{}2) / (1 - 9*x)\textasciicircum{}2.\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{36x + 48x^{2} + 108x^{3}}{1 - 18x + 81x^{2}}.\]
...
\begin{theorem} For A268900, the recurrence $a(n) = 18\,a(n-1) - 81\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}
...
\begin{theorem} For A268900, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{36x + 48x^{2} + 108x^{3}}{1 - 18x + 81x^{2}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 16 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 36, 696, 9720, 118584, 1347192, 14644152, 154472184, 1594323000,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A268900}{A268900}, revision 8, last modified Jan 16 2019.
