...
\title{Proofs of the conjectured column and row recurrences for OEIS A268750}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A268750}{A268750} as

\begin{quote}\itshape T(n,k)=Number of nXk binary arrays with some element plus some horizontally or vertically adjacent neighbor totalling two no more than once\end{quote}
...
k=1: a(n) = 2*a(n-1) +a(n-2) -2*a(n-3) -a(n-4)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) -4*a(n-3) -a(n-4)\\k=3: a(n) = 4*a(n-1) +8*a(n-2) -24*a(n-3) -38*a(n-4) +4*a(n-5) +12*a(n-6) -a(n-8)\\k=4: [order 10]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 12 2016). As of revision 4, last modified Feb 12 2016, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A268750, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - 2\,a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A268750, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) - 4\,a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A268750, the recurrence $a(n) = 4\,a(n-1) + 8\,a(n-2) - 24\,a(n-3) - 38\,a(n-4) + 4\,a(n-5) + 12\,a(n-6) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $21$ states, $13$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A268750, the recurrence $a(n) = 0$ holds for every $n \ge 11$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $38$ states, $22$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A268750}{A268750}, revision 4, last modified Feb 12 2016.
