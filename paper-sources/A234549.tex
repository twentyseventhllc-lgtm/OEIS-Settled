...
\title{Proofs of the conjectured column and row recurrences for OEIS A234549}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234549}{A234549} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 6*a(n-1) +34*a(n-2) -204*a(n-3) -40*a(n-4) +240*a(n-5).\\k=2: [order 15].\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 28 2013). As of revision 7, last modified Jun 20 2022, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A234549, the recurrence $a(n) = 6\,a(n-1) + 34\,a(n-2) - 204\,a(n-3) - 40\,a(n-4) + 240\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $36$ states, $36$ after trimming, and $S = 5$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 5$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A234549, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $216$ states, $216$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234549}{A234549}, revision 7, last modified Jun 20 2022.
