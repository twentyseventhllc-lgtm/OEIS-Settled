...
\title{Proofs of the conjectured column and row recurrences for OEIS A304926}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304926}{A304926} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 5 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -2*a(n-4) for n>6\\k=3: a(n) = a(n-1) +2*a(n-2) +4*a(n-3) +2*a(n-4) -2*a(n-5) -7*a(n-6) -6*a(n-7) for n>9\\k=4: [order 13] for n>18\\k=5: [order 34] for n>41\end{quote}
...
\begin{theorem} For column $k = 1$ of A304926, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A304926, the recurrence $a(n) = a(n-1) + 3\,a(n-2) - 2\,a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A304926, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + 4\,a(n-3) + 2\,a(n-4) - 2\,a(n-5) - 7\,a(n-6) - 6\,a(n-7)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $46$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For column $k = 4$ of A304926, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $99$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 18$; the proved threshold is $n > 18$.

\begin{theorem} For column $k = 5$ of A304926, the recurrence $a(n) = 0$ holds for every $n > 41$, and fails at $n = 41$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $270$ after trimming, and $S = 49$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 41$; after that point more than $S = 49$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304926}{A304926}, revision 4, last modified May 21 2018.
