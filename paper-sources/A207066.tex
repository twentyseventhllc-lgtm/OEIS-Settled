...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207066}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207066}{A207066} as

\begin{quote}\itshape Number of n X 6 0..1 arrays avoiding 0 0 1 and 0 1 0 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (7/120)*n\textasciicircum{}6 + (147/40)*n\textasciicircum{}5 + (49/3)*n\textasciicircum{}4 + (91/8)*n\textasciicircum{}3 - (707/120)*n\textasciicircum{}2 - (91/20)*n.\end{quote}

\begin{quote}\ttfamily G.f.: 21*x*(1 + 14*x - 7*x\textasciicircum{}2 - 8*x\textasciicircum{}3 + 2*x\textasciicircum{}4) / (1 - x)\textasciicircum{}7.\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Jun 18 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = -\frac{91}{20}n - \frac{707}{120}n^{2} + \frac{91}{8}n^{3} + \frac{49}{3}n^{4} + \frac{147}{40}n^{5} + \frac{7}{120}n^{6}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{21x + 294x^{2} - 147x^{3} - 168x^{4} + 42x^{5}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}.\]
...
That refinement leaves $13$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 8$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A207066, $a(n)$ is the polynomial $-\frac{91}{20}n - \frac{707}{120}n^{2} + \frac{91}{8}n^{3} + \frac{49}{3}n^{4} + \frac{147}{40}n^{5} + \frac{7}{120}n^{6}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $6$ is annihilated by the difference operator $(E-1)^{7}$, whose characteristic polynomial is $(t-1)^{7}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 7 = 15$. Its values were computed exactly; all of them vanish, and more than $15$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A207066, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{21x + 294x^{2} - 147x^{3} - 168x^{4} + 42x^{5}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}$.\end{theorem}
...
\begin{theorem} For A207066, the recurrence $a(n) = 7\,a(n-1) - 21\,a(n-2) + 35\,a(n-3) - 35\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 21, 441, 2499, 8799, 23856, 54684, 111426, 208026,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207066}{A207066}, revision 9, last modified Jun 18 2018.
