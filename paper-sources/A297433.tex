...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297433}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297433}{A297433} as

\begin{quote}\itshape Number of 3Xn 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 2 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-3) +8*a(n-4) +a(n-5) -a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical: G.f.: -x*(-1-2*x-11*x\textasciicircum{}2-9*x\textasciicircum{}3+x\textasciicircum{}5)/(1-x-3*x\textasciicircum{}3-8*x\textasciicircum{}4-x\textasciicircum{}5+x\textasciicircum{}6) . - R. J. Mathar, Feb 13 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Dec 30 2017).

The line quoted ending in that signature is contributed by R. J. Mathar on Feb 13 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 6, last modified Feb 13 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 2x^{2} + 11x^{3} + 9x^{4} - x^{6}}{1 - x - 3x^{3} - 8x^{4} - x^{5} + x^{6}}.\]
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $32$; neither removal changes any count.
...
\begin{theorem} For A297433, the recurrence $a(n) = a(n-1) + 3\,a(n-3) + 8\,a(n-4) + a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. Every one of them vanished. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A297433, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 2x^{2} + 11x^{3} + 9x^{4} - x^{6}}{1 - x - 3x^{3} - 8x^{4} - x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 14, 26, 43, 109, 301, 649,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297433}{A297433}, revision 6, last modified Feb 13 2018.
