...
\title{Proofs of the conjectured column and row recurrences for OEIS A317863}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317863}{A317863} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +4*a(n-2) -2*a(n-3) -2*a(n-4) +a(n-5) -4*a(n-6) +4*a(n-7) -a(n-8)\\k=3: [order 26] for n>32\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 09 2018). As of revision 4, last modified Aug 09 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A317863, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317863, the recurrence $a(n) = a(n-1) + 4\,a(n-2) - 2\,a(n-3) - 2\,a(n-4) + a(n-5) - 4\,a(n-6) + 4\,a(n-7) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317863, the recurrence $a(n) = 0$ holds for every $n > 32$, and fails at $n = 32$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $65$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 32$; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317863}{A317863}, revision 4, last modified Aug 09 2018.
