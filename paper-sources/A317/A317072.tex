...
\title{Proofs of the conjectured column and row recurrences for OEIS A317072}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317072}{A317072} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3, 4, 5, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) +2*a(n-3)\\k=3: a(n) = 6*a(n-1) +9*a(n-2) +8*a(n-3) +a(n-4) -5*a(n-5) -2*a(n-6) -a(n-7) -a(n-8)\\k=4: [order 17]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 20 2018). As of revision 4, last modified Jul 20 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A317072, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317072, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317072, the recurrence $a(n) = 6\,a(n-1) + 9\,a(n-2) + 8\,a(n-3) + a(n-4) - 5\,a(n-5) - 2\,a(n-6) - a(n-7) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A317072, the recurrence $a(n) = 0$ holds for every $n \ge 18$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317072}{A317072}, revision 4, last modified Jul 20 2018.
