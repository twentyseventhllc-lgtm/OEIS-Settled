...
\title{Proofs of the conjectured column and row recurrences for OEIS A298970}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298970}{A298970} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 3, 4 or 5 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1)\\k=3: [order 8]\\k=4: [order 23]\end{quote}
...
\begin{theorem} For column $k = 1$ of A298970, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298970, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298970, the recurrence $a(n) = 0$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A298970, the recurrence $a(n) = 0$ holds for every $n \ge 24$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 24$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 24$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298970}{A298970}, revision 4, last modified Jan 30 2018.
