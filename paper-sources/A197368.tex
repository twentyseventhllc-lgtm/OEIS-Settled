...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A197368}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A197368}{A197368} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,4,1,0,1 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +2*a(n-2) +7*a(n-3) -3*a(n-4) -7*a(n-6) -5*a(n-7) +3*a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x + 24*x\textasciicircum{}2 - 32*x\textasciicircum{}3 + 17*x\textasciicircum{}4 - 37*x\textasciicircum{}5 - 9*x\textasciicircum{}6 + 9*x\textasciicircum{}7) / (1 - 2*x - 2*x\textasciicircum{}2 - 7*x\textasciicircum{}3 + 3*x\textasciicircum{}4 + 7*x\textasciicircum{}6 + 5*x\textasciicircum{}7 - 3*x\textasciicircum{}8). - Colin Barker, May 12 2018\end{quote}
...
\begin{multline*} a(n) = 2\,a(n-1) + 2\,a(n-2) + 7\,a(n-3) - 3\,a(n-4) - 7\,a(n-6) - 5\,a(n-7) + \\ 3\,a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + x^{2} + 24x^{3} - 32x^{4} + 17x^{5} - 37x^{6} - 9x^{7} + 9x^{8}}{1 - 2x - 2x^{2} - 7x^{3} + 3x^{4} + 7x^{6} + 5x^{7} - 3x^{8}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 2, \tau(1) = 4, \tau(2) = 1, \tau(3) = 0, \tau(4) = 1. The condition is that for every cell,
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $46$; neither removal changes any count.
...
\begin{theorem} For A197368, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) + 7\,a(n-3) - 3\,a(n-4) - 7\,a(n-6) - 5\,a(n-7) + 3\,a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A197368, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + x^{2} + 24x^{3} - 32x^{4} + 17x^{5} - 37x^{6} - 9x^{7} + 9x^{8}}{1 - 2x - 2x^{2} - 7x^{3} + 3x^{4} + 7x^{6} + 5x^{7} - 3x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 32, 45, 189, 646, 1873, 6209,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A197368}{A197368}, revision 9, last modified May 12 2018.
