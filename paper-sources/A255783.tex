...
\title{Proofs of the conjectured column and row recurrences for OEIS A255783}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A255783}{A255783} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 1 and no row sum 0 and no column sum 0\end{quote}
...
k=1: [linear recurrence of order 18]\\k=2: [order 54] for n>57\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 06 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A255783, the recurrence $a(n) = 0$ holds for every $n \ge 19$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A255783, the recurrence $a(n) = 0$ holds for every $n > 57$, and fails at $n = 57$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 63$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 57$; after that point more than $S = 63$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A255783}{A255783}, revision 6, last modified Jun 02 2025.
