...
\title{Proofs of the conjectured column and row recurrences for OEIS A278807}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A278807}{A278807} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with rows in nondecreasing lexicographic order and columns in nonincreasing lexicographic order\end{quote}
...
k=1: a(n) = (1/2)*n\textasciicircum{}2 + (3/2)*n + 1\end{quote}
...
\begin{theorem} For column $k = 1$ of A278807, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $7$ states, $4$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A278807}{A278807}, revision 4, last modified Nov 28 2016.
