...
\title{Proofs of the conjectured column and row recurrences for OEIS A278778}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A278778}{A278778} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with rows in nondecreasing lexicographic order and columns in nonincreasing lexicographic order, but with exactly two mistakes\end{quote}
...
k=1: a(n) = (1/120)*n\textasciicircum{}5 - (1/24)*n\textasciicircum{}4 + (1/24)*n\textasciicircum{}3 + (1/24)*n\textasciicircum{}2 - (1/20)*n\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 28 2016). As of revision 4, last modified Nov 28 2016, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A278778, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $9$ states, $7$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A278778}{A278778}, revision 4, last modified Nov 28 2016.
