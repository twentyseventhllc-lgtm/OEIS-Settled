...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208843}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208843}{A208843} as

\begin{quote}\itshape Number of 6 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 1 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 20*a(n-2) - 19*a(n-3).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 22*x*(1 + 20*x - 19*x\textasciicircum{}2) / (1 - 2*x - 20*x\textasciicircum{}2 + 19*x\textasciicircum{}3). - Colin Barker, Jul 07 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Mar 01 2012).

The line quoted ending in that signature is contributed by Colin Barker on Jul 07 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Jul 07 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 2\,a(n-1) + 20\,a(n-2) - 19\,a(n-3).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{22x + 440x^{2} - 418x^{3}}{1 - 2x - 20x^{2} + 19x^{3}}.\]
...
For scale: at $n = 8$ there are $2^{48}$ arrays, about $10^{14}$, against $4$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A208843, the recurrence $a(n) = 2\,a(n-1) + 20\,a(n-2) - 19\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,32$. Every one of them vanished. After it, more than $S = 4$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A208843, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{22x + 440x^{2} - 418x^{3}}{1 - 2x - 20x^{2} + 19x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 19 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 22, 484, 990, 11242, 33088, 272206, 992574, 6800596,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208843}{A208843}, revision 9, last modified Jul 07 2018.
