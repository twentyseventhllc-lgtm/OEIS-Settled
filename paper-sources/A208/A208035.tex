...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208035}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208035}{A208035} as

\begin{quote}\itshape Number of n X 4 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 0 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 3*a(n-2) + 6*a(n-3) - a(n-4) - a(n-6) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(9 + 63*x + 100*x\textasciicircum{}2 + 86*x\textasciicircum{}3 - 17*x\textasciicircum{}4 - 9*x\textasciicircum{}5 - 16*x\textasciicircum{}6) / ((1 + x + x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - 3*x - x\textasciicircum{}2 - x\textasciicircum{}3)). - Colin Barker, Jun 27 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 11, last modified Jun 27 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 63x^{2} + 100x^{3} + 86x^{4} - 17x^{5} - 9x^{6} - 16x^{7}}{1 - 2x - 3x^{2} - 6x^{3} + x^{4} + x^{6}}.\]
...
\begin{theorem} For A208035, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) + 6\,a(n-3) - a(n-4) - a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,42$. The last nonvanishing one is at $m = 1$, that is at $n = 7$. After it, more than $S = 7$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A208035, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 63x^{2} + 100x^{3} + 86x^{4} - 17x^{5} - 9x^{6} - 16x^{7}}{1 - 2x - 3x^{2} - 6x^{3} + x^{4} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 289, 961, 3249, 11025, 37249, 126025,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208035}{A208035}, revision 11, last modified Jun 27 2018.
