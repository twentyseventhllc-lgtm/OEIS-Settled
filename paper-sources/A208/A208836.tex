...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208836}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208836}{A208836} as

\begin{quote}\itshape Number of n X 4 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 1 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 7*a(n-1) - 8*a(n-2) - 27*a(n-3) + 45*a(n-4) + 24*a(n-5) - 51*a(n-6) - 3*a(n-7) + 16*a(n-8) - a(n-9) - a(n-10) for n>11.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(10 + 30*x - 338*x\textasciicircum{}2 - 49*x\textasciicircum{}3 + 1531*x\textasciicircum{}4 - 114*x\textasciicircum{}5 - 1834*x\textasciicircum{}6 + 200*x\textasciicircum{}7 + 612*x\textasciicircum{}8 - 63*x\textasciicircum{}9 - 41*x\textasciicircum{}10) / ((1 - x)*(1 + x)*(1 - 3*x + x\textasciicircum{}2)*(1 + x - x\textasciicircum{}2)*(1 - x - x\textasciicircum{}2)*(1 - 4*x - x\textasciicircum{}2)). - Colin Barker, Mar 07 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Mar 01 2012).

The line quoted ending in that signature is contributed by Colin Barker on Mar 07 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Mar 07 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 7\,a(n-1) - 8\,a(n-2) - 27\,a(n-3) + 45\,a(n-4) + 24\,a(n-5) - 51\,a(n-6) - \\ 3\,a(n-7) + 16\,a(n-8) - a(n-9) - a(n-10). \end{multline*}

stated for $n > 11$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{10x + 30x^{2} - 338x^{3} - 49x^{4} + 1531x^{5} - 114x^{6} - 1834x^{7} + 200x^{8} + 612x^{9} - 63x^{10} - 41x^{11}}{1 - 7x + 8x^{2} + 27x^{3} - 45x^{4} - 24x^{5} + 51x^{6} + 3x^{7} - 16x^{8} + x^{9} + x^{10}}.\]
...
That refinement leaves $18$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 12$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A208836, the recurrence $a(n) = 7\,a(n-1) - 8\,a(n-2) - 27\,a(n-3) + 45\,a(n-4) + 24\,a(n-5) - 51\,a(n-6) - 3\,a(n-7) + 16\,a(n-8) - a(n-9) - a(n-10)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 1$, that is at $n = 11$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 11$. The theorem is at least as strong.

\begin{theorem} For A208836, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{10x + 30x^{2} - 338x^{3} - 49x^{4} + 1531x^{5} - 114x^{6} - 1834x^{7} + 200x^{8} + 612x^{9} - 63x^{10} - 41x^{11}}{1 - 7x + 8x^{2} + 27x^{3} - 45x^{4} - 24x^{5} + 51x^{6} + 3x^{7} - 16x^{8} + x^{9} + x^{10}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 10, 100, 282, 855, 3010, 11242, 44275, 179032,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208836}{A208836}, revision 8, last modified Mar 07 2018.
