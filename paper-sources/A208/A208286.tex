...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A208286}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208286}{A208286} as

\begin{quote}\itshape Number of n X 7 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (61/180)*n\textasciicircum{}7 + (121/20)*n\textasciicircum{}6 + (1157/36)*n\textasciicircum{}5 + (425/12)*n\textasciicircum{}4 - (2923/90)*n\textasciicircum{}3 - (22/15)*n\textasciicircum{}2 + 2*n.\end{quote}

\begin{quote}\ttfamily G.f.: 2*x*(21 + 714*x + 1005*x\textasciicircum{}2 - 1156*x\textasciicircum{}3 + 203*x\textasciicircum{}4 + 86*x\textasciicircum{}5 - 19*x\textasciicircum{}6) / (1 - x)\textasciicircum{}8.\end{quote}
...
\[a(n) = 2n - \frac{22}{15}n^{2} - \frac{2923}{90}n^{3} + \frac{425}{12}n^{4} + \frac{1157}{36}n^{5} + \frac{121}{20}n^{6} + \frac{61}{180}n^{7}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{42x + 1428x^{2} + 2010x^{3} - 2312x^{4} + 406x^{5} + 172x^{6} - 38x^{7}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}.\]
...
That refinement leaves $41$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 32$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $2^{56}$ arrays, about $10^{17}$, against $32$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A208286, $a(n)$ is the polynomial $2n - \frac{22}{15}n^{2} - \frac{2923}{90}n^{3} + \frac{425}{12}n^{4} + \frac{1157}{36}n^{5} + \frac{121}{20}n^{6} + \frac{61}{180}n^{7}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $7$ is annihilated by the difference operator $(E-1)^{8}$, whose characteristic polynomial is $(t-1)^{8}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 8 = 40$. Its values were computed exactly; all of them vanish, and more than $40$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A208286, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{42x + 1428x^{2} + 2010x^{3} - 2312x^{4} + 406x^{5} + 172x^{6} - 38x^{7}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}$.\end{theorem}
...
\begin{theorem} For A208286, the recurrence $a(n) = 8\,a(n-1) - 28\,a(n-2) + 56\,a(n-3) - 70\,a(n-4) + 56\,a(n-5) - 28\,a(n-6) + 8\,a(n-7) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,69$. Every one of them vanished. After it, more than $S = 32$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 42, 1764, 14946, 70216, 239490, 665892, 1604862, 3478160,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208286}{A208286}, revision 8, last modified Jun 29 2018.
