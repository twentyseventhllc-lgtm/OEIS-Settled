...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208016}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208016}{A208016} as

\begin{quote}\itshape Number of 6 X n 0..1 arrays avoiding 0 0 0 and 1 0 1 horizontally and 0 0 1 and 0 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 3*a(n-2) + 21*a(n-3) + 9*a(n-4) - 27*a(n-5) - 81*a(n-6) for n>8.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(16 + 240*x + 480*x\textasciicircum{}2 + 2601*x\textasciicircum{}3 + 3015*x\textasciicircum{}4 - 3915*x\textasciicircum{}5 - 15228*x\textasciicircum{}6 - 8505*x\textasciicircum{}7) / ((1 + 3*x\textasciicircum{}2 - 9*x\textasciicircum{}3)*(1 - x - 6*x\textasciicircum{}2 - 9*x\textasciicircum{}3)). - Colin Barker, Jun 26 2018\end{quote}
...
\[a(n) = a(n-1) + 3\,a(n-2) + 21\,a(n-3) + 9\,a(n-4) - 27\,a(n-5) - 81\,a(n-6).\]

stated for $n > 8$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + 240x^{2} + 480x^{3} + 2601x^{4} + 3015x^{5} - 3915x^{6} - 15228x^{7} - 8505x^{8}}{1 - x - 3x^{2} - 21x^{3} - 9x^{4} + 27x^{5} + 81x^{6}}.\]
...
That refinement leaves $25$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 10$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A208016, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 21\,a(n-3) + 9\,a(n-4) - 27\,a(n-5) - 81\,a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,44$. The last nonvanishing one is at $m = 2$, that is at $n = 8$. After it, more than $S = 10$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 8$. The theorem is at least as strong.

\begin{theorem} For A208016, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + 240x^{2} + 480x^{3} + 2601x^{4} + 3015x^{5} - 3915x^{6} - 15228x^{7} - 8505x^{8}}{1 - x - 3x^{2} - 21x^{3} - 9x^{4} + 27x^{5} + 81x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 256, 784, 4489, 15376, 43264, 167281, 609961,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208016}{A208016}, revision 8, last modified Jun 26 2018.
