...
k=1: a(n) = a(n-2)\\k=2: a(n) = 2*a(n-1) +8*a(n-3) -8*a(n-4) -8*a(n-5)\\k=3: [order 15] for n>17\\k=4: [order 40] for n>42\end{quote}
...
\begin{theorem} For column $k = 3$ of A300146, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A300146, the recurrence $a(n) = 0$ holds for every $n > 42$, and fails at $n = 42$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 46$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 42$; after that point more than $S = 46$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
