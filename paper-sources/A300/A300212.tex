...
\title{A proof of the conjectured order of the recurrence for OEIS A300212}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300212}{A300212} as

\begin{quote}\itshape Number of n X 5 0..1 arrays with every element equal to 0, 1, 2, 3, 4, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
That refinement leaves $113$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 106$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A300212, the least order of a linear recurrence that $a$ eventually satisfies is exactly $89$, and that recurrence holds for every $n > 89$, the entry's own figure.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 15 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 512, 11965, 298600, 7452385, 186427449, 4666136435, 116802113451,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300212}{A300212}, revision 6, last modified Dec 27 2023.
