...
\title{A proof of the conjectured order of the recurrence for OEIS A300962}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300962}{A300962} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every element equal to 0, 2, 3, 4 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
That refinement leaves $111$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 93$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 16, 44, 327, 1653, 10135, 60730, 366981,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300962}{A300962}, revision 8, last modified Oct 29 2023.
