...
\title{Proofs of the conjectured column and row recurrences for OEIS A300035}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300035}{A300035} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 2, 3, 4, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 5*a(n-2) +8*a(n-3) +4*a(n-4)\\k=3: [order 19] for n>20\\k=4: [order 66] for n>68\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 23 2018). As of revision 4, last modified Feb 23 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
The condition is $c(i,j) \in \{0, 2, 3, 4, 6, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A300035, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300035, the recurrence $a(n) = 5\,a(n-2) + 8\,a(n-3) + 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300035, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 20$; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 20$; the proved threshold is $n > 20$.

\begin{theorem} For column $k = 4$ of A300035, the recurrence $a(n) = 0$ holds for every $n > 68$, and fails at $n = 68$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 74$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 68$; after that point more than $S = 74$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300035}{A300035}, revision 4, last modified Feb 23 2018.
