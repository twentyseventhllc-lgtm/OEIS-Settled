...
\title{Proofs of the conjectured column and row recurrences for OEIS A300888}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300888}{A300888} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 3, 4 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3) -4*a(n-4) +a(n-5) -2*a(n-6) +a(n-7)\\k=3: [order 21] for n>25\\k=4: [order 96] for n>100\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 14 2018). As of revision 4, last modified Mar 14 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A300888, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300888, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) - 4\,a(n-4) + a(n-5) - 2\,a(n-6) + a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $20$ states, $18$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A300888, the recurrence $a(n) = 0$ holds for every $n > 25$, and fails at $n = 25$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 28$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 25$; after that point more than $S = 28$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 25$; the proved threshold is $n > 25$.

\begin{theorem} For column $k = 4$ of A300888, the recurrence $a(n) = 0$ holds for every $n > 100$, and fails at $n = 100$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 112$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 100$; after that point more than $S = 112$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300888}{A300888}, revision 4, last modified Mar 14 2018.
