...
\title{Proofs of the conjectured column and row recurrences for OEIS A300472}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300472}{A300472} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) -a(n-3) -a(n-4)\\k=3: a(n) = 6*a(n-1) -a(n-2) -a(n-3) +a(n-4) -4*a(n-5) +a(n-6) -a(n-7)\\k=4: [order 22]\end{quote}
...
\begin{theorem} For column $k = 1$ of A300472, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300472, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) - a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300472, the recurrence $a(n) = 6\,a(n-1) - a(n-2) - a(n-3) + a(n-4) - 4\,a(n-5) + a(n-6) - a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $62$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A300472, the recurrence $a(n) = 0$ holds for every $n \ge 23$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $224$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300472}{A300472}, revision 4, last modified Mar 06 2018.
