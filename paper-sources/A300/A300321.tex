...
\title{Proofs of the conjectured column and row recurrences for OEIS A300321}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300321}{A300321} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 3, 4, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 2*a(n-1) +2*a(n-2) +6*a(n-3) -10*a(n-4) -8*a(n-5) for n>6\\k=3: [order 15] for n>17\\k=4: [order 40] for n>43\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 02 2018). As of revision 4, last modified Mar 02 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A300321, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300321, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) + 6\,a(n-3) - 10\,a(n-4) - 8\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300321, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A300321, the recurrence $a(n) = 0$ holds for every $n > 43$, and fails at $n = 43$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 50$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 43$; after that point more than $S = 50$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300321}{A300321}, revision 4, last modified Mar 02 2018.
