...
\title{Proofs of the conjectured column and row recurrences for OEIS A300267}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300267}{A300267} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 3, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +2*a(n-3) -6*a(n-4) -4*a(n-5)\\k=3: [order 18] for n>19\\k=4: [order 64] for n>66\end{quote}
...
\begin{theorem} For column $k = 1$ of A300267, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300267, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300267, the recurrence $a(n) = 0$ holds for every $n > 19$, and fails at $n = 19$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A300267, the recurrence $a(n) = 0$ holds for every $n > 66$, and fails at $n = 66$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 67$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 66$; after that point more than $S = 67$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300267}{A300267}, revision 4, last modified Mar 01 2018.
