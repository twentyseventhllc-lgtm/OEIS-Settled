...
k=1: a(n) = 2*a(n-1) -a(n-2) +a(n-3)\\k=2: a(n) = 3*a(n-1) -a(n-2) +6*a(n-3) -6*a(n-4) +6*a(n-5) -5*a(n-6) +3*a(n-7) -a(n-8)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 17 2017). As of revision 4, last modified Dec 17 2017, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
