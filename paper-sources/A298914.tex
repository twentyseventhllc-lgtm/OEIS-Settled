...
\title{A proof of the conjectured linear recurrence for OEIS A298914}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298914}{A298914} as

\begin{quote}\itshape Number of n X 5 0..1 arrays with every element equal to 0, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +a(n-3) +a(n-5) -a(n-6) -a(n-7) -a(n-8) for n>9.\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Jan 29 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 7, last modified Sep 04 2022, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = a(n-1) + a(n-2) + a(n-3) + a(n-5) - a(n-6) - a(n-7) - \\ a(n-8). \end{multline*}
...
The construction gives $1056$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $21$; neither removal changes any count.
...
\begin{theorem} For A298914, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-3) + a(n-5) - a(n-6) - a(n-7) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,61$. The last nonvanishing one is at $m = 1$, that is at $n = 9$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 36 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 3, 4, 7, 9, 15, 26,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298914}{A298914}, revision 7, last modified Sep 04 2022.
