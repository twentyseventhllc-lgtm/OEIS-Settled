...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207719}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207719}{A207719} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) +10*a(n-2) -8*a(n-3) -9*a(n-4) +6*a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 9*x*(1 - x)*(1 + 6*x + 3*x\textasciicircum{}2 - 6*x\textasciicircum{}3) / (1 - 4*x - 10*x\textasciicircum{}2 + 8*x\textasciicircum{}3 + 9*x\textasciicircum{}4 - 6*x\textasciicircum{}5). - Colin Barker, Jun 25 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 45x^{2} - 27x^{3} - 81x^{4} + 54x^{5}}{1 - 4x - 10x^{2} + 8x^{3} + 9x^{4} - 6x^{5}}.\]
...
\begin{theorem} For A207719, the recurrence $a(n) = 4\,a(n-1) + 10\,a(n-2) - 8\,a(n-3) - 9\,a(n-4) + 6\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A207719, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 45x^{2} - 27x^{3} - 81x^{4} + 54x^{5}}{1 - 4x - 10x^{2} + 8x^{3} + 9x^{4} - 6x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 20 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 387, 2205, 12015, 66339, 364869, 2009223,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207719}{A207719}, revision 8, last modified Jun 25 2018.
