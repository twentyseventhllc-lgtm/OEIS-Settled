...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A264423}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A264423}{A264423} as

\begin{quote}\itshape Number of (2+1) X (n+1) arrays of permutations of 0..n*3+2 with each element having directed index change 0,1 2,2 1,0 -1,2 -2,-1 or -1,-1\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + a(n-3) + 5*a(n-4) + a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 3*x\textasciicircum{}2 + x\textasciicircum{}5) / ((1 + x\textasciicircum{}2 + x\textasciicircum{}3)*(1 - 3*x - x\textasciicircum{}2 + x\textasciicircum{}3 - x\textasciicircum{}4)). - Colin Barker, Jan 07 2019\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 13, last modified Feb 18 2021, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 3x^{3} + x^{6}}{1 - 3x - x^{3} - 5x^{4} - x^{7}}.\]
...
The value $v$ has home $(\lfloor v/3\rfloor,\ v \bmod 3)$ in the row-major layout, and the {\itshape index change} of an element is the displacement from its home to the cell it occupies. The entry lists 0,1, 2,2, 1,0, -1,2, -2,-1, -1,-1, so the allowed displacements are
...
\begin{theorem} For A264423, the recurrence $a(n) = 3\,a(n-1) + a(n-3) + 5\,a(n-4) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A264423, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 3x^{3} + x^{6}}{1 - 3x - x^{3} - 5x^{4} - x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 12, 37, 119, 385, 1252, 4061,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A264423}{A264423}, revision 13, last modified Feb 18 2021.
