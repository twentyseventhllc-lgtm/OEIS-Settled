...
\title{Proofs of the conjectured column and row recurrences for OEIS A269937}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A269937}{A269937} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..6 arrays with some element plus some horizontally, diagonally, antidiagonally or vertically adjacent neighbor totalling six exactly once\end{quote}
...
k=1: a(n) = 12*a(n-1) -36*a(n-2)\\k=2: [order 6] for n>7\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 08 2016). As of revision 4, last modified Mar 08 2016, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A269937, the recurrence $a(n) = 12\,a(n-1) - 36\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $22$ states, $15$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A269937, the recurrence $a(n) = 0$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $141$ states, $92$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A269937}{A269937}, revision 4, last modified Mar 08 2016.
