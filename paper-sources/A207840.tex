...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207840}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207840}{A207840} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 1 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 4*a(n-2) + 5*a(n-3) + 2*a(n-4) - a(n-5) + a(n-6) for n>8.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(3 + 15*x + 6*x\textasciicircum{}2 - 3*x\textasciicircum{}3 - 8*x\textasciicircum{}4 - 5*x\textasciicircum{}5 + 3*x\textasciicircum{}6 - 2*x\textasciicircum{}7) / (1 - x - 4*x\textasciicircum{}2 - 5*x\textasciicircum{}3 - 2*x\textasciicircum{}4 + x\textasciicircum{}5 - x\textasciicircum{}6). - Colin Barker, Feb 21 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 30x^{2} + 12x^{3} - 6x^{4} - 16x^{5} - 10x^{6} + 6x^{7} - 4x^{8}}{1 - x - 4x^{2} - 5x^{3} - 2x^{4} + x^{5} - x^{6}}.\]
...
\begin{theorem} For A207840, the recurrence $a(n) = a(n-1) + 4\,a(n-2) + 5\,a(n-3) + 2\,a(n-4) - a(n-5) + a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,45$. The last nonvanishing one is at $m = 2$, that is at $n = 8$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 8$. The theorem is at least as strong.

\begin{theorem} For A207840, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 30x^{2} + 12x^{3} - 6x^{4} - 16x^{5} - 10x^{6} + 6x^{7} - 4x^{8}}{1 - x - 4x^{2} - 5x^{3} - 2x^{4} + x^{5} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 72, 240, 704, 2080, 6216, 18496,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207840}{A207840}, revision 8, last modified Feb 21 2018.
