...
\title{Proofs of the conjectured column and row recurrences for OEIS A306172}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A306172}{A306172} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 3, 5, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +8*a(n-3) -4*a(n-4)\\k=3: a(n) = a(n-1) +4*a(n-3) +2*a(n-5) -a(n-7) -2*a(n-9) -2*a(n-11) -a(n-12)\\k=4: [order 40] for n>41\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jun 23 2018). As of revision 4, last modified Jun 23 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
The condition is $c(i,j) \in \{0, 1, 3, 5, 6, 7\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A306172, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A306172, the recurrence $a(n) = a(n-1) + 8\,a(n-3) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A306172, the recurrence $a(n) = a(n-1) + 4\,a(n-3) + 2\,a(n-5) - a(n-7) - 2\,a(n-9) - 2\,a(n-11) - a(n-12)$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A306172, the recurrence $a(n) = 0$ holds for every $n > 41$, and fails at $n = 41$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $152$ after trimming, and $S = 43$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 41$; after that point more than $S = 43$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A306172}{A306172}, revision 4, last modified Jun 23 2018.
