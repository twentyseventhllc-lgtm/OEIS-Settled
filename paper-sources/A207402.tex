...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207402}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207402}{A207402} as

\begin{quote}\itshape Number of n X 7 0..1 arrays avoiding 0 0 1 and 0 1 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (1/36)*n\textasciicircum{}7 + (4/9)*n\textasciicircum{}6 + (53/18)*n\textasciicircum{}5 + (91/9)*n\textasciicircum{}4 + (589/36)*n\textasciicircum{}3 + (31/9)*n\textasciicircum{}2 - (58/3)*n + 6.\end{quote}

\begin{quote}\ttfamily G.f.: 2*x*(10 + 120*x - 150*x\textasciicircum{}2 + 135*x\textasciicircum{}3 - 42*x\textasciicircum{}4 - 14*x\textasciicircum{}5 + 14*x\textasciicircum{}6 - 3*x\textasciicircum{}7) / (1 - x)\textasciicircum{}8.\end{quote}
...
\[a(n) = 6 - \frac{58}{3}n + \frac{31}{9}n^{2} + \frac{589}{36}n^{3} + \frac{91}{9}n^{4} + \frac{53}{18}n^{5} + \frac{4}{9}n^{6} + \frac{1}{36}n^{7}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{20x + 240x^{2} - 300x^{3} + 270x^{4} - 84x^{5} - 28x^{6} + 28x^{7} - 6x^{8}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}.\]
...
That refinement leaves $19$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 15$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $2^{56}$ arrays, about $10^{17}$, against $15$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A207402, $a(n)$ is the polynomial $6 - \frac{58}{3}n + \frac{31}{9}n^{2} + \frac{589}{36}n^{3} + \frac{91}{9}n^{4} + \frac{53}{18}n^{5} + \frac{4}{9}n^{6} + \frac{1}{36}n^{7}$ for every $n \ge 1$.\end{theorem}
...
\begin{theorem} For A207402, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{20x + 240x^{2} - 300x^{3} + 270x^{4} - 84x^{5} - 28x^{6} + 28x^{7} - 6x^{8}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}$.\end{theorem}
...
\begin{theorem} For A207402, the recurrence $a(n) = 8\,a(n-1) - 28\,a(n-2) + 56\,a(n-3) - 70\,a(n-4) + 56\,a(n-5) - 28\,a(n-6) + 8\,a(n-7) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. Every one of them vanished. After it, more than $S = 15$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 20, 400, 2340, 8910, 26676, 68060, 154580, 321110,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207402}{A207402}, revision 8, last modified Jun 22 2018.
