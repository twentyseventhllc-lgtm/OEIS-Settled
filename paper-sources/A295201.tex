...
\title{A proof of the conjectured linear recurrence for OEIS A295201}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295201}{A295201} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with each 1 horizontally or vertically adjacent to 2 or 4 1s\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) -2*a(n-2) +2*a(n-3) +5*a(n-4) -a(n-5) -3*a(n-6) -a(n-7) -6*a(n-8) -10*a(n-9) -a(n-10) +a(n-11) +a(n-12) +a(n-13).\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Nov 16 2017).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 11, last modified Mar 14 2026, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 3\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) + 5\,a(n-4) - a(n-5) - 3\,a(n-6) - \\ a(n-7) - 6\,a(n-8) - 10\,a(n-9) - a(n-10) + a(n-11) + a(n-12) + \\ a(n-13). \end{multline*}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $69$; neither removal changes any count.
...
\begin{theorem} For A295201, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) + 5\,a(n-4) - a(n-5) - 3\,a(n-6) - a(n-7) - 6\,a(n-8) - 10\,a(n-9) - a(n-10) + a(n-11) + a(n-12) + a(n-13)$ holds for every $n \ge 14$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,63$. Every one of them vanished. After it, more than $S = 14$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 4, 14, 42, 108, 284, 777, 2146,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295201}{A295201}, revision 11, last modified Mar 14 2026.
