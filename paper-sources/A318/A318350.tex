...
\title{Proofs of the conjectured column and row recurrences for OEIS A318350}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318350}{A318350} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 3 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +2*a(n-3) -a(n-6)\\k=3: [order 19] for n>25\\k=4: [order 37] for n>45\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 24 2018). As of revision 4, last modified Aug 24 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A318350, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318350, the recurrence $a(n) = a(n-1) + 2\,a(n-3) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A318350, the recurrence $a(n) = 0$ holds for every $n > 25$, and fails at $n = 25$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $50$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 25$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 25$; the proved threshold is $n > 25$.

\begin{theorem} For column $k = 4$ of A318350, the recurrence $a(n) = 0$ holds for every $n > 45$, and fails at $n = 45$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $135$ after trimming, and $S = 50$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 45$; after that point more than $S = 50$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318350}{A318350}, revision 4, last modified Aug 24 2018.
