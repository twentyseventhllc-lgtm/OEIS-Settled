...
\title{Proofs of the conjectured column and row recurrences for OEIS A318343}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318343}{A318343} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) -a(n-3) -a(n-4) for n>6\\k=3: a(n) = 2*a(n-1) +2*a(n-2) -3*a(n-3) -2*a(n-4) +2*a(n-5) for n>6\\k=4: [order 8] for n>9\\k=5: [order 15] for n>18\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 24 2018). As of revision 4, last modified Aug 24 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A318343, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318343, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - a(n-3) - a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A318343, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) - 3\,a(n-3) - 2\,a(n-4) + 2\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $28$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 4$ of A318343, the recurrence $a(n) = 0$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $57$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For column $k = 5$ of A318343, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $138$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318343}{A318343}, revision 4, last modified Aug 24 2018.
