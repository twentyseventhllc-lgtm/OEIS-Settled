...
\title{Proofs of the conjectured column and row recurrences for OEIS A304669}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304669}{A304669} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 3, 5 or 6 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +8*a(n-3) -4*a(n-4)\\k=3: a(n) = a(n-1) +4*a(n-3) +2*a(n-5) -a(n-7) -2*a(n-9) -2*a(n-11) -a(n-12)\\k=4: [order 42] for n>43\end{quote}
...
\begin{theorem} For column $k = 1$ of A304669, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A304669, the recurrence $a(n) = a(n-1) + 8\,a(n-3) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A304669, the recurrence $a(n) = a(n-1) + 4\,a(n-3) + 2\,a(n-5) - a(n-7) - 2\,a(n-9) - 2\,a(n-11) - a(n-12)$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A304669, the recurrence $a(n) = 0$ holds for every $n > 43$, and fails at $n = 43$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $152$ after trimming, and $S = 44$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 43$; after that point more than $S = 44$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304669}{A304669}, revision 4, last modified May 16 2018.
