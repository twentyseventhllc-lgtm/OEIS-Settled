...
\title{Proofs of the conjectured column and row recurrences for OEIS A230176}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A230176}{A230176} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays x(i,j) with each element horizontally, vertically, diagonally or antidiagonally next to at least one element with value 2-x(i,j)\end{quote}
...
k=1: a(n) = a(n-1) +2*a(n-2)\\k=2: a(n) = 6*a(n-1) -2*a(n-2) +67*a(n-3) +62*a(n-4) +160*a(n-5) +40*a(n-6) +16*a(n-7)\\k=3: [order 26] for n>27\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 11 2013). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A230176, the recurrence $a(n) = a(n-1) + 2\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A230176, the recurrence $a(n) = 6\,a(n-1) - 2\,a(n-2) + 67\,a(n-3) + 62\,a(n-4) + 160\,a(n-5) + 40\,a(n-6) + 16\,a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $91$ states, $91$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A230176, the recurrence $a(n) = 0$ holds for every $n > 27$, and fails at $n = 27$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $757$ states, $757$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 27$; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A230176}{A230176}, revision 6, last modified Jun 02 2025.
