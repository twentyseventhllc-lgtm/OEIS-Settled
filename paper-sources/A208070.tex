...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208070}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208070}{A208070} as

\begin{quote}\itshape Number of 5 X n 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 0 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 4*a(n-2) + 19*a(n-3) + 10*a(n-4) + 5*a(n-5) - 27*a(n-6) - 2*a(n-7) - 4*a(n-8) + 8*a(n-9) for n>11.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 12*x*(1 + 11*x + 2*x\textasciicircum{}2 - x\textasciicircum{}3 - 86*x\textasciicircum{}4 - 31*x\textasciicircum{}5 - 51*x\textasciicircum{}6 + 114*x\textasciicircum{}7 + 4*x\textasciicircum{}8 + 24*x\textasciicircum{}9 - 32*x\textasciicircum{}10) / ((1 - 2*x - 4*x\textasciicircum{}2 - 8*x\textasciicircum{}3)*(1 + x + 2*x\textasciicircum{}2 - 3*x\textasciicircum{}3 - x\textasciicircum{}5 + x\textasciicircum{}6)). - Colin Barker, Jun 27 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 23 2012).

The line quoted ending in that signature is contributed by Colin Barker on Jun 27 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Jun 27 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = a(n-1) + 4\,a(n-2) + 19\,a(n-3) + 10\,a(n-4) + 5\,a(n-5) - 27\,a(n-6) - \\ 2\,a(n-7) - 4\,a(n-8) + 8\,a(n-9). \end{multline*}

stated for $n > 11$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{12x + 132x^{2} + 24x^{3} - 12x^{4} - 1032x^{5} - 372x^{6} - 612x^{7} + 1368x^{8} + 48x^{9} + 288x^{10} - 384x^{11}}{1 - x - 4x^{2} - 19x^{3} - 10x^{4} - 5x^{5} + 27x^{6} + 2x^{7} + 4x^{8} - 8x^{9}}.\]
...
The construction gives $1056$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $156$; neither removal changes any count.
...
For scale: at $n = 8$ there are $2^{40}$ arrays, about $10^{12}$, against $12$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A208070, the recurrence $a(n) = a(n-1) + 4\,a(n-2) + 19\,a(n-3) + 10\,a(n-4) + 5\,a(n-5) - 27\,a(n-6) - 2\,a(n-7) - 4\,a(n-8) + 8\,a(n-9)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 2$, that is at $n = 11$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 11$. The theorem is at least as strong.

\begin{theorem} For A208070, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{12x + 132x^{2} + 24x^{3} - 12x^{4} - 1032x^{5} - 372x^{6} - 612x^{7} + 1368x^{8} + 48x^{9} + 288x^{10} - 384x^{11}}{1 - x - 4x^{2} - 19x^{3} - 10x^{4} - 5x^{5} + 27x^{6} + 2x^{7} + 4x^{8} - 8x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 12, 144, 216, 1008, 3696, 12960, 48840, 179520,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208070}{A208070}, revision 8, last modified Jun 27 2018.
