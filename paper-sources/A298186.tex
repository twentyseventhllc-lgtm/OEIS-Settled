...
\title{A proof of the conjectured linear recurrence for OEIS A298186}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298186}{A298186} as

\begin{quote}\itshape Number of nX7 0..1 arrays with every element equal to 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) -2*a(n-2) -a(n-3) +2*a(n-4) -2*a(n-5) +a(n-6) -a(n-7) +a(n-8) -a(n-10) +a(n-11) for n>12\end{quote}
...
\begin{multline*} a(n) = 3\,a(n-1) - 2\,a(n-2) - a(n-3) + 2\,a(n-4) - 2\,a(n-5) + a(n-6) - \\ a(n-7) + a(n-8) - a(n-10) + a(n-11). \end{multline*}
...
The construction gives $16512$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $43$; neither removal changes any count.
...
For scale: at $n = 8$ there are $2^{56}$ arrays, about $10^{17}$, against $12$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A298186, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) - a(n-3) + 2\,a(n-4) - 2\,a(n-5) + a(n-6) - a(n-7) + a(n-8) - a(n-10) + a(n-11)$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,67$. The last nonvanishing one is at $m = 1$, that is at $n = 12$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 38 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 8, 8, 9, 10, 14, 19, 27,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298186}{A298186}, revision 4, last modified Jan 14 2018.
