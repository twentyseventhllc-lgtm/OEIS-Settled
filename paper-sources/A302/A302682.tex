...
\title{A proof of the conjectured linear recurrence for OEIS A302682}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302682}{A302682} as

\begin{quote}\itshape Number of 4Xn 0..1 arrays with every element equal to 0, 1, 4 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +5*a(n-3) +5*a(n-4) -3*a(n-5) -3*a(n-6) +2*a(n-7) for n>11\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Apr 11 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 7, last modified Jul 11 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = a(n-1) + a(n-2) + 5\,a(n-3) + 5\,a(n-4) - 3\,a(n-5) - 3\,a(n-6) + \\ 2\,a(n-7). \end{multline*}
...
\begin{theorem} For A302682, the recurrence $a(n) = a(n-1) + a(n-2) + 5\,a(n-3) + 5\,a(n-4) - 3\,a(n-5) - 3\,a(n-6) + 2\,a(n-7)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. The last nonvanishing one is at $m = 4$, that is at $n = 11$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 6, 7, 20, 30, 76, 217, 509,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302682}{A302682}, revision 7, last modified Jul 11 2018.
