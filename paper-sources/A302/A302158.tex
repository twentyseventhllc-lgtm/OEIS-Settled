...
\title{A proof of the conjectured linear recurrence for OEIS A302158}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302158}{A302158} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element equal to 0, 1 or 3 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +8*a(n-3) -6*a(n-4) -4*a(n-6) +4*a(n-7) for n>11\end{quote}
...
\begin{theorem} For A302158, the recurrence $a(n) = a(n-1) + 8\,a(n-3) - 6\,a(n-4) - 4\,a(n-6) + 4\,a(n-7)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,56$. The last nonvanishing one is at $m = 4$, that is at $n = 11$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 9, 7, 17, 31, 57, 111, 265,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302158}{A302158}, revision 4, last modified Apr 02 2018.
