...
\title{Proofs of the conjectured column and row recurrences for OEIS A302877}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302877}{A302877} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 3, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) -a(n-3) -2*a(n-4) +a(n-5)\\k=3: [order 13] for n>16\\k=4: [order 56] for n>59\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = a(n-1) +a(n-2)\\n=2: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3) +12*a(n-4) -16*a(n-5) for n>6\\n=3: [order 20] for n>21\\n=4: [order 60] for n>64\end{quote}
...
\begin{theorem} For column $k = 1$ of A302877, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A302877, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - a(n-3) - 2\,a(n-4) + a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A302877, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A302877, the recurrence $a(n) = 0$ holds for every $n > 59$, and fails at $n = 59$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $262$ after trimming, and $S = 61$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 59$; after that point more than $S = 61$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 59$; the proved threshold is $n > 59$.

\begin{theorem} For row $n = 1$ of A302877, the recurrence $a(k) = a(n-1) + a(n-2)$ holds for every $k \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For row $n = 2$ of A302877, the recurrence $a(k) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) + 12\,a(n-4) - 16\,a(n-5)$ holds for every $k > 6$, and fails at $k = 6$.\end{theorem}
...
\begin{theorem} For row $n = 3$ of A302877, the recurrence $a(k) = 0$ holds for every $k > 21$, and fails at $k = 21$.\end{theorem}
...
\begin{theorem} For row $n = 4$ of A302877, the recurrence $a(k) = 0$ holds for every $k > 64$, and fails at $k = 64$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 74$ blocks after refinement. The model reproduces every published value of row $n = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 64$; after that point more than $S = 74$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302877}{A302877}, revision 4, last modified Apr 15 2018.
