...
\title{Proofs of the conjectured column and row recurrences for OEIS A302528}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302528}{A302528} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 09 2018). As of revision 4, last modified Apr 09 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences and the 4 row recurrences listed above.
...
\begin{theorem} In A302528, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A302528, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + 2\,a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A302528, the recurrence $a(n) = 0$ holds for every $n \ge 19$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A302528, the recurrence $a(n) = 0$ holds for every $n \ge 73$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For row $n = 1$ of A302528, the recurrence $a(k) = a(n-1) + a(n-2)$ holds for every $k \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For row $n = 2$ of A302528, the recurrence $a(k) = a(n-1) + 3\,a(n-2) - 4\,a(n-4)$ holds for every $k > 5$, and fails at $k = 5$.\end{theorem}
...
\begin{theorem} For row $n = 3$ of A302528, the recurrence $a(k) = 0$ holds for every $k > 17$, and fails at $k = 17$.\end{theorem}
...
\begin{theorem} For row $n = 4$ of A302528, the recurrence $a(k) = 0$ holds for every $k > 69$, and fails at $k = 69$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 74$ blocks after refinement. The model reproduces every published value of row $n = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 69$; after that point more than $S = 74$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302528}{A302528}, revision 4, last modified Apr 09 2018.
