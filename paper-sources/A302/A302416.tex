...
\title{A proof of the conjectured linear recurrence for OEIS A302416}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302416}{A302416} as

\begin{quote}\itshape Number of 3Xn 0..1 arrays with every element equal to 0, 1, 2 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) -3*a(n-2) +4*a(n-3) -4*a(n-4) +a(n-5) for n>9\end{quote}
...
\[a(n) = 3\,a(n-1) - 3\,a(n-2) + 4\,a(n-3) - 4\,a(n-4) + a(n-5).\]
...
\begin{theorem} For A302416, the recurrence $a(n) = 3\,a(n-1) - 3\,a(n-2) + 4\,a(n-3) - 4\,a(n-4) + a(n-5)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,51$. The last nonvanishing one is at $m = 4$, that is at $n = 9$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 25, 30, 66, 142, 289, 599, 1268,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302416}{A302416}, revision 4, last modified Apr 07 2018.
