...
\title{Proofs of the conjectured column and row recurrences for OEIS A317815}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317815}{A317815} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 2 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) +a(n-3) -a(n-4) -a(n-5) -a(n-6)\\k=3: [order 17] for n>19\\k=4: [order 48] for n>52\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 07 2018). As of revision 4, last modified Aug 07 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A317815, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317815, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + a(n-3) - a(n-4) - a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317815, the recurrence $a(n) = 0$ holds for every $n > 19$, and fails at $n = 19$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A317815, the recurrence $a(n) = 0$ holds for every $n > 52$, and fails at $n = 52$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $132$ after trimming, and $S = 58$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 52$; after that point more than $S = 58$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 52$; the proved threshold is $n > 52$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317815}{A317815}, revision 4, last modified Aug 07 2018.
