...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = a(n-1) +a(n-2) for n>4\\k=3: a(n) = a(n-1) +a(n-2) +2*a(n-3) for n>6\\k=4: a(n) = a(n-1) +2*a(n-2) +5*a(n-3) +a(n-4) -2*a(n-5) -6*a(n-6) -4*a(n-7) for n>11\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 10 2018). As of revision 4, last modified Jul 10 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
