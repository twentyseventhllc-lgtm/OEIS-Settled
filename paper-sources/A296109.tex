...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296109}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296109}{A296109} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 3 or 4 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-2) + 5*a(n-3) - 2*a(n-4) - 6*a(n-5) - 4*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x - x\textasciicircum{}2 - 8*x\textasciicircum{}3 - 10*x\textasciicircum{}4 - 4*x\textasciicircum{}5) / (1 - x - 2*x\textasciicircum{}2 - 5*x\textasciicircum{}3 + 2*x\textasciicircum{}4 + 6*x\textasciicircum{}5 + 4*x\textasciicircum{}6). - Colin Barker, Mar 22 2018\end{quote}
...
\[a(n) = a(n-1) + 2\,a(n-2) + 5\,a(n-3) - 2\,a(n-4) - 6\,a(n-5) - 4\,a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + x^{2} - x^{3} - 8x^{4} - 10x^{5} - 4x^{6}}{1 - x - 2x^{2} - 5x^{3} + 2x^{4} + 6x^{5} + 4x^{6}}.\]
...
\begin{theorem} For A296109, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + 5\,a(n-3) - 2\,a(n-4) - 6\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296109, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + x^{2} - x^{3} - 8x^{4} - 10x^{5} - 4x^{6}}{1 - x - 2x^{2} - 5x^{3} + 2x^{4} + 6x^{5} + 4x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 2, 3, 4, 8, 17, 31, 71,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296109}{A296109}, revision 9, last modified Mar 22 2018.
