...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A261374}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A261374}{A261374} as

\begin{quote}\itshape Number of (n+2) X (2+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00001011 00010101 or 01010101\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-2) + 6*a(n-4) + 4*a(n-6) - a(n-8) for n > 9.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(39 + 41*x + 43*x\textasciicircum{}2 + 116*x\textasciicircum{}3 - 50*x\textasciicircum{}4 + 67*x\textasciicircum{}5 - 50*x\textasciicircum{}6 - 23*x\textasciicircum{}7 + 9*x\textasciicircum{}8) / ((1 + x + 2*x\textasciicircum{}3 - x\textasciicircum{}4)*(1 - x - 2*x\textasciicircum{}3 - x\textasciicircum{}4)). - Colin Barker, Dec 30 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 11, last modified Dec 30 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{39x + 41x^{2} + 43x^{3} + 116x^{4} - 50x^{5} + 67x^{6} - 50x^{7} - 23x^{8} + 9x^{9}}{1 - x^{2} - 6x^{4} - 4x^{6} + x^{8}}.\]
...
\begin{theorem} For A261374, the recurrence $a(n) = a(n-2) + 6\,a(n-4) + 4\,a(n-6) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,58$. The last nonvanishing one is at $m = 1$, that is at $n = 9$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A261374, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{39x + 41x^{2} + 43x^{3} + 116x^{4} - 50x^{5} + 67x^{6} - 50x^{7} - 23x^{8} + 9x^{9}}{1 - x^{2} - 6x^{4} - 4x^{6} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 39, 41, 82, 157, 266, 470, 864, 1553,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A261374}{A261374}, revision 11, last modified Dec 30 2018.
