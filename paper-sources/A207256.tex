...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207256}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207256}{A207256} as

\begin{quote}\itshape Number of 5 X n 0..1 arrays avoiding 0 0 1 and 1 0 1 horizontally and 0 1 0 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (4/15)*n\textasciicircum{}5 + (32/3)*n\textasciicircum{}4 + (44/3)*n\textasciicircum{}3 - (32/3)*n\textasciicircum{}2 + (16/15)*n.\end{quote}

\begin{quote}\ttfamily G.f.: 16*x*(1 + 10*x - 4*x\textasciicircum{}2 - 6*x\textasciicircum{}3 + x\textasciicircum{}4) / (1 - x)\textasciicircum{}6.\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Jun 21 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = \frac{16}{15}n - \frac{32}{3}n^{2} + \frac{44}{3}n^{3} + \frac{32}{3}n^{4} + \frac{4}{15}n^{5}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + 160x^{2} - 64x^{3} - 96x^{4} + 16x^{5}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}.\]
...
That refinement leaves $8$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 7$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A207256, $a(n)$ is the polynomial $\frac{16}{15}n - \frac{32}{3}n^{2} + \frac{44}{3}n^{3} + \frac{32}{3}n^{4} + \frac{4}{15}n^{5}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $5$ is annihilated by the difference operator $(E-1)^{6}$, whose characteristic polynomial is $(t-1)^{6}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 6 = 13$. Its values were computed exactly; all of them vanish, and more than $13$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A207256, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + 160x^{2} - 64x^{3} - 96x^{4} + 16x^{5}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}$.\end{theorem}

{\itshape Proof.} Let $N(x)$ and $D(x)$ be the numerator and denominator above, $D(0)=1$. The residual test applied to the recurrence read off $D$ --- namely $a(n) = \sum_i c_i a(n-i)$ with $c_i$ the negated coefficients of $D$ --- gives vanishing residuals throughout, so the power series $F(x)=\sum_{n\ge 1}a(n)x^n$ satisfies $[x^k]\bigl(F(x)D(x)\bigr) = 0$ for every $k > 6$. The remaining coefficients of $F(x)D(x)$, for $k \le 15$, were computed from the walk count and agree with $N(x)$ term by term. Hence $F(x)D(x) = N(x)$ identically, which is the assertion. $\square$

\begin{theorem} For A207256, the recurrence $a(n) = 6\,a(n-1) - 15\,a(n-2) + 20\,a(n-3) - 15\,a(n-4) + 6\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,48$. Every one of them vanished. After it, more than $S = 7$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 256, 1232, 3776, 9072, 18688, 34608, 59264,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207256}{A207256}, revision 8, last modified Jun 21 2018.
