...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) +2*a(n-3)\\k=3: a(n) = 6*a(n-1) +9*a(n-2) +8*a(n-3) +a(n-4) -5*a(n-5) -2*a(n-6) -a(n-7) -a(n-8)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 20 2018). As of revision 4, last modified Jul 20 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
