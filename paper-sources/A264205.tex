...
\title{A proof of the conjectured order of the recurrence for OEIS A264205}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A264205}{A264205} as

\begin{quote}\itshape Number of (n+1)X(5+1) arrays of permutations of 0..n*6+5 with each element having index change +-(.,.) 0,0 0,2 or 1,0\end{quote}
...
\begin{theorem} For A264205, the least order of a linear recurrence that $a$ eventually satisfies is exactly $51$, and that recurrence holds for every $n > 51$, the entry's own figure.\end{theorem}

{\itshape Proof.} The count is C-finite of order at most $S = 55$, so the minimal annihilator of the tail $(a(n))_{n \ge M}$ is the same for every $M$ beyond $S$: the nilpotent part of $L$ is killed after $S$ steps. Taking such an $M$ and $116$ consecutive terms from there, the Berlekamp--Massey algorithm over $\mathbb{Q}$ returns the minimal linear recurrence generating them; since the sequence satisfies one of order at most $S$, $2S$ terms determine it. Its order is $51$. That recurrence was then put through the residual test of Section 5: every residual vanishes, followed by more than $S = 55$ consecutive vanishing ones, so it holds for every larger $n$. No recurrence of smaller order can hold eventually, since the tail's minimal annihilator is what the algorithm returned. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 13 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1024, 52441, 3404025, 205062400, 12652875225, 774686906569, 47551947307264, 2916406634175225,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A264205}{A264205}, revision 6, last modified Nov 10 2015.
