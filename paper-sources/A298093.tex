...
\title{Proofs of the conjectured column and row recurrences for OEIS A298093}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298093}{A298093} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 4 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) -2*a(n-2) +4*a(n-3) -10*a(n-4) +4*a(n-5) for n>6\\k=3: a(n) = 2*a(n-1) -a(n-4) -a(n-5) -a(n-6) +a(n-7) +a(n-8) for n>9\\k=4: [order 35] for n>40\\k=5: [order 82] for n>86\end{quote}
...
\begin{theorem} For column $k = 1$ of A298093, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298093, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 4\,a(n-3) - 10\,a(n-4) + 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298093, the recurrence $a(n) = 2\,a(n-1) - a(n-4) - a(n-5) - a(n-6) + a(n-7) + a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $49$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For column $k = 4$ of A298093, the recurrence $a(n) = 0$ holds for every $n > 40$, and fails at $n = 40$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $168$ after trimming, and $S = 44$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 40$; after that point more than $S = 44$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 40$; the proved threshold is $n > 40$.

\begin{theorem} For column $k = 5$ of A298093, the recurrence $a(n) = 0$ holds for every $n > 86$, and fails at $n = 86$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $438$ after trimming, and $S = 98$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 86$; after that point more than $S = 98$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298093}{A298093}, revision 4, last modified Jan 12 2018.
