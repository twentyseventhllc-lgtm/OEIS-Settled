...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A224137}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224137}{A224137} as

\begin{quote}\itshape Number of 6 X n 0..1 arrays with rows nondecreasing and antidiagonals unimodal\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (2/45)*n\textasciicircum{}6 + (8/15)*n\textasciicircum{}5 + (181/36)*n\textasciicircum{}4 + 28*n\textasciicircum{}3 + (19247/180)*n\textasciicircum{}2 + (3952/15)*n - 120 for n>4.\end{quote}

\begin{quote}\ttfamily G.f.: x*(64 + 281*x - 1067*x\textasciicircum{}2 + 617*x\textasciicircum{}3 + 1387*x\textasciicircum{}4 - 1840*x\textasciicircum{}5 + 160*x\textasciicircum{}6 + 696*x\textasciicircum{}7 - 172*x\textasciicircum{}8 - 154*x\textasciicircum{}9 + 60*x\textasciicircum{}10) / (1 - x)\textasciicircum{}7.\end{quote}
...
\[a(n) = -120 + \frac{3952}{15}n + \frac{19247}{180}n^{2} + 28n^{3} + \frac{181}{36}n^{4} + \frac{8}{15}n^{5} + \frac{2}{45}n^{6}.\]

stated for $n > 4$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{64x + 281x^{2} - 1067x^{3} + 617x^{4} + 1387x^{5} - 1840x^{6} + 160x^{7} + 696x^{8} - 172x^{9} - 154x^{10} + 60x^{11}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}.\]
...
The reachable part of that digraph has $234$ states, $234$ after discarding states from which no accepting state is reachable. States from which the same number of arrays can be completed, for every remaining number of rows, are then identified by partition refinement --- start from the partition by acceptance and separate two states as soon as they send different numbers of edges into some block. On the blocks the number of completions of each length is constant, so the quotient counts exactly what the original does.

The refinement, applied first forwards and then in the mirror direction on the edges coming in, leaves $S = 47$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of the $47 \times 47$ quotient matrix. That degree bound is computed here, not assumed, and it is the only one the decision procedure below uses.
...
\begin{theorem} For A224137, $a(n)$ is the polynomial $-120 + \frac{3952}{15}n + \frac{19247}{180}n^{2} + 28n^{3} + \frac{181}{36}n^{4} + \frac{8}{15}n^{5} + \frac{2}{45}n^{6}$ for every $n > 4$, and not at $n = 4$.\end{theorem}

{\itshape Proof.} A polynomial of degree $6$ is annihilated by the difference operator $(E-1)^{7}$, whose characteristic polynomial is $(t-1)^{7}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 7 = 54$. Its values were computed exactly; the last nonvanishing one is at $n = 4$, and more than $54$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A224137, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{64x + 281x^{2} - 1067x^{3} + 617x^{4} + 1387x^{5} - 1840x^{6} + 160x^{7} + 696x^{8} - 172x^{9} - 154x^{10} + 60x^{11}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}$.\end{theorem}
...
\begin{theorem} For A224137, the recurrence $a(n) = 7\,a(n-1) - 21\,a(n-2) + 35\,a(n-3) - 35\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + a(n-7)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,89$. The last nonvanishing one is at $m = 4$, that is at $n = 11$. After it, more than $S = 47$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 64, 729, 2692, 6392, 12874, 24095, 42832, 72888,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224137}{A224137}, revision 9, last modified Aug 27 2018.
