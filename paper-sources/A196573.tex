...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196573}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196573}{A196573} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,2,3,0,4 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +2*a(n-2) +3*a(n-3) +5*a(n-4) -a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x + 8*x\textasciicircum{}2 - 4*x\textasciicircum{}3 - 3*x\textasciicircum{}4 + x\textasciicircum{}5) / ((1 + x)*(1 - 3*x + x\textasciicircum{}2 - 4*x\textasciicircum{}3 - x\textasciicircum{}4 + x\textasciicircum{}5)). - Colin Barker, May 09 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + x^{2} + 8x^{3} - 4x^{4} - 3x^{5} + x^{6}}{1 - 2x - 2x^{2} - 3x^{3} - 5x^{4} + x^{6}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 1, \tau(1) = 2, \tau(2) = 3, \tau(3) = 0, \tau(4) = 4. The condition is that for every cell,
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $22$; neither removal changes any count.
...
\begin{theorem} For A196573, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) + 3\,a(n-3) + 5\,a(n-4) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196573, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + x^{2} + 8x^{3} - 4x^{4} - 3x^{5} + x^{6}}{1 - 2x - 2x^{2} - 3x^{3} - 5x^{4} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 16, 37, 117, 372, 1168, 3613,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196573}{A196573}, revision 8, last modified May 09 2018.
