...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A186454}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A186454}{A186454} as

\begin{quote}\itshape Number of (n+1) X 2 binary arrays with every 2 X 2 subblock commuting with each of its horizontal and vertical 2 X 2 subblock neighbors\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: x*(16 - 11*x + 9*x\textasciicircum{}3 - 6*x\textasciicircum{}4 - 3*x\textasciicircum{}5 + 2*x\textasciicircum{}6 + x\textasciicircum{}7) / ((1 - x)*(1 - x - x\textasciicircum{}2 + x\textasciicircum{}3 - x\textasciicircum{}4)). - Colin Barker, Feb 28 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x - 11x^{2} + 9x^{4} - 6x^{5} - 3x^{6} + 2x^{7} + x^{8}}{1 - 2x + 2x^{3} - 2x^{4} + x^{5}}.\]

\section{The object}

The name is read as follows. The reading is not a guess: it is pinned against the entry's own published terms, and against an independent enumeration, before anything is proved (Section 7).

Let $A$ be an $n' \times 2$ array over $\{0,\dots,1\}$ with $n' = n + 1$ rows.
...
The reachable part of the digraph has $21$ states, $21$ after trimming; identifying states with equal numbers of completions of every length, and then the mirror identification on the edges coming in, leaves $S = 11$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of that $11\times11$ matrix; the bound is computed, not assumed.
...
\begin{theorem} For A186454, the recurrence $a(n) = 2\,a(n-1) - 2\,a(n-3) + 2\,a(n-4) - a(n-5)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,57$. The last nonvanishing one is at $m = 3$, that is at $n = 8$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 8$. The theorem is at least as strong.

\begin{theorem} For A186454, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x - 11x^{2} + 9x^{4} - 6x^{5} - 3x^{6} + 2x^{7} + x^{8}}{1 - 2x + 2x^{3} - 2x^{4} + x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 35 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 21, 42, 61, 106, 151, 245, 359,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A186454}{A186454}, revision 8, last modified Feb 28 2018.
