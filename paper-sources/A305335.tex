...
\title{A proof of the conjectured linear recurrence for OEIS A305335}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305335}{A305335} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 2, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-2) +3*a(n-3) +2*a(n-4) -3*a(n-5) -8*a(n-6) -4*a(n-7) for n>8\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 3\,a(n-2) + 3\,a(n-3) + 2\,a(n-4) - 3\,a(n-5) - 8\,a(n-6) - \\ 4\,a(n-7). \end{multline*}
...
\begin{theorem} For A305335, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 3\,a(n-3) + 2\,a(n-4) - 3\,a(n-5) - 8\,a(n-6) - 4\,a(n-7)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,51$. The last nonvanishing one is at $m = 1$, that is at $n = 8$. After it, more than $S = 10$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 5, 12, 26, 62, 166, 431, 1091,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305335}{A305335}, revision 4, last modified May 30 2018.
