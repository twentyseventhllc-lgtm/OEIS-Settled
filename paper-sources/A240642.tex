...
\title{Proofs of the conjectured column and row recurrences for OEIS A240642}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A240642}{A240642} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays with no element equal to one or three horizontal or vertical neighbors, with new values 0..3 introduced in row major order\end{quote}
...
k=1: a(n) = 4*a(n-1) -3*a(n-2) for n>3\\k=2: a(n) = 6*a(n-1) +10*a(n-2) -6*a(n-3) -9*a(n-4) for n>6\\k=3: a(n) = 14*a(n-1) +54*a(n-2) +82*a(n-3) -14*a(n-4) -54*a(n-5) -81*a(n-6)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 09 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A240642, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $32$ states, $22$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 3$; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 3$; the proved threshold is $n > 3$.

\begin{theorem} For column $k = 2$ of A240642, the recurrence $a(n) = 6\,a(n-1) + 10\,a(n-2) - 6\,a(n-3) - 9\,a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $337$ states, $197$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 3$ of A240642, the recurrence $a(n) = 14\,a(n-1) + 54\,a(n-2) + 82\,a(n-3) - 14\,a(n-4) - 54\,a(n-5) - 81\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4308$ states, $2162$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A240642}{A240642}, revision 6, last modified Jul 23 2025.
