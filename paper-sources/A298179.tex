...
\title{A proof of the conjectured linear recurrence for OEIS A298179}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298179}{A298179} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element equal to 3, 5, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
\begin{theorem} For A298179, the recurrence $a(n) = 2\,a(n-1) + a(n-3) - 2\,a(n-4) - 2\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,54$. Every one of them vanished. After it, more than $S = 5$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 38 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 2, 2, 3, 6, 10, 15, 26,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298179}{A298179}, revision 4, last modified Jan 14 2018.
