...
\title{A proof of the conjectured order of the recurrence for OEIS A260498}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260498}{A260498} as

\begin{quote}\itshape Number of (n+2)X(5+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000001 00000011 or 00001011\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical recurrence of order 88 (see link above)\end{quote}
...
The entry states only that the sequence satisfies a linear recurrence of order $88$ and refers to a linked file for its coefficients, which this note does not use. What is proved below is the corresponding exact statement: the least order of a linear recurrence the sequence eventually satisfies, and the exact index beyond which it holds.
...
That refinement leaves $200$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 112$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 722, 942, 2420, 4471, 5848, 13651, 30590, 44345,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260498}{A260498}, revision 6, last modified Jun 02 2025.
