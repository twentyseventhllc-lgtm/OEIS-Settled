...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A255152}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A255152}{A255152} as

\begin{quote}\itshape Number of (n+2) X (1+2) 0..1 arrays with no 3 x 3 subblock diagonal sum 1 and no antidiagonal sum 1 and no row sum 0 or 1 and no column sum 0 or 1\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) - a(n-2) + 3*a(n-3) + a(n-4) + 2*a(n-5) + 2*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(28 + 6*x + 45*x\textasciicircum{}2 + 27*x\textasciicircum{}3 + 38*x\textasciicircum{}4 + 26*x\textasciicircum{}5) / (1 - 2*x + x\textasciicircum{}2 - 3*x\textasciicircum{}3 - x\textasciicircum{}4 - 2*x\textasciicircum{}5 - 2*x\textasciicircum{}6). - Colin Barker, Dec 18 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{28x + 6x^{2} + 45x^{3} + 27x^{4} + 38x^{5} + 26x^{6}}{1 - 2x + x^{2} - 3x^{3} - x^{4} - 2x^{5} - 2x^{6}}.\]
...
\begin{theorem} For A255152, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 3\,a(n-3) + a(n-4) + 2\,a(n-5) + 2\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A255152, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{28x + 6x^{2} + 45x^{3} + 27x^{4} + 38x^{5} + 26x^{6}}{1 - 2x + x^{2} - 3x^{3} - x^{4} - 2x^{5} - 2x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 28, 62, 141, 331, 773, 1782, 4105, 9484,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A255152}{A255152}, revision 7, last modified Dec 18 2018.
