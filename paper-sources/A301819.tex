...
\title{A proof of the conjectured linear recurrence for OEIS A301819}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301819}{A301819} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element equal to 1 or 2 horizontally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 6*a(n-1) - 13*a(n-2) + 13*a(n-3) - 6*a(n-4) + a(n-5).\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Mar 27 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified May 12 2026, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 6\,a(n-1) - 13\,a(n-2) + 13\,a(n-3) - 6\,a(n-4) + a(n-5).\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $51$; neither removal changes any count.
...
\begin{theorem} For A301819, the recurrence $a(n) = 6\,a(n-1) - 13\,a(n-2) + 13\,a(n-3) - 6\,a(n-4) + a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 8, 25, 68, 177, 457, 1183, 3075,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301819}{A301819}, revision 10, last modified May 12 2026.
