...
\title{A proof of the conjectured linear recurrence for OEIS A250525}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250525}{A250525} as

\begin{quote}\itshape Number of (n+1)X(6+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)-x(i-1,j) in the j direction\end{quote}
...
That refinement leaves $148$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 81$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $3^{56}$ arrays, about $10^{27}$, against $81$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A250525, the recurrence $a(n) = 14\,a(n-1) - 85\,a(n-2) + 294\,a(n-3) - 639\,a(n-4) + 906\,a(n-5) - 839\,a(n-6) + 490\,a(n-7) - 164\,a(n-8) + 24\,a(n-9)$ holds for every $n \ge 10$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,115$. Every one of them vanished. After it, more than $S = 81$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 19 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 35064, 214812, 896191, 3088923, 9580060, 27910024, 78204775, 213775147,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250525}{A250525}, revision 6, last modified Jul 23 2025.
