...
\title{Proofs of the conjectured column and row recurrences for OEIS A256029}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A256029}{A256029} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 or 1 and no antidiagonal sum 0 or 1 and no row sum 1 and no column sum 1\end{quote}
...
k=1: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3) -2*a(n-6) -4*a(n-7) +2*a(n-9) for n>10\\k=2: [order 12]\\k=3: [order 26] for n>28\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 13 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A256029, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) - 2\,a(n-6) - 4\,a(n-7) + 2\,a(n-9)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A256029, the recurrence $a(n) = 0$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A256029, the recurrence $a(n) = 0$ holds for every $n > 28$, and fails at $n = 28$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 28$; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A256029}{A256029}, revision 6, last modified Jun 02 2025.
