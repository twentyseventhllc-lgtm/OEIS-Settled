...
\title{Proofs of the conjectured column and row recurrences for OEIS A252148}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252148}{A252148} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and column sum equal to 0 3 5 6 or 7 and every 3X3 diagonal and antidiagonal sum not equal to 0 3 5 6 or 7\end{quote}
...
k=1: [linear recurrence of order 96] for n>105\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 14 2014). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A252148, the recurrence $a(n) = 0$ holds for every $n > 105$, and fails at $n = 105$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 166$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 105$; after that point more than $S = 166$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 105$; the proved threshold is $n > 105$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252148}{A252148}, revision 6, last modified Jun 02 2025.
