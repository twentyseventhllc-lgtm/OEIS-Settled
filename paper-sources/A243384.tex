...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A243384}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A243384}{A243384} as

\begin{quote}\itshape Number of length n+3 0..3 arrays with no four elements in a row with pattern abba (with a!=b) and new values 0..3 introduced in 0..3 order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 7*a(n-1) - 16*a(n-2) + 18*a(n-3) - 15*a(n-4) + 9*a(n-5) - 3*a(n-6) + a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(14 - 53*x + 67*x\textasciicircum{}2 - 59*x\textasciicircum{}3 + 40*x\textasciicircum{}4 - 13*x\textasciicircum{}5 + 5*x\textasciicircum{}6) / ((1 - x)*(1 - 2*x + x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - 4*x + x\textasciicircum{}2 - x\textasciicircum{}3)). - Colin Barker, Nov 02 2018\end{quote}
...
\begin{multline*} a(n) = 7\,a(n-1) - 16\,a(n-2) + 18\,a(n-3) - 15\,a(n-4) + 9\,a(n-5) - 3\,a(n-6) + \\ a(n-7). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{14x - 53x^{2} + 67x^{3} - 59x^{4} + 40x^{5} - 13x^{6} + 5x^{7}}{1 - 7x + 16x^{2} - 18x^{3} + 15x^{4} - 9x^{5} + 3x^{6} - x^{7}}.\]
...
\begin{theorem} For A243384, the recurrence $a(n) = 7\,a(n-1) - 16\,a(n-2) + 18\,a(n-3) - 15\,a(n-4) + 9\,a(n-5) - 3\,a(n-6) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A243384, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{14x - 53x^{2} + 67x^{3} - 59x^{4} + 40x^{5} - 13x^{6} + 5x^{7}}{1 - 7x + 16x^{2} - 18x^{3} + 15x^{4} - 9x^{5} + 3x^{6} - x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 14, 45, 158, 579, 2165, 8173, 30991, 117755,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A243384}{A243384}, revision 8, last modified Nov 02 2018.
