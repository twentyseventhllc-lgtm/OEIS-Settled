...
\title{A proof of the conjectured linear recurrence for OEIS A301789}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301789}{A301789} as

\begin{quote}\itshape Number of n X 7 0..1 arrays with every element equal to 0 or 1 horizontally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) -a(n-4) -a(n-6).\end{quote}
...
The construction gives $16512$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $91$; neither removal changes any count.
...
For scale: at $n = 8$ there are $2^{56}$ arrays, about $10^{17}$, against $7$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A301789, the recurrence $a(n) = 2\,a(n-1) - a(n-4) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 21, 35, 59, 100, 171, 294, 508, 881,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301789}{A301789}, revision 6, last modified Jun 07 2021.
