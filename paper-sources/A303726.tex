...
\title{A proof of the conjectured linear recurrence for OEIS A303726}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303726}{A303726} as

\begin{quote}\itshape Number of nX7 0..1 arrays with every element unequal to 0, 1, 2 or 3 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) -a(n-4) -a(n-5) +a(n-7) -a(n-8) -a(n-9) +2*a(n-10) for n>18\end{quote}
...
\begin{multline*} a(n) = 2\,a(n-1) - a(n-4) - a(n-5) + a(n-7) - a(n-8) - a(n-9) + \\ 2\,a(n-10). \end{multline*}
...
The construction gives $16512$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $881$; neither removal changes any count.
...
That refinement leaves $62$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 38$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A303726, the recurrence $a(n) = 2\,a(n-1) - a(n-4) - a(n-5) + a(n-7) - a(n-8) - a(n-9) + 2\,a(n-10)$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,85$. The last nonvanishing one is at $m = 8$, that is at $n = 18$. After it, more than $S = 38$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 64, 719, 294, 406, 564, 894, 1476, 2252,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303726}{A303726}, revision 4, last modified Apr 29 2018.
