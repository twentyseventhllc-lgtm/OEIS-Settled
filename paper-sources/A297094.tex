...
\title{Proofs of the conjectured column and row recurrences for OEIS A297094}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297094}{A297094} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no 1 adjacent to 3 king-move neighboring 1s\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +3*a(n-3) -14*a(n-4) +4*a(n-5)\\k=3: [order 19]\\k=4: [order 42]\end{quote}
...
\begin{theorem} For column $k = 1$ of A297094, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297094, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 3\,a(n-3) - 14\,a(n-4) + 4\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297094, the recurrence $a(n) = 0$ holds for every $n \ge 20$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A297094, the recurrence $a(n) = 0$ holds for every $n \ge 43$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 50$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 50$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297094}{A297094}, revision 4, last modified Dec 25 2017.
