...
\title{Proofs of the conjectured column and row recurrences for OEIS A318430}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318430}{A318430} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) -3*a(n-4) -2*a(n-5) -a(n-6)\\k=3: [order 17] for n>20\\k=4: [order 53] for n>59\end{quote}
...
\begin{theorem} For column $k = 1$ of A318430, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318430, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 3\,a(n-4) - 2\,a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A318430, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $67$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 20$; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 20$; the proved threshold is $n > 20$.

\begin{theorem} For column $k = 4$ of A318430, the recurrence $a(n) = 0$ holds for every $n > 59$, and fails at $n = 59$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $241$ after trimming, and $S = 64$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 59$; after that point more than $S = 64$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318430}{A318430}, revision 4, last modified Aug 26 2018.
