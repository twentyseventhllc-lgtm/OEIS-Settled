...
k=1: a(n) = 2*a(n-2) +7*a(n-4) +a(n-5) +2*a(n-6) +a(n-7) -a(n-8) +a(n-9) for n>10\\k=2: a(n) = 2*a(n-1) +a(n-3) +3*a(n-4) -5*a(n-5) -a(n-6) -a(n-8) +a(n-9) for n>10\\k=3: a(n) = 4*a(n-2) +37*a(n-4) -13*a(n-6) -124*a(n-8) +12*a(n-10) +72*a(n-12) for n>13\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 13 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
