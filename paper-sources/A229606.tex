...
\title{Proofs of the conjectured column and row recurrences for OEIS A229606}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A229606}{A229606} as

\begin{quote}\itshape T(n,k) = number of defective 3-colorings of an n X k 0..2 array connected horizontally and vertically with exactly two mistakes, and colors introduced in row-major 0..2 order\end{quote}
...
k=1: a(n) = 6*a(n-1) - 12*a(n-2) + 8*a(n-3) for n > 6.\\k=2: a(n) = 9*a(n-1) - 27*a(n-2) + 27*a(n-3) for n > 5.\\k=3: a(n) = 15*a(n-1) - 81*a(n-2) + 185*a(n-3) - 162*a(n-4) + 60*a(n-5) - 8*a(n-6) for n > 7.\\k=4: [order 9] for n > 11.\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Sep 26 2013). As of revision 7, last modified Apr 27 2021, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A229606, the recurrence $a(n) = 6\,a(n-1) - 12\,a(n-2) + 8\,a(n-3)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $25$ states, $19$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 2$ of A229606, the recurrence $a(n) = 9\,a(n-1) - 27\,a(n-2) + 27\,a(n-3)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $45$ states, $32$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 5$; the proved threshold is $n > 5$.

\begin{theorem} For column $k = 3$ of A229606, the recurrence $a(n) = 15\,a(n-1) - 81\,a(n-2) + 185\,a(n-3) - 162\,a(n-4) + 60\,a(n-5) - 8\,a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $110$ states, $75$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 7$; the proved threshold is $n > 7$.

\begin{theorem} For column $k = 4$ of A229606, the recurrence $a(n) = 0$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $178$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A229606}{A229606}, revision 7, last modified Apr 27 2021.
