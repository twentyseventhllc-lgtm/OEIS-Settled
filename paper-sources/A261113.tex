...
k=1: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>6\\k=2: a(n) = a(n-1) +2*a(n-3) +2*a(n-4) +2*a(n-5) +a(n-6) for n>7\\k=3: a(n) = 3*a(n-2) +10*a(n-4) +5*a(n-6) +2*a(n-8) -2*a(n-10) +a(n-12) for n>13\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 08 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
