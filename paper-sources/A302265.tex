...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1)\\k=3: a(n) = 6*a(n-1) -2*a(n-2) +2*a(n-3) -54*a(n-4) +16*a(n-5) for n>6\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = 2*a(n-1)\\n=2: a(n) = 3*a(n-1) +a(n-2) +4*a(n-3) +4*a(n-4)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 04 2018). As of revision 4, last modified Apr 04 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences and the 2 row recurrences listed above.
