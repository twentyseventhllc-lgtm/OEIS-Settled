...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -a(n-2) +a(n-3) +a(n-4) -2*a(n-5) -a(n-6)\\k=3: a(n) = 4*a(n-1) -3*a(n-2) -3*a(n-3) +2*a(n-4) +3*a(n-5) -2*a(n-6) for n>8\\k=4: a(n) = 4*a(n-1) -4*a(n-2) +2*a(n-5) -3*a(n-7) +2*a(n-8) +a(n-9) -a(n-10) for n>13\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 18 2018). As of revision 4, last modified Mar 18 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
