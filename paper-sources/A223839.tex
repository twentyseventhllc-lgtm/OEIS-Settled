...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A223839}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A223839}{A223839} as

\begin{quote}\itshape Number of 3 X n 0..1 arrays with rows, diagonals and antidiagonals unimodal and columns nondecreasing\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (1/720)*n\textasciicircum{}6 + (1/240)*n\textasciicircum{}5 + (29/144)*n\textasciicircum{}4 + (11/48)*n\textasciicircum{}3 + (1007/360)*n\textasciicircum{}2 - (37/30)*n + 2.\end{quote}

\begin{quote}\ttfamily G.f.: x*(2 - 2*x + x\textasciicircum{}2)*(2 - 4*x + 5*x\textasciicircum{}2 - 4*x\textasciicircum{}3 + 2*x\textasciicircum{}4) / (1 - x)\textasciicircum{}7.\end{quote}
...
\[a(n) = 2 - \frac{37}{30}n + \frac{1007}{360}n^{2} + \frac{11}{48}n^{3} + \frac{29}{144}n^{4} + \frac{1}{240}n^{5} + \frac{1}{720}n^{6}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x - 12x^{2} + 20x^{3} - 22x^{4} + 17x^{5} - 8x^{6} + 2x^{7}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}.\]
...
The refinement, applied first forwards and then in the mirror direction on the edges coming in, leaves $S = 8$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of the $8 \times 8$ quotient matrix. That degree bound is computed here, not assumed, and it is the only one the decision procedure below uses.
...
\begin{theorem} For A223839, $a(n)$ is the polynomial $2 - \frac{37}{30}n + \frac{1007}{360}n^{2} + \frac{11}{48}n^{3} + \frac{29}{144}n^{4} + \frac{1}{240}n^{5} + \frac{1}{720}n^{6}$ for every $n \ge 1$.\end{theorem}
...
\begin{theorem} For A223839, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x - 12x^{2} + 20x^{3} - 22x^{4} + 17x^{5} - 8x^{6} + 2x^{7}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}$.\end{theorem}
...
\begin{theorem} For A223839, the recurrence $a(n) = 7\,a(n-1) - 21\,a(n-2) + 35\,a(n-3) - 35\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,58$. Every one of them vanished. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 16, 48, 118, 255, 503, 926, 1614,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A223839}{A223839}, revision 10, last modified Mar 18 2025.
