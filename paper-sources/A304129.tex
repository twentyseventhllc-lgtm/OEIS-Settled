...
\title{A proof of the conjectured linear recurrence for OEIS A304129}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304129}{A304129} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1, 3 or 6 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-3) +a(n-5) -a(n-6) -5*a(n-7) -2*a(n-8) +2*a(n-10) for n>13\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 3\,a(n-3) + a(n-5) - a(n-6) - 5\,a(n-7) - 2\,a(n-8) + \\ 2\,a(n-10). \end{multline*}
...
\begin{theorem} For A304129, the recurrence $a(n) = a(n-1) + 3\,a(n-3) + a(n-5) - a(n-6) - 5\,a(n-7) - 2\,a(n-8) + 2\,a(n-10)$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,64$. The last nonvanishing one is at $m = 3$, that is at $n = 13$. After it, more than $S = 16$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 17, 19, 44, 80, 140, 244, 436,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304129}{A304129}, revision 4, last modified May 07 2018.
