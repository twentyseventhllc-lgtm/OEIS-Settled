...
\title{Proofs of the conjectured column and row recurrences for OEIS A274858}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A274858}{A274858} as

\begin{quote}\itshape T(n,k) = Number of n X k 0..2 arrays with no element equal to any value at offset (-1,0) (0,-1) or (-2,-2) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 2*a(n-1) for n>2;\\k=3: a(n) = 3*a(n-1) -2*a(n-2) +a(n-3) for n>4;\\k=4: a(n) = 5*a(n-1) -9*a(n-2) +10*a(n-3) -6*a(n-4) +a(n-5) for n>7;\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 09 2016). As of revision 6, last modified Oct 15 2023, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A274858, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n > 2$, and fails at $n = 2$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A274858, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + a(n-3)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $58$ states, $58$ after trimming, and $S = 5$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 4$; after that point more than $S = 5$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 4$; the proved threshold is $n > 4$.

\begin{theorem} For column $k = 4$ of A274858, the recurrence $a(n) = 5\,a(n-1) - 9\,a(n-2) + 10\,a(n-3) - 6\,a(n-4) + a(n-5)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $122$ states, $122$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A274858}{A274858}, revision 6, last modified Oct 15 2023.
