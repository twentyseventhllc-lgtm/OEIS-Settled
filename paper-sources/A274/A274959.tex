...
\title{Proofs of the conjectured column and row recurrences for OEIS A274959}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A274959}{A274959} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (-1,-1) (-2,0) or (0,-2) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 2*a(n-1) for n>3\\k=2: a(n) = 3*a(n-1) for n>3\\k=3: a(n) = 4*a(n-1) +4*a(n-3) +a(n-4) for n>5\\k=4: [order 9] for n>11\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 12 2016). As of revision 4, last modified Jul 12 2016, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A274959, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A274959, the recurrence $a(n) = 3\,a(n-1)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $63$ states, $63$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 3$; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 3$; the proved threshold is $n > 3$.

\begin{theorem} For column $k = 3$ of A274959, the recurrence $a(n) = 4\,a(n-1) + 4\,a(n-3) + a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $152$ states, $152$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 5$; the proved threshold is $n > 5$.

\begin{theorem} For column $k = 4$ of A274959, the recurrence $a(n) = 0$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $443$ states, $443$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A274959}{A274959}, revision 4, last modified Jul 12 2016.
