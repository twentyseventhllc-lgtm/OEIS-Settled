...
\title{Proofs of the conjectured column and row recurrences for OEIS A274895}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A274895}{A274895} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (0,-1) (-1,-1) or (-2,0) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 2*a(n-1) for n>3\\k=2: a(n) = a(n-1) +2*a(n-2) -a(n-4) for n>5\\k=3: a(n) = a(n-1) +4*a(n-2) -6*a(n-4) -a(n-5) +4*a(n-6) -a(n-8) for n>10\\k=4: [order 16] for n>18\\k=5: [order 32] for n>34\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 10 2016). As of revision 4, last modified Jul 10 2016, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
The entry's offsets are (0,-1), (-1,-1), (-2,0). The condition is

\[A[i][j] \ne A[i{+}\delta][j{+}\varepsilon]\quad\text{for every listed }(\delta,\varepsilon)\text{ landing inside the array.}\]

An offset and its negative say the same thing, so the offsets are normalised here to point upwards or, in the same row, leftwards: (-2,0), (-1,-1), (0,-1).
...
\begin{theorem} For column $k = 1$ of A274895, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A274895, the recurrence $a(n) = a(n-1) + 2\,a(n-2) - a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $27$ states, $27$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 5$; the proved threshold is $n > 5$.

\begin{theorem} For column $k = 3$ of A274895, the recurrence $a(n) = a(n-1) + 4\,a(n-2) - 6\,a(n-4) - a(n-5) + 4\,a(n-6) - a(n-8)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $55$ states, $55$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 4$ of A274895, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $121$ states, $121$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 18$; the proved threshold is $n > 18$.

\begin{theorem} For column $k = 5$ of A274895, the recurrence $a(n) = 0$ holds for every $n > 34$, and fails at $n = 34$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $277$ states, $277$ after trimming, and $S = 35$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 34$; after that point more than $S = 35$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A274895}{A274895}, revision 4, last modified Jul 10 2016.
