...
\title{Proofs of the conjectured column and row recurrences for OEIS A241078}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A241078}{A241078} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to exactly one horizontal or vertical neighbor, with new values 0..2 introduced in row major order\end{quote}
...
k=1: a(n) = 2*a(n-1) for n>2\\k=2: a(n) = 6*a(n-1) -8*a(n-2) -4*a(n-3) +a(n-4) +14*a(n-5) -8*a(n-6) for n>7\\k=3: [order 20] for n>21\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 15 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A241078, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n > 2$, and fails at $n = 2$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A241078, the recurrence $a(n) = 6\,a(n-1) - 8\,a(n-2) - 4\,a(n-3) + a(n-4) + 14\,a(n-5) - 8\,a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $190$ states, $93$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 7$; the proved threshold is $n > 7$.

\begin{theorem} For column $k = 3$ of A241078, the recurrence $a(n) = 0$ holds for every $n > 21$, and fails at $n = 21$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $1579$ states, $786$ after trimming, and $S = 22$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 21$; after that point more than $S = 22$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A241078}{A241078}, revision 6, last modified Jul 23 2025.
