...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250462}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250462}{A250462} as

\begin{quote}\itshape Number of (n+1)X(2+1) 0..1 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) + 3*a(n-2) - 10*a(n-3) + 4*a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(31 + 10*x - 72*x\textasciicircum{}2 + 2*x\textasciicircum{}3 + 32*x\textasciicircum{}4) / ((1 - x)\textasciicircum{}2*(1 - 2*x - 8*x\textasciicircum{}2 - 4*x\textasciicircum{}3)). - Colin Barker, Nov 14 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 12, last modified Nov 14 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 4\,a(n-1) + 3\,a(n-2) - 10\,a(n-3) + 4\,a(n-5).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{31x + 10x^{2} - 72x^{3} + 2x^{4} + 32x^{5}}{1 - 4x - 3x^{2} + 10x^{3} - 4x^{5}}.\]
...
\begin{theorem} For A250462, the recurrence $a(n) = 4\,a(n-1) + 3\,a(n-2) - 10\,a(n-3) + 4\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250462, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{31x + 10x^{2} - 72x^{3} + 2x^{4} + 32x^{5}}{1 - 4x - 3x^{2} + 10x^{3} - 4x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 31, 134, 557, 2322, 9651, 40124, 166765, 693150,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250462}{A250462}, revision 12, last modified Nov 14 2018.
