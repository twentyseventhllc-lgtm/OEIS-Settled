...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A244177}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A244177}{A244177} as

\begin{quote}\itshape Number of length n+4 0..2 arrays with no five consecutive elements with pattern ababa or abbba (possibly a=b) and new values 0..2 introduced in 0..2 order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 3*a(n-2) + 4*a(n-3) + 5*a(n-4) + 3*a(n-5) + 2*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(38 + 70*x + 84*x\textasciicircum{}2 + 85*x\textasciicircum{}3 + 50*x\textasciicircum{}4 + 27*x\textasciicircum{}5) / (1 - x - 3*x\textasciicircum{}2 - 4*x\textasciicircum{}3 - 5*x\textasciicircum{}4 - 3*x\textasciicircum{}5 - 2*x\textasciicircum{}6). - Colin Barker, Nov 03 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{38x + 70x^{2} + 84x^{3} + 85x^{4} + 50x^{5} + 27x^{6}}{1 - x - 3x^{2} - 4x^{3} - 5x^{4} - 3x^{5} - 2x^{6}}.\]
...
The reachable part of that digraph has $107$ states, $107$ after trimming. Identifying states with the same number of completions of every length, and then the mirror identification on the edges coming in, leaves $S = 11$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of the $11\times11$ quotient matrix; that degree bound is computed here, not assumed.
...
\begin{theorem} For A244177, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 4\,a(n-3) + 5\,a(n-4) + 3\,a(n-5) + 2\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A244177, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{38x + 70x^{2} + 84x^{3} + 85x^{4} + 50x^{5} + 27x^{6}}{1 - x - 3x^{2} - 4x^{3} - 5x^{4} - 3x^{5} - 2x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 38, 108, 306, 867, 2457, 6963, 19732, 55918,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A244177}{A244177}, revision 8, last modified Nov 03 2018.
