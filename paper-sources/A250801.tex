...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250801}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250801}{A250801} as

\begin{quote}\itshape Number of (4+1) X (n+1) 0..1 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 2*a(n-2) - 9*a(n-3) + 9*a(n-4) + 6*a(n-5) - 8*a(n-6) -a(n-7) + 2*a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(70 - 35*x - 125*x\textasciicircum{}2 + 148*x\textasciicircum{}3 + 82*x\textasciicircum{}4 - 125*x\textasciicircum{}5 - 15*x\textasciicircum{}6 + 32*x\textasciicircum{}7) / ((1 - x)\textasciicircum{}3*(1 + x)\textasciicircum{}2*(1 - 2*x)*(1 - x - x\textasciicircum{}2)). - Colin Barker, Nov 20 2018\end{quote}
...
\begin{multline*} a(n) = 4\,a(n-1) - 2\,a(n-2) - 9\,a(n-3) + 9\,a(n-4) + 6\,a(n-5) - 8\,a(n-6) - \\ a(n-7) + 2\,a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{140x - 70x^{2} - 250x^{3} + 296x^{4} + 164x^{5} - 250x^{6} - 30x^{7} + 64x^{8}}{1 - 4x + 2x^{2} + 9x^{3} - 9x^{4} - 6x^{5} + 8x^{6} + x^{7} - 2x^{8}}.\]
...
That refinement leaves $23$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 10$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A250801, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) - 9\,a(n-3) + 9\,a(n-4) + 6\,a(n-5) - 8\,a(n-6) - a(n-7) + 2\,a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250801, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{140x - 70x^{2} - 250x^{3} + 296x^{4} + 164x^{5} - 250x^{6} - 30x^{7} + 64x^{8}}{1 - 4x + 2x^{2} + 9x^{3} - 9x^{4} - 6x^{5} + 8x^{6} + x^{7} - 2x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 140, 490, 1430, 3776, 9258, 21610, 48600, 106426,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250801}{A250801}, revision 7, last modified Nov 20 2018.
