...
\title{Proofs of the conjectured column and row recurrences for OEIS A304855}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304855}{A304855} as

\begin{quote}\itshape T(n,k) = Number of n X k 0..1 arrays with every element unequal to 0, 1, 3, 4 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=2: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3) -4*a(n-4) for n > 5;\\k=4: [order 41] for n > 44.\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 19 2018). As of revision 6, last modified Apr 26 2021, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 2$ of A304855, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) - 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A304855, the recurrence $a(n) = 0$ holds for every $n > 44$, and fails at $n = 44$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $176$ after trimming, and $S = 50$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 44$; after that point more than $S = 50$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304855}{A304855}, revision 6, last modified Apr 26 2021.
