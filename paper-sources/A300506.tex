...
\title{Proofs of the conjectured column and row recurrences for OEIS A300506}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300506}{A300506} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 3 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3) -4*a(n-4) +a(n-5) -2*a(n-6) +a(n-7)\\k=3: [order 26] for n>31\\k=4: [order 93] for n>103\end{quote}
...
\begin{theorem} For column $k = 1$ of A300506, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300506, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) - 4\,a(n-4) + a(n-5) - 2\,a(n-6) + a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300506, the recurrence $a(n) = 0$ holds for every $n > 31$, and fails at $n = 31$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $60$ after trimming, and $S = 31$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 31$; after that point more than $S = 31$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 31$; the proved threshold is $n > 31$.

\begin{theorem} For column $k = 4$ of A300506, the recurrence $a(n) = 0$ holds for every $n > 103$, and fails at $n = 103$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $235$ after trimming, and $S = 111$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 103$; after that point more than $S = 111$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 103$; the proved threshold is $n > 103$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300506}{A300506}, revision 4, last modified Mar 07 2018.
