...
\title{Proofs of the conjectured column and row recurrences for OEIS A234184}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234184}{A234184} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..5 arrays with every 2X2 subblock having the absolute values of all six edge and diagonal differences no larger than 1\end{quote}
...
k=1: a(n) = 10*a(n-1) -27*a(n-2) +3*a(n-3) +36*a(n-4) +12*a(n-5)\\k=2: [order 11]\end{quote}
...
\begin{theorem} For column $k = 1$ of A234184, the recurrence $a(n) = 10\,a(n-1) - 27\,a(n-2) + 3\,a(n-3) + 36\,a(n-4) + 12\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $36$ states, $36$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A234184, the recurrence $a(n) = 0$ holds for every $n \ge 12$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $216$ states, $216$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234184}{A234184}, revision 6, last modified Jun 02 2025.
