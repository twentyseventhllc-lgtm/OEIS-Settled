...
\title{Proofs of the conjectured column and row recurrences for OEIS A317823}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317823}{A317823} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) -3*a(n-4) -2*a(n-5) -a(n-6)\\k=3: [order 16] for n>18\\k=4: [order 37] for n>43\\k=5: [order 94] for n>99\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 07 2018). As of revision 4, last modified Aug 07 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A317823, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317823, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 3\,a(n-4) - 2\,a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317823, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $59$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 18$; the proved threshold is $n > 18$.

\begin{theorem} For column $k = 4$ of A317823, the recurrence $a(n) = 0$ holds for every $n > 43$, and fails at $n = 43$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $179$ after trimming, and $S = 49$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 43$; after that point more than $S = 49$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 43$; the proved threshold is $n > 43$.

\begin{theorem} For column $k = 5$ of A317823, the recurrence $a(n) = 0$ holds for every $n > 99$, and fails at $n = 99$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $557$ after trimming, and $S = 124$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 99$; after that point more than $S = 124$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317823}{A317823}, revision 4, last modified Aug 07 2018.
