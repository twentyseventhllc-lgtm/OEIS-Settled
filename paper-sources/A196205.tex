...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196205}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196205}{A196205} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,0,3,4,1 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) - 2*a(n-2) - 2*a(n-4) + 3*a(n-5) - 2*a(n-6) + 2*a(n-7) - 2*a(n-8) + a(n-9).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(4 - 4*x - 9*x\textasciicircum{}2 + 9*x\textasciicircum{}3 - 3*x\textasciicircum{}4 + 8*x\textasciicircum{}5 - 7*x\textasciicircum{}6 + x\textasciicircum{}7 + 2*x\textasciicircum{}8) / ((1 - x)*(1 - x - x\textasciicircum{}3)*(1 - x - x\textasciicircum{}2 + x\textasciicircum{}4 - x\textasciicircum{}5)). - Colin Barker, Mar 01 2018\end{quote}
...
\begin{multline*} a(n) = 3\,a(n-1) - 2\,a(n-2) - 2\,a(n-4) + 3\,a(n-5) - 2\,a(n-6) + 2\,a(n-7) - \\ 2\,a(n-8) + a(n-9). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x - 4x^{2} - 9x^{3} + 9x^{4} - 3x^{5} + 8x^{6} - 7x^{7} + x^{8} + 2x^{9}}{1 - 3x + 2x^{2} + 2x^{4} - 3x^{5} + 2x^{6} - 2x^{7} + 2x^{8} - x^{9}}.\]
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $30$; neither removal changes any count.
...
\begin{theorem} For A196205, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) - 2\,a(n-4) + 3\,a(n-5) - 2\,a(n-6) + 2\,a(n-7) - 2\,a(n-8) + a(n-9)$ holds for every $n \ge 10$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,62$. Every one of them vanished. After it, more than $S = 9$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A196205, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x - 4x^{2} - 9x^{3} + 9x^{4} - 3x^{5} + 8x^{6} - 7x^{7} + x^{8} + 2x^{9}}{1 - 3x + 2x^{2} + 2x^{4} - 3x^{5} + 2x^{6} - 2x^{7} + 2x^{8} - x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 38 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 8, 7, 14, 17, 27, 42, 58,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196205}{A196205}, revision 8, last modified Mar 01 2018.
