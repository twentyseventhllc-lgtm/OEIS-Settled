...
\title{A proof of the conjectured linear recurrence for OEIS A300345}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300345}{A300345} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element equal to 1 or 2 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +a(n-2) -2*a(n-5) -2*a(n-6) -2*a(n-7) -a(n-8) for n>10\end{quote}
...
\[a(n) = 2\,a(n-1) + a(n-2) - 2\,a(n-5) - 2\,a(n-6) - 2\,a(n-7) - a(n-8).\]
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $27$; neither removal changes any count.
...
\begin{theorem} For A300345, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - 2\,a(n-5) - 2\,a(n-6) - 2\,a(n-7) - a(n-8)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,56$. The last nonvanishing one is at $m = 2$, that is at $n = 10$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 6, 9, 26, 57, 133, 306, 709,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300345}{A300345}, revision 4, last modified Mar 03 2018.
