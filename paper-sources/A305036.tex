...
\title{A proof of the conjectured linear recurrence for OEIS A305036}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305036}{A305036} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +5*a(n-3) +a(n-4) -4*a(n-6) -4*a(n-7) for n>11\end{quote}
...
\[a(n) = a(n-1) + 5\,a(n-3) + a(n-4) - 4\,a(n-6) - 4\,a(n-7).\]
...
\begin{theorem} For A305036, the recurrence $a(n) = a(n-1) + 5\,a(n-3) + a(n-4) - 4\,a(n-6) - 4\,a(n-7)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,58$. The last nonvanishing one is at $m = 4$, that is at $n = 11$. After it, more than $S = 14$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 5, 7, 19, 26, 61, 143, 277,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305036}{A305036}, revision 4, last modified May 24 2018.
