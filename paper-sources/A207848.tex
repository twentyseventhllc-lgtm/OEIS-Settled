...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207848}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207848}{A207848} as

\begin{quote}\itshape Number of 5 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 1 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 15*a(n-2) + 4*a(n-3) - 32*a(n-4) - 4*a(n-5) + 15*a(n-6) - a(n-8) for n>9.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 16*x*(1 + 16*x + 29*x\textasciicircum{}2 - 68*x\textasciicircum{}3 - 58*x\textasciicircum{}4 + 88*x\textasciicircum{}5 - 5*x\textasciicircum{}6 - 8*x\textasciicircum{}7 + x\textasciicircum{}8) / (1 - 15*x\textasciicircum{}2 - 4*x\textasciicircum{}3 + 32*x\textasciicircum{}4 + 4*x\textasciicircum{}5 - 15*x\textasciicircum{}6 + x\textasciicircum{}8). - Colin Barker, Jun 26 2018\end{quote}
...
\[a(n) = 15\,a(n-2) + 4\,a(n-3) - 32\,a(n-4) - 4\,a(n-5) + 15\,a(n-6) - a(n-8).\]

stated for $n > 9$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + 256x^{2} + 464x^{3} - 1088x^{4} - 928x^{5} + 1408x^{6} - 80x^{7} - 128x^{8} + 16x^{9}}{1 - 15x^{2} - 4x^{3} + 32x^{4} + 4x^{5} - 15x^{6} + x^{8}}.\]
...
\begin{theorem} For A207848, the recurrence $a(n) = 15\,a(n-2) + 4\,a(n-3) - 32\,a(n-4) - 4\,a(n-5) + 15\,a(n-6) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,47$. The last nonvanishing one is at $m = 1$, that is at $n = 9$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A207848, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + 256x^{2} + 464x^{3} - 1088x^{4} - 928x^{5} + 1408x^{6} - 80x^{7} - 128x^{8} + 16x^{9}}{1 - 15x^{2} - 4x^{3} + 32x^{4} + 4x^{5} - 15x^{6} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 256, 704, 2816, 10144, 38208, 140032, 524480,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207848}{A207848}, revision 9, last modified Jun 26 2018.
