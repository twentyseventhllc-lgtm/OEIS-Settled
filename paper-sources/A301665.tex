...
\title{A proof of the conjectured linear recurrence for OEIS A301665}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301665}{A301665} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element equal to 1, 2 or 4 horizontally or vertically adjacent elements, with upper left element zero\end{quote}
...
That refinement leaves $72$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 72$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A301665, the recurrence of order $70$ displayed in Section 1 holds for every $n \ge 71$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,169$. Every one of them vanished. After it, more than $S = 72$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 23, 118, 564, 2793, 14394, 71564, 359659,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301665}{A301665}, revision 4, last modified Mar 25 2018.
