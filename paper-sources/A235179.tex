...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A235179}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A235179}{A235179} as

\begin{quote}\itshape Number of (n+1) X (1+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 16*a(n-2) - 65*a(n-4) + 14*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 4*x*(18 + 53*x - 144*x\textasciicircum{}2 - 424*x\textasciicircum{}3 + 30*x\textasciicircum{}4 + 95*x\textasciicircum{}5) / ((1 - 7*x\textasciicircum{}2)*(1 - 9*x\textasciicircum{}2 + 2*x\textasciicircum{}4)). - Colin Barker, Oct 17 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{72x + 212x^{2} - 576x^{3} - 1696x^{4} + 120x^{5} + 380x^{6}}{1 - 16x^{2} + 65x^{4} - 14x^{6}}.\]
...
\begin{theorem} For A235179, the recurrence $a(n) = 16\,a(n-2) - 65\,a(n-4) + 14\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,44$. Every one of them vanished. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A235179, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{72x + 212x^{2} - 576x^{3} - 1696x^{4} + 120x^{5} + 380x^{6}}{1 - 16x^{2} + 65x^{4} - 14x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 72, 212, 576, 1696, 4656, 13736, 38064, 112504,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A235179}{A235179}, revision 10, last modified Jun 19 2022.
