...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A298163}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298163}{A298163} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every element equal to 3, 4, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + a(n-2) - a(n-3) - 2*a(n-4) - 2*a(n-5) + a(n-6) + a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x\textasciicircum{}2*(2 - 2*x - 2*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + x\textasciicircum{}4 + 2*x\textasciicircum{}5) / ((1 - x - x\textasciicircum{}2)*(1 - x - x\textasciicircum{}2 - x\textasciicircum{}3 + x\textasciicircum{}5)). - Colin Barker, Mar 23 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Jan 14 2018).

The line quoted ending in that signature is contributed by Colin Barker on Mar 23 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 7, last modified Mar 23 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 2\,a(n-1) + a(n-2) - a(n-3) - 2\,a(n-4) - 2\,a(n-5) + a(n-6) + \\ a(n-7). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x^{2} - 2x^{3} - 2x^{4} - 2x^{5} + x^{6} + 2x^{7}}{1 - 2x - x^{2} + x^{3} + 2x^{4} + 2x^{5} - x^{6} - x^{7}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $16$; neither removal changes any count.
...
\begin{theorem} For A298163, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - a(n-3) - 2\,a(n-4) - 2\,a(n-5) + a(n-6) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A298163, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x^{2} - 2x^{3} - 2x^{4} - 2x^{5} + x^{6} + 2x^{7}}{1 - 2x - x^{2} + x^{3} + 2x^{4} + 2x^{5} - x^{6} - x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 36 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 2, 2, 4, 6, 11, 18, 31,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298163}{A298163}, revision 7, last modified Mar 23 2018.
