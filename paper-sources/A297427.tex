...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297427}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297427}{A297427} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 2 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 12*a(n-3) + 2*a(n-4) + 2*a(n-5) - 7*a(n-6) - a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 11*x + 14*x\textasciicircum{}2 + 4*x\textasciicircum{}3 - 5*x\textasciicircum{}4 - 8*x\textasciicircum{}5 - x\textasciicircum{}6) / (1 - x - 12*x\textasciicircum{}3 - 2*x\textasciicircum{}4 - 2*x\textasciicircum{}5 + 7*x\textasciicircum{}6 + x\textasciicircum{}7). - Colin Barker, Feb 28 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 11x^{2} + 14x^{3} + 4x^{4} - 5x^{5} - 8x^{6} - x^{7}}{1 - x - 12x^{3} - 2x^{4} - 2x^{5} + 7x^{6} + x^{7}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $42$; neither removal changes any count.
...
\begin{theorem} For A297427, the recurrence $a(n) = a(n-1) + 12\,a(n-3) + 2\,a(n-4) + 2\,a(n-5) - 7\,a(n-6) - a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297427, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 11x^{2} + 14x^{3} + 4x^{4} - 5x^{5} - 8x^{6} - x^{7}}{1 - x - 12x^{3} - 2x^{4} - 2x^{5} + 7x^{6} + x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 12, 26, 42, 183, 513, 1085, 3332,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297427}{A297427}, revision 8, last modified Feb 28 2019.
