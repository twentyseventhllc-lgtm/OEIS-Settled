...
\title{Proofs of the conjectured column and row recurrences for OEIS A298930}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298930}{A298930} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 2, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -4*a(n-4)\\k=3: [order 18] for n>19\\k=4: [order 60] for n>61\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 29 2018). As of revision 6, last modified Jan 29 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A298930, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298930, the recurrence $a(n) = a(n-1) + 3\,a(n-2) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298930, the recurrence $a(n) = 0$ holds for every $n > 19$, and fails at $n = 19$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $59$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 19$; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 19$; the proved threshold is $n > 19$.

\begin{theorem} For column $k = 4$ of A298930, the recurrence $a(n) = 0$ holds for every $n > 61$, and fails at $n = 61$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $217$ after trimming, and $S = 62$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 61$; after that point more than $S = 62$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298930}{A298930}, revision 6, last modified Jan 29 2018.
