...
\title{Proofs of the conjectured column and row recurrences for OEIS A298924}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298924}{A298924} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-2)\\k=2: a(n) = 8*a(n-3)\\k=3: a(n) = a(n-1) +a(n-2) +a(n-6) -a(n-7) -a(n-8)\\k=4: a(n) = a(n-1) +a(n-2) +a(n-6) -a(n-7) -a(n-8)\\k=5: a(n) = a(n-1) +a(n-2) +a(n-6) -a(n-7) -a(n-8) for n>11\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 29 2018). As of revision 4, last modified Jan 29 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
The condition is $c(i,j) \in \{1, 3, 5, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A298924, the recurrence $a(n) = a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298924, the recurrence $a(n) = 8\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298924, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-6) - a(n-7) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A298924, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-6) - a(n-7) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $58$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 5$ of A298924, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-6) - a(n-7) - a(n-8)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $92$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298924}{A298924}, revision 4, last modified Jan 29 2018.
