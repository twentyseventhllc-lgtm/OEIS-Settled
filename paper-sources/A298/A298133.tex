...
\title{Proofs of the conjectured column and row recurrences for OEIS A298133}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298133}{A298133} as

\begin{quote}\itshape T(n,k) is the number of n X k 0..1 arrays with every element equal to 2, 3, 4 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1).\\k=2: a(n) = 3*a(n-2) + 4*a(n-3) + 2*a(n-4).\\k=3: [order 18].\\k=4: [order 53] for n>56.\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 13 2018). As of revision 6, last modified Jul 31 2026, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A298133, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298133, the recurrence $a(n) = 3\,a(n-2) + 4\,a(n-3) + 2\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298133, the recurrence $a(n) = 0$ holds for every $n \ge 19$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A298133, the recurrence $a(n) = 0$ holds for every $n > 56$, and fails at $n = 56$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $252$ after trimming, and $S = 60$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 56$; after that point more than $S = 60$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298133}{A298133}, revision 6, last modified Jul 31 2026.
