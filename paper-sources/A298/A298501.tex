...
\title{Proofs of the conjectured column and row recurrences for OEIS A298501}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298501}{A298501} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 3, 4 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +3*a(n-2) +8*a(n-3) -4*a(n-4) -16*a(n-5) for n>6\\k=3: [order 15] for n>16\\k=4: [order 51] for n>54\end{quote}
...
\begin{theorem} For column $k = 1$ of A298501, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298501, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 8\,a(n-3) - 4\,a(n-4) - 16\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298501, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $51$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 16$; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 16$; the proved threshold is $n > 16$.

\begin{theorem} For column $k = 4$ of A298501, the recurrence $a(n) = 0$ holds for every $n > 54$, and fails at $n = 54$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $202$ after trimming, and $S = 59$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 54$; after that point more than $S = 59$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298501}{A298501}, revision 4, last modified Jan 20 2018.
