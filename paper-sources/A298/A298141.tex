...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A298141}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298141}{A298141} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element equal to 2, 3, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-2) + 2*a(n-5) - 4*a(n-6) - 8*a(n-7) - a(n-8) + a(n-9).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x\textasciicircum{}2*(1 - x)*(2 - 2*x\textasciicircum{}2 - 3*x\textasciicircum{}3 - 2*x\textasciicircum{}4 - 6*x\textasciicircum{}5 - 3*x\textasciicircum{}6) / ((1 + x)*(1 - 2*x - 2*x\textasciicircum{}5 + 6*x\textasciicircum{}6 + 2*x\textasciicircum{}7 - x\textasciicircum{}8)). - Colin Barker, Mar 22 2018\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 2\,a(n-2) + 2\,a(n-5) - 4\,a(n-6) - 8\,a(n-7) - a(n-8) + \\ a(n-9). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x^{2} - 2x^{3} - 2x^{4} - x^{5} + x^{6} - 4x^{7} + 3x^{8} + 3x^{9}}{1 - x - 2x^{2} - 2x^{5} + 4x^{6} + 8x^{7} + x^{8} - x^{9}}.\]
...
\begin{theorem} For A298141, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + 2\,a(n-5) - 4\,a(n-6) - 8\,a(n-7) - a(n-8) + a(n-9)$ holds for every $n \ge 10$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,63$. Every one of them vanished. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A298141, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x^{2} - 2x^{3} - 2x^{4} - x^{5} + x^{6} - 4x^{7} + 3x^{8} + 3x^{9}}{1 - x - 2x^{2} - 2x^{5} + 4x^{6} + 8x^{7} + x^{8} - x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 2, 0, 2, 1, 6, 8, 15,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298141}{A298141}, revision 8, last modified Mar 22 2018.
