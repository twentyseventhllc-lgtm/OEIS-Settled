...
\title{Proofs of the conjectured column and row recurrences for OEIS A298494}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298494}{A298494} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 4 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -2*a(n-2) +8*a(n-3) -20*a(n-4) +8*a(n-5) for n>6\\k=3: [order 11] for n>12\\k=4: [order 41] for n>43\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 20 2018). As of revision 4, last modified Jan 20 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A298494, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298494, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 8\,a(n-3) - 20\,a(n-4) + 8\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298494, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $55$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 12$; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 12$; the proved threshold is $n > 12$.

\begin{theorem} For column $k = 4$ of A298494, the recurrence $a(n) = 0$ holds for every $n > 43$, and fails at $n = 43$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $184$ after trimming, and $S = 47$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 43$; after that point more than $S = 47$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298494}{A298494}, revision 4, last modified Jan 20 2018.
