...
\title{Proofs of the conjectured column and row recurrences for OEIS A298841}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298841}{A298841} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 2, 3, 4, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) +2*a(n-3)\\k=3: [order 12] for n>14\\k=4: [order 45] for n>48\end{quote}
...
\begin{theorem} In A298841, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298841, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298841, the recurrence $a(n) = 0$ holds for every $n > 14$, and fails at $n = 14$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $67$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 14$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 14$; the proved threshold is $n > 14$.

\begin{theorem} For column $k = 4$ of A298841, the recurrence $a(n) = 0$ holds for every $n > 48$, and fails at $n = 48$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $260$ after trimming, and $S = 55$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 48$; after that point more than $S = 55$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298841}{A298841}, revision 4, last modified Jan 27 2018.
