...
\title{Proofs of the conjectured column and row recurrences for OEIS A298070}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298070}{A298070} as

\begin{quote}\itshape T(n,k) is the number of n X k 0..1 arrays with every element equal to 2, 3, 4 or 6 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1).\\k=2: a(n) = 3*a(n-2) + 4*a(n-3) + 2*a(n-4).\\k=3: [order 17].\\k=4: [order 62] for n>63.\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 11 2018). As of revision 6, last modified Jul 31 2026, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A298070, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298070, the recurrence $a(n) = 3\,a(n-2) + 4\,a(n-3) + 2\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298070, the recurrence $a(n) = 0$ holds for every $n \ge 18$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $67$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A298070, the recurrence $a(n) = 0$ holds for every $n > 63$, and fails at $n = 63$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $260$ after trimming, and $S = 71$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 63$; after that point more than $S = 71$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298070}{A298070}, revision 6, last modified Jul 31 2026.
