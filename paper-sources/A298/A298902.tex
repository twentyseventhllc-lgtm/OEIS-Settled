...
\title{Proofs of the conjectured column and row recurrences for OEIS A298902}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298902}{A298902} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 3, 4, 5, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +a(n-2)\\k=3: a(n) = 2*a(n-1) +a(n-2) -2*a(n-3) +a(n-4) -2*a(n-5)\\k=4: [order 16]\\k=5: [order 61]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 28 2018). As of revision 4, last modified Jan 28 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
The condition is $c(i,j) \in \{3, 4, 5, 6, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} In A298902, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298902, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298902, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - 2\,a(n-3) + a(n-4) - 2\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A298902, the recurrence $a(n) = 0$ holds for every $n \ge 17$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $58$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 5$ of A298902, the recurrence $a(n) = 0$ holds for every $n \ge 62$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $305$ after trimming, and $S = 69$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 69$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298902}{A298902}, revision 4, last modified Jan 28 2018.
