...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A198662}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A198662}{A198662} as

\begin{quote}\itshape Number of n X 1 0..4 arrays with values 0..4 introduced in row major order and each element equal to one or two horizontal and vertical neighbors\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) + a(n-2) - 17*a(n-3) + 6*a(n-4) + 21*a(n-5) - 6*a(n-6) - 8*a(n-7) for n>8.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x\textasciicircum{}2*(1 - 3*x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - 3*x + 3*x\textasciicircum{}3) / ((1 - x)*(1 + x)*(1 - 2*x)*(1 - x - x\textasciicircum{}2)*(1 - x - 4*x\textasciicircum{}2)). - Colin Barker, Mar 02 2018\end{quote}
...
\begin{multline*} a(n) = 4\,a(n-1) + a(n-2) - 17\,a(n-3) + 6\,a(n-4) + 21\,a(n-5) - 6\,a(n-6) - \\ 8\,a(n-7). \end{multline*}

stated for $n > 8$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x^{2} - 3x^{3} - 3x^{4} + 11x^{5} + 3x^{6} - 9x^{7} - 3x^{8}}{1 - 4x - x^{2} + 17x^{3} - 6x^{4} - 21x^{5} + 6x^{6} + 8x^{7}}.\]
...
The reachable part of the digraph has $111$ states, $57$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 9$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of that $9\times9$ matrix; the bound is computed, not assumed.
...
\begin{theorem} For A198662, the recurrence $a(n) = 4\,a(n-1) + a(n-2) - 17\,a(n-3) + 6\,a(n-4) + 21\,a(n-5) - 6\,a(n-6) - 8\,a(n-7)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}
...
\begin{theorem} For A198662, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x^{2} - 3x^{3} - 3x^{4} + 11x^{5} + 3x^{6} - 9x^{7} - 3x^{8}}{1 - 4x - x^{2} + 17x^{3} - 6x^{4} - 21x^{5} + 6x^{6} + 8x^{7}}$.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A198662}{A198662}, revision 9, last modified Mar 02 2018.
