...
Here the refinement leaves $S = 100$ blocks. Writing $L$ for the $S\times S$ quotient matrix, $\tilde w$ for the start weights summed over each block and $\tilde f$ for the acceptance indicator,
...
{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,162$. The last nonvanishing one is at $m = 8$, that is at $n = 43$. After it, more than $S = 100$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
