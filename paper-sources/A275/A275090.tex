...
\title{Proofs of the conjectured column and row recurrences for OEIS A275090}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A275090}{A275090} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (0,-2) (-1,-2) or (-2,-1) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 4*a(n-1) -3*a(n-2)\\k=2: a(n) = 6*a(n-1) for n>3\\k=3: [order 9] for n>10\\k=4: [order 14] for n>15\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 16 2016). As of revision 4, last modified Jul 16 2016, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A275090, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A275090, the recurrence $a(n) = 6\,a(n-1)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A275090, the recurrence $a(n) = 0$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $224$ states, $224$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 4$ of A275090, the recurrence $a(n) = 0$ holds for every $n > 15$, and fails at $n = 15$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $585$ states, $585$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 15$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A275090}{A275090}, revision 4, last modified Jul 16 2016.
