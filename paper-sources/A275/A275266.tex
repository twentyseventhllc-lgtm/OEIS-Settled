...
\title{Proofs of the conjectured column and row recurrences for OEIS A275266}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A275266}{A275266} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (-2,-1) (-2,1) or (-1,-1) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 4*a(n-1) -3*a(n-2)\\k=2: [order 9] for n>10\\k=3: [order 14] for n>17\\k=4: [order 43] for n>47\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = 4*a(n-1) -3*a(n-2)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 21 2016). As of revision 4, last modified Jul 21 2016, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences and the 1 row recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A275266, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A275266, the recurrence $a(n) = 0$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $61$ states, $61$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 3$ of A275266, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $278$ states, $278$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 17$; the proved threshold is $n > 17$.

\begin{theorem} For column $k = 4$ of A275266, the recurrence $a(n) = 0$ holds for every $n > 47$, and fails at $n = 47$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $1063$ states, $1063$ after trimming, and $S = 71$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 47$; after that point more than $S = 71$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 47$; the proved threshold is $n > 47$.

\begin{theorem} For row $n = 1$ of A275266, the recurrence $a(k) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $k \ge 3$, at every index at which it can be stated.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A275266}{A275266}, revision 4, last modified Jul 21 2016.
