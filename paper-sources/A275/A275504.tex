...
\title{Proofs of the conjectured column and row recurrences for OEIS A275504}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A275504}{A275504} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (-2,-1) (-2,0) or (-1,-1) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 2*a(n-1) for n>3\\k=2: a(n) = a(n-1) +2*a(n-2) -a(n-4) for n>6\\k=3: a(n) = 2*a(n-1) +2*a(n-2) -2*a(n-3) -4*a(n-4) +3*a(n-5) +a(n-6) -a(n-7) for n>11\\k=4: [order 16] for n>20\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 30 2016). As of revision 4, last modified Jul 30 2016, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A275504, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A275504, the recurrence $a(n) = a(n-1) + 2\,a(n-2) - a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $51$ states, $51$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 3$ of A275504, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) - 2\,a(n-3) - 4\,a(n-4) + 3\,a(n-5) + a(n-6) - a(n-7)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $136$ states, $136$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 11$; the proved threshold is $n > 11$.

\begin{theorem} For column $k = 4$ of A275504, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $613$ states, $613$ after trimming, and $S = 31$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 20$; after that point more than $S = 31$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A275504}{A275504}, revision 4, last modified Jul 30 2016.
