...
\title{A proof of the conjectured order of the recurrence for OEIS A299749}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299749}{A299749} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element equal to 0, 1, 2, 3, 5, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
\begin{theorem} For A299749, the least order of a linear recurrence that $a$ eventually satisfies is exactly $65$, and that recurrence holds for every $n > 66$, the entry's own figure.\end{theorem}

{\itshape Proof.} The count is C-finite of order at most $S = 67$, so the minimal annihilator of the tail $(a(n))_{n \ge M}$ is the same for every $M$ beyond $S$: the nilpotent part of $L$ is killed after $S$ steps. Taking such an $M$ and $140$ consecutive terms from there, the Berlekamp--Massey algorithm over $\mathbb{Q}$ returns the minimal linear recurrence generating them; since the sequence satisfies one of order at most $S$, $2S$ terms determine it. Its order is $65$. That recurrence was then put through the residual test of Section 5: the last nonvanishing residual is at $n = 66$, followed by more than $S = 67$ consecutive vanishing ones, so it holds for every larger $n$. No recurrence of smaller order can hold eventually, since the tail's minimal annihilator is what the algorithm returned. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 20 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 88, 375, 1998, 11733, 64703, 358219, 2010841,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299749}{A299749}, revision 4, last modified Feb 18 2018.
