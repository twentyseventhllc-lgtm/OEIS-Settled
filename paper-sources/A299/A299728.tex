...
\title{Proofs of the conjectured column and row recurrences for OEIS A299728}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299728}{A299728} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 3, 4, 5, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +4*a(n-3) for n>4\\k=3: a(n) = 6*a(n-1) +8*a(n-2) +20*a(n-3) +25*a(n-4) +11*a(n-5) +2*a(n-6) for n>7\\k=4: [order 16] for n>17\end{quote}
...
\begin{theorem} For column $k = 1$ of A299728, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A299728, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 4\,a(n-3)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A299728, the recurrence $a(n) = 6\,a(n-1) + 8\,a(n-2) + 20\,a(n-3) + 25\,a(n-4) + 11\,a(n-5) + 2\,a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 7$; the proved threshold is $n > 7$.

\begin{theorem} For column $k = 4$ of A299728, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299728}{A299728}, revision 4, last modified Feb 17 2018.
