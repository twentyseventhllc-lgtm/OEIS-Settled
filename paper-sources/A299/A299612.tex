...
\title{Proofs of the conjectured column and row recurrences for OEIS A299612}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299612}{A299612} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 4, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) -2*a(n-2) +4*a(n-3) -10*a(n-4) +4*a(n-5) for n>6\\k=3: [order 18] for n>19\\k=4: [order 72] for n>73\end{quote}
...
\begin{theorem} For column $k = 1$ of A299612, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A299612, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 4\,a(n-3) - 10\,a(n-4) + 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A299612, the recurrence $a(n) = 0$ holds for every $n > 19$, and fails at $n = 19$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $64$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 19$; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 19$; the proved threshold is $n > 19$.

\begin{theorem} For column $k = 4$ of A299612, the recurrence $a(n) = 0$ holds for every $n > 73$, and fails at $n = 73$.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299612}{A299612}, revision 4, last modified Feb 14 2018.
