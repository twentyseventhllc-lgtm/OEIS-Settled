...
\title{Proofs of the conjectured column and row recurrences for OEIS A299321}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299321}{A299321} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 3, 4, 5, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -2*a(n-4) for n>5\\k=3: [order 10] for n>12\\k=4: [order 29] for n>30\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 06 2018). As of revision 4, last modified Feb 06 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
The condition is $c(i,j) \in \{0, 3, 4, 5, 6, 7\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A299321, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A299321, the recurrence $a(n) = a(n-1) + 3\,a(n-2) - 2\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A299321, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A299321, the recurrence $a(n) = 0$ holds for every $n > 30$, and fails at $n = 30$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $158$ after trimming, and $S = 31$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 30$; after that point more than $S = 31$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299321}{A299321}, revision 4, last modified Feb 06 2018.
