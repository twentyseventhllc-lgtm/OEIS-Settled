...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A125343}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A125343}{A125343} as

\begin{quote}\itshape Number of base 6 circular n-digit numbers with adjacent digits differing by 4 or less\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily G.f.: (1 - x\textasciicircum{}2 - 8*x\textasciicircum{}3) / ((1 - x)*(1 - 5*x - 4*x\textasciicircum{}2)).\end{quote}

\begin{quote}\ttfamily a(n) = 6*a(n-1) - a(n-2) - 4*a(n-3) for n>3.\end{quote}
...
\[\sum_{n \ge 0} a(n)\,x^n \;=\; \frac{1 - x^{2} - 8x^{3}}{1 - 6x + x^{2} + 4x^{3}}.\]
...
\[|d_i - d_{i+1}| \le 4\quad (1 \le i < n), \qquad |d_n - d_1| \le 4.\]
...
Let $G$ be the digraph on the vertex set $\{0,\dots,5\}$ with an edge from $i$ to $j$ exactly when $|i-j| \le 4$, and let $M$ be its adjacency matrix, a $6 \times 6$ symmetric 0--1 band matrix with $M_{ij} = 1$ iff $|i-j| \le 4$.
...
\begin{theorem} For A125343, the generating function is exactly $\sum_{n\ge 0} a(n)x^n = \frac{1 - x^{2} - 8x^{3}}{1 - 6x + x^{2} + 4x^{3}}$.\end{theorem}
...
\begin{theorem} For A125343, the recurrence $a(n) = 6\,a(n-1) - a(n-2) - 4\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (0) rather than fitted. The first of them are 1, 6, 34, 186, 1058, 6026, 34354, 195866,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A125343}{A125343}, revision 17, last modified Aug 12 2026.
