...
k=1: a(n) = a(n-2) +3*a(n-4) +2*a(n-6) for n>7\\k=2: a(n) = a(n-1) +2*a(n-3) +a(n-4)\\k=3: a(n) = 2*a(n-2) +9*a(n-4) +4*a(n-6) -6*a(n-8) +4*a(n-10) for n>11\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 04 2015). As of revision 8, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
