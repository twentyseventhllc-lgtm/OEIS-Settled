...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A208139}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208139}{A208139} as

\begin{quote}\itshape Number of n X 5 0..1 arrays avoiding 0 0 1 and 0 1 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = n\textasciicircum{}5 + 4*n\textasciicircum{}4 + 5*n\textasciicircum{}3 + 2*n\textasciicircum{}2.\end{quote}

\begin{quote}\ttfamily G.f.: 12*x*(1 + 6*x + 3*x\textasciicircum{}2) / (1 - x)\textasciicircum{}6.\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 13, last modified Jun 17 2026, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 2n^{2} + 5n^{3} + 4n^{4} + n^{5}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{12x + 72x^{2} + 36x^{3}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}.\]
...
For scale: at $n = 8$ there are $2^{40}$ arrays, about $10^{12}$, against $8$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A208139, $a(n)$ is the polynomial $2n^{2} + 5n^{3} + 4n^{4} + n^{5}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $5$ is annihilated by the difference operator $(E-1)^{6}$, whose characteristic polynomial is $(t-1)^{6}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 6 = 14$. Its values were computed exactly; all of them vanish, and more than $14$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A208139, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{12x + 72x^{2} + 36x^{3}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}$.\end{theorem}
...
\begin{theorem} For A208139, the recurrence $a(n) = 6\,a(n-1) - 15\,a(n-2) + 20\,a(n-3) - 15\,a(n-4) + 6\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 12, 144, 720, 2400, 6300, 14112, 28224, 51840,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208139}{A208139}, revision 13, last modified Jun 17 2026.
