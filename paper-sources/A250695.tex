...
\title{A proof of the conjectured linear recurrence for OEIS A250695}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250695}{A250695} as

\begin{quote}\itshape Number of (4+1)X(n+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction\end{quote}
...
That refinement leaves $98$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 51$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $4^{40}$ arrays, about $10^{24}$, against $51$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A250695, the recurrence of order $16$ displayed in Section 1 holds for every $n \ge 17$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,91$. Every one of them vanished. After it, more than $S = 51$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 18 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 11736, 76320, 383440, 1670016, 6671458, 25235016, 92169818, 329292956,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250695}{A250695}, revision 6, last modified Jul 23 2025.
