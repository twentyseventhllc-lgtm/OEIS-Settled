...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A295937}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295937}{A295937} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 1 or 3 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + a(n-2) + 9*a(n-3) - 4*a(n-4) - 4*a(n-5) - 4*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(2 + 6*x + 5*x\textasciicircum{}2 - 8*x\textasciicircum{}3 - 8*x\textasciicircum{}4 - 4*x\textasciicircum{}5) / (1 - x - x\textasciicircum{}2 - 9*x\textasciicircum{}3 + 4*x\textasciicircum{}4 + 4*x\textasciicircum{}5 + 4*x\textasciicircum{}6). - Colin Barker, Feb 22 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x + 6x^{2} + 5x^{3} - 8x^{4} - 8x^{5} - 4x^{6}}{1 - x - x^{2} - 9x^{3} + 4x^{4} + 4x^{5} + 4x^{6}}.\]
...
\begin{theorem} For A295937, the recurrence $a(n) = a(n-1) + a(n-2) + 9\,a(n-3) - 4\,a(n-4) - 4\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A295937, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x + 6x^{2} + 5x^{3} - 8x^{4} - 8x^{5} - 4x^{6}}{1 - x - x^{2} - 9x^{3} + 4x^{4} + 4x^{5} + 4x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 8, 15, 33, 104, 228, 529, 1469,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295937}{A295937}, revision 8, last modified Feb 22 2019.
