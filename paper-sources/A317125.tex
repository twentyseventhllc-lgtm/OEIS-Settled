...
\title{Proofs of the conjectured column and row recurrences for OEIS A317125}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317125}{A317125} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) -6*a(n-3) -8*a(n-4) for n>5\\k=3: [order 16]\\k=4: [order 57] for n>59\end{quote}
...
\begin{theorem} For column $k = 1$ of A317125, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317125, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) - 6\,a(n-3) - 8\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317125, the recurrence $a(n) = 0$ holds for every $n \ge 17$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A317125, the recurrence $a(n) = 0$ holds for every $n > 59$, and fails at $n = 59$.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317125}{A317125}, revision 4, last modified Jul 21 2018.
