...
\title{A proof of the conjectured linear recurrence for OEIS A305042}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305042}{A305042} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 2, 3, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +a(n-2) +5*a(n-3) -13*a(n-4) -8*a(n-5) +2*a(n-6) +19*a(n-7) +9*a(n-8) -4*a(n-9) -8*a(n-10) -4*a(n-11) for n>13\end{quote}
...
\begin{multline*} a(n) = 2\,a(n-1) + a(n-2) + 5\,a(n-3) - 13\,a(n-4) - 8\,a(n-5) + 2\,a(n-6) + \\ 19\,a(n-7) + 9\,a(n-8) - 4\,a(n-9) - 8\,a(n-10) - 4\,a(n-11). \end{multline*}
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $47$; neither removal changes any count.
...
\begin{theorem} For A305042, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + 5\,a(n-3) - 13\,a(n-4) - 8\,a(n-5) + 2\,a(n-6) + 19\,a(n-7) + 9\,a(n-8) - 4\,a(n-9) - 8\,a(n-10) - 4\,a(n-11)$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,66$. The last nonvanishing one is at $m = 2$, that is at $n = 13$. After it, more than $S = 15$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 7, 5, 9, 15, 17, 40, 73,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305042}{A305042}, revision 4, last modified May 24 2018.
