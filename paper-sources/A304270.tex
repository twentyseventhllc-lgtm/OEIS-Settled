...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +2*a(n-3) for n>4\\k=3: a(n) = a(n-1) +3*a(n-3) for n>4\\k=4: a(n) = a(n-1) +a(n-2) +5*a(n-3) +a(n-4) -3*a(n-5) -3*a(n-6) for n>7\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 09 2018). As of revision 4, last modified May 09 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
