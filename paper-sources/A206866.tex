...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A206866}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A206866}{A206866} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 horizontally and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) + 8*a(n-3) + 4*a(n-4) - 2*a(n-5) - a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(7 + 21*x + 15*x\textasciicircum{}2 - 11*x\textasciicircum{}3 - 9*x\textasciicircum{}4 - x\textasciicircum{}5) / (1 - 4*x - 8*x\textasciicircum{}3 - 4*x\textasciicircum{}4 + 2*x\textasciicircum{}5 + x\textasciicircum{}6). - Colin Barker, Jun 17 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{7x + 21x^{2} + 15x^{3} - 11x^{4} - 9x^{5} - x^{6}}{1 - 4x - 8x^{3} - 4x^{4} + 2x^{5} + x^{6}}.\]
...
\begin{theorem} For A206866, the recurrence $a(n) = 4\,a(n-1) + 8\,a(n-3) + 4\,a(n-4) - 2\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A206866, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{7x + 21x^{2} + 15x^{3} - 11x^{4} - 9x^{5} - x^{6}}{1 - 4x - 8x^{3} - 4x^{4} + 2x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 7, 49, 211, 889, 3967, 17737, 78799, 350017,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A206866}{A206866}, revision 11, last modified Jun 17 2018.
