...
\title{Proofs of the conjectured column and row recurrences for OEIS A256748}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A256748}{A256748} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 3 and no antidiagonal sum 3 and no row sum 0 and no column sum 0\end{quote}
...
k=1: [linear recurrence of order 9] for n>12\\k=2: [order 8] for n>13\\k=3: [order 12] for n>17\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 09 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A256748, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 12$; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 12$; the proved threshold is $n > 12$.

\begin{theorem} For column $k = 2$ of A256748, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 3$ of A256748, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 36$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 36$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A256748}{A256748}, revision 6, last modified Jun 02 2025.
