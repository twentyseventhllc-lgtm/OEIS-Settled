...
\title{Proofs of the conjectured column and row recurrences for OEIS A230275}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A230275}{A230275} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 3, and upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 4*a(n-1) -a(n-2) -2*a(n-3)\\k=3: [order 10]\\k=4: [order 34]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 14 2013). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A230275, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A230275, the recurrence $a(n) = 4\,a(n-1) - a(n-2) - 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $73$ states, $63$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A230275, the recurrence $a(n) = 0$ holds for every $n \ge 11$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $715$ states, $693$ after trimming, and $S = 18$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 18$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A230275, the recurrence $a(n) = 0$ holds for every $n \ge 35$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $6541$ states, $6495$ after trimming, and $S = 75$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 75$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A230275}{A230275}, revision 6, last modified Jul 23 2025.
