...
\title{Proofs of the conjectured column and row recurrences for OEIS A296963}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296963}{A296963} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 adjacent to 2, 3 or 5 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 4*a(n-1) -5*a(n-2) +11*a(n-3) -18*a(n-4) +4*a(n-5)\\k=3: [order 21]\\k=4: [order 51]\end{quote}
...
\begin{theorem} For column $k = 1$ of A296963, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A296963, the recurrence $a(n) = 4\,a(n-1) - 5\,a(n-2) + 11\,a(n-3) - 18\,a(n-4) + 4\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A296963, the recurrence $a(n) = 0$ holds for every $n \ge 22$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 22$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 22$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A296963, the recurrence $a(n) = 0$ holds for every $n \ge 52$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $268$ after trimming, and $S = 55$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 55$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296963}{A296963}, revision 4, last modified Dec 22 2017.
