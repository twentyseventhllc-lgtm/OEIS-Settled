...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207110}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207110}{A207110} as

\begin{quote}\itshape Number of n X 7 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (1/280)*n\textasciicircum{}7 + (7/45)*n\textasciicircum{}6 + (47/15)*n\textasciicircum{}5 + (206/9)*n\textasciicircum{}4 + (4111/120)*n\textasciicircum{}3 - (1037/45)*n\textasciicircum{}2 - (4493/210)*n + 9.\end{quote}

\begin{quote}\ttfamily G.f.: x*(25 + 425*x - 901*x\textasciicircum{}2 + 419*x\textasciicircum{}3 + 249*x\textasciicircum{}4 - 269*x\textasciicircum{}5 + 79*x\textasciicircum{}6 - 9*x\textasciicircum{}7) / (1 - x)\textasciicircum{}8.\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 11, last modified Jul 22 2025, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 9 - \frac{4493}{210}n - \frac{1037}{45}n^{2} + \frac{4111}{120}n^{3} + \frac{206}{9}n^{4} + \frac{47}{15}n^{5} + \frac{7}{45}n^{6} + \frac{1}{280}n^{7}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{25x + 425x^{2} - 901x^{3} + 419x^{4} + 249x^{5} - 269x^{6} + 79x^{7} - 9x^{8}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}.\]
...
That refinement leaves $116$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 58$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A207110, $a(n)$ is the polynomial $9 - \frac{4493}{210}n - \frac{1037}{45}n^{2} + \frac{4111}{120}n^{3} + \frac{206}{9}n^{4} + \frac{47}{15}n^{5} + \frac{7}{45}n^{6} + \frac{1}{280}n^{7}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $7$ is annihilated by the difference operator $(E-1)^{8}$, whose characteristic polynomial is $(t-1)^{8}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 8 = 66$. Its values were computed exactly; all of them vanish, and more than $66$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A207110, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{25x + 425x^{2} - 901x^{3} + 419x^{4} + 249x^{5} - 269x^{6} + 79x^{7} - 9x^{8}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}$.\end{theorem}
...
\begin{theorem} For A207110, the recurrence $a(n) = 8\,a(n-1) - 28\,a(n-2) + 56\,a(n-3) - 70\,a(n-4) + 56\,a(n-5) - 28\,a(n-6) + 8\,a(n-7) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,98$. Every one of them vanished. After it, more than $S = 58$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 25, 625, 3399, 11511, 30415, 68737, 139341, 260597,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207110}{A207110}, revision 11, last modified Jul 22 2025.
