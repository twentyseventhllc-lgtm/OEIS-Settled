...
\title{Proofs of the conjectured column and row recurrences for OEIS A298087}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298087}{A298087} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 2, 3, 5 or 6 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -4*a(n-4)\\k=3: [order 16] for n>17\\k=4: [order 57] for n>58\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 12 2018). As of revision 4, last modified Jan 12 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A298087, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298087, the recurrence $a(n) = a(n-1) + 3\,a(n-2) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298087, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $59$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 17$; the proved threshold is $n > 17$.

\begin{theorem} For column $k = 4$ of A298087, the recurrence $a(n) = 0$ holds for every $n > 58$, and fails at $n = 58$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $218$ after trimming, and $S = 58$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 58$; after that point more than $S = 58$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298087}{A298087}, revision 4, last modified Jan 12 2018.
