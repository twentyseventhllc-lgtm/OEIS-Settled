...
\title{Proofs of the conjectured column and row recurrences for OEIS A317871}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317871}{A317871} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 2, 3 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) +a(n-3) -a(n-4) -a(n-5) -a(n-6)\\k=3: [order 18]\\k=4: [order 83] for n>86\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 09 2018). As of revision 4, last modified Aug 09 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A317871, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317871, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + a(n-3) - a(n-4) - a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317871, the recurrence $a(n) = 0$ holds for every $n \ge 19$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $44$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A317871, the recurrence $a(n) = 0$ holds for every $n > 86$, and fails at $n = 86$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $190$ after trimming, and $S = 90$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 86$; after that point more than $S = 90$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317871}{A317871}, revision 4, last modified Aug 09 2018.
