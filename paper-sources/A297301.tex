...
\title{A proof of the conjectured linear recurrence for OEIS A297301}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297301}{A297301} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays with every 1 horizontally or antidiagonally adjacent to 2 neighboring 1s\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 6*a(n-2) + 7*a(n-3) - 4*a(n-4) + 3*a(n-5) + a(n-6) - 5*a(n-7) + 4*a(n-8) - 2*a(n-9) - a(n-10) - 5*a(n-12) - 3*a(n-13) for n > 16.\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Dec 27 2017).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified May 26 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 4\,a(n-1) - 6\,a(n-2) + 7\,a(n-3) - 4\,a(n-4) + 3\,a(n-5) + a(n-6) - \\ 5\,a(n-7) + 4\,a(n-8) - 2\,a(n-9) - a(n-10) - 5\,a(n-12) - 3\,a(n-13). \end{multline*}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $81$; neither removal changes any count.
...
\begin{theorem} For A297301, the recurrence $a(n) = 4\,a(n-1) - 6\,a(n-2) + 7\,a(n-3) - 4\,a(n-4) + 3\,a(n-5) + a(n-6) - 5\,a(n-7) + 4\,a(n-8) - 2\,a(n-9) - a(n-10) - 5\,a(n-12) - 3\,a(n-13)$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,66$. The last nonvanishing one is at $m = 3$, that is at $n = 16$. After it, more than $S = 18$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 1, 5, 10, 21, 50, 130, 332,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297301}{A297301}, revision 10, last modified May 26 2018.
