...
\title{Proofs of the conjectured column and row recurrences for OEIS A320402}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A320402}{A320402} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +a(n-3) -a(n-4)\\k=3: [order 12] for n>14\\k=4: [order 40] for n>41\end{quote}
...
\begin{theorem} For column $k = 1$ of A320402, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A320402, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A320402, the recurrence $a(n) = 0$ holds for every $n > 14$, and fails at $n = 14$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A320402, the recurrence $a(n) = 0$ holds for every $n > 41$, and fails at $n = 41$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 42$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 41$; after that point more than $S = 42$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A320402}{A320402}, revision 4, last modified Oct 12 2018.
