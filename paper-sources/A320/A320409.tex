...
\title{Proofs of the conjectured column and row recurrences for OEIS A320409}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A320409}{A320409} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 4 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +3*a(n-2) -2*a(n-3) -2*a(n-4) -a(n-6) +a(n-7)\\k=3: [order 23] for n>25\\k=4: [order 83] for n>85\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 12 2018). As of revision 4, last modified Oct 12 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A320409, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A320409, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 2\,a(n-3) - 2\,a(n-4) - a(n-6) + a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A320409, the recurrence $a(n) = 0$ holds for every $n > 25$, and fails at $n = 25$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 27$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 25$; after that point more than $S = 27$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 25$; the proved threshold is $n > 25$.

\begin{theorem} For column $k = 4$ of A320409, the recurrence $a(n) = 0$ holds for every $n > 85$, and fails at $n = 85$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $263$ after trimming, and $S = 95$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 85$; after that point more than $S = 95$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 85$; the proved threshold is $n > 85$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A320409}{A320409}, revision 4, last modified Oct 12 2018.
