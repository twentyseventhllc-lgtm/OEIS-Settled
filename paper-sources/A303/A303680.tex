...
\title{A proof of the conjectured linear recurrence for OEIS A303680}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303680}{A303680} as

\begin{quote}\itshape Number of nX6 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-4) +a(n-7) for n>12\end{quote}
...
The construction gives $4160$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $96$; neither removal changes any count.
...
\begin{theorem} For A303680, the recurrence $a(n) = a(n-1) + a(n-4) + a(n-7)$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,68$. The last nonvanishing one is at $m = 5$, that is at $n = 12$. After it, more than $S = 18$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 13, 49, 39, 64, 110, 177, 245, 335,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303680}{A303680}, revision 4, last modified Apr 28 2018.
