...
\title{Proofs of the conjectured column and row recurrences for OEIS A303969}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303969}{A303969} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 3 or 5 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +8*a(n-3) -4*a(n-4)\\k=3: a(n) = a(n-1) +2*a(n-3) +2*a(n-4) +a(n-5) +a(n-6) -a(n-7) -a(n-11) -a(n-12)\\k=4: [order 36] for n>37\\k=5: [order 24] for n>35\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 03 2018). As of revision 4, last modified May 03 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A303969, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A303969, the recurrence $a(n) = a(n-1) + 8\,a(n-3) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A303969, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + 2\,a(n-4) + a(n-5) + a(n-6) - a(n-7) - a(n-11) - a(n-12)$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A303969, the recurrence $a(n) = 0$ holds for every $n > 37$, and fails at $n = 37$.\end{theorem}
...
\begin{theorem} For column $k = 5$ of A303969, the recurrence $a(n) = 0$ holds for every $n > 35$, and fails at $n = 35$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $219$ after trimming, and $S = 48$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 35$; after that point more than $S = 48$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303969}{A303969}, revision 4, last modified May 03 2018.
