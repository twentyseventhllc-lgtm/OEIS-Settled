...
\title{Proofs of the conjectured column and row recurrences for OEIS A303896}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303896}{A303896} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3 or 4 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) +4*a(n-3) +4*a(n-4)\\k=3: [order 13] for n>14\\k=4: [order 35] for n>40\\k=5: [order 92] for n>98\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 02 2018). As of revision 4, last modified May 02 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A303896, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A303896, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 4\,a(n-3) + 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A303896, the recurrence $a(n) = 0$ holds for every $n > 14$, and fails at $n = 14$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $64$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 14$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 14$; the proved threshold is $n > 14$.

\begin{theorem} For column $k = 4$ of A303896, the recurrence $a(n) = 0$ holds for every $n > 40$, and fails at $n = 40$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $217$ after trimming, and $S = 47$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 40$; after that point more than $S = 47$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 40$; the proved threshold is $n > 40$.

\begin{theorem} For column $k = 5$ of A303896, the recurrence $a(n) = 0$ holds for every $n > 98$, and fails at $n = 98$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $723$ after trimming, and $S = 118$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 98$; after that point more than $S = 118$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 98$; the proved threshold is $n > 98$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303896}{A303896}, revision 4, last modified May 02 2018.
