...
\title{A proof of the conjectured linear recurrence for OEIS A303723}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303723}{A303723} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every element unequal to 0, 1, 2 or 3 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +a(n-3) +a(n-4) -a(n-5) -2*a(n-6) for n>8.\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Apr 29 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 6, last modified Apr 30 2022, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\begin{theorem} For A303723, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-3) + a(n-4) - a(n-5) - 2\,a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. The last nonvanishing one is at $m = 2$, that is at $n = 8$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 49, 46, 84, 133, 221, 406, 743,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303723}{A303723}, revision 6, last modified Apr 30 2022.
