...
\title{A proof of the conjectured linear recurrence for OEIS A303795}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303795}{A303795} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 2 or 4 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +2*a(n-3) -a(n-5) -2*a(n-6) -2*a(n-7) for n>10\end{quote}
...
\begin{theorem} For A303795, the recurrence $a(n) = a(n-1) + a(n-2) + 2\,a(n-3) - a(n-5) - 2\,a(n-6) - 2\,a(n-7)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,56$. The last nonvanishing one is at $m = 3$, that is at $n = 10$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 12, 17, 30, 70, 121, 221, 421,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303795}{A303795}, revision 4, last modified Apr 30 2018.
