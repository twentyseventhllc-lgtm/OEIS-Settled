...
\title{A proof of the conjectured order of the recurrence for OEIS A303687}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303687}{A303687} as

\begin{quote}\itshape Number of nX5 0..1 arrays with every element unequal to 2, 3 or 4 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical recurrence of order 93 (see link above)\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Apr 28 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 5, last modified Apr 28 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
That refinement leaves $125$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 110$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $2^{40}$ arrays, about $10^{12}$, against $110$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 61, 27, 343, 349, 2809, 4619, 29475,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303687}{A303687}, revision 5, last modified Apr 28 2018.
