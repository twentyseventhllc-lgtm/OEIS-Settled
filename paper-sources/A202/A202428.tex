...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A202428}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A202428}{A202428} as

\begin{quote}\itshape Number of (n+2) X 3 binary arrays avoiding patterns 000 and 001 in rows, columns and nw-to-se diagonals\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +7*a(n-2) +6*a(n-3) -a(n-4) -2*a(n-5) +a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 6*x*(18 + 47*x + 33*x\textasciicircum{}2 - 7*x\textasciicircum{}3 - 11*x\textasciicircum{}4 + 6*x\textasciicircum{}5) / (1 - x - 7*x\textasciicircum{}2 - 6*x\textasciicircum{}3 + x\textasciicircum{}4 + 2*x\textasciicircum{}5 - x\textasciicircum{}6). - Colin Barker, May 28 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Dec 19 2011).

The line quoted ending in that signature is contributed by Colin Barker on May 28 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified May 28 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = a(n-1) + 7\,a(n-2) + 6\,a(n-3) - a(n-4) - 2\,a(n-5) + a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{108x + 282x^{2} + 198x^{3} - 42x^{4} - 66x^{5} + 36x^{6}}{1 - x - 7x^{2} - 6x^{3} + x^{4} + 2x^{5} - x^{6}}.\]
...
\begin{theorem} For A202428, the recurrence $a(n) = a(n-1) + 7\,a(n-2) + 6\,a(n-3) - a(n-4) - 2\,a(n-5) + a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A202428, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{108x + 282x^{2} + 198x^{3} - 42x^{4} - 66x^{5} + 36x^{6}}{1 - x - 7x^{2} - 6x^{3} + x^{4} + 2x^{5} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 108, 390, 1344, 4680, 16254, 56508, 196350, 682452,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A202428}{A202428}, revision 9, last modified May 28 2018.
