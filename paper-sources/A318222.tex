...
\title{Proofs of the conjectured column and row recurrences for OEIS A318222}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318222}{A318222} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 2, 3, 4 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) +2*a(n-3) +3*a(n-4) +a(n-5) for n>6\\k=3: [order 11]\\k=4: [order 36] for n>39\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 21 2018). As of revision 4, last modified Aug 21 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A318222, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318222, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + 2\,a(n-3) + 3\,a(n-4) + a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A318222, the recurrence $a(n) = 0$ holds for every $n \ge 12$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $52$ after trimming, and $S = 17$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 17$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A318222, the recurrence $a(n) = 0$ holds for every $n > 39$, and fails at $n = 39$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $200$ after trimming, and $S = 44$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 39$; after that point more than $S = 44$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318222}{A318222}, revision 4, last modified Aug 21 2018.
