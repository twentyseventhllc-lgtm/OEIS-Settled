...
\title{A proof of the conjectured linear recurrence for OEIS A305581}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305581}{A305581} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 2, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +a(n-3) for n>5\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Jun 05 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 15, last modified Jun 20 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\begin{theorem} For A305581, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-3)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,48$. The last nonvanishing one is at $m = 2$, that is at $n = 5$. After it, more than $S = 5$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 3, 6, 8, 19, 33, 60, 112,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305581}{A305581}, revision 15, last modified Jun 20 2018.
