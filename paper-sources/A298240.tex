...
The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 15 2018). As of revision 8, last modified Jan 29 2018, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
Let $q = 2$ and let $A$ be an $n \times 1$ array with entries in $\{0,1,\dots,1\}$, rows indexed $0 \le i < n$ and columns $0 \le j < 1$.
...
\[c(i,j) \;=\; \#\bigl\{(\delta,\varepsilon)\in\mathcal{N} \;:\; 0\le i+\delta<n,\ 0\le j+\varepsilon<1,\ A[i+\delta][j+\varepsilon] = A[i][j]\bigr\},\]
