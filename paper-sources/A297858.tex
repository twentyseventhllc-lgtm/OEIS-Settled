...
\title{Proofs of the conjectured column and row recurrences for OEIS A297858}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297858}{A297858} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2 or 4 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) -2*a(n-2) +4*a(n-3) -10*a(n-4) +4*a(n-5) for n>6\\k=3: a(n) = 2*a(n-1) -a(n-4) -a(n-5) -a(n-6) +a(n-7) +a(n-8) for n>9\\k=4: [order 32] for n>37\\k=5: [order 76] for n>81\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 07 2018). As of revision 4, last modified Jan 07 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A297858, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297858, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 4\,a(n-3) - 10\,a(n-4) + 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297858, the recurrence $a(n) = 2\,a(n-1) - a(n-4) - a(n-5) - a(n-6) + a(n-7) + a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A297858, the recurrence $a(n) = 0$ holds for every $n > 37$, and fails at $n = 37$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $162$ after trimming, and $S = 42$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 37$; after that point more than $S = 42$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 37$; the proved threshold is $n > 37$.

\begin{theorem} For column $k = 5$ of A297858, the recurrence $a(n) = 0$ holds for every $n > 81$, and fails at $n = 81$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $429$ after trimming, and $S = 93$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 81$; after that point more than $S = 93$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 81$; the proved threshold is $n > 81$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297858}{A297858}, revision 4, last modified Jan 07 2018.
