...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208689}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208689}{A208689} as

\begin{quote}\itshape Number of 3 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 1 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 4*a(n-2) - 3*a(n-3).\end{quote}

\begin{quote}\ttfamily G.f.: 6*x*(1 + 4*x - 3*x\textasciicircum{}2) / ((1 - 3*x)*(1 + x - x\textasciicircum{}2)).\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 24x^{2} - 18x^{3}}{1 - 2x - 4x^{2} + 3x^{3}}.\]
...
\begin{theorem} For A208689, the recurrence $a(n) = 2\,a(n-1) + 4\,a(n-2) - 3\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208689, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 24x^{2} - 18x^{3}}{1 - 2x - 4x^{2} + 3x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 78, 282, 768, 2430, 7086, 21588,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208689}{A208689}, revision 8, last modified Mar 07 2018.
