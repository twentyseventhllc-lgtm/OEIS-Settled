...
\title{A proof of the conjectured generating function for OEIS A250655}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250655}{A250655} as
...
\begin{quote}\ttfamily Empirical g.f.: x*(81 - 74*x) / ((1 - x)*(1 - 2*x)). - Colin Barker, Nov 15 2018\end{quote}

The line quoted above is contributed by Colin Barker on Nov 15 2018.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{81x - 74x^{2}}{1 - 3x + 2x^{2}}.\]
...
That refinement leaves $10$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 4$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $2^{64}$ arrays, about $10^{19}$, against $4$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A250655, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{81x - 74x^{2}}{1 - 3x + 2x^{2}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 81, 169, 345, 697, 1401, 2809, 5625, 11257,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250655}{A250655}, revision 7, last modified Nov 15 2018.
