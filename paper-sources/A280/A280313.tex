...
\title{Proofs of the conjectured column and row recurrences for OEIS A280313}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A280313}{A280313} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to more than one of its horizontal and vertical neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = 6*a(n-1) -6*a(n-2) -16*a(n-3) +12*a(n-4) +24*a(n-5) +8*a(n-6) for n>10\\k=2: [order 12]\\k=3: [order 36] for n>38\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 31 2016). As of revision 4, last modified Dec 31 2016, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A280313, the recurrence $a(n) = 6\,a(n-1) - 6\,a(n-2) - 16\,a(n-3) + 12\,a(n-4) + 24\,a(n-5) + 8\,a(n-6)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A280313, the recurrence $a(n) = 0$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $366$ states, $247$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A280313, the recurrence $a(n) = 0$ holds for every $n > 38$, and fails at $n = 38$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $2764$ states, $1630$ after trimming, and $S = 53$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 38$; after that point more than $S = 53$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A280313}{A280313}, revision 4, last modified Dec 31 2016.
