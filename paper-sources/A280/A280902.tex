...
\title{Proofs of the conjectured column and row recurrences for OEIS A280902}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A280902}{A280902} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to more than one of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: [order 6] for n>8\\k=3: [order 6] for n>8\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 10 2017). As of revision 4, last modified Jan 10 2017, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} In A280902, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A280902, the recurrence $a(n) = 0$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $380$ states, $269$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 8$; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 8$; the proved threshold is $n > 8$.

\begin{theorem} For column $k = 3$ of A280902, the recurrence $a(n) = 0$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $2861$ states, $1784$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 8$; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A280902}{A280902}, revision 4, last modified Jan 10 2017.
