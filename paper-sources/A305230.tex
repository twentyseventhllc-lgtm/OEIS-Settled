...
\title{Proofs of the conjectured column and row recurrences for OEIS A305230}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305230}{A305230} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +a(n-2) -4*a(n-3) -4*a(n-4) for n>5\\k=3: a(n) = 2*a(n-1) +a(n-2) -3*a(n-4) -a(n-5) +2*a(n-6) for n>9\\k=4: a(n) = a(n-1) +3*a(n-2) +a(n-3) -a(n-4) -3*a(n-5) -2*a(n-6) +2*a(n-7) for n>9\\k=5: [order 12] for n>16\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 27 2018). As of revision 4, last modified May 27 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A305230, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A305230, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 4\,a(n-3) - 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A305230, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - 3\,a(n-4) - a(n-5) + 2\,a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $38$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For column $k = 4$ of A305230, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + a(n-3) - a(n-4) - 3\,a(n-5) - 2\,a(n-6) + 2\,a(n-7)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $88$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For column $k = 5$ of A305230, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $220$ after trimming, and $S = 22$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 16$; after that point more than $S = 22$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305230}{A305230}, revision 4, last modified May 27 2018.
