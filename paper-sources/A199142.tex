...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A199142}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A199142}{A199142} as

\begin{quote}\itshape Number of n X 1 0..3 arrays with values 0..3 introduced in row major order and each element equal to one or two horizontal and vertical neighbors\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + a(n-2) - 7*a(n-3) + a(n-4) + 3*a(n-5) for n>6.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x\textasciicircum{}2*(1 - 2*x - 2*x\textasciicircum{}2 + 3*x\textasciicircum{}3 + x\textasciicircum{}4) / ((1 - x)*(1 - x - x\textasciicircum{}2)*(1 - x - 3*x\textasciicircum{}2)). - Colin Barker, Feb 22 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x^{2} - 2x^{3} - 2x^{4} + 3x^{5} + x^{6}}{1 - 3x - x^{2} + 7x^{3} - x^{4} - 3x^{5}}.\]
...
The reachable part of the digraph has $61$ states, $32$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 7$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of that $7\times7$ matrix; the bound is computed, not assumed.
...
\begin{theorem} For A199142, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 7\,a(n-3) + a(n-4) + 3\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 1$, that is at $n = 6$. After it, more than $S = 7$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 6$. The theorem is at least as strong.

\begin{theorem} For A199142, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x^{2} - 2x^{3} - 2x^{4} + 3x^{5} + x^{6}}{1 - 3x - x^{2} + 7x^{3} - x^{4} - 3x^{5}}$.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A199142}{A199142}, revision 8, last modified Feb 22 2018.
