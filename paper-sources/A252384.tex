...
\title{Proofs of the conjectured column and row recurrences for OEIS A252384}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252384}{A252384} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 3 5 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 3 5 6 or 7\end{quote}
...
k=1: a(n) = 5*a(n-3) -8*a(n-6) +5*a(n-9) -a(n-12) for n>22\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: [linear recurrence of order 65] for n>79\end{quote}
...
\begin{theorem} For column $k = 1$ of A252384, the recurrence $a(n) = 5\,a(n-3) - 8\,a(n-6) + 5\,a(n-9) - a(n-12)$ holds for every $n > 22$, and fails at $n = 22$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 73$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 22$; after that point more than $S = 73$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 22$; the proved threshold is $n > 22$.

\begin{theorem} For row $n = 1$ of A252384, the recurrence $a(k) = 0$ holds for every $k > 79$, and fails at $k = 79$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 130$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 79$; after that point more than $S = 130$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $k > 79$; the proved threshold is $k > 79$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252384}{A252384}, revision 6, last modified Jun 02 2025.
