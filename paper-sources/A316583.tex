...
\title{Proofs of the conjectured column and row recurrences for OEIS A316583}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316583}{A316583} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3, 4, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) +4*a(n-3) +4*a(n-4)\\k=3: [order 15] for n>16\\k=4: [order 46] for n>48\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 07 2018). As of revision 4, last modified Jul 07 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
The condition is $c(i,j) \in \{1, 2, 3, 4, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A316583, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316583, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 4\,a(n-3) + 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316583, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A316583, the recurrence $a(n) = 0$ holds for every $n > 48$, and fails at $n = 48$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 55$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 48$; after that point more than $S = 55$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316583}{A316583}, revision 4, last modified Jul 07 2018.
