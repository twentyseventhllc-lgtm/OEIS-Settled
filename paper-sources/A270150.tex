...
\title{Proofs of the conjectured column and row recurrences for OEIS A270150}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A270150}{A270150} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..7 arrays with some element plus some horizontally, diagonally, antidiagonally or vertically adjacent neighbor totalling seven exactly once\end{quote}
...
k=1: a(n) = 14*a(n-1) -49*a(n-2)\\k=2: a(n) = 66*a(n-1) -1021*a(n-2) -2244*a(n-3) -1156*a(n-4) for n>5\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 12 2016). As of revision 4, last modified Mar 12 2016, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A270150, the recurrence $a(n) = 14\,a(n-1) - 49\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $25$ states, $17$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A270150, the recurrence $a(n) = 66\,a(n-1) - 1021\,a(n-2) - 2244\,a(n-3) - 1156\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $185$ states, $121$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A270150}{A270150}, revision 4, last modified Mar 12 2016.
