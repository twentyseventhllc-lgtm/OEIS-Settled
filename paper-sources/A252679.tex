...
\title{Proofs of the conjectured column and row recurrences for OEIS A252679}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252679}{A252679} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 5 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 5 6 or 7\end{quote}
...
k=1: [linear recurrence of order 48] for n>57\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: [linear recurrence of order 38] for n>48\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 20 2014). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence and the 1 row recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A252679, the recurrence $a(n) = 0$ holds for every $n > 57$, and fails at $n = 57$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 107$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 57$; after that point more than $S = 107$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 57$; the proved threshold is $n > 57$.

\begin{theorem} For row $n = 1$ of A252679, the recurrence $a(k) = 0$ holds for every $k > 48$, and fails at $k = 48$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 122$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 48$; after that point more than $S = 122$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $k > 48$; the proved threshold is $k > 48$.
...
\item 1 line: state space 16781312 > cap 3000000
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252679}{A252679}, revision 6, last modified Jun 02 2025.
