...
k=1: a(n) = a(n-1)\\k=2: a(n) = 3*a(n-1) +a(n-2) -2*a(n-4)\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = a(n-1) +a(n-2)\\n=2: a(n) = 2*a(n-1) +3*a(n-2) +6*a(n-3) +10*a(n-4) +4*a(n-5) for n>6\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 20 2018). As of revision 6, last modified May 11 2023, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences and the 2 row recurrences listed above.
