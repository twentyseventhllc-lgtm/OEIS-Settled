...
\title{A proof of the conjectured linear recurrence for OEIS A303964}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303964}{A303964} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 3 or 5 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +2*a(n-3) +2*a(n-4) +a(n-5) +a(n-6) -a(n-7) -a(n-11) -a(n-12)\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 2\,a(n-3) + 2\,a(n-4) + a(n-5) + a(n-6) - a(n-7) - \\ a(n-11) - a(n-12). \end{multline*}
...
\begin{theorem} For A303964, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + 2\,a(n-4) + a(n-5) + a(n-6) - a(n-7) - a(n-11) - a(n-12)$ holds for every $n \ge 13$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,62$. Every one of them vanished. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 9, 13, 26, 54, 101, 186, 361,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303964}{A303964}, revision 4, last modified May 03 2018.
