...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196012}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196012}{A196012} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element equal to the number its horizontal and vertical neighbors within one of itself\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +3*a(n-4) -a(n-5) -2*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 - x)*(1 + x + x\textasciicircum{}2 + 5*x\textasciicircum{}3 + 6*x\textasciicircum{}4) / (1 - x - x\textasciicircum{}2 - 3*x\textasciicircum{}4 + x\textasciicircum{}5 + 2*x\textasciicircum{}6). - Colin Barker, May 08 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Sep 26 2011).
...
\[a(n) = a(n-1) + a(n-2) + 3\,a(n-4) - a(n-5) - 2\,a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 4x^{4} + x^{5} - 6x^{6}}{1 - x - x^{2} - 3x^{4} + x^{5} + 2x^{6}}.\]
...
The condition is that for every cell, the number of its neighbours whose value is within one of itself equals the cell's own value.
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $42$; neither removal changes any count.
...
\begin{theorem} For A196012, the recurrence $a(n) = a(n-1) + a(n-2) + 3\,a(n-4) - a(n-5) - 2\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196012, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 4x^{4} + x^{5} - 6x^{6}}{1 - x - x^{2} - 3x^{4} + x^{5} + 2x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 1, 2, 7, 13, 16, 32, 65,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196012}{A196012}, revision 9, last modified May 08 2018.
