...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A231338}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A231338}{A231338} as

\begin{quote}\itshape Number of (n+1) X (2+1) 0..3 arrays with no element unequal to a strict majority of its horizontal and vertical neighbors, with values 0..3 introduced in row major order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) - 6*a(n-2) + 2*a(n-3) - 8*a(n-4) + 6*a(n-5) - a(n-6) + 3*a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(4 - 11*x + x\textasciicircum{}2 - 5*x\textasciicircum{}3 + 10*x\textasciicircum{}4 + 2*x\textasciicircum{}5 + 3*x\textasciicircum{}6) / ((1 - x)*(1 - 3*x)*(1 + x + x\textasciicircum{}2)*(1 - 2*x - x\textasciicircum{}3)). - Colin Barker, Sep 28 2018\end{quote}
...
\begin{multline*} a(n) = 5\,a(n-1) - 6\,a(n-2) + 2\,a(n-3) - 8\,a(n-4) + 6\,a(n-5) - a(n-6) + \\ 3\,a(n-7). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x - 11x^{2} + x^{3} - 5x^{4} + 10x^{5} + 2x^{6} + 3x^{7}}{1 - 5x + 6x^{2} - 2x^{3} + 8x^{4} - 6x^{5} + x^{6} - 3x^{7}}.\]
...
The reachable part of the digraph has $3565$ states, $254$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 12$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of that $12\times12$ matrix. The bound is computed, not assumed.
...
\begin{theorem} For A231338, the recurrence $a(n) = 5\,a(n-1) - 6\,a(n-2) + 2\,a(n-3) - 8\,a(n-4) + 6\,a(n-5) - a(n-6) + 3\,a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A231338, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x - 11x^{2} + x^{3} - 5x^{4} + 10x^{5} + 2x^{6} + 3x^{7}}{1 - 5x + 6x^{2} - 2x^{3} + 8x^{4} - 6x^{5} + x^{6} - 3x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 9, 22, 59, 159, 439, 1236, 3527,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A231338}{A231338}, revision 8, last modified Sep 28 2018.
