...
\title{Proofs of the conjectured column and row recurrences for OEIS A326105}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A326105}{A326105} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 3 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) -a(n-3) -a(n-4)\\k=3: a(n) = 5*a(n-1) +5*a(n-2) -2*a(n-3) -3*a(n-6) -2*a(n-7) -a(n-8) -a(n-9)\\k=4: [order 22]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jun 06 2019). As of revision 4, last modified Jun 06 2019, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A326105, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A326105, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) - a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A326105, the recurrence $a(n) = 5\,a(n-1) + 5\,a(n-2) - 2\,a(n-3) - 3\,a(n-6) - 2\,a(n-7) - a(n-8) - a(n-9)$ holds for every $n \ge 10$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $66$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A326105, the recurrence $a(n) = 0$ holds for every $n \ge 23$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $236$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A326105}{A326105}, revision 4, last modified Jun 06 2019.
