...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) -a(n-3) -a(n-4)\\k=3: a(n) = 5*a(n-1) +5*a(n-2) -2*a(n-3) -3*a(n-6) -2*a(n-7) -a(n-8) -a(n-9)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jun 06 2019). As of revision 4, last modified Jun 06 2019, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
