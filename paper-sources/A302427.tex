...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) -a(n-3) -2*a(n-4) +a(n-5)\\k=3: a(n) = a(n-1) +10*a(n-3) -8*a(n-4) for n>9\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = a(n-1) +a(n-2)\\n=2: a(n) = a(n-1) +16*a(n-4) -8*a(n-5) for n>6\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 07 2018). As of revision 4, last modified Apr 07 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences and the 2 row recurrences listed above.
