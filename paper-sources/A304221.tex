...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +4*a(n-3) -2*a(n-4) for n>5\\k=3: a(n) = a(n-1) +a(n-3) +2*a(n-4) for n>5\\k=4: a(n) = a(n-1) +2*a(n-3) +a(n-4) +a(n-5) -2*a(n-7) for n>9\\k=5: a(n) = a(n-1) +3*a(n-3) +a(n-4) +a(n-6) -2*a(n-7) -a(n-10) for n>14\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 08 2018). As of revision 5, last modified May 08 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
