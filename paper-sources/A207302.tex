...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207302}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207302}{A207302} as

\begin{quote}\itshape Number of n X 5 0..1 arrays avoiding 0 0 0 and 1 0 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (8/3)*n\textasciicircum{}4 + (49/3)*n\textasciicircum{}3 + (16/3)*n\textasciicircum{}2 - (43/3)*n + 3.\end{quote}

\begin{quote}\ttfamily G.f.: x*(13 + 104*x - 50*x\textasciicircum{}2 - 6*x\textasciicircum{}3 + 3*x\textasciicircum{}4) / (1 - x)\textasciicircum{}5.\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Jun 21 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 3 - \frac{43}{3}n + \frac{16}{3}n^{2} + \frac{49}{3}n^{3} + \frac{8}{3}n^{4}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{13x + 104x^{2} - 50x^{3} - 6x^{4} + 3x^{5}}{1 - 5x + 10x^{2} - 10x^{3} + 5x^{4} - x^{5}}.\]
...
That refinement leaves $22$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 15$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A207302, $a(n)$ is the polynomial $3 - \frac{43}{3}n + \frac{16}{3}n^{2} + \frac{49}{3}n^{3} + \frac{8}{3}n^{4}$ for every $n \ge 1$.\end{theorem}
...
\begin{theorem} For A207302, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{13x + 104x^{2} - 50x^{3} - 6x^{4} + 3x^{5}}{1 - 5x + 10x^{2} - 10x^{3} + 5x^{4} - x^{5}}$.\end{theorem}
...
\begin{theorem} For A207302, the recurrence $a(n) = 5\,a(n-1) - 10\,a(n-2) + 10\,a(n-3) - 5\,a(n-4) + a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,58$. Every one of them vanished. After it, more than $S = 15$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 13, 169, 665, 1759, 3773, 7093, 12169, 19515,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207302}{A207302}, revision 10, last modified Jun 21 2018.
