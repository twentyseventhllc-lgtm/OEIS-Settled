...
\title{Proofs of the conjectured column and row recurrences for OEIS A303040}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303040}{A303040} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 3, 4 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +a(n-2) -a(n-3) -2*a(n-4) +a(n-5)\\k=3: [order 13] for n>16\\k=4: [order 70]\end{quote}
...
\begin{theorem} For column $k = 1$ of A303040, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A303040, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - a(n-3) - 2\,a(n-4) + a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A303040, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A303040, the recurrence $a(n) = 0$ holds for every $n \ge 71$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $260$ after trimming, and $S = 72$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 72$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For row $n = 1$ of A303040, the recurrence $a(k) = a(n-1) + a(n-2)$ holds for every $k \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For row $n = 2$ of A303040, the recurrence $a(k) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) + 12\,a(n-4) - 16\,a(n-5)$ holds for every $k > 6$, and fails at $k = 6$.\end{theorem}
...
\begin{theorem} For row $n = 3$ of A303040, the recurrence $a(k) = 0$ holds for every $k > 12$, and fails at $k = 12$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $54$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of row $n = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 12$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $k > 12$; the proved threshold is $k > 12$.

\begin{theorem} For row $n = 4$ of A303040, the recurrence $a(k) = 0$ holds for every $k > 62$, and fails at $k = 62$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $256$ after trimming, and $S = 69$ blocks after refinement. The model reproduces every published value of row $n = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 62$; after that point more than $S = 69$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303040}{A303040}, revision 4, last modified Apr 17 2018.
