...
\title{Proofs of the conjectured column and row recurrences for OEIS A269276}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A269276}{A269276} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling three exactly once\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 18*a(n-1) -81*a(n-2)\\k=3: a(n) = 42*a(n-1) -441*a(n-2)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 21 2016). As of revision 4, last modified Feb 21 2016, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} In A269276, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A269276, the recurrence $a(n) = 18\,a(n-1) - 81\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $45$ states, $29$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A269276, the recurrence $a(n) = 42\,a(n-1) - 441\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $161$ states, $97$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A269276}{A269276}, revision 4, last modified Feb 21 2016.
