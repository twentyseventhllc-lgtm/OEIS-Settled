...
k=1: a(n) = 2*a(n-1) +a(n-2) -2*a(n-4) -2*a(n-5) for n>7\\k=2: a(n) = a(n-1) +a(n-2) +5*a(n-3) +2*a(n-4) +a(n-5) -a(n-7) -a(n-8) for n>9\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 19 2015). As of revision 9, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
