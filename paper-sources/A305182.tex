...
\title{Proofs of the conjectured column and row recurrences for OEIS A305182}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305182}{A305182} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 5, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-2) +2*a(n-3) -2*a(n-4) for n>6\\k=3: a(n) = 4*a(n-3) +4*a(n-4) -a(n-6) -8*a(n-7) -2*a(n-9) -4*a(n-10) for n>13\\k=4: [order 11] for n>18\\k=5: [order 43] for n>50\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 26 2018). As of revision 4, last modified May 26 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
The entry publishes its table by upward antidiagonals, so every published term is a value $T(n,k)$ with both indices known. All 73 of them are used as the data gate below: the models are required to reproduce the whole published triangle, not merely the column or row a given line is about.
...
\begin{theorem} For column $k = 1$ of A305182, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A305182, the recurrence $a(n) = 3\,a(n-2) + 2\,a(n-3) - 2\,a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A305182, the recurrence $a(n) = 4\,a(n-3) + 4\,a(n-4) - a(n-6) - 8\,a(n-7) - 2\,a(n-9) - 4\,a(n-10)$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $48$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 4$ of A305182, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $83$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 18$; the proved threshold is $n > 18$.

\begin{theorem} For column $k = 5$ of A305182, the recurrence $a(n) = 0$ holds for every $n > 50$, and fails at $n = 50$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $277$ after trimming, and $S = 60$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 50$; after that point more than $S = 60$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 50$; the proved threshold is $n > 50$.
...
\item {\bfseries The published table.} Every one of the 73 values $T(n,k)$ the entry publishes was recomputed from the models and agrees exactly, in integer arithmetic, with the indices read off the entry's offset (1) and its stated shape.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305182}{A305182}, revision 4, last modified May 26 2018.
