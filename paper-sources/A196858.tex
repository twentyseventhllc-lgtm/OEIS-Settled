...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196858}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196858}{A196858} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,4,2,0,1 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) -a(n-2) +3*a(n-3) -2*a(n-4) +2*a(n-5) -a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x + 2*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + 2*x\textasciicircum{}4 - x\textasciicircum{}5) / (1 - 2*x + x\textasciicircum{}2 - 3*x\textasciicircum{}3 + 2*x\textasciicircum{}4 - 2*x\textasciicircum{}5 + x\textasciicircum{}6). - Colin Barker, May 09 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + x^{2} + 2x^{3} - 2x^{4} + 2x^{5} - x^{6}}{1 - 2x + x^{2} - 3x^{3} + 2x^{4} - 2x^{5} + x^{6}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 3, \tau(1) = 4, \tau(2) = 2, \tau(3) = 0, \tau(4) = 1. The condition is that for every cell,
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $28$; neither removal changes any count.
...
For scale: at $n = 8$ there are $5^{24}$ arrays, about $10^{17}$, against $7$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A196858, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 3\,a(n-3) - 2\,a(n-4) + 2\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196858, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + x^{2} + 2x^{3} - 2x^{4} + 2x^{5} - x^{6}}{1 - 2x + x^{2} - 3x^{3} + 2x^{4} - 2x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 7, 12, 26, 56, 113, 235,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196858}{A196858}, revision 8, last modified May 09 2018.
