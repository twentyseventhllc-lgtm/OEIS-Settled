...
\title{Proofs of the conjectured column and row recurrences for OEIS A316815}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316815}{A316815} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3, 4, 5 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1)\\k=3: a(n) = 6*a(n-1) +10*a(n-2) -7*a(n-3) -64*a(n-4) -51*a(n-5) for n>6\\k=4: [order 17] for n>18\end{quote}
...
\begin{theorem} For column $k = 1$ of A316815, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316815, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316815, the recurrence $a(n) = 6\,a(n-1) + 10\,a(n-2) - 7\,a(n-3) - 64\,a(n-4) - 51\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 4$ of A316815, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316815}{A316815}, revision 4, last modified Jul 14 2018.
