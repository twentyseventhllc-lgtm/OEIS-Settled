...
\title{A proof of the conjectured linear recurrence for OEIS A317760}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317760}{A317760} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1 or 2 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +a(n-2) -2*a(n-3) -a(n-4) +a(n-5) for n>6\end{quote}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $40$; neither removal changes any count.
...
\begin{theorem} For A317760, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - 2\,a(n-3) - a(n-4) + a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,52$. The last nonvanishing one is at $m = 1$, that is at $n = 6$. After it, more than $S = 9$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 20, 28, 43, 72, 127, 232, 432,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317760}{A317760}, revision 4, last modified Aug 06 2018.
