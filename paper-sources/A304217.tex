...
\title{A proof of the conjectured linear recurrence for OEIS A304217}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304217}{A304217} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1, 3 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +2*a(n-3) +a(n-4) +a(n-5) -2*a(n-7) for n>9\end{quote}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $43$; neither removal changes any count.
...
\begin{theorem} For A304217, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + a(n-4) + a(n-5) - 2\,a(n-7)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,56$. The last nonvanishing one is at $m = 2$, that is at $n = 9$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 17, 17, 32, 62, 108, 190, 348,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304217}{A304217}, revision 4, last modified May 08 2018.
