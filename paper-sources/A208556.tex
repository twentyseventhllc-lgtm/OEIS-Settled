...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208556}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208556}{A208556} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 0 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 6*a(n-2) - 8*a(n-3).\end{quote}

\begin{quote}\ttfamily G.f.: 9*x*(1 + 6*x - 8*x\textasciicircum{}2) / ((1 - x)*(1 + 2*x)*(1 - 4*x)).\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 28 2012).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Mar 07 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 54x^{2} - 72x^{3}}{1 - 3x - 6x^{2} + 8x^{3}}.\]
...
\begin{theorem} For A208556, the recurrence $a(n) = 3\,a(n-1) + 6\,a(n-2) - 8\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208556, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 54x^{2} - 72x^{3}}{1 - 3x - 6x^{2} + 8x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 225, 1089, 3969, 16641, 65025, 263169,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208556}{A208556}, revision 9, last modified Mar 07 2018.
