...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A259946}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A259946}{A259946} as

\begin{quote}\itshape Number of (n+2) X (2+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00000000 00000001 or 00000101\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 2*a(n-2) + a(n-3) + 6*a(n-4) - 14*a(n-5) - 4*a(n-6) + 2*a(n-7) - 4*a(n-8) + 4*a(n-9) for n>10.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(60 + 50*x + 89*x\textasciicircum{}2 + 9*x\textasciicircum{}3 - 514*x\textasciicircum{}4 - 66*x\textasciicircum{}5 + 34*x\textasciicircum{}6 - 90*x\textasciicircum{}7 + 144*x\textasciicircum{}8 - 12*x\textasciicircum{}9) / ((1 + x - x\textasciicircum{}2)*(1 - 3*x + 2*x\textasciicircum{}2 - 6*x\textasciicircum{}3 + 2*x\textasciicircum{}4 + 6*x\textasciicircum{}5 + 4*x\textasciicircum{}7)). - Colin Barker, Dec 27 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Dec 27 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 2\,a(n-1) + 2\,a(n-2) + a(n-3) + 6\,a(n-4) - 14\,a(n-5) - 4\,a(n-6) + \\ 2\,a(n-7) - 4\,a(n-8) + 4\,a(n-9). \end{multline*}

stated for $n > 10$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{60x + 50x^{2} + 89x^{3} + 9x^{4} - 514x^{5} - 66x^{6} + 34x^{7} - 90x^{8} + 144x^{9} - 12x^{10}}{1 - 2x - 2x^{2} - x^{3} - 6x^{4} + 14x^{5} + 4x^{6} - 2x^{7} + 4x^{8} - 4x^{9}}.\]
...
The condition is that $w(i,j)$ is a cyclic rotation of one of \texttt{00000000}, \texttt{00000001}, \texttt{00000101}.
...
That refinement leaves $16$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 15$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A259946, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) + a(n-3) + 6\,a(n-4) - 14\,a(n-5) - 4\,a(n-6) + 2\,a(n-7) - 4\,a(n-8) + 4\,a(n-9)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,54$. The last nonvanishing one is at $m = 1$, that is at $n = 10$. After it, more than $S = 15$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 10$. The theorem is at least as strong.

\begin{theorem} For A259946, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{60x + 50x^{2} + 89x^{3} + 9x^{4} - 514x^{5} - 66x^{6} + 34x^{7} - 90x^{8} + 144x^{9} - 12x^{10}}{1 - 2x - 2x^{2} - x^{3} - 6x^{4} + 14x^{5} + 4x^{6} - 2x^{7} + 4x^{8} - 4x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 60, 170, 549, 1507, 4128, 11933, 34337, 97374,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A259946}{A259946}, revision 10, last modified Dec 27 2018.
