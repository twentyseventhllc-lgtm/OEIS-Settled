...
\title{Proofs of the conjectured column and row recurrences for OEIS A301450}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301450}{A301450} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 3, 5 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) -a(n-3) -a(n-4)\\k=3: [order 13]\\k=4: [order 40]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 21 2018). As of revision 4, last modified Mar 21 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A301450, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A301450, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) - a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A301450, the recurrence $a(n) = 0$ holds for every $n \ge 14$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A301450, the recurrence $a(n) = 0$ holds for every $n \ge 41$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 50$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 50$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301450}{A301450}, revision 4, last modified Mar 21 2018.
