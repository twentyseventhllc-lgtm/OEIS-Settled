...
\title{A proof of the conjectured linear recurrence for OEIS A196423}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196423}{A196423} as

\begin{quote}\itshape Number of n X 1 0..4 arrays with each element equal to the number its horizontal and vertical neighbors less than itself\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +3*a(n-3) +a(n-4).\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Oct 02 2011).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 15, last modified Dec 26 2024, the entry records no proof and links to none, so the statement is open. This note settles it.
...
Let $q = 5$ and let $A$ be an $n \times 1$ array over $\{0,\dots,4\}$.
...
Let $R = \{0,\dots,4\}^{1}$ be the set of rows, so $|R| = 5$, and adjoin a symbol $\top$ standing for ``no row above''. Take as states the pairs
...
The construction gives $30$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $10$; neither removal changes any count.
...
\begin{theorem} For A196423, the recurrence $a(n) = a(n-1) + a(n-2) + 3\,a(n-3) + a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 5, 11, 26, 55, 119, 263,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196423}{A196423}, revision 15, last modified Dec 26 2024.
