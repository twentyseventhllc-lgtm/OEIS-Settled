...
\title{A proof of the conjectured linear recurrence for OEIS A298489}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298489}{A298489} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element equal to 0, 1, 2, 4 or 7 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) -2*a(n-2) +3*a(n-3) -9*a(n-4) +4*a(n-5) -a(n-6) +9*a(n-7) -3*a(n-8) -a(n-9) -4*a(n-10) +2*a(n-11) for n>12\end{quote}
...
\begin{multline*} a(n) = 3\,a(n-1) - 2\,a(n-2) + 3\,a(n-3) - 9\,a(n-4) + 4\,a(n-5) - a(n-6) + \\ 9\,a(n-7) - 3\,a(n-8) - a(n-9) - 4\,a(n-10) + 2\,a(n-11). \end{multline*}
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $55$; neither removal changes any count.
...
\begin{theorem} For A298489, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 3\,a(n-3) - 9\,a(n-4) + 4\,a(n-5) - a(n-6) + 9\,a(n-7) - 3\,a(n-8) - a(n-9) - 4\,a(n-10) + 2\,a(n-11)$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,65$. The last nonvanishing one is at $m = 1$, that is at $n = 12$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 35 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 13, 20, 27, 43, 70, 106, 169,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298489}{A298489}, revision 4, last modified Jan 20 2018.
