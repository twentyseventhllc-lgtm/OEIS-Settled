...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A303714}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303714}{A303714} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element unequal to 0, 1 or 5 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +2*a(n-3) for n>4.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 3*x-x\textasciicircum{}2*(3+2*x+3*x\textasciicircum{}2)/(-1+x+2*x\textasciicircum{}3). - R. J. Mathar, Apr 30 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Apr 29 2018).

The line quoted ending in that signature is contributed by R. J. Mathar on Apr 30 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified May 17 2023, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{3x + 2x^{3} - 3x^{4}}{1 - x - 2x^{3}}.\]
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $11$; neither removal changes any count.
...
\begin{theorem} For A303714, the recurrence $a(n) = a(n-1) + 2\,a(n-3)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,49$. The last nonvanishing one is at $m = 1$, that is at $n = 4$. After it, more than $S = 4$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 4$. The theorem is at least as strong.

\begin{theorem} For A303714, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{3x + 2x^{3} - 3x^{4}}{1 - x - 2x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 36 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 3, 5, 8, 14, 24, 40, 68,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303714}{A303714}, revision 8, last modified May 17 2023.
