...
\title{A proof of the conjectured order of the recurrence for OEIS A250514}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250514}{A250514} as

\begin{quote}\itshape Number of (n+1)X(3+1) 0..3 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical recurrence of order 50 (see link above)\end{quote}
...
The entry states only that the sequence satisfies a linear recurrence of order $50$ and refers to a linked file for its coefficients, which this note does not use. What is proved below is the corresponding exact statement: the least order of a linear recurrence the sequence eventually satisfies, and the exact index beyond which it holds.
...
For scale: at $n = 8$ there are $4^{32}$ arrays, about $10^{19}$, against $52$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\item {\bfseries The published terms.} The walk count reproduces all 14 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5996, 205022, 5941982, 166583429, 4464289109, 117638074883, 3049397341176, 78567969110807,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250514}{A250514}, revision 6, last modified Jul 23 2025.
