...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +2*a(n-2) +4*a(n-3)\\k=3: a(n) = 2*a(n-1) +3*a(n-2) +6*a(n-3) -3*a(n-4) +2*a(n-5)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 27 2017). As of revision 4, last modified Nov 27 2017, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
