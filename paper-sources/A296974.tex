...
\title{Proofs of the conjectured column and row recurrences for OEIS A296974}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296974}{A296974} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 adjacent to 2, 4 or 5 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) -a(n-2) +6*a(n-3) -4*a(n-4) +2*a(n-5)\\k=3: [order 17]\\k=4: [order 48]\end{quote}
...
The condition is $c(i,j) \in \{2, 4, 5\}$ for every cell carrying the value 1. Cells carrying any other value are unconstrained. Offsets leaving the array are not counted, so border cells have fewer neighbours than interior ones.
...
\begin{theorem} For column $k = 1$ of A296974, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A296974, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 6\,a(n-3) - 4\,a(n-4) + 2\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A296974, the recurrence $a(n) = 0$ holds for every $n \ge 18$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A296974, the recurrence $a(n) = 0$ holds for every $n \ge 49$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $259$ after trimming, and $S = 53$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 53$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296974}{A296974}, revision 4, last modified Dec 22 2017.
