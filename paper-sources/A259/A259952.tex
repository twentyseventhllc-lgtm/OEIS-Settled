...
\title{Proofs of the conjectured column and row recurrences for OEIS A259952}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A259952}{A259952} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000000 00000001 or 00000101\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2) +2*a(n-3) +6*a(n-4) -a(n-6) +a(n-7) -a(n-8) for n>9\\k=2: [order 9] for n>10\\k=3: [order 16] for n>17\end{quote}
...
\begin{theorem} For column $k = 1$ of A259952, the recurrence $a(n) = a(n-1) + a(n-2) + 2\,a(n-3) + 6\,a(n-4) - a(n-6) + a(n-7) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A259952, the recurrence $a(n) = 0$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 3$ of A259952, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 24$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 24$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A259952}{A259952}, revision 6, last modified Jun 02 2025.
