...
\title{Proofs of the conjectured column and row recurrences for OEIS A223669}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A223669}{A223669} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with rows, diagonals and antidiagonals unimodal\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1)\\k=3: a(n) = 6*a(n-1) -2*a(n-2) +11*a(n-3) +10*a(n-4) -30*a(n-5) -12*a(n-6)\\k=4: [order 23]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 25 2013). As of revision 8, last modified Oct 03 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A223669, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A223669, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $10$ states, $10$ after trimming, and $S = 2$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 2$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A223669, the recurrence $a(n) = 6\,a(n-1) - 2\,a(n-2) + 11\,a(n-3) + 10\,a(n-4) - 30\,a(n-5) - 12\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $37$ states, $37$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A223669, the recurrence $a(n) = 0$ holds for every $n \ge 24$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $150$ states, $150$ after trimming, and $S = 33$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 33$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A223669}{A223669}, revision 8, last modified Oct 03 2025.
