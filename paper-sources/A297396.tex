...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297396}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297396}{A297396} as

\begin{quote}\itshape Number of 3 X n 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 1 neighboring 1\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + a(n-2) + 8*a(n-3) + a(n-4) - 2*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 8*x + 9*x\textasciicircum{}2 + x\textasciicircum{}3 - 2*x\textasciicircum{}4 - 2*x\textasciicircum{}5) / (1 - x - x\textasciicircum{}2 - 8*x\textasciicircum{}3 - x\textasciicircum{}4 + 2*x\textasciicircum{}6). - Colin Barker, Feb 28 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 8x^{2} + 9x^{3} + x^{4} - 2x^{5} - 2x^{6}}{1 - x - x^{2} - 8x^{3} - x^{4} + 2x^{6}}.\]
...
\begin{theorem} For A297396, the recurrence $a(n) = a(n-1) + a(n-2) + 8\,a(n-3) + a(n-4) - 2\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297396, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 8x^{2} + 9x^{3} + x^{4} - 2x^{5} - 2x^{6}}{1 - x - x^{2} - 8x^{3} - x^{4} + 2x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 9, 19, 37, 127, 323, 763, 2121,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297396}{A297396}, revision 7, last modified Feb 28 2019.
