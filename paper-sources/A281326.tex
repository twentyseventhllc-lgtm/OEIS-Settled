...
\title{Proofs of the conjectured column and row recurrences for OEIS A281326}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281326}{A281326} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to more than one of its horizontal and antidiagonal neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 16*a(n-1) -72*a(n-2) +64*a(n-3) -16*a(n-4)\\k=3: a(n) = 28*a(n-1) -180*a(n-2) -224*a(n-3) -64*a(n-4) for n>5\\k=4: [order 8] for n>9\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 20 2017). As of revision 4, last modified Jan 20 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A281326, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A281326, the recurrence $a(n) = 16\,a(n-1) - 72\,a(n-2) + 64\,a(n-3) - 16\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $282$ states, $171$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A281326, the recurrence $a(n) = 28\,a(n-1) - 180\,a(n-2) - 224\,a(n-3) - 64\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $2068$ states, $1042$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 5$; the proved threshold is $n > 5$.

\begin{theorem} For column $k = 4$ of A281326, the recurrence $a(n) = 0$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $16015$ states, $6526$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281326}{A281326}, revision 4, last modified Jan 20 2017.
