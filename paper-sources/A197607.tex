...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A197607}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A197607}{A197607} as

\begin{quote}\itshape Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 4,2,1,1,1 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + a(n-2) + 4*a(n-3) + a(n-4) + 2*a(n-5) + 2*a(n-6) - a(n-7) - a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x)*(1 + 2*x + x\textasciicircum{}2 + 5*x\textasciicircum{}3 - 2*x\textasciicircum{}4 - x\textasciicircum{}6) / (1 - 2*x - x\textasciicircum{}2 - 4*x\textasciicircum{}3 - x\textasciicircum{}4 - 2*x\textasciicircum{}5 - 2*x\textasciicircum{}6 + x\textasciicircum{}7 + x\textasciicircum{}8). - Colin Barker, Mar 02 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Mar 02 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 2\,a(n-1) + a(n-2) + 4\,a(n-3) + a(n-4) + 2\,a(n-5) + 2\,a(n-6) - \\ a(n-7) - a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 3x^{2} + 3x^{3} + 6x^{4} + 3x^{5} - 2x^{6} - x^{7} - x^{8}}{1 - 2x - x^{2} - 4x^{3} - x^{4} - 2x^{5} - 2x^{6} + x^{7} + x^{8}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 4, \tau(1) = 2, \tau(2) = 1, \tau(3) = 1, \tau(4) = 1. The condition is that for every cell,
...
\begin{theorem} For A197607, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + 4\,a(n-3) + a(n-4) + 2\,a(n-5) + 2\,a(n-6) - a(n-7) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A197607, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 3x^{2} + 3x^{3} + 6x^{4} + 3x^{5} - 2x^{6} - x^{7} - x^{8}}{1 - 2x - x^{2} - 4x^{3} - x^{4} - 2x^{5} - 2x^{6} + x^{7} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 5, 14, 43, 124, 352, 1025, 2977,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A197607}{A197607}, revision 8, last modified Mar 02 2018.
