...
\title{A proof of the conjectured linear recurrence for OEIS A304136}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304136}{A304136} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element unequal to 0, 2, 3 or 6 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +5*a(n-3) -5*a(n-4) -5*a(n-5) -4*a(n-6) +4*a(n-7) +4*a(n-8) for n>10.\end{quote}
...
\begin{multline*} a(n) = a(n-1) + a(n-2) + 5\,a(n-3) - 5\,a(n-4) - 5\,a(n-5) - 4\,a(n-6) + \\ 4\,a(n-7) + 4\,a(n-8). \end{multline*}
...
\begin{theorem} For A304136, the recurrence $a(n) = a(n-1) + a(n-2) + 5\,a(n-3) - 5\,a(n-4) - 5\,a(n-5) - 4\,a(n-6) + 4\,a(n-7) + 4\,a(n-8)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,63$. The last nonvanishing one is at $m = 2$, that is at $n = 10$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 7, 5, 9, 15, 16, 33, 57,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304136}{A304136}, revision 6, last modified Mar 17 2023.
