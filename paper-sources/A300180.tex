...
Here the refinement leaves $S = 29$ blocks. Writing $L$ for the $S\times S$ quotient matrix, $\tilde w$ for the start weights summed over each block and $\tilde f$ for the acceptance indicator,
...
For scale: at $n = 8$ there are $2^{48}$ arrays, about $10^{14}$, against $29$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,63$. Every one of them vanished. After it, more than $S = 29$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
