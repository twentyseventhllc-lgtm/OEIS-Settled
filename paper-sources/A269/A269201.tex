...
\title{Proofs of the conjectured column and row recurrences for OEIS A269201}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A269201}{A269201} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays with some element plus some horizontally, diagonally or antidiagonally adjacent neighbor totalling three no more than once\end{quote}
...
k=1: a(n) = 4*a(n-1)\\k=2: a(n) = 14*a(n-1) -49*a(n-2) for n>3\\k=3: a(n) = 30*a(n-1) -237*a(n-2) +180*a(n-3) -36*a(n-4) for n>5\end{quote}
...
\begin{theorem} For column $k = 1$ of A269201, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $5$ states, $5$ after trimming, and $S = 1$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 1$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A269201, the recurrence $a(n) = 14\,a(n-1) - 49\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $45$ states, $29$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 3$; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 3$; the proved threshold is $n > 3$.

\begin{theorem} For column $k = 3$ of A269201, the recurrence $a(n) = 30\,a(n-1) - 237\,a(n-2) + 180\,a(n-3) - 36\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $161$ states, $97$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A269201}{A269201}, revision 4, last modified Feb 20 2016.
