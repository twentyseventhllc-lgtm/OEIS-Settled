...
\title{Proofs of the conjectured column and row recurrences for OEIS A269767}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A269767}{A269767} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..5 arrays with some element plus some horizontally or vertically adjacent neighbor totalling five exactly once\end{quote}
...
k=1: a(n) = 10*a(n-1) -25*a(n-2)\\k=2: a(n) = 42*a(n-1) -441*a(n-2) for n>3\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 04 2016). As of revision 4, last modified Mar 04 2016, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A269767, the recurrence $a(n) = 10\,a(n-1) - 25\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A269767, the recurrence $a(n) = 42\,a(n-1) - 441\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $103$ states, $67$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 3$; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A269767}{A269767}, revision 4, last modified Mar 04 2016.
