...
\title{Proofs of the conjectured column and row recurrences for OEIS A269888}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A269888}{A269888} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..5 arrays with some element plus some horizontally, diagonally, antidiagonally or vertically adjacent neighbor totalling five exactly once\end{quote}
...
k=1: a(n) = 10*a(n-1) -25*a(n-2)\\k=2: a(n) = 30*a(n-1) -197*a(n-2) -420*a(n-3) -196*a(n-4) for n>5\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 06 2016). As of revision 4, last modified Mar 06 2016, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A269888, the recurrence $a(n) = 10\,a(n-1) - 25\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $19$ states, $13$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A269888, the recurrence $a(n) = 30\,a(n-1) - 197\,a(n-2) - 420\,a(n-3) - 196\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $103$ states, $67$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A269888}{A269888}, revision 4, last modified Mar 06 2016.
