...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296309}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296309}{A296309} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with each 1 adjacent to 3 or 6 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 4*a(n-3) - a(n-4) - 2*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 3*x + 3*x\textasciicircum{}2 - x\textasciicircum{}3 - 2*x\textasciicircum{}4 - 2*x\textasciicircum{}5) / (1 - x - 4*x\textasciicircum{}3 + x\textasciicircum{}4 + 2*x\textasciicircum{}6). - Colin Barker, Feb 22 2019\end{quote}
...
\[a(n) = a(n-1) + 4\,a(n-3) - a(n-4) - 2\,a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 3x^{2} + 3x^{3} - x^{4} - 2x^{5} - 2x^{6}}{1 - x - 4x^{3} + x^{4} + 2x^{6}}.\]
...
\begin{theorem} For A296309, the recurrence $a(n) = a(n-1) + 4\,a(n-3) - a(n-4) - 2\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296309, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 3x^{2} + 3x^{3} - x^{4} - 2x^{5} - 2x^{6}}{1 - x - 4x^{3} + x^{4} + 2x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 33 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 4, 7, 10, 23, 45, 76, 150,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296309}{A296309}, revision 8, last modified Feb 22 2019.
