...
\title{Proofs of the conjectured column and row recurrences for OEIS A299595}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299595}{A299595} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +2*a(n-3) +a(n-5) for n>6\\k=3: a(n) = a(n-1) +2*a(n-3) +a(n-4) +a(n-5) for n>8\end{quote}
...
\begin{theorem} For column $k = 1$ of A299595, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A299595, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A299595, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + a(n-4) + a(n-5)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $12$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 8$; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299595}{A299595}, revision 4, last modified Feb 13 2018.
