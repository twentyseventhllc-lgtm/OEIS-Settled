...
\title{Proofs of the conjectured column and row recurrences for OEIS A318552}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318552}{A318552} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 4 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +4*a(n-2) -2*a(n-3) -2*a(n-4) +a(n-5) -4*a(n-6) +4*a(n-7) -a(n-8)\\k=3: [order 31]\end{quote}
...
\begin{theorem} For column $k = 1$ of A318552, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318552, the recurrence $a(n) = a(n-1) + 4\,a(n-2) - 2\,a(n-3) - 2\,a(n-4) + a(n-5) - 4\,a(n-6) + 4\,a(n-7) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A318552, the recurrence $a(n) = 0$ holds for every $n \ge 32$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $66$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318552}{A318552}, revision 4, last modified Aug 28 2018.
