...
\title{Proofs of the conjectured column and row recurrences for OEIS A230184}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A230184}{A230184} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value 2-x(i,j)\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 5*a(n-1) -4*a(n-2)\\k=3: a(n) = 9*a(n-1) -4*a(n-2)\\k=4: a(n) = 26*a(n-1) -97*a(n-2) +89*a(n-3) -18*a(n-4) +a(n-5)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 11 2013). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A230184, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A230184, the recurrence $a(n) = 5\,a(n-1) - 4\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $55$ states, $37$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A230184, the recurrence $a(n) = 9\,a(n-1) - 4\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $433$ states, $253$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A230184, the recurrence $a(n) = 26\,a(n-1) - 97\,a(n-2) + 89\,a(n-3) - 18\,a(n-4) + a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $3727$ states, $2053$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A230184}{A230184}, revision 6, last modified Jun 02 2025.
