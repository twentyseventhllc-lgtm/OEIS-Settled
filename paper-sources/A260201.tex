...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A260201}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260201}{A260201} as

\begin{quote}\itshape Number of (n+2) X (2+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00000000 00000001 or 00010001\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + a(n-2) + 5*a(n-3) + 2*a(n-4) + a(n-5) - a(n-7) - a(n-8) for n>9.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(44 + 36*x + 83*x\textasciicircum{}2 - 28*x\textasciicircum{}3 - 11*x\textasciicircum{}4 - 17*x\textasciicircum{}5 - 21*x\textasciicircum{}6 - 5*x\textasciicircum{}7 + 12*x\textasciicircum{}8) / (1 - x - x\textasciicircum{}2 - 5*x\textasciicircum{}3 - 2*x\textasciicircum{}4 - x\textasciicircum{}5 + x\textasciicircum{}7 + x\textasciicircum{}8). - Colin Barker, Dec 28 2018\end{quote}
...
\begin{multline*} a(n) = a(n-1) + a(n-2) + 5\,a(n-3) + 2\,a(n-4) + a(n-5) - a(n-7) - \\ a(n-8). \end{multline*}

stated for $n > 9$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{44x + 36x^{2} + 83x^{3} - 28x^{4} - 11x^{5} - 17x^{6} - 21x^{7} - 5x^{8} + 12x^{9}}{1 - x - x^{2} - 5x^{3} - 2x^{4} - x^{5} + x^{7} + x^{8}}.\]
...
\begin{theorem} For A260201, the recurrence $a(n) = a(n-1) + a(n-2) + 5\,a(n-3) + 2\,a(n-4) + a(n-5) - a(n-7) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. The last nonvanishing one is at $m = 1$, that is at $n = 9$. After it, more than $S = 15$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A260201, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{44x + 36x^{2} + 83x^{3} - 28x^{4} - 11x^{5} - 17x^{6} - 21x^{7} - 5x^{8} + 12x^{9}}{1 - x - x^{2} - 5x^{3} - 2x^{4} - x^{5} + x^{7} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 44, 80, 207, 479, 1163, 2864, 6895, 16690,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260201}{A260201}, revision 8, last modified Dec 28 2018.
