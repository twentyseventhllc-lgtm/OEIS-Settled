...
\title{Proofs of the conjectured column and row recurrences for OEIS A300465}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300465}{A300465} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 2, 3 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +2*a(n-2) +a(n-3)\\k=3: [order 15]\\k=4: [order 52] for n>54\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 06 2018). As of revision 4, last modified Mar 06 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A300465, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300465, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300465, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A300465, the recurrence $a(n) = 0$ holds for every $n > 54$, and fails at $n = 54$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $179$ after trimming, and $S = 57$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 54$; after that point more than $S = 57$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 54$; the proved threshold is $n > 54$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300465}{A300465}, revision 4, last modified Mar 06 2018.
