...
\title{Proofs of the conjectured column and row recurrences for OEIS A316576}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316576}{A316576} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 2, 3, 4, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +2*a(n-3) -6*a(n-4) -4*a(n-5) for n>6\\k=3: [order 19] for n>20\\k=4: [order 49] for n>52\end{quote}
...
The condition is $c(i,j) \in \{0, 2, 3, 4, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A316576, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316576, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316576, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A316576, the recurrence $a(n) = 0$ holds for every $n > 52$, and fails at $n = 52$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $218$ after trimming, and $S = 57$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 52$; after that point more than $S = 57$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316576}{A316576}, revision 4, last modified Jul 07 2018.
