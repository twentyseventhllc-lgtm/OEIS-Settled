...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296124}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296124}{A296124} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with each 1 adjacent to 2 or 5 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + a(n-2) + 5*a(n-3) - 18*a(n-4) - 14*a(n-5) + 3*a(n-6) + a(n-7) + a(n-8) - a(n-9).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x)*(1 + 9*x - 19*x\textasciicircum{}2 - 15*x\textasciicircum{}3 + 3*x\textasciicircum{}4 + x\textasciicircum{}5 + x\textasciicircum{}6 - x\textasciicircum{}7) / (1 - 3*x - x\textasciicircum{}2 - 5*x\textasciicircum{}3 + 18*x\textasciicircum{}4 + 14*x\textasciicircum{}5 - 3*x\textasciicircum{}6 - x\textasciicircum{}7 - x\textasciicircum{}8 + x\textasciicircum{}9). - Colin Barker, Feb 22 2019\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Feb 22 2019, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 3\,a(n-1) + a(n-2) + 5\,a(n-3) - 18\,a(n-4) - 14\,a(n-5) + 3\,a(n-6) + \\ a(n-7) + a(n-8) - a(n-9). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 10x^{2} - 10x^{3} - 34x^{4} - 12x^{5} + 4x^{6} + 2x^{7} - x^{9}}{1 - 3x - x^{2} - 5x^{3} + 18x^{4} + 14x^{5} - 3x^{6} - x^{7} - x^{8} + x^{9}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $68$; neither removal changes any count.
...
\begin{theorem} For A296124, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 5\,a(n-3) - 18\,a(n-4) - 14\,a(n-5) + 3\,a(n-6) + a(n-7) + a(n-8) - a(n-9)$ holds for every $n \ge 10$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296124, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 10x^{2} - 10x^{3} - 34x^{4} - 12x^{5} + 4x^{6} + 2x^{7} - x^{9}}{1 - 3x - x^{2} - 5x^{3} + 18x^{4} + 14x^{5} - 3x^{6} - x^{7} - x^{8} + x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 13, 30, 74, 287, 841, 2463, 7953,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296124}{A296124}, revision 10, last modified Feb 22 2019.
