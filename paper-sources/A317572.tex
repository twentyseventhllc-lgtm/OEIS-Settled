...
\title{Proofs of the conjectured column and row recurrences for OEIS A317572}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317572}{A317572} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3, 4, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +4*a(n-3)\\k=3: [order 11] for n>12\\k=4: [order 38] for n>39\end{quote}
...
The condition is $c(i,j) \in \{0, 1, 2, 3, 4, 6, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A317572, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317572, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 4\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317572, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A317572, the recurrence $a(n) = 0$ holds for every $n > 39$, and fails at $n = 39$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 44$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 39$; after that point more than $S = 44$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317572}{A317572}, revision 4, last modified Jul 31 2018.
