...
\title{A proof of the conjectured linear recurrence for OEIS A281053}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281053}{A281053} as

\begin{quote}\itshape Number of nX5 0..1 arrays with no element equal to more than one of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) -6*a(n-2) -4*a(n-3) +5*a(n-4) +9*a(n-5) -12*a(n-7) -3*a(n-8) +3*a(n-9) +6*a(n-10) -a(n-12) -a(n-13) for n>16\end{quote}
...
\begin{multline*} a(n) = 5\,a(n-1) - 6\,a(n-2) - 4\,a(n-3) + 5\,a(n-4) + 9\,a(n-5) - 12\,a(n-7) - \\ 3\,a(n-8) + 3\,a(n-9) + 6\,a(n-10) - a(n-12) - a(n-13). \end{multline*}
...
The reachable part of the digraph has $2321$ states, $516$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 32$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of that $32\times32$ matrix; the bound is computed, not assumed.
...
\begin{theorem} For A281053, the recurrence $a(n) = 5\,a(n-1) - 6\,a(n-2) - 4\,a(n-3) + 5\,a(n-4) + 9\,a(n-5) - 12\,a(n-7) - 3\,a(n-8) + 3\,a(n-9) + 6\,a(n-10) - a(n-12) - a(n-13)$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,76$. The last nonvanishing one is at $m = 3$, that is at $n = 16$. After it, more than $S = 32$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 104, 618, 2452, 7803, 22106, 58005, 144283,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281053}{A281053}, revision 4, last modified Jan 13 2017.
