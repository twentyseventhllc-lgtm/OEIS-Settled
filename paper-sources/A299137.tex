...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A299137}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299137}{A299137} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element equal to 1, 2, 3, 4, 5 or 7 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 6*a(n-1) +8*a(n-2) -12*a(n-3) +8*a(n-4) +7*a(n-5) -4*a(n-6) -2*a(n-7) -7*a(n-8) -5*a(n-9) +2*a(n-10) for n>11.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 12*x + 13*x\textasciicircum{}2 - 7*x\textasciicircum{}3 - 13*x\textasciicircum{}4 + 41*x\textasciicircum{}5 - 41*x\textasciicircum{}6 - 106*x\textasciicircum{}7 - 37*x\textasciicircum{}8 + 14*x\textasciicircum{}9 + 2*x\textasciicircum{}10) / ((1 - x)*(1 - 5*x - 13*x\textasciicircum{}2 - x\textasciicircum{}3 - 9*x\textasciicircum{}4 - 16*x\textasciicircum{}5 - 12*x\textasciicircum{}6 - 10*x\textasciicircum{}7 - 3*x\textasciicircum{}8 + 2*x\textasciicircum{}9)). - Colin Barker, Feb 17 2018\end{quote}
...
\begin{multline*} a(n) = 6\,a(n-1) + 8\,a(n-2) - 12\,a(n-3) + 8\,a(n-4) + 7\,a(n-5) - 4\,a(n-6) - \\ 2\,a(n-7) - 7\,a(n-8) - 5\,a(n-9) + 2\,a(n-10). \end{multline*}

stated for $n > 11$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 12x^{2} + 13x^{3} - 7x^{4} - 13x^{5} + 41x^{6} - 41x^{7} - 106x^{8} - 37x^{9} + 14x^{10} + 2x^{11}}{1 - 6x - 8x^{2} + 12x^{3} - 8x^{4} - 7x^{5} + 4x^{6} + 2x^{7} + 7x^{8} + 5x^{9} - 2x^{10}}.\]
...
The condition is $c(i,j) \in \{1, 2, 3, 4, 5, 7\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For A299137, the recurrence $a(n) = 6\,a(n-1) + 8\,a(n-2) - 12\,a(n-3) + 8\,a(n-4) + 7\,a(n-5) - 4\,a(n-6) - 2\,a(n-7) - 7\,a(n-8) - 5\,a(n-9) + 2\,a(n-10)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,48$. The last nonvanishing one is at $m = 1$, that is at $n = 11$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 11$. The theorem is at least as strong.

\begin{theorem} For A299137, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 12x^{2} + 13x^{3} - 7x^{4} - 13x^{5} + 41x^{6} - 41x^{7} - 106x^{8} - 37x^{9} + 14x^{10} + 2x^{11}}{1 - 6x - 8x^{2} + 12x^{3} - 8x^{4} - 7x^{5} + 4x^{6} + 2x^{7} + 7x^{8} + 5x^{9} - 2x^{10}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 20 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 18, 129, 899, 6205, 43066, 298361, 2068149,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299137}{A299137}, revision 8, last modified Feb 17 2018.
