...
\title{A proof of the conjectured linear recurrence for OEIS A230610}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A230610}{A230610} as

\begin{quote}\itshape Number of nX4 0..2 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value (x(i,j)+1) mod 3, and upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 15*a(n-1) -45*a(n-2) +42*a(n-3) -12*a(n-4) +a(n-5).\end{quote}
...
The reachable part of the digraph has $3385$ states, $1757$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 11$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of that $11\times11$ matrix; the bound is computed, not assumed.
...
\begin{theorem} For A230610, the recurrence $a(n) = 15\,a(n-1) - 45\,a(n-2) + 42\,a(n-3) - 12\,a(n-4) + a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 18 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 2, 30, 348, 3956, 44916, 509978, 5790456,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A230610}{A230610}, revision 8, last modified Jul 23 2025.
