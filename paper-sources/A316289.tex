...
\title{Proofs of the conjectured column and row recurrences for OEIS A316289}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316289}{A316289} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
The lines carry no separate signature; the entry itself is by R. H. Hardin (Jun 28 2018). As of revision 4, last modified Jun 28 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A316289, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316289, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 4\,a(n-3) - 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316289, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A316289, the recurrence $a(n) = 0$ holds for every $n > 25$, and fails at $n = 25$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $171$ after trimming, and $S = 29$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 25$; after that point more than $S = 29$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 25$; the proved threshold is $n > 25$.

\begin{theorem} For column $k = 5$ of A316289, the recurrence $a(n) = 0$ holds for every $n > 89$, and fails at $n = 89$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $676$ after trimming, and $S = 105$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 89$; after that point more than $S = 105$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316289}{A316289}, revision 4, last modified Jun 28 2018.
