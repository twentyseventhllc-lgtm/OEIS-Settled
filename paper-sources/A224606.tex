...
\title{Proofs of the conjectured column and row recurrences for OEIS A224606}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224606}{A224606} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..2 matrices with each 3X3 subblock idempotent\end{quote}
...
The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 11 2013). As of revision 8, last modified Oct 03 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A224606, the recurrence $a(n) = 3\,a(n-1) - 3\,a(n-2) + 2\,a(n-3) - a(n-4) - a(n-5) + a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $756$ states, $756$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 7$; the proved threshold is $n > 7$.

\begin{theorem} For column $k = 2$ of A224606, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) - a(n-3) + a(n-4) - a(n-5) + 2\,a(n-6) - a(n-7)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $6642$ states, $6642$ after trimming, and $S = 18$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 18$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224606}{A224606}, revision 8, last modified Oct 03 2025.
