...
\title{A proof of the conjectured linear recurrence for OEIS A317759}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317759}{A317759} as

\begin{quote}\itshape Number of nX2 0..1 arrays with every element unequal to 0, 1 or 2 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
The construction gives $20$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $12$; neither removal changes any count.
...
\begin{theorem} For A317759, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - a(n-3) - a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,46$. The last nonvanishing one is at $m = 2$, that is at $n = 6$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 6, 10, 20, 42, 89, 190, 407,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317759}{A317759}, revision 4, last modified Aug 06 2018.
