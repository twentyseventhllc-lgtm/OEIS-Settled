...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208558}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208558}{A208558} as

\begin{quote}\itshape Number of 6 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 0 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) +12*a(n-2) -27*a(n-3).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 16*x*(1 + 12*x - 27*x\textasciicircum{}2) / ((1 + 3*x)*(1 - 7*x + 9*x\textasciicircum{}2)). - Colin Barker, Jul 04 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Jul 04 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + 192x^{2} - 432x^{3}}{1 - 4x - 12x^{2} + 27x^{3}}.\]
...
\begin{theorem} For A208558, the recurrence $a(n) = 4\,a(n-1) + 12\,a(n-2) - 27\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208558, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + 192x^{2} - 432x^{3}}{1 - 4x - 12x^{2} + 27x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 20 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 256, 784, 5776, 25600, 150544, 753424, 4129024,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208558}{A208558}, revision 9, last modified Jul 04 2018.
