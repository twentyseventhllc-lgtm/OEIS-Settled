...
\title{A proof of the conjectured generating function for OEIS A250847}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250847}{A250847} as

\begin{quote}\itshape Number of (n+1) X (2+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical g.f.: x*(400 - 1543*x + 2527*x\textasciicircum{}2 - 1344*x\textasciicircum{}3) / ((1 - x)*(1 - 2*x)*(1 - 3*x)*(1 - 4*x)). - Colin Barker, Nov 21 2018\end{quote}

The line quoted above is contributed by Colin Barker on Nov 21 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Nov 21 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{400x - 1543x^{2} + 2527x^{3} - 1344x^{4}}{1 - 10x + 35x^{2} - 50x^{3} + 24x^{4}}.\]
...
For scale: at $n = 8$ there are $4^{24}$ arrays, about $10^{14}$, against $9$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A250847, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{400x - 1543x^{2} + 2527x^{3} - 1344x^{4}}{1 - 10x + 35x^{2} - 50x^{3} + 24x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 19 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 400, 2457, 13097, 63631, 291165, 1280447, 5480917, 23024631,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250847}{A250847}, revision 9, last modified Nov 21 2018.
