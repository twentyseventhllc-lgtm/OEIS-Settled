...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1)\\k=3: a(n) = 7*a(n-1) +5*a(n-2) +9*a(n-3) -a(n-4) -6*a(n-5)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 15 2018). As of revision 4, last modified Feb 15 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
