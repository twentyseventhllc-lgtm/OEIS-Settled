...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207083}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207083}{A207083} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 horizontally and 0 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 6*a(n-1) - 5*a(n-2) - 14*a(n-3) + 11*a(n-4) + 4*a(n-5) - a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(7 + 7*x - 18*x\textasciicircum{}2 + 14*x\textasciicircum{}3 + 3*x\textasciicircum{}4 - x\textasciicircum{}5) / ((1 - x)*(1 - 2*x - x\textasciicircum{}2)*(1 - 3*x - 5*x\textasciicircum{}2 + x\textasciicircum{}3)). - Colin Barker, Jun 18 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 15 2012).

The line quoted ending in that signature is contributed by Colin Barker on Jun 18 2018.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{7x + 7x^{2} - 18x^{3} + 14x^{4} + 3x^{5} - x^{6}}{1 - 6x + 5x^{2} + 14x^{3} - 11x^{4} - 4x^{5} + x^{6}}.\]
...
\begin{theorem} For A207083, the recurrence $a(n) = 6\,a(n-1) - 5\,a(n-2) - 14\,a(n-3) + 11\,a(n-4) + 4\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A207083, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{7x + 7x^{2} - 18x^{3} + 14x^{4} + 3x^{5} - x^{6}}{1 - 6x + 5x^{2} + 14x^{3} - 11x^{4} - 4x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 7, 49, 241, 1117, 4891, 20953, 88465, 370753,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207083}{A207083}, revision 8, last modified Jun 18 2018.
