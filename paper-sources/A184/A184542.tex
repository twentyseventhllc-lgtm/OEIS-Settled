...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A184542}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A184542}{A184542} as

\begin{quote}\itshape Number of (n+2) X 5 binary arrays with each 3 X 3 subblock having rows and columns in lexicographically nondecreasing order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (1/120)*n\textasciicircum{}5 + (5/24)*n\textasciicircum{}4 + (49/24)*n\textasciicircum{}3 + (739/24)*n\textasciicircum{}2 + (1659/20)*n + 31.\end{quote}

\begin{quote}\ttfamily G.f.: x*(147 - 542*x + 796*x\textasciicircum{}2 - 578*x\textasciicircum{}3 + 209*x\textasciicircum{}4 - 31*x\textasciicircum{}5) / (1 - x)\textasciicircum{}6.\end{quote}

\begin{quote}\ttfamily a(n) = 6*a(n-1) - 15*a(n-2) + 20*a(n-3) - 15*a(n-4) + 6*a(n-5) - a(n-6) for n>5.\end{quote}
...
\[a(n) = 31 + \frac{1659}{20}n + \frac{739}{24}n^{2} + \frac{49}{24}n^{3} + \frac{5}{24}n^{4} + \frac{1}{120}n^{5}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{147x - 542x^{2} + 796x^{3} - 578x^{4} + 209x^{5} - 31x^{6}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}.\]
...
That refinement leaves $21$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 12$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A184542, $a(n)$ is the polynomial $31 + \frac{1659}{20}n + \frac{739}{24}n^{2} + \frac{49}{24}n^{3} + \frac{5}{24}n^{4} + \frac{1}{120}n^{5}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $5$ is annihilated by the difference operator $(E-1)^{6}$, whose characteristic polynomial is $(t-1)^{6}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 6 = 18$. Its values were computed exactly; all of them vanish, and more than $18$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A184542, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{147x - 542x^{2} + 796x^{3} - 578x^{4} + 209x^{5} - 31x^{6}}{1 - 6x + 15x^{2} - 20x^{3} + 15x^{4} - 6x^{5} + x^{6}}$.\end{theorem}
...
\begin{theorem} For A184542, the recurrence $a(n) = 6\,a(n-1) - 15\,a(n-2) + 20\,a(n-3) - 15\,a(n-4) + 6\,a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,59$. Every one of them vanished. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 5$; the proved threshold is $n > 6$.
...
\item {\bfseries The published terms.} The walk count reproduces all 35 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 147, 340, 631, 1048, 1627, 2413, 3461, 4837,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A184542}{A184542}, revision 8, last modified Apr 13 2018.
