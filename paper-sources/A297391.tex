...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297391}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297391}{A297391} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 1 neighboring 1\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: x*(4 + 9*x - 5*x\textasciicircum{}3 - 3*x\textasciicircum{}4 - 3*x\textasciicircum{}5 - x\textasciicircum{}6) / (1 - x - 6*x\textasciicircum{}2 - 4*x\textasciicircum{}3 + 3*x\textasciicircum{}4 + x\textasciicircum{}5 + 2*x\textasciicircum{}6 + x\textasciicircum{}7). - Colin Barker, Feb 28 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x + 9x^{2} - 5x^{4} - 3x^{5} - 3x^{6} - x^{7}}{1 - x - 6x^{2} - 4x^{3} + 3x^{4} + x^{5} + 2x^{6} + x^{7}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $47$; neither removal changes any count.
...
\begin{theorem} For A297391, the recurrence $a(n) = a(n-1) + 6\,a(n-2) + 4\,a(n-3) - 3\,a(n-4) - a(n-5) - 2\,a(n-6) - a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297391, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x + 9x^{2} - 5x^{4} - 3x^{5} - 3x^{6} - x^{7}}{1 - x - 6x^{2} - 4x^{3} + 3x^{4} + x^{5} + 2x^{6} + x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 13, 37, 126, 385, 1243, 3924, 12477,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297391}{A297391}, revision 8, last modified Feb 28 2019.
