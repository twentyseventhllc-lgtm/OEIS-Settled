...
\title{A proof of the conjectured linear recurrence for OEIS A301320}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301320}{A301320} as

\begin{quote}\itshape Number of nX5 0..1 arrays with every element equal to 0, 1, 2, 5 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) -3*a(n-2) -4*a(n-3) +4*a(n-4) +2*a(n-6) -5*a(n-8) +2*a(n-9) +4*a(n-10) -2*a(n-11) -2*a(n-12) +a(n-14) for n>16\end{quote}
...
\begin{multline*} a(n) = 4\,a(n-1) - 3\,a(n-2) - 4\,a(n-3) + 4\,a(n-4) + 2\,a(n-6) - 5\,a(n-8) + \\ 2\,a(n-9) + 4\,a(n-10) - 2\,a(n-11) - 2\,a(n-12) + a(n-14). \end{multline*}
...
The construction gives $1056$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $274$; neither removal changes any count.
...
That refinement leaves $57$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 35$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A301320, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2) - 4\,a(n-3) + 4\,a(n-4) + 2\,a(n-6) - 5\,a(n-8) + 2\,a(n-9) + 4\,a(n-10) - 2\,a(n-11) - 2\,a(n-12) + a(n-14)$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,81$. The last nonvanishing one is at $m = 2$, that is at $n = 16$. After it, more than $S = 35$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 138, 293, 726, 1649, 3649, 8035, 17751,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301320}{A301320}, revision 4, last modified Mar 18 2018.
