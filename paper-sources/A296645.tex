...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296645}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296645}{A296645} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 0, 1 or 3 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 3*a(n-2) + 9*a(n-3) - 4*a(n-4) - 12*a(n-5) - 4*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(4 + 8*x + 5*x\textasciicircum{}2 - 16*x\textasciicircum{}3 - 16*x\textasciicircum{}4 - 4*x\textasciicircum{}5) / (1 - x - 3*x\textasciicircum{}2 - 9*x\textasciicircum{}3 + 4*x\textasciicircum{}4 + 12*x\textasciicircum{}5 + 4*x\textasciicircum{}6). - Colin Barker, Feb 23 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x + 8x^{2} + 5x^{3} - 16x^{4} - 16x^{5} - 4x^{6}}{1 - x - 3x^{2} - 9x^{3} + 4x^{4} + 12x^{5} + 4x^{6}}.\]
...
\begin{theorem} For A296645, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 9\,a(n-3) - 4\,a(n-4) - 12\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296645, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x + 8x^{2} + 5x^{3} - 16x^{4} - 16x^{5} - 4x^{6}}{1 - x - 3x^{2} - 9x^{3} + 4x^{4} + 12x^{5} + 4x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 12, 29, 85, 248, 664, 1897, 5385,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296645}{A296645}, revision 8, last modified Feb 23 2019.
