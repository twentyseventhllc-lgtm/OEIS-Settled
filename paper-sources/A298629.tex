...
\title{Proofs of the conjectured column and row recurrences for OEIS A298629}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298629}{A298629} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 2, 3, 4, 5 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) +2*a(n-3)\\k=3: [order 11] for n>12\\k=4: [order 48]\end{quote}
...
\begin{theorem} In A298629, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298629, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298629, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $67$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 12$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 12$; the proved threshold is $n > 12$.

\begin{theorem} For column $k = 4$ of A298629, the recurrence $a(n) = 0$ holds for every $n \ge 49$, at every index at which it can be stated.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298629}{A298629}, revision 4, last modified Jan 23 2018.
