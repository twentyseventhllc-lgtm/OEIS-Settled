...
\title{Proofs of the conjectured column and row recurrences for OEIS A241119}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A241119}{A241119} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays with no element equal to exactly three horizontal or vertical neighbors, with new values 0..3 introduced in row major order\end{quote}
...
k=1: a(n) = 7*a(n-1) -14*a(n-2) +8*a(n-3)\\k=2: [order 8] for n>10\\k=3: [order 20] for n>21\end{quote}
...
\begin{theorem} For column $k = 1$ of A241119, the recurrence $a(n) = 7\,a(n-1) - 14\,a(n-2) + 8\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $32$ states, $32$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A241119, the recurrence $a(n) = 0$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $710$ states, $357$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 3$ of A241119, the recurrence $a(n) = 0$ holds for every $n > 21$, and fails at $n = 21$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $9785$ states, $4895$ after trimming, and $S = 23$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 21$; after that point more than $S = 23$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A241119}{A241119}, revision 6, last modified Jul 23 2025.
