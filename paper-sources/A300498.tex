...
\title{Proofs of the conjectured column and row recurrences for OEIS A300498}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300498}{A300498} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -a(n-2) +a(n-3) +a(n-4) -2*a(n-5) -a(n-6)\\k=3: [order 24] for n>25\\k=4: [order 83] for n>86\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 07 2018). As of revision 4, last modified Mar 07 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A300498, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300498, the recurrence $a(n) = 3\,a(n-1) - a(n-2) + a(n-3) + a(n-4) - 2\,a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300498, the recurrence $a(n) = 0$ holds for every $n > 25$, and fails at $n = 25$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A300498, the recurrence $a(n) = 0$ holds for every $n > 86$, and fails at $n = 86$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $242$ after trimming, and $S = 89$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 86$; after that point more than $S = 89$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300498}{A300498}, revision 4, last modified Mar 07 2018.
