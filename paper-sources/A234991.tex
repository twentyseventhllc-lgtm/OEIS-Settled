...
\title{Proofs of the conjectured column and row recurrences for OEIS A234991}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234991}{A234991} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
k=1: [linear recurrence of order 8].\\k=2: [order 28].\end{quote}
...
\begin{theorem} For column $k = 1$ of A234991, the recurrence $a(n) = 0$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A234991, the recurrence $a(n) = 0$ holds for every $n \ge 29$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $216$ states, $150$ after trimming, and $S = 29$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 29$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234991}{A234991}, revision 6, last modified Jun 20 2022.
