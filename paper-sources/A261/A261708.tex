...
\title{A proof of the conjectured linear recurrence for OEIS A261708}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A261708}{A261708} as

\begin{quote}\itshape Number of (n+2)X(5+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000001 00000101 or 00000111\end{quote}
...
That refinement leaves $95$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 58$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A261708, the recurrence $a(n) = 2\,a(n-7) - 2\,a(n-21) + a(n-28)$ holds for every $n > 34$, and fails at $n = 34$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,136$. The last nonvanishing one is at $m = 6$, that is at $n = 34$. After it, more than $S = 58$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 44 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 628, 892, 1168, 734, 676, 1818, 984, 748,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A261708}{A261708}, revision 4, last modified Aug 28 2015.
