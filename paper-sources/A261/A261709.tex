...
\title{Proofs of the conjectured column and row recurrences for OEIS A261709}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A261709}{A261709} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000001 00000101 or 00000111\end{quote}
...
k=1: [linear recurrence of order 9] for n>11\\k=2: [order 15] for n>17\\k=3: [order 14] for n>20\\k=4: a(n) = 3*a(n-7) +5*a(n-10) +a(n-13) -a(n-14) -a(n-17) -2*a(n-21) for n>27\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 28 2015). As of revision 4, last modified Aug 28 2015, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A261709, the recurrence $a(n) = 0$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 11$; the proved threshold is $n > 11$.

\begin{theorem} For column $k = 2$ of A261709, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A261709, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 31$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 20$; after that point more than $S = 31$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 20$; the proved threshold is $n > 20$.

\begin{theorem} For column $k = 4$ of A261709, the recurrence $a(n) = 3\,a(n-7) + 5\,a(n-10) + a(n-13) - a(n-14) - a(n-17) - 2\,a(n-21)$ holds for every $n > 27$, and fails at $n = 27$.\end{theorem}

{\itshape Proof.} The width is $6$; the digraph has $4160$ states, $4160$ after trimming, and $S = 42$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 27$; after that point more than $S = 42$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A261709}{A261709}, revision 4, last modified Aug 28 2015.
