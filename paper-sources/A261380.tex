...
k=1: a(n) = a(n-2) +3*a(n-4) +2*a(n-6) for n>12\\k=2: a(n) = a(n-2) +6*a(n-4) +4*a(n-6) -a(n-8) for n>9\\k=3: a(n) = a(n-2) +11*a(n-4) +13*a(n-6) -2*a(n-8) -2*a(n-10) +4*a(n-12) for n>17\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 17 2015). As of revision 4, last modified Aug 17 2015, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
