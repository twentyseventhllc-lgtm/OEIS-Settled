...
\title{Proofs of the conjectured column and row recurrences for OEIS A296157}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296157}{A296157} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 adjacent to 3 or 5 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) -a(n-2) +a(n-3)\\k=3: [order 15]\\k=4: [order 43]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 06 2017). As of revision 4, last modified Dec 06 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A296157, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A296157, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A296157, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $51$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A296157, the recurrence $a(n) = 0$ holds for every $n \ge 44$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $191$ after trimming, and $S = 48$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 48$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296157}{A296157}, revision 4, last modified Dec 06 2017.
