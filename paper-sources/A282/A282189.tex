...
\title{Proofs of the conjectured column and row recurrences for OEIS A282189}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A282189}{A282189} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to more than four of its king-move neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 16*a(n-1) -48*a(n-2) -116*a(n-3) -160*a(n-4) -96*a(n-5) -36*a(n-6) for n>9\\k=3: [order 30] for n>33\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 08 2017). As of revision 4, last modified Feb 08 2017, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} In A282189, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A282189, the recurrence $a(n) = 16\,a(n-1) - 48\,a(n-2) - 116\,a(n-3) - 160\,a(n-4) - 96\,a(n-5) - 36\,a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $199$ states, $199$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For column $k = 3$ of A282189, the recurrence $a(n) = 0$ holds for every $n > 33$, and fails at $n = 33$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $3174$ states, $2376$ after trimming, and $S = 43$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 33$; after that point more than $S = 43$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A282189}{A282189}, revision 4, last modified Feb 08 2017.
