...
\title{Proofs of the conjectured column and row recurrences for OEIS A252640}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252640}{A252640} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 3 4 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 3 4 6 or 7\end{quote}
...
k=1: [linear recurrence of order 18] for n>24\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: [linear recurrence of order 67] for n>74\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 19 2014). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence and the 1 row recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A252640, the recurrence $a(n) = 0$ holds for every $n > 24$, and fails at $n = 24$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 85$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 24$; after that point more than $S = 85$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 24$; the proved threshold is $n > 24$.

\begin{theorem} For row $n = 1$ of A252640, the recurrence $a(k) = 0$ holds for every $k > 74$, and fails at $k = 74$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 113$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 74$; after that point more than $S = 113$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252640}{A252640}, revision 6, last modified Jun 02 2025.
