...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A295525}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295525}{A295525} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 horizontally or vertically adjacent to 1, 2 or 4 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 2*a(n-2) + 5*a(n-3) - a(n-5) - a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(2 + 6*x + 5*x\textasciicircum{}2 - x\textasciicircum{}3 - 2*x\textasciicircum{}4 - x\textasciicircum{}5) / (1 - 2*x - 2*x\textasciicircum{}2 - 5*x\textasciicircum{}3 + x\textasciicircum{}5 + x\textasciicircum{}6). - Colin Barker, Feb 21 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x + 6x^{2} + 5x^{3} - x^{4} - 2x^{5} - x^{6}}{1 - 2x - 2x^{2} - 5x^{3} + x^{5} + x^{6}}.\]
...
\begin{theorem} For A295525, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) + 5\,a(n-3) - a(n-5) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A295525, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x + 6x^{2} + 5x^{3} - x^{4} - 2x^{5} - x^{6}}{1 - 2x - 2x^{2} - 5x^{3} + x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 10, 29, 87, 280, 876, 2735, 8583,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295525}{A295525}, revision 8, last modified Feb 21 2018.
