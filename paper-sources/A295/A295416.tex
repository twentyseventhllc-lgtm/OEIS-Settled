...
\title{Proofs of the conjectured column and row recurrences for OEIS A295416}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295416}{A295416} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 horizontally or vertically adjacent to 1, 2 or 3 1s\end{quote}
...
k=1: a(n) = 2*a(n-1) -a(n-2) +a(n-3)\\k=2: a(n) = 3*a(n-1) +a(n-2) +4*a(n-3)\\k=3: a(n) = 6*a(n-1) +28*a(n-3) +3*a(n-4) +49*a(n-5) +33*a(n-6) -34*a(n-7) -22*a(n-8)\\k=4: [order 14]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 22 2017). As of revision 4, last modified Nov 22 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A295416, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A295416, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 4\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A295416, the recurrence $a(n) = 6\,a(n-1) + 28\,a(n-3) + 3\,a(n-4) + 49\,a(n-5) + 33\,a(n-6) - 34\,a(n-7) - 22\,a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A295416, the recurrence $a(n) = 0$ holds for every $n \ge 15$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295416}{A295416}, revision 4, last modified Nov 22 2017.
