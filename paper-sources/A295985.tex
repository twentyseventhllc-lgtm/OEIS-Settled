...
\title{Proofs of the conjectured column and row recurrences for OEIS A295985}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295985}{A295985} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 adjacent to 2 or 3 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 3*a(n-1) -2*a(n-2) +9*a(n-3) -10*a(n-4) -4*a(n-5) -4*a(n-6)\\k=3: [order 10]\\k=4: [order 27]\\k=5: [order 67]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 01 2017). As of revision 4, last modified Dec 01 2017, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A295985, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A295985, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 9\,a(n-3) - 10\,a(n-4) - 4\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $20$ states, $20$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A295985, the recurrence $a(n) = 0$ holds for every $n \ge 11$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $58$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A295985, the recurrence $a(n) = 0$ holds for every $n \ge 28$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $213$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 5$ of A295985, the recurrence $a(n) = 0$ holds for every $n \ge 68$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $788$ after trimming, and $S = 90$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 90$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295985}{A295985}, revision 4, last modified Dec 01 2017.
