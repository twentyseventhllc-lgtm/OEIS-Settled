...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207495}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207495}{A207495} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 0 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) -2*a(n-2) -14*a(n-3) +6*a(n-4) +8*a(n-5) -a(n-6) -a(n-7) for n>8.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(6 + 6*x - 66*x\textasciicircum{}2 - 57*x\textasciicircum{}3 + 119*x\textasciicircum{}4 + 72*x\textasciicircum{}5 - 19*x\textasciicircum{}6 - 11*x\textasciicircum{}7) / ((1 - x)*(1 - 2*x - x\textasciicircum{}2)*(1 - 2*x - 5*x\textasciicircum{}2 + x\textasciicircum{}4)). - Colin Barker, Mar 05 2018\end{quote}
...
\begin{multline*} a(n) = 5\,a(n-1) - 2\,a(n-2) - 14\,a(n-3) + 6\,a(n-4) + 8\,a(n-5) - a(n-6) - \\ a(n-7). \end{multline*}

stated for $n > 8$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 6x^{2} - 66x^{3} - 57x^{4} + 119x^{5} + 72x^{6} - 19x^{7} - 11x^{8}}{1 - 5x + 2x^{2} + 14x^{3} - 6x^{4} - 8x^{5} + x^{6} + x^{7}}.\]
...
\begin{theorem} For A207495, the recurrence $a(n) = 5\,a(n-1) - 2\,a(n-2) - 14\,a(n-3) + 6\,a(n-4) + 8\,a(n-5) - a(n-6) - a(n-7)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}
...
\begin{theorem} For A207495, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 6x^{2} - 66x^{3} - 57x^{4} + 119x^{5} + 72x^{6} - 19x^{7} - 11x^{8}}{1 - 5x + 2x^{2} + 14x^{3} - 6x^{4} - 8x^{5} + x^{6} + x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 102, 297, 932, 2974, 9723, 32164,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207495}{A207495}, revision 8, last modified Mar 05 2018.
