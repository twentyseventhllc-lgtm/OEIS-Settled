...
\title{Proofs of the conjectured column and row recurrences for OEIS A297951}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297951}{A297951} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 3 or 5 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +2*a(n-3) -6*a(n-4) -4*a(n-5) for n>6\\k=3: [order 20] for n>21\\k=4: [order 66] for n>69\end{quote}
...
\begin{theorem} For column $k = 1$ of A297951, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297951, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297951, the recurrence $a(n) = 0$ holds for every $n > 21$, and fails at $n = 21$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A297951, the recurrence $a(n) = 0$ holds for every $n > 69$, and fails at $n = 69$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $263$ after trimming, and $S = 74$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 69$; after that point more than $S = 74$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297951}{A297951}, revision 4, last modified Jan 09 2018.
