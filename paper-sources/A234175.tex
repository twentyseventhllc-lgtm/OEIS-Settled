...
\title{Proofs of the conjectured column and row recurrences for OEIS A234175}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234175}{A234175} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 154*a(n-2) -4144*a(n-4) +16176*a(n-6).\end{quote}
...
The entry publishes its table by upward antidiagonals, so every published term is a value $T(n,k)$ with both indices known. All 24 of them are used as the data gate below: the models are required to reproduce the whole published triangle, not merely the column or row a given line is about.
...
\begin{theorem} For column $k = 1$ of A234175, the recurrence $a(n) = 154\,a(n-2) - 4144\,a(n-4) + 16176\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published table.} Every one of the 24 values $T(n,k)$ the entry publishes was recomputed from the models and agrees exactly, in integer arithmetic, with the indices read off the entry's offset (1) and its stated shape.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234175}{A234175}, revision 6, last modified Jun 20 2022.
