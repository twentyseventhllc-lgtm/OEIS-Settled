...
\title{Proofs of the conjectured column and row recurrences for OEIS A304149}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304149}{A304149} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3 or 6 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 2*a(n-1) +3*a(n-2) -2*a(n-3) -6*a(n-4) -4*a(n-5) for n>6\\k=3: [order 14] for n>16\\k=4: [order 17] for n>22\\k=5: [order 93] for n>96\end{quote}
...
\begin{theorem} For column $k = 1$ of A304149, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A304149, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A304149, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A304149, the recurrence $a(n) = 0$ holds for every $n > 22$, and fails at $n = 22$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $149$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 22$; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 22$; the proved threshold is $n > 22$.

\begin{theorem} For column $k = 5$ of A304149, the recurrence $a(n) = 0$ holds for every $n > 96$, and fails at $n = 96$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $557$ after trimming, and $S = 113$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 96$; after that point more than $S = 113$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304149}{A304149}, revision 4, last modified May 07 2018.
