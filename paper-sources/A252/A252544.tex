...
\title{Proofs of the conjectured column and row recurrences for OEIS A252544}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252544}{A252544} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 1 3 4 6 or 7 and every 3X3 column and antidiagonal sum not equal to 1 3 4 6 or 7\end{quote}
...
k=1: a(n) = 18*a(n-3) -32*a(n-6) for n>8\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = 34*a(n-3) +2*a(n-5) -320*a(n-6) -36*a(n-8) +512*a(n-9) +64*a(n-11) for n>15\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 18 2014). As of revision 7, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence and the 1 row recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A252544, the recurrence $a(n) = 18\,a(n-3) - 32\,a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 18$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 8$; after that point more than $S = 18$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 8$; the proved threshold is $n > 8$.

\begin{theorem} For row $n = 1$ of A252544, the recurrence $a(k) = 34\,a(n-3) + 2\,a(n-5) - 320\,a(n-6) - 36\,a(n-8) + 512\,a(n-9) + 64\,a(n-11)$ holds for every $k > 15$, and fails at $k = 15$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 39$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 15$; after that point more than $S = 39$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item 9 lines: fewer than six published terms in that line
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252544}{A252544}, revision 7, last modified Jun 02 2025.
