...
\title{A proof of the conjectured linear recurrence for OEIS A252450}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252450}{A252450} as

\begin{quote}\itshape Number of (n+2) X (1+2) 0..3 arrays with every 3 X 3 subblock row, column, diagonal and antidiagonal sum not equal to 0 2 4 6 or 7\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) -3*a(n-2) +9*a(n-3) -9*a(n-4) +4*a(n-5) -22*a(n-6) -14*a(n-7) -19*a(n-8) -8*a(n-9) +8*a(n-10) +18*a(n-11) +24*a(n-12) +20*a(n-13) +6*a(n-14) -4*a(n-15) -8*a(n-16) -4*a(n-17) for n>20.\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Dec 17 2014).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Feb 27 2026, the entry records no proof and links to none, so the statement is open. This note settles it.
...
Every row and column and diagonal and antidiagonal sum of every $3\times3$ subblock must avoid $\{0, 2, 4, 6, 7\}$.
...
That refinement leaves $58$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 32$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $4^{24}$ arrays, about $10^{14}$, against $32$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A252450, the recurrence $a(n) = 3\,a(n-1) - 3\,a(n-2) + 9\,a(n-3) - 9\,a(n-4) + 4\,a(n-5) - 22\,a(n-6) - 14\,a(n-7) - 19\,a(n-8) - 8\,a(n-9) + 8\,a(n-10) + 18\,a(n-11) + 24\,a(n-12) + 20\,a(n-13) + 6\,a(n-14) - 4\,a(n-15) - 8\,a(n-16) - 4\,a(n-17)$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,81$. The last nonvanishing one is at $m = 3$, that is at $n = 20$. After it, more than $S = 32$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 661, 228, 323, 747, 1731, 3938, 9013, 20820,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252450}{A252450}, revision 8, last modified Feb 27 2026.
