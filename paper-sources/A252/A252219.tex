...
\title{Proofs of the conjectured column and row recurrences for OEIS A252219}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252219}{A252219} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 0 2 5 6 or 7\end{quote}
...
k=1: [linear recurrence of order 61] for n>69\end{quote}
...
\begin{theorem} For column $k = 1$ of A252219, the recurrence $a(n) = 0$ holds for every $n > 69$, and fails at $n = 69$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 114$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 69$; after that point more than $S = 114$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252219}{A252219}, revision 6, last modified Jun 02 2025.
