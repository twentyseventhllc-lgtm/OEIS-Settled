...
\title{Proofs of the conjectured column and row recurrences for OEIS A252032}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252032}{A252032} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 0 2 or 4\end{quote}
...
k=1: [linear recurrence of order 11] for n>14\\k=2: [order 14] for n>18\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 12 2014). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A252032, the recurrence $a(n) = 0$ holds for every $n > 14$, and fails at $n = 14$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $756$ states, $756$ after trimming, and $S = 23$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 14$; after that point more than $S = 23$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 14$; the proved threshold is $n > 14$.

\begin{theorem} For column $k = 2$ of A252032, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $6642$ states, $6642$ after trimming, and $S = 28$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 28$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252032}{A252032}, revision 6, last modified Jun 02 2025.
