...
\title{Proofs of the conjectured column and row recurrences for OEIS A252433}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252433}{A252433} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 4 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 4 6 or 7\end{quote}
...
k=1: [linear recurrence of order 27] for n>34\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: [linear recurrence of order 29] for n>44\end{quote}
...
\begin{theorem} For column $k = 1$ of A252433, the recurrence $a(n) = 0$ holds for every $n > 34$, and fails at $n = 34$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 96$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 34$; after that point more than $S = 96$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 34$; the proved threshold is $n > 34$.

\begin{theorem} For row $n = 1$ of A252433, the recurrence $a(k) = 0$ holds for every $k > 44$, and fails at $k = 44$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 113$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 44$; after that point more than $S = 113$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252433}{A252433}, revision 6, last modified Jun 02 2025.
