...
\title{Proofs of the conjectured column and row recurrences for OEIS A252695}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252695}{A252695} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every consecutive three elements in every row and column having exactly two distinct values, and in every diagonal and antidiagonal not having exactly two distinct values, and new values 0 upwards introduced in row major order\end{quote}
...
k=1: [linear recurrence of order 13] for n>14\\k=2: [order 10] for n>11\\k=3: [order 9] for n>10\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 20 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A252695, the recurrence $a(n) = 0$ holds for every $n > 14$, and fails at $n = 14$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $397$ states, $397$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 14$; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 14$; the proved threshold is $n > 14$.

\begin{theorem} For column $k = 2$ of A252695, the recurrence $a(n) = 0$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $672$ states, $672$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 11$; the proved threshold is $n > 11$.

\begin{theorem} For column $k = 3$ of A252695, the recurrence $a(n) = 0$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $2044$ states, $2044$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252695}{A252695}, revision 6, last modified Jul 23 2025.
