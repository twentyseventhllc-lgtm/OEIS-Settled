...
\title{Proofs of the conjectured column and row recurrences for OEIS A252278}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252278}{A252278} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and column sum not equal to 2 3 4 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 2 3 4 6 or 7\end{quote}
...
k=1: a(n) = 6*a(n-3) -7*a(n-6) +2*a(n-9) for n>12\end{quote}
...
\begin{theorem} For column $k = 1$ of A252278, the recurrence $a(n) = 6\,a(n-3) - 7\,a(n-6) + 2\,a(n-9)$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 29$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 12$; after that point more than $S = 29$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252278}{A252278}, revision 6, last modified Jun 02 2025.
