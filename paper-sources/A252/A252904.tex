...
\title{Proofs of the conjectured column and row recurrences for OEIS A252904}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252904}{A252904} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every consecutive three elements in every row and column having exactly two distinct values, and new values 0 upwards introduced in row major order\end{quote}
...
k=1: [linear recurrence of order 18]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 24 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A252904, the recurrence $a(n) = 0$ holds for every $n \ge 19$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $1660$ states, $1660$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252904}{A252904}, revision 6, last modified Jul 23 2025.
