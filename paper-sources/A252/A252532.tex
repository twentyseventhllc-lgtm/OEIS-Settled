...
\title{Proofs of the conjectured column and row recurrences for OEIS A252532}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252532}{A252532} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 3 6 or 7\end{quote}
...
k=1: a(n) = 4*a(n-3) -3*a(n-6) -4*a(n-9) +4*a(n-12) for n>23\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = 4*a(n-3) -3*a(n-6) -4*a(n-9) +4*a(n-12) for n>39\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 18 2014). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence and the 1 row recurrence listed above.
...
Every row and diagonal sum of every $3\times3$ subblock must lie in $\{0, 2, 3, 6, 7\}$.

Every column and antidiagonal sum of every $3\times3$ subblock must avoid $\{0, 2, 3, 6, 7\}$.
...
\begin{theorem} For column $k = 1$ of A252532, the recurrence $a(n) = 4\,a(n-3) - 3\,a(n-6) - 4\,a(n-9) + 4\,a(n-12)$ holds for every $n > 23$, and fails at $n = 23$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 80$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 23$; after that point more than $S = 80$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 23$; the proved threshold is $n > 23$.

\begin{theorem} For row $n = 1$ of A252532, the recurrence $a(k) = 4\,a(n-3) - 3\,a(n-6) - 4\,a(n-9) + 4\,a(n-12)$ holds for every $k > 39$, and fails at $k = 39$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 86$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 39$; after that point more than $S = 86$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item 2 lines: transition work 1074790400 > cap 25000000

\item 2 lines: state space 16781312 > cap 3000000
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252532}{A252532}, revision 6, last modified Jun 02 2025.
