...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A252271}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252271}{A252271} as

\begin{quote}\itshape Number of (n+2) X (1+2) 0..3 arrays with every 3 X 3 subblock row and column sum not equal to 2 3 4 6 or 7 and every 3 X 3 diagonal and antidiagonal sum equal to 2 3 4 6 or 7\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 6*a(n-3) - 7*a(n-6) + 2*a(n-9) for n>12.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(206 + 185*x + 232*x\textasciicircum{}2 - 914*x\textasciicircum{}3 - 575*x\textasciicircum{}4 - 540*x\textasciicircum{}5 + 770*x\textasciicircum{}6 + 306*x\textasciicircum{}7 + 188*x\textasciicircum{}8 - 180*x\textasciicircum{}9 - 40*x\textasciicircum{}10 + 2*x\textasciicircum{}11) / ((1 - x)*(1 + x + x\textasciicircum{}2)*(1 - 5*x\textasciicircum{}3 + 2*x\textasciicircum{}6)). - Colin Barker, Dec 03 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{206x + 185x^{2} + 232x^{3} - 914x^{4} - 575x^{5} - 540x^{6} + 770x^{7} + 306x^{8} + 188x^{9} - 180x^{10} - 40x^{11} + 2x^{12}}{1 - 6x^{3} + 7x^{6} - 2x^{9}}.\]
...
Every row and column sum of every $3\times3$ subblock must avoid $\{2, 3, 4, 6, 7\}$.

Every diagonal and antidiagonal sum of every $3\times3$ subblock must lie in $\{2, 3, 4, 6, 7\}$.
...
That refinement leaves $40$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 29$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $4^{24}$ arrays, about $10^{14}$, against $29$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A252271, the recurrence $a(n) = 6\,a(n-3) - 7\,a(n-6) + 2\,a(n-9)$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,74$. The last nonvanishing one is at $m = 3$, that is at $n = 12$. After it, more than $S = 29$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 12$. The theorem is at least as strong.

\begin{theorem} For A252271, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{206x + 185x^{2} + 232x^{3} - 914x^{4} - 575x^{5} - 540x^{6} + 770x^{7} + 306x^{8} + 188x^{9} - 180x^{10} - 40x^{11} + 2x^{12}}{1 - 6x^{3} + 7x^{6} - 2x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 206, 185, 232, 322, 535, 852, 1260, 2221,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252271}{A252271}, revision 8, last modified Dec 03 2018.
