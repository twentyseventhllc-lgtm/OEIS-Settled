...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A224599}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224599}{A224599} as

\begin{quote}\itshape Number of (n+2) X 3 0..2 matrices with each 3 X 3 subblock idempotent\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: x*(122 - 188*x + 129*x\textasciicircum{}2 - 185*x\textasciicircum{}3 + 37*x\textasciicircum{}4 + 115*x\textasciicircum{}5 - 36*x\textasciicircum{}6) / ((1 - x)\textasciicircum{}2*(1 + x\textasciicircum{}2)*(1 - x - x\textasciicircum{}2)). - Colin Barker, Sep 01 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Apr 11 2013).

The line quoted ending in that signature is contributed by Colin Barker on Sep 01 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Sep 01 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{122x - 188x^{2} + 129x^{3} - 185x^{4} + 37x^{5} + 115x^{6} - 36x^{7}}{1 - 3x + 3x^{2} - 2x^{3} + x^{4} + x^{5} - x^{6}}.\]

\section{The object}

The name is read as follows. The reading is not a guess: it is pinned against the entry's own published terms, and against an independent enumeration, before anything is proved (Section 7).

Let $q = 3$ and let $A$ be an $n' \times 3$ matrix over $\{0,\dots,2\}$ with $n' = n + 2$ rows.
...
\begin{theorem} For A224599, the recurrence $a(n) = 3\,a(n-1) - 3\,a(n-2) + 2\,a(n-3) - a(n-4) - a(n-5) + a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,57$. The last nonvanishing one is at $m = 1$, that is at $n = 7$. After it, more than $S = 14$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A224599, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{122x - 188x^{2} + 129x^{3} - 185x^{4} + 37x^{5} + 115x^{6} - 36x^{7}}{1 - 3x + 3x^{2} - 2x^{3} + x^{4} + x^{5} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 122, 178, 297, 416, 628, 1045, 1694, 2668,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224599}{A224599}, revision 8, last modified Sep 01 2018.
