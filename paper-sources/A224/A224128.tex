...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A224128}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224128}{A224128} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with rows nondecreasing and antidiagonals unimodal\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 2*a(n-2) + 2*a(n-3).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(2 + x\textasciicircum{}2) / (1 - 4*x + 2*x\textasciicircum{}2 - 2*x\textasciicircum{}3). - Colin Barker, Aug 27 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Aug 27 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x + 2x^{3}}{1 - 4x + 2x^{2} - 2x^{3}}.\]
...
\begin{theorem} For A224128, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A224128, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x + 2x^{3}}{1 - 4x + 2x^{2} - 2x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 16, 58, 208, 748, 2692, 9688, 34864,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224128}{A224128}, revision 10, last modified Aug 27 2018.
