...
\title{Proofs of the conjectured column and row recurrences for OEIS A224391}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224391}{A224391} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays with diagonals and antidiagonals unimodal and rows nondecreasing\end{quote}
...
k=1: a(n) = 4*a(n-1)\\k=2: a(n) = 10*a(n-1)\\k=3: [order 15]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 05 2013). As of revision 8, last modified Oct 03 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A224391, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $5$ states, $5$ after trimming, and $S = 2$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 2$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A224391, the recurrence $a(n) = 10\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A224391, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $108$ states, $108$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224391}{A224391}, revision 8, last modified Oct 03 2025.
