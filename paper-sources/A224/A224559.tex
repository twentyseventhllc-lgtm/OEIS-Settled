...
\title{Proofs of the conjectured column and row recurrences for OEIS A224559}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224559}{A224559} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 matrices with each 3X3 subblock idempotent\end{quote}
...
k=1: a(n) = 3*a(n-1) -3*a(n-2) +2*a(n-3) -a(n-4) -a(n-5) +a(n-6) for n>7\\k=2: a(n) = 3*a(n-1) -2*a(n-2) -a(n-3) +a(n-4) -a(n-5) +2*a(n-6) -a(n-7) for n>9\end{quote}
...
\begin{theorem} For column $k = 1$ of A224559, the recurrence $a(n) = 3\,a(n-1) - 3\,a(n-2) + 2\,a(n-3) - a(n-4) - a(n-5) + a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A224559, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) - a(n-3) + a(n-4) - a(n-5) + 2\,a(n-6) - a(n-7)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item 1 line: line not under an Empirical header
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224559}{A224559}, revision 8, last modified Oct 03 2025.
