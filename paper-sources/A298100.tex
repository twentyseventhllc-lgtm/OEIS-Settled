...
\title{Proofs of the conjectured column and row recurrences for OEIS A298100}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298100}{A298100} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 3, 4, 5 or 6 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +a(n-2)\\k=3: a(n) = a(n-1) +2*a(n-2) -a(n-3) +a(n-4) -2*a(n-5)\\k=4: [order 16]\\k=5: [order 61]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 12 2018). As of revision 4, last modified Jan 12 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} In A298100, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298100, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298100, the recurrence $a(n) = a(n-1) + 2\,a(n-2) - a(n-3) + a(n-4) - 2\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A298100, the recurrence $a(n) = 0$ holds for every $n \ge 17$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 5$ of A298100, the recurrence $a(n) = 0$ holds for every $n \ge 62$, at every index at which it can be stated.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298100}{A298100}, revision 4, last modified Jan 12 2018.
