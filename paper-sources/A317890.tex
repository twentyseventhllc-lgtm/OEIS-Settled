...
\title{A proof of the conjectured linear recurrence for OEIS A317890}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317890}{A317890} as

\begin{quote}\itshape Number of nX2 0..1 arrays with every element unequal to 2, 3 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +a(n-2) +2*a(n-3) +3*a(n-4) +a(n-5) for n>6\end{quote}
...
\[a(n) = 2\,a(n-1) + a(n-2) + 2\,a(n-3) + 3\,a(n-4) + a(n-5).\]
...
The construction gives $20$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $15$; neither removal changes any count.
...
\begin{theorem} For A317890, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + 2\,a(n-3) + 3\,a(n-4) + a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 1, 1, 2, 5, 16, 45, 123,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317890}{A317890}, revision 4, last modified Aug 10 2018.
