...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208421}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208421}{A208421} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 0 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 7*a(n-2) - 6*a(n-3) - 3*a(n-4) + 2*a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 9*x*(1 + 6*x - 5*x\textasciicircum{}2 - 3*x\textasciicircum{}3 + 2*x\textasciicircum{}4) / (1 - 3*x - 7*x\textasciicircum{}2 + 6*x\textasciicircum{}3 + 3*x\textasciicircum{}4 - 2*x\textasciicircum{}5). - Colin Barker, Jul 02 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 54x^{2} - 45x^{3} - 27x^{4} + 18x^{5}}{1 - 3x - 7x^{2} + 6x^{3} + 3x^{4} - 2x^{5}}.\]
...
\begin{theorem} For A208421, the recurrence $a(n) = 3\,a(n-1) + 7\,a(n-2) - 6\,a(n-3) - 3\,a(n-4) + 2\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208421, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 54x^{2} - 45x^{3} - 27x^{4} + 18x^{5}}{1 - 3x - 7x^{2} + 6x^{3} + 3x^{4} - 2x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 261, 1269, 5139, 22509, 95265, 409239,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208421}{A208421}, revision 8, last modified Jul 02 2018.
