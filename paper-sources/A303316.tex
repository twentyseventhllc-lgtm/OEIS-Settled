...
\title{A proof of the conjectured linear recurrence for OEIS A303316}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303316}{A303316} as

\begin{quote}\itshape Number of 4Xn 0..1 arrays with every element equal to 0, 1, 4, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +5*a(n-3) +9*a(n-4) -3*a(n-5) -7*a(n-6) -2*a(n-7) for n>11\end{quote}
...
\begin{multline*} a(n) = a(n-1) + a(n-2) + 5\,a(n-3) + 9\,a(n-4) - 3\,a(n-5) - 7\,a(n-6) - \\ 2\,a(n-7). \end{multline*}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $65$; neither removal changes any count.
...
\begin{theorem} For A303316, the recurrence $a(n) = a(n-1) + a(n-2) + 5\,a(n-3) + 9\,a(n-4) - 3\,a(n-5) - 7\,a(n-6) - 2\,a(n-7)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,54$. The last nonvanishing one is at $m = 4$, that is at $n = 11$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 6, 11, 36, 50, 128, 439, 1085,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303316}{A303316}, revision 4, last modified Apr 21 2018.
