...
\title{Proofs of the conjectured column and row recurrences for OEIS A276248}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A276248}{A276248} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (-2,-1) (-1,1) or (0,-2) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 4*a(n-1) -3*a(n-2)\\k=2: a(n) = 4*a(n-1) for n>2\\k=3: a(n) = 5*a(n-1) -4*a(n-2) +17*a(n-3) -83*a(n-4) +54*a(n-5) +56*a(n-6) for n>9\\k=4: [order 36] for n>37\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 25 2016). As of revision 4, last modified Aug 25 2016, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A276248, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A276248, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n > 2$, and fails at $n = 2$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $64$ states, $64$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 2$; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 2$; the proved threshold is $n > 2$.

\begin{theorem} For column $k = 3$ of A276248, the recurrence $a(n) = 5\,a(n-1) - 4\,a(n-2) + 17\,a(n-3) - 83\,a(n-4) + 54\,a(n-5) + 56\,a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $150$ states, $150$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 9$; the proved threshold is $n > 9$.

\begin{theorem} For column $k = 4$ of A276248, the recurrence $a(n) = 0$ holds for every $n > 37$, and fails at $n = 37$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $436$ states, $436$ after trimming, and $S = 38$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 37$; after that point more than $S = 38$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A276248}{A276248}, revision 4, last modified Aug 25 2016.
