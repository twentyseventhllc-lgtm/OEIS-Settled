...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208289}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208289}{A208289} as

\begin{quote}\itshape Number of 5 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 10*a(n-2) - 2*a(n-3) - 7*a(n-4) + a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 10*x*(1 + 7*x - x\textasciicircum{}2 - 6*x\textasciicircum{}3 + x\textasciicircum{}5) / (1 - 3*x - 10*x\textasciicircum{}2 + 2*x\textasciicircum{}3 + 7*x\textasciicircum{}4 - x\textasciicircum{}6). - Colin Barker, Jun 30 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{10x + 70x^{2} - 10x^{3} - 60x^{4} + 10x^{6}}{1 - 3x - 10x^{2} + 2x^{3} + 7x^{4} - x^{6}}.\]
...
\begin{theorem} For A208289, the recurrence $a(n) = 3\,a(n-1) + 10\,a(n-2) - 2\,a(n-3) - 7\,a(n-4) + a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208289, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{10x + 70x^{2} - 10x^{3} - 60x^{4} + 10x^{6}}{1 - 3x - 10x^{2} + 2x^{3} + 7x^{4} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 10, 100, 390, 2090, 9900, 49130, 239490, 1175440,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208289}{A208289}, revision 8, last modified Jun 30 2018.
