...
\title{Proofs of the conjectured column and row recurrences for OEIS A224310}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224310}{A224310} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with diagonals and rows unimodal and antidiagonals nondecreasing\end{quote}
...
k=1: a(n) = 3*a(n-1)\\k=2: a(n) = 6*a(n-1)\\k=3: [order 17]\\k=4: [order 30] for n>35\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 03 2013). As of revision 8, last modified Oct 03 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A224310, the recurrence $a(n) = 3\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $4$ states, $4$ after trimming, and $S = 2$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 2$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A224310, the recurrence $a(n) = 6\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $16$ states, $16$ after trimming, and $S = 2$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 2$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A224310, the recurrence $a(n) = 0$ holds for every $n \ge 18$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $57$ states, $57$ after trimming, and $S = 27$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 27$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A224310, the recurrence $a(n) = 0$ holds for every $n > 35$, and fails at $n = 35$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $182$ states, $182$ after trimming, and $S = 60$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 35$; after that point more than $S = 60$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224310}{A224310}, revision 8, last modified Oct 03 2025.
