...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A209225}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A209225}{A209225} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 1 0 and 1 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 4*a(n-3) + 16*a(n-4) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(9 + 72*x + 45*x\textasciicircum{}2 + 34*x\textasciicircum{}3 - 160*x\textasciicircum{}4 - 1008*x\textasciicircum{}5 - 784*x\textasciicircum{}6) / ((1 + 4*x\textasciicircum{}2)*(1 - x - 4*x\textasciicircum{}2)). - Colin Barker, Jul 08 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Jul 08 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 72x^{2} + 45x^{3} + 34x^{4} - 160x^{5} - 1008x^{6} - 784x^{7}}{1 - x - 4x^{3} - 16x^{4}}.\]
...
\begin{theorem} For A209225, the recurrence $a(n) = a(n-1) + 4\,a(n-3) + 16\,a(n-4)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,59$. The last nonvanishing one is at $m = 3$, that is at $n = 7$. After it, more than $S = 23$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A209225, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 72x^{2} + 45x^{3} + 34x^{4} - 160x^{5} - 1008x^{6} - 784x^{7}}{1 - x - 4x^{3} - 16x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 126, 196, 504, 1296, 3312, 8464,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A209225}{A209225}, revision 10, last modified Jul 08 2018.
