...
\title{Proofs of the conjectured column and row recurrences for OEIS A240636}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A240636}{A240636} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no element equal to exactly three horizontal or vertical neighbors, with new values 0..1 introduced in row major order\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +a(n-2) -2*a(n-4)\\k=3: a(n) = 4*a(n-1) +4*a(n-2) +2*a(n-3) +7*a(n-4) +6*a(n-5) -a(n-6) -a(n-7) -a(n-8)\\k=4: [order 32]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 09 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
Call a cell {\itshape bad} when $c(i,j) \in \{3\}$.
...
\begin{theorem} For column $k = 1$ of A240636, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A240636, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 2\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A240636, the recurrence $a(n) = 4\,a(n-1) + 4\,a(n-2) + 2\,a(n-3) + 7\,a(n-4) + 6\,a(n-5) - a(n-6) - a(n-7) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $134$ states, $69$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A240636, the recurrence $a(n) = 0$ holds for every $n \ge 33$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $521$ states, $265$ after trimming, and $S = 34$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 34$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A240636}{A240636}, revision 6, last modified Jul 23 2025.
