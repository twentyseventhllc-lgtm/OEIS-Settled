...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207455}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207455}{A207455} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 10*a(n-2) + 3*a(n-3) - 9*a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 8*x*(1 + 7*x + 3*x\textasciicircum{}2 - 9*x\textasciicircum{}3) / (1 - x - 10*x\textasciicircum{}2 - 3*x\textasciicircum{}3 + 9*x\textasciicircum{}4). - Colin Barker, Jun 23 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 13, last modified Sep 04 2022, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{8x + 56x^{2} + 24x^{3} - 72x^{4}}{1 - x - 10x^{2} - 3x^{3} + 9x^{4}}.\]
...
\begin{theorem} For A207455, the recurrence $a(n) = a(n-1) + 10\,a(n-2) + 3\,a(n-3) - 9\,a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A207455, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{8x + 56x^{2} + 24x^{3} - 72x^{4}}{1 - x - 10x^{2} - 3x^{3} + 9x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 64, 168, 760, 2560, 10088, 36456, 138176,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207455}{A207455}, revision 13, last modified Sep 04 2022.
