...
\title{Proofs of the conjectured column and row recurrences for OEIS A300881}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300881}{A300881} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 4 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -a(n-2) +a(n-3) +a(n-4) -2*a(n-5) -a(n-6)\\k=3: [order 25] for n>26\\k=4: [order 97] for n>98\end{quote}
...
\begin{theorem} For column $k = 1$ of A300881, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300881, the recurrence $a(n) = 3\,a(n-1) - a(n-2) + a(n-3) + a(n-4) - 2\,a(n-5) - a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300881, the recurrence $a(n) = 0$ holds for every $n > 26$, and fails at $n = 26$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 26$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 26$; after that point more than $S = 26$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 26$; the proved threshold is $n > 26$.

\begin{theorem} For column $k = 4$ of A300881, the recurrence $a(n) = 0$ holds for every $n > 98$, and fails at $n = 98$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $262$ after trimming, and $S = 98$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 98$; after that point more than $S = 98$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300881}{A300881}, revision 4, last modified Mar 14 2018.
