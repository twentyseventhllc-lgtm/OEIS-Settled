...
\title{Proofs of the conjectured column and row recurrences for OEIS A316801}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316801}{A316801} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 2, 3, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) +3*a(n-2) -4*a(n-3) -6*a(n-4) +4*a(n-5) for n>6\\k=3: [order 15] for n>18\\k=4: [order 65] for n>66\end{quote}
...
The condition is $c(i,j) \in \{2, 3, 5, 6, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} In A316801, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316801, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 4\,a(n-3) - 6\,a(n-4) + 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316801, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A316801, the recurrence $a(n) = 0$ holds for every $n > 66$, and fails at $n = 66$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $246$ after trimming, and $S = 69$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 66$; after that point more than $S = 69$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316801}{A316801}, revision 4, last modified Jul 14 2018.
