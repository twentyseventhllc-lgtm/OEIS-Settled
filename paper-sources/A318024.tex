...
\title{Proofs of the conjectured column and row recurrences for OEIS A318024}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318024}{A318024} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +3*a(n-2) -2*a(n-3) -2*a(n-4) -a(n-6) +a(n-7)\\k=3: [order 18] for n>23\\k=4: [order 70] for n>74\end{quote}
...
\begin{theorem} For column $k = 1$ of A318024, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318024, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 2\,a(n-3) - 2\,a(n-4) - a(n-6) + a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $20$ states, $17$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A318024, the recurrence $a(n) = 0$ holds for every $n > 23$, and fails at $n = 23$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $65$ after trimming, and $S = 26$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 23$; after that point more than $S = 26$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 23$; the proved threshold is $n > 23$.

\begin{theorem} For column $k = 4$ of A318024, the recurrence $a(n) = 0$ holds for every $n > 74$, and fails at $n = 74$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $245$ after trimming, and $S = 82$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 74$; after that point more than $S = 82$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318024}{A318024}, revision 4, last modified Aug 12 2018.
