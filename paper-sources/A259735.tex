...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A259735}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A259735}{A259735} as

\begin{quote}\itshape Number of (n+2) X (1+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00010101 or 01010101\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-2) + 3*a(n-4) + 2*a(n-6) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(20 + 22*x + 22*x\textasciicircum{}2 + 40*x\textasciicircum{}3 - x\textasciicircum{}4 + 24*x\textasciicircum{}5 - 2*x\textasciicircum{}6) / (1 - x\textasciicircum{}2 - 3*x\textasciicircum{}4 - 2*x\textasciicircum{}6). - Colin Barker, Dec 26 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{20x + 22x^{2} + 22x^{3} + 40x^{4} - x^{5} + 24x^{6} - 2x^{7}}{1 - x^{2} - 3x^{4} - 2x^{6}}.\]
...
\begin{theorem} For A259735, the recurrence $a(n) = a(n-2) + 3\,a(n-4) + 2\,a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,59$. The last nonvanishing one is at $m = 1$, that is at $n = 7$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A259735, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{20x + 22x^{2} + 22x^{3} + 40x^{4} - x^{5} + 24x^{6} - 2x^{7}}{1 - x^{2} - 3x^{4} - 2x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 20, 22, 42, 62, 101, 152, 265, 382,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A259735}{A259735}, revision 8, last modified Dec 26 2018.
