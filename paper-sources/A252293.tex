...
\title{Proofs of the conjectured column and row recurrences for OEIS A252293}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252293}{A252293} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 0 1 or 4\end{quote}
...
k=1: [linear recurrence of order 89] for n>91\end{quote}
...
\begin{theorem} For column $k = 1$ of A252293, the recurrence $a(n) = 0$ holds for every $n > 91$, and fails at $n = 91$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $756$ states, $756$ after trimming, and $S = 101$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 91$; after that point more than $S = 101$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252293}{A252293}, revision 6, last modified Jun 02 2025.
