...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A229749}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A229749}{A229749} as

\begin{quote}\itshape Number of defective 4-colorings of an n X 2 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..3 order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 6*a(n-1) - 12*a(n-2) + 8*a(n-3) for n>5.\end{quote}

\begin{quote}\ttfamily G.f.: x\textasciicircum{}2*(3 + 42*x + 98*x\textasciicircum{}2 + 4*x\textasciicircum{}3) / (1 - 2*x)\textasciicircum{}3.\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{3x^{2} + 42x^{3} + 98x^{4} + 4x^{5}}{1 - 6x + 12x^{2} - 8x^{3}}.\]
...
The reachable part of the digraph has $87$ states, $58$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 8$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of that $8\times8$ matrix.
...
\begin{theorem} For A229749, the recurrence $a(n) = 6\,a(n-1) - 12\,a(n-2) + 8\,a(n-3)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,42$. The last nonvanishing one is at $m = 2$, that is at $n = 5$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 5$. The theorem is at least as strong.

\begin{theorem} For A229749, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{3x^{2} + 42x^{3} + 98x^{4} + 4x^{5}}{1 - 6x + 12x^{2} - 8x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 3, 60, 422, 1840, 6456, 20032, 57440,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A229749}{A229749}, revision 7, last modified Sep 21 2018.
