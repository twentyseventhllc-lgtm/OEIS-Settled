...
\title{Proofs of the conjectured column and row recurrences for OEIS A229685}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A229685}{A229685} as

\begin{quote}\itshape T(n,k) = number of defective 3-colorings of an n X k 0..2 array connected diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order\end{quote}
...
k=1: a(n) = a(n-1).\\k=2: a(n) = 12*a(n-1) - 48*a(n-2) + 64*a(n-3) for n > 5.\\k=3: a(n) = 18*a(n-1) - 108*a(n-2) + 216*a(n-3) for n > 4.\\k=4: a(n) = 27*a(n-1) - 243*a(n-2) + 729*a(n-3) for n > 6.\end{quote}
...
\begin{theorem} In A229685, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A229685, the recurrence $a(n) = 12\,a(n-1) - 48\,a(n-2) + 64\,a(n-3)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $54$ states, $41$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 5$; the proved threshold is $n > 5$.

\begin{theorem} For column $k = 3$ of A229685, the recurrence $a(n) = 18\,a(n-1) - 108\,a(n-2) + 216\,a(n-3)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $139$ states, $104$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 4$; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 4$; the proved threshold is $n > 4$.

\begin{theorem} For column $k = 4$ of A229685, the recurrence $a(n) = 27\,a(n-1) - 243\,a(n-2) + 729\,a(n-3)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $376$ states, $279$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A229685}{A229685}, revision 9, last modified Apr 28 2021.
