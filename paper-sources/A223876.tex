...
\title{Proofs of the conjectured column and row recurrences for OEIS A223876}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A223876}{A223876} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays with rows, diagonals and antidiagonals unimodal\end{quote}
...
k=1: a(n) = 4*a(n-1)\\k=2: a(n) = 16*a(n-1)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 28 2013). As of revision 8, last modified Oct 03 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A223876, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A223876, the recurrence $a(n) = 16\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $50$ states, $50$ after trimming, and $S = 2$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 2$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A223876}{A223876}, revision 8, last modified Oct 03 2025.
