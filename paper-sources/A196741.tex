...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196741}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196741}{A196741} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,3,1,0,4 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +5*a(n-2) +12*a(n-3) +2*a(n-4) -11*a(n-5) +4*a(n-6) -5*a(n-7) +a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x + 9*x\textasciicircum{}2 + 4*x\textasciicircum{}3 + 2*x\textasciicircum{}4 - 10*x\textasciicircum{}5 - 2*x\textasciicircum{}6 + 2*x\textasciicircum{}7) / (1 - 2*x - 5*x\textasciicircum{}2 - 12*x\textasciicircum{}3 - 2*x\textasciicircum{}4 + 11*x\textasciicircum{}5 - 4*x\textasciicircum{}6 + 5*x\textasciicircum{}7 - x\textasciicircum{}8). - Colin Barker, May 09 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified May 09 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 2\,a(n-1) + 5\,a(n-2) + 12\,a(n-3) + 2\,a(n-4) - 11\,a(n-5) + 4\,a(n-6) - \\ 5\,a(n-7) + a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + x^{2} + 9x^{3} + 4x^{4} + 2x^{5} - 10x^{6} - 2x^{7} + 2x^{8}}{1 - 2x - 5x^{2} - 12x^{3} - 2x^{4} + 11x^{5} - 4x^{6} + 5x^{7} - x^{8}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 2, \tau(1) = 3, \tau(2) = 1, \tau(3) = 0, \tau(4) = 4. The condition is that for every cell,
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $40$; neither removal changes any count.
...
For scale: at $n = 8$ there are $5^{24}$ arrays, about $10^{17}$, against $8$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A196741, the recurrence $a(n) = 2\,a(n-1) + 5\,a(n-2) + 12\,a(n-3) + 2\,a(n-4) - 11\,a(n-5) + 4\,a(n-6) - 5\,a(n-7) + a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196741, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + x^{2} + 9x^{3} + 4x^{4} + 2x^{5} - 10x^{6} - 2x^{7} + 2x^{8}}{1 - 2x - 5x^{2} - 12x^{3} - 2x^{4} + 11x^{5} - 4x^{6} + 5x^{7} - x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 20, 71, 282, 1144, 4559, 18153,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196741}{A196741}, revision 8, last modified May 09 2018.
