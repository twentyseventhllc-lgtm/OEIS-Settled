...
\title{Proofs of the conjectured column and row recurrences for OEIS A316518}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316518}{A316518} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +a(n-2) -4*a(n-3) -4*a(n-4) for n>5\\k=3: a(n) = a(n-1) +2*a(n-2) +3*a(n-3) +a(n-4) -a(n-5) for n>8\\k=4: [order 10] for n>13\\k=5: [order 25] for n>28\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 05 2018). As of revision 4, last modified Jul 05 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A316518, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316518, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 4\,a(n-3) - 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316518, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + 3\,a(n-3) + a(n-4) - a(n-5)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A316518, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}
...
\begin{theorem} For column $k = 5$ of A316518, the recurrence $a(n) = 0$ holds for every $n > 28$, and fails at $n = 28$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $360$ after trimming, and $S = 39$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 28$; after that point more than $S = 39$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316518}{A316518}, revision 4, last modified Jul 05 2018.
