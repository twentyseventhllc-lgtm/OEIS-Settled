...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296968}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296968}{A296968} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 2, 4 or 5 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) - a(n-2) + 6*a(n-3) - 4*a(n-4) + 2*a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 3*x + 2*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + 2*x\textasciicircum{}4) / (1 - 2*x + x\textasciicircum{}2 - 6*x\textasciicircum{}3 + 4*x\textasciicircum{}4 - 2*x\textasciicircum{}5). - Colin Barker, Feb 25 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 3x^{2} + 2x^{3} - 2x^{4} + 2x^{5}}{1 - 2x + x^{2} - 6x^{3} + 4x^{4} - 2x^{5}}.\]
...
\begin{theorem} For A296968, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 6\,a(n-3) - 4\,a(n-4) + 2\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296968, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 3x^{2} + 2x^{3} - 2x^{4} + 2x^{5}}{1 - 2x + x^{2} - 6x^{3} + 4x^{4} - 2x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 5, 11, 21, 59, 145, 323, 793,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296968}{A296968}, revision 7, last modified Feb 25 2019.
