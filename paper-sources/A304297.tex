...
\title{A proof of the conjectured linear recurrence for OEIS A304297}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304297}{A304297} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 1, 2, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-2) +3*a(n-3) +2*a(n-4) -4*a(n-6) -2*a(n-7) -a(n-8) -2*a(n-9)\end{quote}
...
\begin{multline*} a(n) = a(n-2) + 3\,a(n-3) + 2\,a(n-4) - 4\,a(n-6) - 2\,a(n-7) - a(n-8) - \\ 2\,a(n-9). \end{multline*}
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $19$; neither removal changes any count.
...
\begin{theorem} For A304297, the recurrence $a(n) = a(n-2) + 3\,a(n-3) + 2\,a(n-4) - 4\,a(n-6) - 2\,a(n-7) - a(n-8) - 2\,a(n-9)$ holds for every $n \ge 10$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 38 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 2, 3, 5, 6, 14, 22, 33,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304297}{A304297}, revision 4, last modified May 10 2018.
