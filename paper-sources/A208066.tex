...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208066}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208066}{A208066} as

\begin{quote}\itshape Number of n X 5 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 0 1 1 vertically\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: 16*x*(1 + 14*x + 10*x\textasciicircum{}2 + 7*x\textasciicircum{}3 - 3*x\textasciicircum{}4 - 5*x\textasciicircum{}5 + 2*x\textasciicircum{}6 + 2*x\textasciicircum{}7 - x\textasciicircum{}8) / ((1 - x)\textasciicircum{}5*(1 + x)\textasciicircum{}3). - Colin Barker, Jun 27 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + 224x^{2} + 160x^{3} + 112x^{4} - 48x^{5} - 80x^{6} + 32x^{7} + 32x^{8} - 16x^{9}}{1 - 2x - 2x^{2} + 6x^{3} - 6x^{5} + 2x^{6} + 2x^{7} - x^{8}}.\]
...
That refinement leaves $42$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 29$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A208066, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) - 6\,a(n-3) + 6\,a(n-5) - 2\,a(n-6) - 2\,a(n-7) + a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,75$. The last nonvanishing one is at $m = 1$, that is at $n = 9$. After it, more than $S = 29$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A208066, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + 224x^{2} + 160x^{3} + 112x^{4} - 48x^{5} - 80x^{6} + 32x^{7} + 32x^{8} - 16x^{9}}{1 - 2x - 2x^{2} + 6x^{3} - 6x^{5} + 2x^{6} + 2x^{7} - x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 256, 704, 1936, 3696, 7056, 11424, 18496,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208066}{A208066}, revision 8, last modified Jun 27 2018.
