...
\title{A proof of the conjectured linear recurrence for OEIS A304226}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304226}{A304226} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1, 2 or 7 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +5*a(n-3) -4*a(n-6) for n>8\end{quote}
...
\begin{theorem} For A304226, the recurrence $a(n) = a(n-1) + 5\,a(n-3) - 4\,a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,52$. The last nonvanishing one is at $m = 2$, that is at $n = 8$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 5, 6, 11, 20, 46, 97, 189,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304226}{A304226}, revision 4, last modified May 08 2018.
