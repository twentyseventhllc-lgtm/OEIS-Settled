...
\title{Proofs of the conjectured column and row recurrences for OEIS A252369}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252369}{A252369} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 1 3 4 6 or 7\end{quote}
...
The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 17 2014). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A252369, the recurrence $a(n) = a(n-1) + 3\,a(n-3) + 4\,a(n-4) + 4\,a(n-5) + 2\,a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 8$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252369}{A252369}, revision 6, last modified Jun 02 2025.
