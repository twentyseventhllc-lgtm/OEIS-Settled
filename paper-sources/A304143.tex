...
\title{A proof of the conjectured linear recurrence for OEIS A304143}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304143}{A304143} as

\begin{quote}\itshape Number of nX2 0..1 arrays with every element unequal to 1, 2, 3 or 6 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +3*a(n-2) -2*a(n-3) -6*a(n-4) -4*a(n-5) for n>6.\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (May 07 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 6, last modified May 08 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\[a(n) = 2\,a(n-1) + 3\,a(n-2) - 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5).\]
...
\begin{theorem} For A304143, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 7, 14, 33, 70, 157, 346, 769,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304143}{A304143}, revision 6, last modified May 08 2018.
