...
\title{A proof of the conjectured order of the recurrence for OEIS A224194}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A224194}{A224194} as

\begin{quote}\itshape Number of 7Xn 0..2 arrays with rows unimodal and columns nondecreasing\end{quote}
...
\begin{theorem} For A224194, the least degree of a polynomial that $a$ eventually agrees with is exactly $28$, and the agreement holds for every $n > 0$, the entry's own figure.\end{theorem}

{\itshape Proof.} A sequence agrees eventually with a polynomial of degree $d$ exactly when its $(d+1)$-st difference vanishes eventually. That difference is a fixed linear combination of $a(n),\dots,a(n+d+1)$, so it is C-finite of order at most $S = 112$ as well, and $S$ consecutive vanishing values force the rest. Taking differences of the tail gives the least such $d$, namely $28$; the residual test then gives vanishing throughout. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 18 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 36, 1296, 28176, 424326, 4844576, 44435746, 340780382, 2249643632,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A224194}{A224194}, revision 10, last modified Oct 03 2025.
