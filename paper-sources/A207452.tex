...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207452}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207452}{A207452} as

\begin{quote}\itshape Number of n X 7 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 168*n\textasciicircum{}4 - 84*n\textasciicircum{}3 - 84*n\textasciicircum{}2 + 42*n.\end{quote}

\begin{quote}\ttfamily G.f.: 42*x*(1 + 37*x + 55*x\textasciicircum{}2 + 3*x\textasciicircum{}3) / (1 - x)\textasciicircum{}5.\end{quote}
...
\[a(n) = 42n - 84n^{2} - 84n^{3} + 168n^{4}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{42x + 1554x^{2} + 2310x^{3} + 126x^{4}}{1 - 5x + 10x^{2} - 10x^{3} + 5x^{4} - x^{5}}.\]
...
That refinement leaves $12$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 9$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $2^{56}$ arrays, about $10^{17}$, against $9$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A207452, $a(n)$ is the polynomial $42n - 84n^{2} - 84n^{3} + 168n^{4}$ for every $n \ge 1$.\end{theorem}
...
\begin{theorem} For A207452, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{42x + 1554x^{2} + 2310x^{3} + 126x^{4}}{1 - 5x + 10x^{2} - 10x^{3} + 5x^{4} - x^{5}}$.\end{theorem}
...
\begin{theorem} For A207452, the recurrence $a(n) = 5\,a(n-1) - 10\,a(n-2) + 10\,a(n-3) - 5\,a(n-4) + a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 42, 1764, 10710, 36456, 92610, 196812, 370734, 640080,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207452}{A207452}, revision 11, last modified Jun 23 2018.
