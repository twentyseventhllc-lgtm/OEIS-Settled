...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A242544}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A242544}{A242544} as

\begin{quote}\itshape Number of length n+3 0..3 arrays with no four elements in a row with pattern aabb (possibly a=b) and new values 0..3 introduced in 0..3 order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) -3*a(n-2) +10*a(n-3) -2*a(n-4) -12*a(n-5) -9*a(n-6) -18*a(n-7) -9*a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical: G.f.: -x*(-13+10*x-21*x\textasciicircum{}2+50*x\textasciicircum{}3+88*x\textasciicircum{}4+78*x\textasciicircum{}5+99*x\textasciicircum{}6+42*x\textasciicircum{}7) / ( (x\textasciicircum{}2+x-1)*(3*x\textasciicircum{}2+3*x-1)*(3*x\textasciicircum{}2+1)*(x\textasciicircum{}2+1) ). - R. J. Mathar, May 18 2014\end{quote}
...
\begin{multline*} a(n) = 4\,a(n-1) - 3\,a(n-2) + 10\,a(n-3) - 2\,a(n-4) - 12\,a(n-5) - 9\,a(n-6) - \\ 18\,a(n-7) - 9\,a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{13x - 10x^{2} + 21x^{3} - 50x^{4} - 88x^{5} - 78x^{6} - 99x^{7} - 42x^{8}}{1 - 4x + 3x^{2} - 10x^{3} + 2x^{4} + 12x^{5} + 9x^{6} + 18x^{7} + 9x^{8}}.\]
...
The reachable part of that digraph has $104$ states, $104$ after trimming. Identifying states with the same number of completions of every length, and then the mirror identification on the edges coming in, leaves $S = 12$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of the $12\times12$ quotient matrix; that degree bound is computed here, not assumed.
...
\begin{theorem} For A242544, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2) + 10\,a(n-3) - 2\,a(n-4) - 12\,a(n-5) - 9\,a(n-6) - 18\,a(n-7) - 9\,a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A242544, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{13x - 10x^{2} + 21x^{3} - 50x^{4} - 88x^{5} - 78x^{6} - 99x^{7} - 42x^{8}}{1 - 4x + 3x^{2} - 10x^{3} + 2x^{4} + 12x^{5} + 9x^{6} + 18x^{7} + 9x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 13, 42, 150, 554, 2072, 7808, 29536, 111878,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A242544}{A242544}, revision 8, last modified Jul 23 2025.
