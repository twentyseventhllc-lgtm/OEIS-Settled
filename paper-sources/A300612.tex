...
\title{Proofs of the conjectured column and row recurrences for OEIS A300612}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300612}{A300612} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2, 3 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) -3*a(n-4) -2*a(n-5) -a(n-6) for n>7\\k=3: [order 17] for n>18\\k=4: [order 72] for n>73\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 09 2018). As of revision 4, last modified Mar 09 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A300612, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300612, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 3\,a(n-4) - 2\,a(n-5) - a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $20$ states, $18$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 7$; the proved threshold is $n > 7$.

\begin{theorem} For column $k = 3$ of A300612, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 22$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 22$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 18$; the proved threshold is $n > 18$.

\begin{theorem} For column $k = 4$ of A300612, the recurrence $a(n) = 0$ holds for every $n > 73$, and fails at $n = 73$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 75$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 73$; after that point more than $S = 75$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300612}{A300612}, revision 4, last modified Mar 09 2018.
