...
\title{Proofs of the conjectured column and row recurrences for OEIS A235007}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A235007}{A235007} as

\begin{quote}\itshape T(n,k) the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
k=1: [linear recurrence of order 12].\end{quote}
...
\begin{theorem} For column $k = 1$ of A235007, the recurrence $a(n) = 0$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $64$ states, $56$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A235007}{A235007}, revision 6, last modified Jun 20 2022.
