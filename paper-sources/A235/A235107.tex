...
\title{Proofs of the conjectured column and row recurrences for OEIS A235107}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A235107}{A235107} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 7*a(n-1) -2*a(n-2) -47*a(n-3) +3*a(n-4) +84*a(n-5) +36*a(n-6).\\k=2: [order 19].\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 03 2014). As of revision 6, last modified Jun 20 2022, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A235107, the recurrence $a(n) = 7\,a(n-1) - 2\,a(n-2) - 47\,a(n-3) + 3\,a(n-4) + 84\,a(n-5) + 36\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $49$ states, $42$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A235107, the recurrence $a(n) = 0$ holds for every $n \ge 20$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $343$ states, $252$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A235107}{A235107}, revision 6, last modified Jun 20 2022.
