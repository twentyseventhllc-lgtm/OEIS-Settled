...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A235011}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A235011}{A235011} as

\begin{quote}\itshape Number of (n+1) X (2+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 4*a(n-2) - 14*a(n-3) - 4*a(n-4) + 16*a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 8*x*(13 - 11*x - 75*x\textasciicircum{}2 + 26*x\textasciicircum{}3 + 104*x\textasciicircum{}4) / ((1 - 2*x)*(1 - 2*x\textasciicircum{}2)*(1 - x - 4*x\textasciicircum{}2)). - Colin Barker, Oct 16 2018\end{quote}
...
\[a(n) = 3\,a(n-1) + 4\,a(n-2) - 14\,a(n-3) - 4\,a(n-4) + 16\,a(n-5).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{104x - 88x^{2} - 600x^{3} + 208x^{4} + 832x^{5}}{1 - 3x - 4x^{2} + 14x^{3} + 4x^{4} - 16x^{5}}.\]
...
For scale: at $n = 8$ there are $5^{24}$ arrays, about $10^{17}$, against $6$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A235011, the recurrence $a(n) = 3\,a(n-1) + 4\,a(n-2) - 14\,a(n-3) - 4\,a(n-4) + 16\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A235011, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{104x - 88x^{2} - 600x^{3} + 208x^{4} + 832x^{5}}{1 - 3x - 4x^{2} + 14x^{3} + 4x^{4} - 16x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 104, 224, 488, 1112, 2568, 6088, 14600, 35560,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A235011}{A235011}, revision 9, last modified Jun 19 2022.
