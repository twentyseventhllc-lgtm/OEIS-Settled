...
\title{Proofs of the conjectured column and row recurrences for OEIS A208379}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208379}{A208379} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}
...
k=1: a(n) = 2*n\\k=2: a(n) = 4*n\textasciicircum{}2\\k=3: a(n) = 36*n - 36 for n>1\\k=4: a(n) = 20*n\textasciicircum{}2 + 40*n - 60 for n>1\\k=5: a(n) = 96*n\textasciicircum{}2 - 32*n - 64 for n>1\end{quote}
...
\begin{theorem} For column $k = 1$ of A208379, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A208379, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A208379, the recurrence $a(n) = 0$ holds for every $n > 1$, and fails at $n = 1$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $42$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 1$; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 1$; the proved threshold is $n > 1$.

\begin{theorem} For column $k = 4$ of A208379, the recurrence $a(n) = 0$ holds for every $n > 1$, and fails at $n = 1$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $110$ after trimming, and $S = 5$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 1$; after that point more than $S = 5$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 1$; the proved threshold is $n > 1$.

\begin{theorem} For column $k = 5$ of A208379, the recurrence $a(n) = 0$ holds for every $n > 1$, and fails at $n = 1$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $272$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 1$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208379}{A208379}, revision 9, last modified Oct 03 2025.
