...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207457}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207457}{A207457} as

\begin{quote}\itshape Number of 6 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 16*a(n-2) + 5*a(n-3) - 25*a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 12*x*(1 + 11*x + 5*x\textasciicircum{}2 - 25*x\textasciicircum{}3) / (1 - x - 16*x\textasciicircum{}2 - 5*x\textasciicircum{}3 + 25*x\textasciicircum{}4). - Colin Barker, Jun 23 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 17 2012).

The line quoted ending in that signature is contributed by Colin Barker on Jun 23 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 11, last modified Jun 23 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{12x + 132x^{2} + 60x^{3} - 300x^{4}}{1 - x - 16x^{2} - 5x^{3} + 25x^{4}}.\]
...
\begin{theorem} For A207457, the recurrence $a(n) = a(n-1) + 16\,a(n-2) + 5\,a(n-3) - 25\,a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A207457, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{12x + 132x^{2} + 60x^{3} - 300x^{4}}{1 - x - 16x^{2} - 5x^{3} + 25x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 12, 144, 396, 2460, 9216, 46956, 196812, 932688,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207457}{A207457}, revision 11, last modified Jun 23 2018.
