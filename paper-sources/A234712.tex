...
\title{Proofs of the conjectured column and row recurrences for OEIS A234712}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234712}{A234712} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 6 (constant-stress 1 X 1 tilings)\end{quote}
...
\begin{quote}\ttfamily Empirical for column k (the k=3..4 recurrence works also for k=1..2; apparently all rows and columns satisfy the same order 36 recurrence):\\
k=1: [linear recurrence of order 7].\\k=2: [order 31].\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 29 2013). As of revision 9, last modified Jun 20 2022, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A234712, the recurrence $a(n) = 0$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $64$ states, $64$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A234712, the recurrence $a(n) = 0$ holds for every $n \ge 32$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $512$ states, $512$ after trimming, and $S = 35$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 35$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234712}{A234712}, revision 9, last modified Jun 20 2022.
