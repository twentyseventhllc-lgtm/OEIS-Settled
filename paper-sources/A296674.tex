...
\title{Proofs of the conjectured column and row recurrences for OEIS A296674}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296674}{A296674} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 adjacent to 0, 2 or 3 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +5*a(n-3) -10*a(n-4) -12*a(n-5) -4*a(n-6)\\k=3: [order 12]\\k=4: [order 27]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 18 2017). As of revision 8, last modified Jun 19 2020, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A296674, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A296674, the recurrence $a(n) = 3\,a(n-1) + 5\,a(n-3) - 10\,a(n-4) - 12\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A296674, the recurrence $a(n) = 0$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $61$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A296674, the recurrence $a(n) = 0$ holds for every $n \ge 28$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $217$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296674}{A296674}, revision 8, last modified Jun 19 2020.
