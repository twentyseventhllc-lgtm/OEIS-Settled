...
\title{Proofs of the conjectured column and row recurrences for OEIS A235078}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A235078}{A235078} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 61*a(n-2) -813*a(n-4) +4271*a(n-6) -9694*a(n-8) +8264*a(n-10) -1536*a(n-12).\\k=2: [order 73].\end{quote}
...
The entry publishes its table by upward antidiagonals, so every published term is a value $T(n,k)$ with both indices known. All 28 of them are used as the data gate below: the models are required to reproduce the whole published triangle, not merely the column or row a given line is about.
...
\begin{theorem} For column $k = 1$ of A235078, the recurrence $a(n) = 61\,a(n-2) - 813\,a(n-4) + 4271\,a(n-6) - 9694\,a(n-8) + 8264\,a(n-10) - 1536\,a(n-12)$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $64$ states, $56$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A235078, the recurrence $a(n) = 0$ holds for every $n \ge 74$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $512$ states, $392$ after trimming, and $S = 90$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 90$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item {\bfseries The published table.} Every one of the 28 values $T(n,k)$ the entry publishes was recomputed from the models and agrees exactly, in integer arithmetic, with the indices read off the entry's offset (1) and its stated shape.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A235078}{A235078}, revision 6, last modified Jun 20 2022.
