...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A188826}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A188826}{A188826} as

\begin{quote}\itshape Number of 4 X n binary arrays without the pattern 0 1 diagonally or antidiagonally\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 5*a(n-2) - 15*a(n-3) + 3*a(n-4) + 5*a(n-5) - a(n-6) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(16 - 23*x - 74*x\textasciicircum{}2 + 128*x\textasciicircum{}3 - 5*x\textasciicircum{}4 - 39*x\textasciicircum{}5 + 7*x\textasciicircum{}6) / ((1 - 5*x + 6*x\textasciicircum{}2 - x\textasciicircum{}3)*(1 + 2*x - x\textasciicircum{}2 - x\textasciicircum{}3)). - Colin Barker, Apr 30 2018\end{quote}
...
\[a(n) = 3\,a(n-1) + 5\,a(n-2) - 15\,a(n-3) + 3\,a(n-4) + 5\,a(n-5) - a(n-6).\]

stated for $n > 7$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x - 23x^{2} - 74x^{3} + 128x^{4} - 5x^{5} - 39x^{6} + 7x^{7}}{1 - 3x - 5x^{2} + 15x^{3} - 3x^{4} - 5x^{5} + x^{6}}.\]
...
The reachable part of the digraph has $17$ states, $17$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 8$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of that $8\times8$ matrix. The bound is computed, not assumed.
...
\begin{theorem} For A188826, the recurrence $a(n) = 3\,a(n-1) + 5\,a(n-2) - 15\,a(n-3) + 3\,a(n-4) + 5\,a(n-5) - a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,44$. The last nonvanishing one is at $m = 1$, that is at $n = 7$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A188826, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x - 23x^{2} - 74x^{3} + 128x^{4} - 5x^{5} - 39x^{6} + 7x^{7}}{1 - 3x - 5x^{2} + 15x^{3} - 3x^{4} - 5x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 25, 81, 256, 841, 2704, 8836, 28561,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A188826}{A188826}, revision 9, last modified Apr 30 2018.
