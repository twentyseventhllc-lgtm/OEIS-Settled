...
\title{Proofs of the conjectured column and row recurrences for OEIS A263672}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A263672}{A263672} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..6 arrays with each row and column divisible by 11, read as a base-7 number with top and left being the most significant digits\end{quote}
...
k=1: a(n) = 5*a(n-1) +67*a(n-10) -335*a(n-11)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 23 2015). As of revision 4, last modified Oct 23 2015, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A263672, the recurrence $a(n) = 5\,a(n-1) + 67\,a(n-10) - 335\,a(n-11)$ holds for every $n \ge 12$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $11$ states, $11$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A263672}{A263672}, revision 4, last modified Oct 23 2015.
