...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A261259}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A261259}{A261259} as

\begin{quote}\itshape Number of (n+2) X (2+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00010101 00100101 or 01010101\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-3) + a(n-4) + 2*a(n-5) for n>6.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(41 + 11*x + 58*x\textasciicircum{}2 + 40*x\textasciicircum{}3 + 41*x\textasciicircum{}4 - 14*x\textasciicircum{}5) / (1 - x - 2*x\textasciicircum{}3 - x\textasciicircum{}4 - 2*x\textasciicircum{}5). - Colin Barker, Dec 30 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{41x + 11x^{2} + 58x^{3} + 40x^{4} + 41x^{5} - 14x^{6}}{1 - x - 2x^{3} - x^{4} - 2x^{5}}.\]
...
That refinement leaves $13$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 10$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A261259, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + a(n-4) + 2\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,51$. The last nonvanishing one is at $m = 1$, that is at $n = 6$. After it, more than $S = 10$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 6$. The theorem is at least as strong.

\begin{theorem} For A261259, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{41x + 11x^{2} + 58x^{3} + 40x^{4} + 41x^{5} - 14x^{6}}{1 - x - 2x^{3} - x^{4} - 2x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 41, 52, 110, 232, 418, 758, 1436, 2724,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A261259}{A261259}, revision 8, last modified Dec 30 2018.
