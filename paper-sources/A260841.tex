...
\title{Proofs of the conjectured column and row recurrences for OEIS A260841}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260841}{A260841} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000001 00000011 or 00010101\end{quote}
...
k=1: [linear recurrence of order 14] for n>15\\k=2: [order 18] for n>19\\k=3: [order 22] for n>24\\k=4: [order 33] for n>34\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 01 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
The condition is that $w(i,j)$ is a cyclic rotation of one of \texttt{00000001}, \texttt{00000011}, \texttt{00010101}.
...
\begin{theorem} For column $k = 1$ of A260841, the recurrence $a(n) = 0$ holds for every $n > 15$, and fails at $n = 15$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 15$; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 15$; the proved threshold is $n > 15$.

\begin{theorem} For column $k = 2$ of A260841, the recurrence $a(n) = 0$ holds for every $n > 19$, and fails at $n = 19$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 19$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 19$; the proved threshold is $n > 19$.

\begin{theorem} For column $k = 3$ of A260841, the recurrence $a(n) = 0$ holds for every $n > 24$, and fails at $n = 24$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 31$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 24$; after that point more than $S = 31$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 24$; the proved threshold is $n > 24$.

\begin{theorem} For column $k = 4$ of A260841, the recurrence $a(n) = 0$ holds for every $n > 34$, and fails at $n = 34$.\end{theorem}

{\itshape Proof.} The width is $6$; the digraph has $4160$ states, $4160$ after trimming, and $S = 49$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 34$; after that point more than $S = 49$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260841}{A260841}, revision 6, last modified Jun 02 2025.
