...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207737}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207737}{A207737} as

\begin{quote}\itshape Number of n X 4 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 1 and 1 0 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 8*a(n-1) -11*a(n-2) -12*a(n-3) +13*a(n-4) +4*a(n-5) -a(n-6) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(9 + 9*x - 188*x\textasciicircum{}2 - 40*x\textasciicircum{}3 + 235*x\textasciicircum{}4 + 55*x\textasciicircum{}5 - 16*x\textasciicircum{}6) / ((1 - x)*(1 + x)*(1 - 6*x + x\textasciicircum{}2)*(1 - 2*x - x\textasciicircum{}2)). - Colin Barker, Jun 25 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 9x^{2} - 188x^{3} - 40x^{4} + 235x^{5} + 55x^{6} - 16x^{7}}{1 - 8x + 11x^{2} + 12x^{3} - 13x^{4} - 4x^{5} + x^{6}}.\]
...
\begin{theorem} For A207737, the recurrence $a(n) = 8\,a(n-1) - 11\,a(n-2) - 12\,a(n-3) + 13\,a(n-4) + 4\,a(n-5) - a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,39$. The last nonvanishing one is at $m = 1$, that is at $n = 7$. After it, more than $S = 7$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A207737, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 9x^{2} - 188x^{3} - 40x^{4} + 235x^{5} + 55x^{6} - 16x^{7}}{1 - 8x + 11x^{2} + 12x^{3} - 13x^{4} - 4x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 20 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 361, 1849, 10201, 58081, 335241, 1946025,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207737}{A207737}, revision 8, last modified Jun 25 2018.
