...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207936}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207936}{A207936} as

\begin{quote}\itshape Number of n X 6 0..1 arrays avoiding 0 0 1 and 1 0 0 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7).\end{quote}

\begin{quote}\ttfamily G.f.: 2*x*(11 + 165*x - 110*x\textasciicircum{}2 - 59*x\textasciicircum{}3 + 68*x\textasciicircum{}4 - 15*x\textasciicircum{}5 + x\textasciicircum{}6) / (1 - x)\textasciicircum{}7.\end{quote}

\begin{quote}\ttfamily a(n) = (720 - 1584*n - 6206*n\textasciicircum{}2 + 7335*n\textasciicircum{}3 + 6505*n\textasciicircum{}4 + 1089*n\textasciicircum{}5 + 61*n\textasciicircum{}6) / 360.\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{22x + 330x^{2} - 220x^{3} - 118x^{4} + 136x^{5} - 30x^{6} + 2x^{7}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}.\]

Written out, the claim is

\[a(n) = 2 - \frac{22}{5}n - \frac{3103}{180}n^{2} + \frac{163}{8}n^{3} + \frac{1301}{72}n^{4} + \frac{121}{40}n^{5} + \frac{61}{360}n^{6}.\]
...
That refinement leaves $37$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 24$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,

\[a(n) \;=\; \tilde w^{\mathsf T} L^{\,n-1} \tilde f \qquad (n \ge 1),\]

and $a$ satisfies the linear recurrence given by the characteristic polynomial of $L$, of degree $24$. This is the only bound used below, and it is computed, not assumed.

For scale: at $n = 8$ there are $2^{48}$ arrays, about $10^{14}$, against $24$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A207936, the recurrence $a(n) = 7\,a(n-1) - 21\,a(n-2) + 35\,a(n-3) - 35\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,63$. Every one of them vanished. After it, more than $S = 24$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A207936, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{22x + 330x^{2} - 220x^{3} - 118x^{4} + 136x^{5} - 30x^{6} + 2x^{7}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}$.\end{theorem}
...
\begin{theorem} For A207936, $a(n)$ is the polynomial $2 - \frac{22}{5}n - \frac{3103}{180}n^{2} + \frac{163}{8}n^{3} + \frac{1301}{72}n^{4} + \frac{121}{40}n^{5} + \frac{61}{360}n^{6}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $6$ is annihilated by the difference operator $(E-1)^{7}$, whose characteristic polynomial is $(t-1)^{7}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 7 = 31$. Its values were computed exactly; all of them vanish, and more than $31$ consecutive vanishing values follow, so every later one vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 22, 484, 2706, 9430, 25490, 58602, 120276, 226850,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207936}{A207936}, revision 8, last modified Jun 26 2018.
