...
\title{Proofs of the conjectured column and row recurrences for OEIS A207024}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207024}{A207024} as

\begin{quote}\itshape T(n,k) = Number of n X k 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}
...
k=1: a(n) = 2*n\\k=2: a(n) = 4*n\textasciicircum{}2\\k=3: a(n) = 12*n\textasciicircum{}2 - 6*n\\k=4: a(n) = 6*n\textasciicircum{}3 + (27/2)*n\textasciicircum{}2 - (21/2)*n\\k=5: a(n) = (13/6)*n\textasciicircum{}4 + 13*n\textasciicircum{}3 + (52/3)*n\textasciicircum{}2 - (39/2)*n\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 14 2012). As of revision 7, last modified Mar 26 2023, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A207024, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A207024, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A207024, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A207024, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $90$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 5$ of A207024, the recurrence $a(n) = 0$ holds for every $n \ge 1$, at every index at which it can be stated.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207024}{A207024}, revision 7, last modified Mar 26 2023.
