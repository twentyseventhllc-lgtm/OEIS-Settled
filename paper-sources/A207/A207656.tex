...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207656}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207656}{A207656} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 1 0 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) - a(n-2) - a(n-3) for n>4.\end{quote}

\begin{quote}\ttfamily G.f.: 6*x*(1 + 3*x - 3*x\textasciicircum{}2 - 2*x\textasciicircum{}3) / ((1 - x)*(1 - 2*x - x\textasciicircum{}2)).\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 18x^{2} - 18x^{3} - 12x^{4}}{1 - 3x + x^{2} + x^{3}}.\]
...
\begin{theorem} For A207656, the recurrence $a(n) = 3\,a(n-1) - a(n-2) - a(n-3)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,42$. The last nonvanishing one is at $m = 1$, that is at $n = 4$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 4$. The theorem is at least as strong.

\begin{theorem} For A207656, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 18x^{2} - 18x^{3} - 12x^{4}}{1 - 3x + x^{2} + x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 84, 198, 474, 1140, 2748, 6630,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207656}{A207656}, revision 9, last modified Jun 25 2018.
