...
\title{Proofs of the conjectured column and row recurrences for OEIS A263439}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A263439}{A263439} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..4 arrays with each row and column divisible by 13, read as a base-5 number with top and left being the most significant digits\end{quote}
...
k=1: a(n) = 2*a(n-1) +9*a(n-4) -18*a(n-5) -14*a(n-8) +28*a(n-9) +a(n-12) -2*a(n-13)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 18 2015). As of revision 4, last modified Oct 18 2015, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A263439, the recurrence $a(n) = 2\,a(n-1) + 9\,a(n-4) - 18\,a(n-5) - 14\,a(n-8) + 28\,a(n-9) + a(n-12) - 2\,a(n-13)$ holds for every $n \ge 14$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $13$ states, $13$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A263439}{A263439}, revision 4, last modified Oct 18 2015.
