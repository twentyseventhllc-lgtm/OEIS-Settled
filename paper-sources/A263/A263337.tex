...
\title{Proofs of the conjectured column and row recurrences for OEIS A263337}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A263337}{A263337} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with each row and column divisible by 13, read as a base-3 number with top and left being the most significant digits\end{quote}
...
k=1: a(n) = 3*a(n-1) +a(n-3) -3*a(n-4)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 15 2015). As of revision 3, last modified Oct 15 2015, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A263337, the recurrence $a(n) = 3\,a(n-1) + a(n-3) - 3\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $13$ states, $13$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A263337}{A263337}, revision 3, last modified Oct 15 2015.
