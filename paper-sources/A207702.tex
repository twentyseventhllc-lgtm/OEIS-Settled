...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207702}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207702}{A207702} as

\begin{quote}\itshape Number of n X 7 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}
...
\begin{quote}\ttfamily G.f.: 2*x*(20 + 640*x + 775*x\textasciicircum{}2 - 1013*x\textasciicircum{}3 + 259*x\textasciicircum{}4 + 31*x\textasciicircum{}5 - 12*x\textasciicircum{}6) / (1 - x)\textasciicircum{}8.\end{quote}

\begin{quote}\ttfamily a(n) = (n*(162 - 327*n - 881*n\textasciicircum{}2 + 1296*n\textasciicircum{}3 + 997*n\textasciicircum{}4 + 183*n\textasciicircum{}5 + 10*n\textasciicircum{}6)) / 36.\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{40x + 1280x^{2} + 1550x^{3} - 2026x^{4} + 518x^{5} + 62x^{6} - 24x^{7}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}.\]

Written out, the claim is

\[a(n) = \frac{9}{2}n - \frac{109}{12}n^{2} - \frac{881}{36}n^{3} + 36n^{4} + \frac{997}{36}n^{5} + \frac{61}{12}n^{6} + \frac{5}{18}n^{7}.\]
...
That refinement leaves $48$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 31$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
For scale: at $n = 8$ there are $2^{56}$ arrays, about $10^{17}$, against $31$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A207702, the recurrence $a(n) = 8\,a(n-1) - 28\,a(n-2) + 56\,a(n-3) - 70\,a(n-4) + 56\,a(n-5) - 28\,a(n-6) + 8\,a(n-7) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,68$. Every one of them vanished. After it, more than $S = 31$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A207702, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{40x + 1280x^{2} + 1550x^{3} - 2026x^{4} + 518x^{5} + 62x^{6} - 24x^{7}}{1 - 8x + 28x^{2} - 56x^{3} + 70x^{4} - 56x^{5} + 28x^{6} - 8x^{7} + x^{8}}$.\end{theorem}
...
\begin{theorem} For A207702, $a(n)$ is the polynomial $\frac{9}{2}n - \frac{109}{12}n^{2} - \frac{881}{36}n^{3} + 36n^{4} + \frac{997}{36}n^{5} + \frac{61}{12}n^{6} + \frac{5}{18}n^{7}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $7$ is annihilated by the difference operator $(E-1)^{8}$, whose characteristic polynomial is $(t-1)^{8}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 8 = 39$. Its values were computed exactly; all of them vanish, and more than $39$ consecutive vanishing values follow, so every later one vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 40, 1600, 13230, 61254, 206910, 571350, 1369900, 2956980,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207702}{A207702}, revision 8, last modified Jun 25 2018.
