...
\title{A proof of the conjectured linear recurrence for OEIS A316422}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316422}{A316422} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element unequal to 0, 1, 3, 5, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +5*a(n-3) +2*a(n-5) -2*a(n-6) -a(n-7) -2*a(n-8) -3*a(n-9) -2*a(n-11) -a(n-12).\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Jul 02 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 6, last modified Jul 16 2025, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = a(n-1) + 5\,a(n-3) + 2\,a(n-5) - 2\,a(n-6) - a(n-7) - 2\,a(n-8) - \\ 3\,a(n-9) - 2\,a(n-11) - a(n-12). \end{multline*}
...
\begin{theorem} For A316422, the recurrence $a(n) = a(n-1) + 5\,a(n-3) + 2\,a(n-5) - 2\,a(n-6) - a(n-7) - 2\,a(n-8) - 3\,a(n-9) - 2\,a(n-11) - a(n-12)$ holds for every $n \ge 13$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,60$. Every one of them vanished. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 9, 14, 32, 75, 148, 311, 687,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316422}{A316422}, revision 6, last modified Jul 16 2025.
