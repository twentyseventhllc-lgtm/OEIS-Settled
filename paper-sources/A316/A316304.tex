...
\title{Proofs of the conjectured column and row recurrences for OEIS A316304}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316304}{A316304} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 4, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +8*a(n-3) -8*a(n-4) -8*a(n-5) for n>6\\k=3: [order 12] for n>13\\k=4: [order 65] for n>67\end{quote}
...
\begin{theorem} For column $k = 1$ of A316304, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316304, the recurrence $a(n) = 2\,a(n-1) + 8\,a(n-3) - 8\,a(n-4) - 8\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316304, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $54$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 4$ of A316304, the recurrence $a(n) = 0$ holds for every $n > 67$, and fails at $n = 67$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $262$ after trimming, and $S = 70$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 67$; after that point more than $S = 70$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316304}{A316304}, revision 4, last modified Jun 29 2018.
