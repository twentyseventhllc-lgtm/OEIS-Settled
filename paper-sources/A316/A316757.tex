...
\title{Proofs of the conjectured column and row recurrences for OEIS A316757}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316757}{A316757} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 4, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +a(n-2) +8*a(n-3) -16*a(n-5) for n>6\\k=3: [order 16] for n>17\\k=4: [order 71] for n>72\end{quote}
...
\begin{theorem} For column $k = 1$ of A316757, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316757, the recurrence $a(n) = a(n-1) + a(n-2) + 8\,a(n-3) - 16\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316757, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $54$ after trimming, and $S = 18$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 18$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 17$; the proved threshold is $n > 17$.

\begin{theorem} For column $k = 4$ of A316757, the recurrence $a(n) = 0$ holds for every $n > 72$, and fails at $n = 72$.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316757}{A316757}, revision 4, last modified Jul 12 2018.
