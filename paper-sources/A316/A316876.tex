...
\title{Proofs of the conjectured column and row recurrences for OEIS A316876}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316876}{A316876} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 3, 5, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) -6*a(n-3) -8*a(n-4) for n>5\\k=3: [order 16] for n>17\\k=4: [order 56] for n>58\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 15 2018). As of revision 4, last modified Jul 15 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A316876, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316876, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) - 6\,a(n-3) - 8\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316876, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A316876, the recurrence $a(n) = 0$ holds for every $n > 58$, and fails at $n = 58$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 65$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 58$; after that point more than $S = 65$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316876}{A316876}, revision 4, last modified Jul 15 2018.
