...
\title{Proofs of the conjectured column and row recurrences for OEIS A316511}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316511}{A316511} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 2, 3, 4, 5, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 3*a(n-1) +a(n-2) +2*a(n-3) -2*a(n-4) -4*a(n-5) for n>6\\k=3: [order 13] for n>15\\k=4: [order 31] for n>32\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 05 2018). As of revision 4, last modified Jul 05 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A316511, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316511, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 2\,a(n-3) - 2\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316511, the recurrence $a(n) = 0$ holds for every $n > 15$, and fails at $n = 15$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 15$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 15$; the proved threshold is $n > 15$.

\begin{theorem} For column $k = 4$ of A316511, the recurrence $a(n) = 0$ holds for every $n > 32$, and fails at $n = 32$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 39$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 32$; after that point more than $S = 39$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316511}{A316511}, revision 4, last modified Jul 05 2018.
