...
\title{Proofs of the conjectured column and row recurrences for OEIS A316733}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316733}{A316733} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 2, 3, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -4*a(n-4) for n>5\\k=3: [order 11] for n>13\\k=4: [order 9] for n>13\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 11 2018). As of revision 10, last modified Aug 04 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
The condition is $c(i,j) \in \{0, 2, 3, 6, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A316733, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316733, the recurrence $a(n) = a(n-1) + 3\,a(n-2) - 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316733, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $47$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 13$; the proved threshold is $n > 13$.

\begin{theorem} For column $k = 4$ of A316733, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $67$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316733}{A316733}, revision 10, last modified Aug 04 2018.
