...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296798}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296798}{A296798} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 1, 3 or 4 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 3*a(n-2) + 9*a(n-3) - 4*a(n-4) - 12*a(n-5) - 16*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 2*x)*(2 + 2*x - 3*x\textasciicircum{}2 - 10*x\textasciicircum{}3 - 8*x\textasciicircum{}4) / ((1 - x\textasciicircum{}2 - 2*x\textasciicircum{}3)*(1 - x - 2*x\textasciicircum{}2 - 8*x\textasciicircum{}3)). - Colin Barker, Feb 25 2019\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 8, last modified Feb 25 2019, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x + 6x^{2} + x^{3} - 16x^{4} - 28x^{5} - 16x^{6}}{1 - x - 3x^{2} - 9x^{3} + 4x^{4} + 12x^{5} + 16x^{6}}.\]
...
\begin{theorem} For A296798, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 9\,a(n-3) - 4\,a(n-4) - 12\,a(n-5) - 16\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296798, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x + 6x^{2} + x^{3} - 16x^{4} - 28x^{5} - 16x^{6}}{1 - x - 3x^{2} - 9x^{3} + 4x^{4} + 12x^{5} + 16x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 8, 15, 41, 122, 308, 855, 2405,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296798}{A296798}, revision 8, last modified Feb 25 2019.
