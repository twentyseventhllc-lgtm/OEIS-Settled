...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207769}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207769}{A207769} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 0 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) - 2*a(n-2) + a(n-3) - 2*a(n-4) + a(n-6) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 6*x*(1 + 3*x - 2*x\textasciicircum{}2 + x\textasciicircum{}3 + 3*x\textasciicircum{}4 - x\textasciicircum{}5 - x\textasciicircum{}6) / ((1 - x)\textasciicircum{}2*(1 - x - x\textasciicircum{}2 - 2*x\textasciicircum{}3 - x\textasciicircum{}4)). - Colin Barker, Jun 25 2018\end{quote}
...
\[a(n) = 3\,a(n-1) - 2\,a(n-2) + a(n-3) - 2\,a(n-4) + a(n-6).\]

stated for $n > 7$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 18x^{2} - 12x^{3} + 6x^{4} + 18x^{5} - 6x^{6} - 6x^{7}}{1 - 3x + 2x^{2} - x^{3} + 2x^{4} - x^{6}}.\]
...
\begin{theorem} For A207769, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + a(n-3) - 2\,a(n-4) + a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,48$. The last nonvanishing one is at $m = 1$, that is at $n = 7$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A207769, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 18x^{2} - 12x^{3} + 6x^{4} + 18x^{5} - 6x^{6} - 6x^{7}}{1 - 3x + 2x^{2} - x^{3} + 2x^{4} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 84, 192, 450, 972, 2040, 4278,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207769}{A207769}, revision 8, last modified Jun 25 2018.
