...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296984}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296984}{A296984} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 3, 4 or 5 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + a(n-2) + 3*a(n-3) - 4*a(n-4) - 2*a(n-5) - 4*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x + 2*x\textasciicircum{}2)*(1 - x - 2*x\textasciicircum{}2 - 2*x\textasciicircum{}3) / (1 - 2*x - x\textasciicircum{}2 - 3*x\textasciicircum{}3 + 4*x\textasciicircum{}4 + 2*x\textasciicircum{}5 + 4*x\textasciicircum{}6). - Colin Barker, Feb 25 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x - x^{3} - 6x^{4} - 6x^{5} - 4x^{6}}{1 - 2x - x^{2} - 3x^{3} + 4x^{4} + 2x^{5} + 4x^{6}}.\]
...
\begin{theorem} For A296984, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + 3\,a(n-3) - 4\,a(n-4) - 2\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296984, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x - x^{3} - 6x^{4} - 6x^{5} - 4x^{6}}{1 - 2x - x^{2} - 3x^{3} + 4x^{4} + 2x^{5} + 4x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 2, 4, 7, 14, 33, 77, 185,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296984}{A296984}, revision 7, last modified Feb 25 2019.
