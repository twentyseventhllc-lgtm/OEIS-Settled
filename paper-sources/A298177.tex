...
k=1: a(n) = a(n-1)\\k=2: a(n) = 3*a(n-2) +2*a(n-3) -2*a(n-4)\\k=3: a(n) = a(n-1) +2*a(n-2) +2*a(n-5) -4*a(n-6) -8*a(n-7) -a(n-8) +a(n-9)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 14 2018). As of revision 4, last modified Jan 14 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
