...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A223951}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A223951}{A223951} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays with diagonals and antidiagonals unimodal and rows nondecreasing\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (1/3)*n\textasciicircum{}4 + (2/3)*n\textasciicircum{}3 + (37/6)*n\textasciicircum{}2 + (107/6)*n + 23 for n>2.\end{quote}

\begin{quote}\ttfamily G.f.: x*(16 + x - 68*x\textasciicircum{}2 + 86*x\textasciicircum{}3 - 7*x\textasciicircum{}4 - 33*x\textasciicircum{}5 + 13*x\textasciicircum{}6) / (1 - x)\textasciicircum{}5.\end{quote}
...
\[a(n) = 23 + \frac{107}{6}n + \frac{37}{6}n^{2} + \frac{2}{3}n^{3} + \frac{1}{3}n^{4}.\]

stated for $n > 2$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + x^{2} - 68x^{3} + 86x^{4} - 7x^{5} - 33x^{6} + 13x^{7}}{1 - 5x + 10x^{2} - 10x^{3} + 5x^{4} - x^{5}}.\]
...
\begin{theorem} For A223951, $a(n)$ is the polynomial $23 + \frac{107}{6}n + \frac{37}{6}n^{2} + \frac{2}{3}n^{3} + \frac{1}{3}n^{4}$ for every $n > 2$, and not at $n = 2$.\end{theorem}

{\itshape Proof.} A polynomial of degree $4$ is annihilated by the difference operator $(E-1)^{5}$, whose characteristic polynomial is $(t-1)^{5}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 5 = 18$. Its values were computed exactly; the last nonvanishing one is at $n = 2$, and more than $18$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A223951, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + x^{2} - 68x^{3} + 86x^{4} - 7x^{5} - 33x^{6} + 13x^{7}}{1 - 5x + 10x^{2} - 10x^{3} + 5x^{4} - x^{5}}$.\end{theorem}
...
\begin{theorem} For A223951, the recurrence $a(n) = 5\,a(n-1) - 10\,a(n-2) + 10\,a(n-3) - 5\,a(n-4) + a(n-5)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,60$. The last nonvanishing one is at $m = 2$, that is at $n = 7$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 36 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 81, 177, 321, 558, 928, 1479, 2267,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A223951}{A223951}, revision 7, last modified Aug 24 2018.
