...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208288}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208288}{A208288} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 6*a(n-2) - 4*a(n-3) - 2*a(n-4) + a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 8*x*(1 + 5*x - 3*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + x\textasciicircum{}4) / ((1 + x - x\textasciicircum{}2)*(1 - 4*x - x\textasciicircum{}2 + x\textasciicircum{}3)). - Colin Barker, Jun 30 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Jun 30 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{8x + 40x^{2} - 24x^{3} - 16x^{4} + 8x^{5}}{1 - 3x - 6x^{2} + 4x^{3} + 2x^{4} - x^{5}}.\]
...
\begin{theorem} For A208288, the recurrence $a(n) = 3\,a(n-1) + 6\,a(n-2) - 4\,a(n-3) - 2\,a(n-4) + a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208288, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{8x + 40x^{2} - 24x^{3} - 16x^{4} + 8x^{5}}{1 - 3x - 6x^{2} + 4x^{3} + 2x^{4} - x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 64, 216, 984, 3984, 16872, 70216, 294192,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208288}{A208288}, revision 10, last modified Jun 30 2018.
