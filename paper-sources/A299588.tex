...
\title{Proofs of the conjectured column and row recurrences for OEIS A299588}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299588}{A299588} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 3, 4, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-2)\\k=2: a(n) = 2*a(n-1) +8*a(n-3) -8*a(n-4) -8*a(n-5)\\k=3: [order 19] for n>20\\k=4: [order 64] for n>65\end{quote}
...
\begin{theorem} For column $k = 1$ of A299588, the recurrence $a(n) = a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A299588, the recurrence $a(n) = 2\,a(n-1) + 8\,a(n-3) - 8\,a(n-4) - 8\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A299588, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A299588, the recurrence $a(n) = 0$ holds for every $n > 65$, and fails at $n = 65$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 69$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 65$; after that point more than $S = 69$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299588}{A299588}, revision 4, last modified Feb 13 2018.
