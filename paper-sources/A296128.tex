...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1) +4*a(n-3)\\k=3: a(n) = 2*a(n-1) -a(n-2) +8*a(n-3) -7*a(n-4) +2*a(n-5) -2*a(n-6)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 05 2017). As of revision 4, last modified Dec 05 2017, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
