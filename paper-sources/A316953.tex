...
\title{Proofs of the conjectured column and row recurrences for OEIS A316953}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316953}{A316953} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3, 4, 5, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) +2*a(n-3)\\k=3: a(n) = 6*a(n-1) +10*a(n-2) +15*a(n-3) +8*a(n-4) +3*a(n-5) +a(n-6) +2*a(n-7)\\k=4: [order 13]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 17 2018). As of revision 4, last modified Jul 17 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A316953, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316953, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316953, the recurrence $a(n) = 6\,a(n-1) + 10\,a(n-2) + 15\,a(n-3) + 8\,a(n-4) + 3\,a(n-5) + a(n-6) + 2\,a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A316953, the recurrence $a(n) = 0$ holds for every $n \ge 14$, at every index at which it can be stated.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316953}{A316953}, revision 4, last modified Jul 17 2018.
