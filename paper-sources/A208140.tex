...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A208140}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208140}{A208140} as

\begin{quote}\itshape Number of n X 6 0..1 arrays avoiding 0 0 1 and 0 1 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (4/9)*n\textasciicircum{}6 + (8/3)*n\textasciicircum{}5 + (52/9)*n\textasciicircum{}4 + (16/3)*n\textasciicircum{}3 + (16/9)*n\textasciicircum{}2.\end{quote}

\begin{quote}\ttfamily G.f.: 16*x*(1 + x)*(1 + 8*x + x\textasciicircum{}2) / (1 - x)\textasciicircum{}7.\end{quote}
...
\[a(n) = \frac{16}{9}n^{2} + \frac{16}{3}n^{3} + \frac{52}{9}n^{4} + \frac{8}{3}n^{5} + \frac{4}{9}n^{6}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + 144x^{2} + 144x^{3} + 16x^{4}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}.\]
...
For scale: at $n = 8$ there are $2^{48}$ arrays, about $10^{14}$, against $10$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A208140, $a(n)$ is the polynomial $\frac{16}{9}n^{2} + \frac{16}{3}n^{3} + \frac{52}{9}n^{4} + \frac{8}{3}n^{5} + \frac{4}{9}n^{6}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $6$ is annihilated by the difference operator $(E-1)^{7}$, whose characteristic polynomial is $(t-1)^{7}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 7 = 17$. Its values were computed exactly; all of them vanish, and more than $17$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A208140, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + 144x^{2} + 144x^{3} + 16x^{4}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}$.\end{theorem}
...
\begin{theorem} For A208140, the recurrence $a(n) = 7\,a(n-1) - 21\,a(n-2) + 35\,a(n-3) - 35\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 256, 1600, 6400, 19600, 50176, 112896, 230400,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208140}{A208140}, revision 8, last modified Jun 28 2018.
