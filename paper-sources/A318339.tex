...
\title{A proof of the conjectured linear recurrence for OEIS A318339}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318339}{A318339} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1, 2 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +4*a(n-2) +a(n-3) -5*a(n-4) -2*a(n-5) +2*a(n-6) +a(n-7) -a(n-8) for n>9\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 4\,a(n-2) + a(n-3) - 5\,a(n-4) - 2\,a(n-5) + 2\,a(n-6) + \\ a(n-7) - a(n-8). \end{multline*}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $57$; neither removal changes any count.
...
\begin{theorem} For A318339, the recurrence $a(n) = a(n-1) + 4\,a(n-2) + a(n-3) - 5\,a(n-4) - 2\,a(n-5) + 2\,a(n-6) + a(n-7) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,59$. The last nonvanishing one is at $m = 1$, that is at $n = 9$. After it, more than $S = 17$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 20, 36, 67, 148, 343, 817, 1975,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318339}{A318339}, revision 4, last modified Aug 24 2018.
