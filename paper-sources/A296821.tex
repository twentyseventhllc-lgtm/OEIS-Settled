...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296821}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296821}{A296821} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 2, 3 or 4 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 5*a(n-3) - 2*a(n-4) - 10*a(n-5) - 8*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 3*x + 3*x\textasciicircum{}2 - 12*x\textasciicircum{}3 - 18*x\textasciicircum{}4 - 8*x\textasciicircum{}5) / (1 - 3*x - 5*x\textasciicircum{}3 + 2*x\textasciicircum{}4 + 10*x\textasciicircum{}5 + 8*x\textasciicircum{}6). - Colin Barker, Feb 25 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 3x^{2} + 3x^{3} - 12x^{4} - 18x^{5} - 8x^{6}}{1 - 3x - 5x^{3} + 2x^{4} + 10x^{5} + 8x^{6}}.\]
...
\begin{theorem} For A296821, the recurrence $a(n) = 3\,a(n-1) + 5\,a(n-3) - 2\,a(n-4) - 10\,a(n-5) - 8\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296821, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 3x^{2} + 3x^{3} - 12x^{4} - 18x^{5} - 8x^{6}}{1 - 3x - 5x^{3} + 2x^{4} + 10x^{5} + 8x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 6, 21, 56, 178, 609, 1997, 6511,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296821}{A296821}, revision 7, last modified Feb 25 2019.
