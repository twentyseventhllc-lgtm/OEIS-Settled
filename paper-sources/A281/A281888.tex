...
\title{Proofs of the conjectured column and row recurrences for OEIS A281888}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281888}{A281888} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no element equal to more than four of its king-move neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = a(n-1)\\k=3: [order 14] for n>17\\k=4: [order 46] for n>49\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 01 2017). As of revision 4, last modified Feb 01 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A281888, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} In A281888, column $k = 2$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}

{\itshape Proof.} With that width the digraph has no state at all: no array of the required kind exists for any number of rows, so the sequence is $0,0,0,\dots$ and every linear recurrence holds. The entry's own published terms for column $k = 2$ are all zero, in agreement. This is an elementary case and is recorded as one. $\square$

\begin{theorem} For column $k = 3$ of A281888, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $196$ states, $129$ after trimming, and $S = 22$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 22$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 17$; the proved threshold is $n > 17$.

\begin{theorem} For column $k = 4$ of A281888, the recurrence $a(n) = 0$ holds for every $n > 49$, and fails at $n = 49$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $762$ states, $493$ after trimming, and $S = 72$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 49$; after that point more than $S = 72$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281888}{A281888}, revision 4, last modified Feb 01 2017.
