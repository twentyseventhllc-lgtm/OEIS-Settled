...
\title{Proofs of the conjectured column and row recurrences for OEIS A281165}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281165}{A281165} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no element equal to more than three of its king-move neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: [order 15]\\k=3: [order 27] for n>32\\k=4: [order 54] for n>61\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 16 2017). As of revision 4, last modified Jan 16 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A281165, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A281165, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $69$ states, $53$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A281165, the recurrence $a(n) = 0$ holds for every $n > 32$, and fails at $n = 32$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $249$ states, $173$ after trimming, and $S = 43$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 32$; after that point more than $S = 43$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 32$; the proved threshold is $n > 32$.

\begin{theorem} For column $k = 4$ of A281165, the recurrence $a(n) = 0$ holds for every $n > 61$, and fails at $n = 61$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $916$ states, $581$ after trimming, and $S = 107$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 61$; after that point more than $S = 107$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281165}{A281165}, revision 4, last modified Jan 16 2017.
