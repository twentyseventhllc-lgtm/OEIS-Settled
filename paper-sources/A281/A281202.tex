...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A281202}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281202}{A281202} as

\begin{quote}\itshape Number of n X 5 0..1 arrays with no element equal to more than one of its horizontal and antidiagonal neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 4*a(n-2) - 2*a(n-4) + 4*a(n-5) - a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(5 + 32*x - 12*x\textasciicircum{}2 - 26*x\textasciicircum{}3 - 25*x\textasciicircum{}4 + 2*x\textasciicircum{}5 + 12*x\textasciicircum{}6 + 4*x\textasciicircum{}7) / ((1 - x)\textasciicircum{}2*(1 - x - x\textasciicircum{}2 - x\textasciicircum{}3)\textasciicircum{}2). - Colin Barker, Feb 17 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{5x + 32x^{2} - 12x^{3} - 26x^{4} - 25x^{5} + 2x^{6} + 12x^{7} + 4x^{8}}{1 - 4x + 4x^{2} + 2x^{4} - 4x^{5} + x^{8}}.\]
...
The reachable part of the digraph has $1365$ states, $174$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 13$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of that $13\times13$ matrix; the bound is computed, not assumed.
...
\begin{theorem} For A281202, the recurrence $a(n) = 4\,a(n-1) - 4\,a(n-2) - 2\,a(n-4) + 4\,a(n-5) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A281202, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{5x + 32x^{2} - 12x^{3} - 26x^{4} - 25x^{5} + 2x^{6} + 12x^{7} + 4x^{8}}{1 - 4x + 4x^{2} + 2x^{4} - 4x^{5} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 52, 176, 470, 1141, 2602, 5712, 12208,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281202}{A281202}, revision 8, last modified Feb 17 2019.
