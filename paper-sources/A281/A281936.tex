...
\title{Proofs of the conjectured column and row recurrences for OEIS A281936}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281936}{A281936} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no element equal to more than four of its king-move neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 6*a(n-1) -3*a(n-2) -14*a(n-3) -21*a(n-4) -12*a(n-5) -4*a(n-6)\\k=3: [order 21] for n>26\\k=4: [order 69] for n>74\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 02 2017). As of revision 4, last modified Feb 02 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} In A281936, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A281936, the recurrence $a(n) = 6\,a(n-1) - 3\,a(n-2) - 14\,a(n-3) - 21\,a(n-4) - 12\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $37$ states, $37$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A281936, the recurrence $a(n) = 0$ holds for every $n > 26$, and fails at $n = 26$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $260$ states, $194$ after trimming, and $S = 35$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 26$; after that point more than $S = 35$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 26$; the proved threshold is $n > 26$.

\begin{theorem} For column $k = 4$ of A281936, the recurrence $a(n) = 0$ holds for every $n > 74$, and fails at $n = 74$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $1018$ states, $747$ after trimming, and $S = 113$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 74$; after that point more than $S = 113$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281936}{A281936}, revision 4, last modified Feb 02 2017.
