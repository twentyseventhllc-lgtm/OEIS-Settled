...
\title{Proofs of the conjectured column and row recurrences for OEIS A281540}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281540}{A281540} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to more than one of its king-move neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = 6*a(n-1) -6*a(n-2) -16*a(n-3) +12*a(n-4) +24*a(n-5) +8*a(n-6) for n>10\\k=2: [order 12]\\k=3: [order 22] for n>30\\k=4: [order 36] for n>50\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 23 2017). As of revision 4, last modified Jan 23 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A281540, the recurrence $a(n) = 6\,a(n-1) - 6\,a(n-2) - 16\,a(n-3) + 12\,a(n-4) + 24\,a(n-5) + 8\,a(n-6)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $58$ states, $44$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 2$ of A281540, the recurrence $a(n) = 0$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $320$ states, $173$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A281540, the recurrence $a(n) = 0$ holds for every $n > 30$, and fails at $n = 30$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $2153$ states, $784$ after trimming, and $S = 45$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 30$; after that point more than $S = 45$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 30$; the proved threshold is $n > 30$.

\begin{theorem} For column $k = 4$ of A281540, the recurrence $a(n) = 0$ holds for every $n > 50$, and fails at $n = 50$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $14581$ states, $3199$ after trimming, and $S = 116$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 50$; after that point more than $S = 116$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281540}{A281540}, revision 4, last modified Jan 23 2017.
