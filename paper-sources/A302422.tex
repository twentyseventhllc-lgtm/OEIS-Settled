...
\title{A proof of the conjectured linear recurrence for OEIS A302422}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302422}{A302422} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element equal to 0, 1, 3 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +10*a(n-3) -8*a(n-4) for n>9\end{quote}
...
\begin{theorem} For A302422, the recurrence $a(n) = a(n-1) + 10\,a(n-3) - 8\,a(n-4)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 5$, that is at $n = 9$. After it, more than $S = 10$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 30 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 9, 11, 21, 51, 93, 207, 517,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302422}{A302422}, revision 4, last modified Apr 07 2018.
