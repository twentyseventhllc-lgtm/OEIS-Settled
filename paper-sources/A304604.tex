...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -2*a(n-4) for n>6\\k=3: a(n) = a(n-1) +2*a(n-2) +4*a(n-3) +2*a(n-4) -2*a(n-5) -7*a(n-6) -6*a(n-7) for n>10\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 15 2018). As of revision 4, last modified May 15 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
