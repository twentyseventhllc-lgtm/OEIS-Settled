...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A260363}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260363}{A260363} as

\begin{quote}\itshape Number of (n+2) X (1+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00000001 00000011 or 00001111\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) - a(n-2) + 5*a(n-3) - a(n-4) + 5*a(n-5) - 3*a(n-6) - a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(24 + 15*x + 89*x\textasciicircum{}2 + 33*x\textasciicircum{}3 + 23*x\textasciicircum{}4 - 2*x\textasciicircum{}5 - 50*x\textasciicircum{}6) / (1 - x + x\textasciicircum{}2 - 5*x\textasciicircum{}3 + x\textasciicircum{}4 - 5*x\textasciicircum{}5 + 3*x\textasciicircum{}6 + x\textasciicircum{}7). - Colin Barker, Dec 29 2018\end{quote}
...
\begin{multline*} a(n) = a(n-1) - a(n-2) + 5\,a(n-3) - a(n-4) + 5\,a(n-5) - 3\,a(n-6) - \\ a(n-7). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{48x + 30x^{2} + 178x^{3} + 66x^{4} + 46x^{5} - 4x^{6} - 100x^{7}}{1 - x + x^{2} - 5x^{3} + x^{4} - 5x^{5} + 3x^{6} + x^{7}}.\]
...
That refinement leaves $17$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 12$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A260363, the recurrence $a(n) = a(n-1) - a(n-2) + 5\,a(n-3) - a(n-4) + 5\,a(n-5) - 3\,a(n-6) - a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A260363, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{48x + 30x^{2} + 178x^{3} + 66x^{4} + 46x^{5} - 4x^{6} - 100x^{7}}{1 - x + x^{2} - 5x^{3} + x^{4} - 5x^{5} + 3x^{6} + x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 48, 78, 208, 436, 616, 1378, 2880, 4904,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260363}{A260363}, revision 8, last modified Dec 29 2018.
