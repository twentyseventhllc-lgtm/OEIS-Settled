...
\title{Proofs of the conjectured column and row recurrences for OEIS A299942}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299942}{A299942} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 3, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) +a(n-2) +2*a(n-3) -2*a(n-4) -4*a(n-5)\\k=3: [order 11] for n>12\\k=4: [order 22] for n>28\end{quote}
...
\begin{theorem} For column $k = 1$ of A299942, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A299942, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 2\,a(n-3) - 2\,a(n-4) - 4\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A299942, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $58$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 12$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 12$; the proved threshold is $n > 12$.

\begin{theorem} For column $k = 4$ of A299942, the recurrence $a(n) = 0$ holds for every $n > 28$, and fails at $n = 28$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $198$ after trimming, and $S = 36$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 28$; after that point more than $S = 36$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299942}{A299942}, revision 4, last modified Feb 22 2018.
