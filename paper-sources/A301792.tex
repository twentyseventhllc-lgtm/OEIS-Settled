...
\title{A proof of the conjectured linear recurrence for OEIS A301792}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301792}{A301792} as

\begin{quote}\itshape Number of 3Xn 0..1 arrays with every element equal to 0 or 1 horizontally or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $25$; neither removal changes any count.
...
\begin{theorem} For A301792, the recurrence $a(n) = 2\,a(n-1) - a(n-3)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,51$. The last nonvanishing one is at $m = 2$, that is at $n = 5$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 36 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 13, 12, 17, 25, 38, 59, 93,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301792}{A301792}, revision 4, last modified Mar 26 2018.
