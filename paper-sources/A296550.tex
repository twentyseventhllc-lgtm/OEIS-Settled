...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296550}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296550}{A296550} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with each 1 horizontally, vertically or antidiagonally adjacent to 3 or 6 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-4) + a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 2*x\textasciicircum{}2 + 2*x\textasciicircum{}3 + x\textasciicircum{}5 + x\textasciicircum{}6) / (1 - x - 2*x\textasciicircum{}4 - x\textasciicircum{}7). - Colin Barker, Feb 23 2019\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 14, last modified Jan 21 2022, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 2x^{3} + 2x^{4} + x^{6} + x^{7}}{1 - x - 2x^{4} - x^{7}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $13$; neither removal changes any count.
...
\begin{theorem} For A296550, the recurrence $a(n) = a(n-1) + 2\,a(n-4) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296550, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 2x^{3} + 2x^{4} + x^{6} + x^{7}}{1 - x - 2x^{4} - x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 39 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 1, 3, 5, 7, 10, 17, 28,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296550}{A296550}, revision 14, last modified Jan 21 2022.
