...
n=1: a(n) = 2*a(n-1)\\n=2: a(n) = 3*a(n-1) +a(n-2) -6*a(n-3) -2*a(n-4) +4*a(n-5) for n>6\\n=3: a(n) = 3*a(n-1) -3*a(n-2) +4*a(n-3) -4*a(n-4) +a(n-5) for n>9\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 07 2018). As of revision 4, last modified Apr 07 2018, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences and the 3 row recurrences listed above.
