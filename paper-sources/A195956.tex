...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A195956}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A195956}{A195956} as

\begin{quote}\itshape Number of n X 2 0..4 arrays with each element equal to the number its horizontal and vertical neighbors unequal to itself\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-3) -a(n-4) -2*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 5*x\textasciicircum{}2 - 4*x\textasciicircum{}3 - 2*x\textasciicircum{}4 - 8*x\textasciicircum{}5) / ((1 - x)*(1 + x + x\textasciicircum{}2)*(1 - x - 2*x\textasciicircum{}3)). - Colin Barker, May 08 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 5x^{3} - 4x^{4} - 2x^{5} - 8x^{6}}{1 - x - 3x^{3} + x^{4} + 2x^{6}}.\]
...
The construction gives $650$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $22$; neither removal changes any count.
...
\begin{theorem} For A195956, the recurrence $a(n) = a(n-1) + 3\,a(n-3) - a(n-4) - 2\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A195956, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 5x^{3} - 4x^{4} - 2x^{5} - 8x^{6}}{1 - x - 3x^{3} + x^{4} + 2x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 1, 6, 5, 5, 14, 21, 29,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A195956}{A195956}, revision 9, last modified May 08 2018.
