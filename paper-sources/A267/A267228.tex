...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A267228}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A267228}{A267228} as

\begin{quote}\itshape Number of length-n 0..4 arrays with no following elements greater than or equal to the first repeated value\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 14*a(n-1) -75*a(n-2) +190*a(n-3) -224*a(n-4) +96*a(n-5) for n>6.\end{quote}

\begin{quote}\ttfamily G.f.: x*(5 - 45*x + 135*x\textasciicircum{}2 - 145*x\textasciicircum{}3 + 20*x\textasciicircum{}4 + 24*x\textasciicircum{}5) / ((1 - x)*(1 - 2*x)*(1 - 3*x)*(1 - 4*x)\textasciicircum{}2).\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{5x - 45x^{2} + 135x^{3} - 145x^{4} + 20x^{5} + 24x^{6}}{1 - 14x + 75x^{2} - 190x^{3} + 224x^{4} - 96x^{5}}.\]
...
\begin{theorem} For A267228, the recurrence $a(n) = 14\,a(n-1) - 75\,a(n-2) + 190\,a(n-3) - 224\,a(n-4) + 96\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,40$. The last nonvanishing one is at $m = 1$, that is at $n = 6$. After it, more than $S = 7$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 6$. The theorem is at least as strong.

\begin{theorem} For A267228, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{5x - 45x^{2} + 135x^{3} - 145x^{4} + 20x^{5} + 24x^{6}}{1 - 14x + 75x^{2} - 190x^{3} + 224x^{4} - 96x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 22 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 25, 110, 470, 1980, 8274, 34396, 142474,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A267228}{A267228}, revision 8, last modified Feb 05 2018.
