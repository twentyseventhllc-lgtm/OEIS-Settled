...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208079}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208079}{A208079} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 0 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) + 5*a(n-2) - a(n-3).\end{quote}

\begin{quote}\ttfamily G.f.: 9*x*(1 + 4*x - x\textasciicircum{}2) / ((1 + x)*(1 - 6*x + x\textasciicircum{}2)).\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 36x^{2} - 9x^{3}}{1 - 5x - 5x^{2} + x^{3}}.\]
...
\begin{theorem} For A208079, the recurrence $a(n) = 5\,a(n-1) + 5\,a(n-2) - a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A208079, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 36x^{2} - 9x^{3}}{1 - 5x - 5x^{2} + x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 20 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 441, 2601, 15129, 88209, 514089, 2996361,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208079}{A208079}, revision 10, last modified Jan 20 2018.
