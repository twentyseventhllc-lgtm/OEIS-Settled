...
\title{A proof of the conjectured recurrence and generating function for OEIS A302676}
...
Here the refinement leaves $S = 12$ blocks. Writing $L$ for the $S\times S$ quotient matrix, $\tilde w$ for the start weights summed over each block and $\tilde f$ for the acceptance indicator,
...
{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,56$. The last nonvanishing one is at $m = 4$, that is at $n = 8$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
