...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A302676}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302676}{A302676} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every element equal to 0, 1, 4 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-2) - a(n-4) for  n > 8.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(5 - 3*x\textasciicircum{}2 - 2*x\textasciicircum{}3 - 6*x\textasciicircum{}4 - 4*x\textasciicircum{}5 + 3*x\textasciicircum{}6 + 2*x\textasciicircum{}7) / (1 - x - 2*x\textasciicircum{}2 + x\textasciicircum{}4). - Colin Barker, Jun 17 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 31, last modified Aug 24 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{5x - 3x^{3} - 2x^{4} - 6x^{5} - 4x^{6} + 3x^{7} + 2x^{8}}{1 - x - 2x^{2} + x^{4}}.\]
...
\begin{theorem} For A302676, the recurrence $a(n) = a(n-1) + 2\,a(n-2) - a(n-4)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. The last nonvanishing one is at $m = 4$, that is at $n = 8$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 8$. The theorem is at least as strong.

\begin{theorem} For A302676, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{5x - 3x^{3} - 2x^{4} - 6x^{5} - 4x^{6} + 3x^{7} + 2x^{8}}{1 - x - 2x^{2} + x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 5, 12, 20, 33, 64, 121, 231,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302676}{A302676}, revision 31, last modified Aug 24 2018.
