...
\title{A proof of the conjectured linear recurrence for OEIS A301902}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A301902}{A301902} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element equal to 0, 2 or 3 horizontally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) +a(n-2) -4*a(n-3).\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Mar 28 2018).

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 6, last modified Sep 21 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\begin{theorem} For A301902, the recurrence $a(n) = 3\,a(n-1) + a(n-2) - 4\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 2, 3, 7, 16, 43, 117, 330,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A301902}{A301902}, revision 6, last modified Sep 21 2018.
