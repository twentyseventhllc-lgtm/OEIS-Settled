...
\title{Proofs of the conjectured column and row recurrences for OEIS A297889}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297889}{A297889} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2 or 4 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 3*a(n-1) -2*a(n-2) +8*a(n-3) -20*a(n-4) +8*a(n-5) for n>6\\k=3: [order 10] for n>11\\k=4: [order 30] for n>34\\k=5: [order 86] for n>91\end{quote}
...
\begin{theorem} For column $k = 1$ of A297889, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297889, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 8\,a(n-3) - 20\,a(n-4) + 8\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297889, the recurrence $a(n) = 0$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $53$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 11$; the proved threshold is $n > 11$.

\begin{theorem} For column $k = 4$ of A297889, the recurrence $a(n) = 0$ holds for every $n > 34$, and fails at $n = 34$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $171$ after trimming, and $S = 40$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 34$; after that point more than $S = 40$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 34$; the proved threshold is $n > 34$.

\begin{theorem} For column $k = 5$ of A297889, the recurrence $a(n) = 0$ holds for every $n > 91$, and fails at $n = 91$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $495$ after trimming, and $S = 101$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 91$; after that point more than $S = 101$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297889}{A297889}, revision 4, last modified Jan 07 2018.
