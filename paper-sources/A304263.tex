...
\title{Proofs of the conjectured column and row recurrences for OEIS A304263}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304263}{A304263} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 2, 3, 5 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 2*a(n-1) +3*a(n-2) -4*a(n-3) -6*a(n-4) +4*a(n-5) for n>6\\k=3: [order 17] for n>18\\k=4: [order 63] for n>64\end{quote}
...
\begin{theorem} In A304263, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A304263, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 4\,a(n-3) - 6\,a(n-4) + 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A304263, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A304263, the recurrence $a(n) = 0$ holds for every $n > 64$, and fails at $n = 64$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $238$ after trimming, and $S = 67$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 64$; after that point more than $S = 67$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304263}{A304263}, revision 4, last modified May 09 2018.
