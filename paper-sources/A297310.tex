...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297310}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297310}{A297310} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every 1 horizontally or antidiagonally adjacent to 1 or 2 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) + 26*a(n-2) + 22*a(n-3) - 43*a(n-4) - a(n-5) + 7*a(n-6) - a(n-7) - a(n-8) for n>9.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(7 + 38*x + 51*x\textasciicircum{}2 - 50*x\textasciicircum{}3 - 93*x\textasciicircum{}4 + 50*x\textasciicircum{}5 + 2*x\textasciicircum{}6 - 10*x\textasciicircum{}7 + x\textasciicircum{}8) / ((1 - 7*x - 5*x\textasciicircum{}2 - x\textasciicircum{}3)*(1 + 3*x - 6*x\textasciicircum{}3 + 4*x\textasciicircum{}4 - x\textasciicircum{}5)). - Colin Barker, Feb 26 2019\end{quote}
...
\begin{multline*} a(n) = 4\,a(n-1) + 26\,a(n-2) + 22\,a(n-3) - 43\,a(n-4) - a(n-5) + 7\,a(n-6) - \\ a(n-7) - a(n-8). \end{multline*}

stated for $n > 9$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{7x + 38x^{2} + 51x^{3} - 50x^{4} - 93x^{5} + 50x^{6} + 2x^{7} - 10x^{8} + x^{9}}{1 - 4x - 26x^{2} - 22x^{3} + 43x^{4} + x^{5} - 7x^{6} + x^{7} + x^{8}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $160$; neither removal changes any count.
...
That refinement leaves $17$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 10$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A297310, the recurrence $a(n) = 4\,a(n-1) + 26\,a(n-2) + 22\,a(n-3) - 43\,a(n-4) - a(n-5) + 7\,a(n-6) - a(n-7) - a(n-8)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}
...
\begin{theorem} For A297310, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{7x + 38x^{2} + 51x^{3} - 50x^{4} - 93x^{5} + 50x^{6} + 2x^{7} - 10x^{8} + x^{9}}{1 - 4x - 26x^{2} - 22x^{3} + 43x^{4} + x^{5} - 7x^{6} + x^{7} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 19 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 7, 66, 497, 3808, 29212, 223995, 1717882, 13174266,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297310}{A297310}, revision 7, last modified Feb 26 2019.
