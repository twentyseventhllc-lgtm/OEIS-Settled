...
\title{A proof of the conjectured linear recurrence for OEIS A317736}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317736}{A317736} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 2 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = -a(n-1) -a(n-2) +2*a(n-3) +7*a(n-4) +7*a(n-5) +4*a(n-6) -5*a(n-7) -10*a(n-8) -9*a(n-9) -3*a(n-10) for n>13\end{quote}
...
\begin{multline*} a(n) = -a(n-1) - a(n-2) + 2\,a(n-3) + 7\,a(n-4) + 7\,a(n-5) + 4\,a(n-6) - \\ 5\,a(n-7) - 10\,a(n-8) - 9\,a(n-9) - 3\,a(n-10). \end{multline*}
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $34$; neither removal changes any count.
...
\begin{theorem} For A317736, the recurrence $a(n) = -a(n-1) - a(n-2) + 2\,a(n-3) + 7\,a(n-4) + 7\,a(n-5) + 4\,a(n-6) - 5\,a(n-7) - 10\,a(n-8) - 9\,a(n-9) - 3\,a(n-10)$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,71$. The last nonvanishing one is at $m = 3$, that is at $n = 13$. After it, more than $S = 14$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 41 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 1, 1, 3, 2, 4, 6, 14,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317736}{A317736}, revision 4, last modified Aug 05 2018.
