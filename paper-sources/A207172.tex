...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207172}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207172}{A207172} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 1 0 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 13*a(n-3) + 3*a(n-4) + 3*a(n-5) - 27*a(n-6) - 18*a(n-7) + 27*a(n-9) for n>11.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(8 + 56*x + 104*x\textasciicircum{}2 + 331*x\textasciicircum{}3 + 153*x\textasciicircum{}4 - 60*x\textasciicircum{}5 - 819*x\textasciicircum{}6 - 828*x\textasciicircum{}7 - 54*x\textasciicircum{}8 + 837*x\textasciicircum{}9 + 324*x\textasciicircum{}10) / ((1 - x - 13*x\textasciicircum{}3 - 3*x\textasciicircum{}4 - 3*x\textasciicircum{}5 + 27*x\textasciicircum{}6 + 18*x\textasciicircum{}7 - 27*x\textasciicircum{}9)). - Colin Barker, Jun 20 2018\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 13\,a(n-3) + 3\,a(n-4) + 3\,a(n-5) - 27\,a(n-6) - 18\,a(n-7) + \\ 27\,a(n-9). \end{multline*}

stated for $n > 11$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{8x + 56x^{2} + 104x^{3} + 331x^{4} + 153x^{5} - 60x^{6} - 819x^{7} - 828x^{8} - 54x^{9} + 837x^{10} + 324x^{11}}{1 - x - 13x^{3} - 3x^{4} - 3x^{5} + 27x^{6} + 18x^{7} - 27x^{9}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $72$; neither removal changes any count.
...
That refinement leaves $23$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 11$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A207172, the recurrence $a(n) = a(n-1) + 13\,a(n-3) + 3\,a(n-4) + 3\,a(n-5) - 27\,a(n-6) - 18\,a(n-7) + 27\,a(n-9)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,51$. The last nonvanishing one is at $m = 2$, that is at $n = 11$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 11$. The theorem is at least as strong.

\begin{theorem} For A207172, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{8x + 56x^{2} + 104x^{3} + 331x^{4} + 153x^{5} - 60x^{6} - 819x^{7} - 828x^{8} - 54x^{9} + 837x^{10} + 324x^{11}}{1 - x - 13x^{3} - 3x^{4} - 3x^{5} + 27x^{6} + 18x^{7} - 27x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 64, 168, 603, 1612, 3952, 11452, 32021,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207172}{A207172}, revision 8, last modified Jun 20 2018.
