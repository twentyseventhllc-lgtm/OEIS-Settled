...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A235080}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A235080}{A235080} as

\begin{quote}\itshape Number of (n+1) X (1+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 6*a(n-2) - 10*a(n-3) - 5*a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(19 + 7*x - 103*x\textasciicircum{}2 - 43*x\textasciicircum{}3) / ((1 - 2*x - x\textasciicircum{}2)*(1 - 5*x\textasciicircum{}2)). - Colin Barker, Oct 17 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{38x + 14x^{2} - 206x^{3} - 86x^{4}}{1 - 2x - 6x^{2} + 10x^{3} + 5x^{4}}.\]
...
\begin{theorem} For A235080, the recurrence $a(n) = 2\,a(n-1) + 6\,a(n-2) - 10\,a(n-3) - 5\,a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A235080, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{38x + 14x^{2} - 206x^{3} - 86x^{4}}{1 - 2x - 6x^{2} + 10x^{3} + 5x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 38, 90, 202, 478, 1078, 2554, 5786, 13726,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A235080}{A235080}, revision 9, last modified Jun 19 2022.
