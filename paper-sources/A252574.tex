...
\title{Proofs of the conjectured column and row recurrences for OEIS A252574}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252574}{A252574} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 1 2 5 6 or 7 and every 3X3 column and antidiagonal sum not equal to 1 2 5 6 or 7\end{quote}
...
k=1: [linear recurrence of order 54] for n>60\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: [linear recurrence of order 33] for n>43\end{quote}
...
\begin{theorem} For column $k = 1$ of A252574, the recurrence $a(n) = 0$ holds for every $n > 60$, and fails at $n = 60$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 112$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 60$; after that point more than $S = 112$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 60$; the proved threshold is $n > 60$.

\begin{theorem} For row $n = 1$ of A252574, the recurrence $a(k) = 0$ holds for every $k > 43$, and fails at $k = 43$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $4160$ states, $4160$ after trimming, and $S = 107$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $k = 43$; after that point more than $S = 107$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252574}{A252574}, revision 6, last modified Jun 02 2025.
