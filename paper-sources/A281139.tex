...
\title{Proofs of the conjectured column and row recurrences for OEIS A281139}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281139}{A281139} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to more than one of its king-move neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = 4*a(n-1) -8*a(n-3) -4*a(n-4) for n>7\\k=2: [order 8]\\k=3: [order 14] for n>19\\k=4: [order 24] for n>33\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 15 2017). As of revision 4, last modified Jan 15 2017, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A281139, the recurrence $a(n) = 4\,a(n-1) - 8\,a(n-3) - 4\,a(n-4)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A281139, the recurrence $a(n) = 0$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $223$ states, $115$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A281139, the recurrence $a(n) = 0$ holds for every $n > 19$, and fails at $n = 19$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $1390$ states, $438$ after trimming, and $S = 26$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 19$; after that point more than $S = 26$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 19$; the proved threshold is $n > 19$.

\begin{theorem} For column $k = 4$ of A281139, the recurrence $a(n) = 0$ holds for every $n > 33$, and fails at $n = 33$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $8226$ states, $1409$ after trimming, and $S = 57$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 33$; after that point more than $S = 57$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281139}{A281139}, revision 4, last modified Jan 15 2017.
