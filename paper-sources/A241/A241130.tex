...
\title{Proofs of the conjectured column and row recurrences for OEIS A241130}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A241130}{A241130} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to exactly two horizontal and vertical neighbors, with new values 0..2 introduced in row major order\end{quote}
...
k=1: a(n) = 2*a(n-1) +2*a(n-2) for n>4\\k=2: a(n) = 6*a(n-1) +2*a(n-2) -9*a(n-3) -10*a(n-4) +8*a(n-5) for n>6\\k=3: [order 26]\end{quote}
...
\begin{theorem} For column $k = 1$ of A241130, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $30$ states, $16$ after trimming, and $S = 5$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 4$; after that point more than $S = 5$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 4$; the proved threshold is $n > 4$.

\begin{theorem} For column $k = 2$ of A241130, the recurrence $a(n) = 6\,a(n-1) + 2\,a(n-2) - 9\,a(n-3) - 10\,a(n-4) + 8\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $192$ states, $95$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 3$ of A241130, the recurrence $a(n) = 0$ holds for every $n \ge 27$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $1592$ states, $799$ after trimming, and $S = 28$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 28$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A241130}{A241130}, revision 6, last modified Jul 23 2025.
