...
\title{Proofs of the conjectured column and row recurrences for OEIS A241108}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A241108}{A241108} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to one or three horizontal or vertical neighbors, with new values 0..2 introduced in row major order\end{quote}
...
k=1: a(n) = 2*a(n-1) for n>2\\k=2: a(n) = 2*a(n-1) +4*a(n-2) for n>4\\k=3: a(n) = 3*a(n-1) +8*a(n-2) +16*a(n-3)\\k=4: [order 10]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 16 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A241108, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n > 2$, and fails at $n = 2$.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $16$ states, $10$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 2$; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 2$; the proved threshold is $n > 2$.

\begin{theorem} For column $k = 2$ of A241108, the recurrence $a(n) = 2\,a(n-1) + 4\,a(n-2)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $93$ states, $49$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 4$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 4$; the proved threshold is $n > 4$.

\begin{theorem} For column $k = 3$ of A241108, the recurrence $a(n) = 3\,a(n-1) + 8\,a(n-2) + 16\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $668$ states, $290$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A241108, the recurrence $a(n) = 0$ holds for every $n \ge 11$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $5244$ states, $2020$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A241108}{A241108}, revision 6, last modified Jul 23 2025.
