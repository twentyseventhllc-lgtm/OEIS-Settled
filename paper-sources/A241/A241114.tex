...
\title{Proofs of the conjectured column and row recurrences for OEIS A241114}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A241114}{A241114} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to exactly three horizontal or vertical neighbors, with new values 0..2 introduced in row major order\end{quote}
...
k=1: a(n) = 4*a(n-1) -3*a(n-2)\\k=2: a(n) = 8*a(n-1) +4*a(n-2) -2*a(n-3) -8*a(n-4) for n>6\\k=3: [order 12] for n>13\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 16 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A241114, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A241114, the recurrence $a(n) = 8\,a(n-1) + 4\,a(n-2) - 2\,a(n-3) - 8\,a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $198$ states, $101$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 3$ of A241114, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $1593$ states, $799$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 13$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A241114}{A241114}, revision 6, last modified Jul 23 2025.
