...
\title{Proofs of the conjectured column and row recurrences for OEIS A296651}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296651}{A296651} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 adjacent to 0, 1 or 3 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2) +a(n-3)\\k=2: a(n) = a(n-1) +3*a(n-2) +9*a(n-3) -4*a(n-4) -12*a(n-5) -4*a(n-6)\\k=3: [order 10]\\k=4: [order 22]\end{quote}
...
\begin{theorem} For column $k = 1$ of A296651, the recurrence $a(n) = a(n-1) + a(n-2) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A296651, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 9\,a(n-3) - 4\,a(n-4) - 12\,a(n-5) - 4\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A296651, the recurrence $a(n) = 0$ holds for every $n \ge 11$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $50$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A296651, the recurrence $a(n) = 0$ holds for every $n \ge 23$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $164$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296651}{A296651}, revision 4, last modified Dec 17 2017.
