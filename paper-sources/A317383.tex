...
\title{Proofs of the conjectured column and row recurrences for OEIS A317383}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317383}{A317383} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = a(n-1) +3*a(n-2) -2*a(n-4) for n>6\\k=3: a(n) = a(n-1) +3*a(n-2) +5*a(n-3) +2*a(n-4) -3*a(n-5) -10*a(n-6) -8*a(n-7) for n>10\\k=4: [order 19] for n>23\\k=5: [order 43] for n>49\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 26 2018). As of revision 4, last modified Jul 26 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
The condition is $c(i,j) \in \{0, 1, 2, 5, 6, 7, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A317383, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A317383, the recurrence $a(n) = a(n-1) + 3\,a(n-2) - 2\,a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A317383, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + 5\,a(n-3) + 2\,a(n-4) - 3\,a(n-5) - 10\,a(n-6) - 8\,a(n-7)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $48$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 4$ of A317383, the recurrence $a(n) = 0$ holds for every $n > 23$, and fails at $n = 23$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $127$ after trimming, and $S = 28$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 23$; after that point more than $S = 28$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 23$; the proved threshold is $n > 23$.

\begin{theorem} For column $k = 5$ of A317383, the recurrence $a(n) = 0$ holds for every $n > 49$, and fails at $n = 49$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $368$ after trimming, and $S = 61$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 49$; after that point more than $S = 61$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317383}{A317383}, revision 4, last modified Jul 26 2018.
