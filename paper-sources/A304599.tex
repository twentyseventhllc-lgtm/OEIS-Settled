...
\title{A proof of the conjectured linear recurrence for OEIS A304599}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304599}{A304599} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 2, 5 or 6 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +2*a(n-2) +4*a(n-3) +2*a(n-4) -2*a(n-5) -7*a(n-6) -6*a(n-7) for n>10\end{quote}
...
\begin{theorem} For A304599, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + 4\,a(n-3) + 2\,a(n-4) - 2\,a(n-5) - 7\,a(n-6) - 6\,a(n-7)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. The last nonvanishing one is at $m = 3$, that is at $n = 10$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 5, 13, 23, 53, 131, 330, 793,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304599}{A304599}, revision 4, last modified May 15 2018.
