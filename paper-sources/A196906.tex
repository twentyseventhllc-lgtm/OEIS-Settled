...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196906}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196906}{A196906} as

\begin{quote}\itshape Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,1,2,0,4 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-2) + 3*a(n-3) + 2*a(n-4) - a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(2 + 4*x + 3*x\textasciicircum{}2 + 3*x\textasciicircum{}3 - x\textasciicircum{}4 - x\textasciicircum{}5) / (1 - x - 2*x\textasciicircum{}2 - 3*x\textasciicircum{}3 - 2*x\textasciicircum{}4 + x\textasciicircum{}6). - Colin Barker, Mar 01 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x + 4x^{2} + 3x^{3} + 3x^{4} - x^{5} - x^{6}}{1 - x - 2x^{2} - 3x^{3} - 2x^{4} + x^{6}}.\]
...
\begin{theorem} For A196906, the recurrence $a(n) = a(n-1) + 2\,a(n-2) + 3\,a(n-3) + 2\,a(n-4) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196906, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x + 4x^{2} + 3x^{3} + 3x^{4} - x^{5} - x^{6}}{1 - x - 2x^{2} - 3x^{3} - 2x^{4} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 6, 13, 34, 81, 199, 487, 1190,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196906}{A196906}, revision 8, last modified Mar 01 2018.
