...
\title{Proofs of the conjectured column and row recurrences for OEIS A253025}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A253025}{A253025} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every consecutive three elements in every row and column not having exactly two distinct values, and in every diagonal and antidiagonal having exactly two distinct values, and new values 0 upwards introduced in row major order\end{quote}
...
k=1: a(n) = 3*a(n-3) for n>5\\k=2: a(n) = 6*a(n-3) -10*a(n-6) +3*a(n-9) for n>11\\k=3: a(n) = 5*a(n-3) -6*a(n-6) -a(n-9) +a(n-12) for n>15\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 26 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A253025, the recurrence $a(n) = 3\,a(n-3)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $186$ states, $186$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 5$; the proved threshold is $n > 5$.

\begin{theorem} For column $k = 2$ of A253025, the recurrence $a(n) = 6\,a(n-3) - 10\,a(n-6) + 3\,a(n-9)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $380$ states, $380$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 11$; the proved threshold is $n > 11$.

\begin{theorem} For column $k = 3$ of A253025, the recurrence $a(n) = 5\,a(n-3) - 6\,a(n-6) - a(n-9) + a(n-12)$ holds for every $n > 15$, and fails at $n = 15$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $771$ states, $771$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 15$; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item 2 lines: time limit reached while building the model
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A253025}{A253025}, revision 6, last modified Jul 23 2025.
