...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207171}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207171}{A207171} as

\begin{quote}\itshape Number of 3 X n 0..1 arrays avoiding 0 0 0 and 1 0 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 9*a(n-3) + 2*a(n-4) + 2*a(n-5) - 12*a(n-6) - 8*a(n-7) + 8*a(n-9) for n>11.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(6 + 30*x + 54*x\textasciicircum{}2 + 117*x\textasciicircum{}3 + 27*x\textasciicircum{}4 - 36*x\textasciicircum{}5 - 203*x\textasciicircum{}6 - 134*x\textasciicircum{}7 + 28*x\textasciicircum{}8 + 120*x\textasciicircum{}9 + 16*x\textasciicircum{}10) / (1 - x - 9*x\textasciicircum{}3 - 2*x\textasciicircum{}4 - 2*x\textasciicircum{}5 + 12*x\textasciicircum{}6 + 8*x\textasciicircum{}7 - 8*x\textasciicircum{}9). - Colin Barker, Mar 05 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Mar 05 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = a(n-1) + 9\,a(n-3) + 2\,a(n-4) + 2\,a(n-5) - 12\,a(n-6) - 8\,a(n-7) + \\ 8\,a(n-9). \end{multline*}

stated for $n > 11$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 30x^{2} + 54x^{3} + 117x^{4} + 27x^{5} - 36x^{6} - 203x^{7} - 134x^{8} + 28x^{9} + 120x^{10} + 16x^{11}}{1 - x - 9x^{3} - 2x^{4} - 2x^{5} + 12x^{6} + 8x^{7} - 8x^{9}}.\]
...
That refinement leaves $18$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 11$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A207171, the recurrence $a(n) = a(n-1) + 9\,a(n-3) + 2\,a(n-4) + 2\,a(n-5) - 12\,a(n-6) - 8\,a(n-7) + 8\,a(n-9)$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,53$. The last nonvanishing one is at $m = 2$, that is at $n = 11$. After it, more than $S = 11$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 11$. The theorem is at least as strong.

\begin{theorem} For A207171, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 30x^{2} + 54x^{3} + 117x^{4} + 27x^{5} - 36x^{6} - 203x^{7} - 134x^{8} + 28x^{9} + 120x^{10} + 16x^{11}}{1 - x - 9x^{3} - 2x^{4} - 2x^{5} + 12x^{6} + 8x^{7} - 8x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 90, 261, 624, 1482, 3808, 9512,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207171}{A207171}, revision 9, last modified Mar 05 2018.
