...
\title{A proof of the conjectured linear recurrence and generating function and closed form for OEIS A207401}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207401}{A207401} as

\begin{quote}\itshape Number of n X 6 0..1 arrays avoiding 0 0 1 and 0 1 1 horizontally and 0 0 1 and 1 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = (1/9)*n\textasciicircum{}6 + (4/3)*n\textasciicircum{}5 + (58/9)*n\textasciicircum{}4 + (40/3)*n\textasciicircum{}3 + (49/9)*n\textasciicircum{}2 - (44/3)*n + 4.\end{quote}

\begin{quote}\ttfamily G.f.: 4*x*(4 + 36*x - 40*x\textasciicircum{}2 + 25*x\textasciicircum{}3 - 3*x\textasciicircum{}4 - 3*x\textasciicircum{}5 + x\textasciicircum{}6) / (1 - x)\textasciicircum{}7.\end{quote}
...
\[a(n) = 4 - \frac{44}{3}n + \frac{49}{9}n^{2} + \frac{40}{3}n^{3} + \frac{58}{9}n^{4} + \frac{4}{3}n^{5} + \frac{1}{9}n^{6}.\]

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{16x + 144x^{2} - 160x^{3} + 100x^{4} - 12x^{5} - 12x^{6} + 4x^{7}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}.\]
...
The construction gives $4160$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $272$; neither removal changes any count.
...
For scale: at $n = 8$ there are $2^{48}$ arrays, about $10^{14}$, against $12$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A207401, $a(n)$ is the polynomial $4 - \frac{44}{3}n + \frac{49}{9}n^{2} + \frac{40}{3}n^{3} + \frac{58}{9}n^{4} + \frac{4}{3}n^{5} + \frac{1}{9}n^{6}$ for every $n \ge 1$.\end{theorem}

{\itshape Proof.} A polynomial of degree $6$ is annihilated by the difference operator $(E-1)^{7}$, whose characteristic polynomial is $(t-1)^{7}$. So $a(n) - P(n)$, with $P$ the claimed polynomial, satisfies the linear recurrence whose characteristic polynomial is the product of that with the characteristic polynomial of $L$, of degree $S + 7 = 19$. Its values were computed exactly; all of them vanish, and more than $19$ consecutive vanishing values follow, so every later one vanishes as well. $\square$

\begin{theorem} For A207401, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{16x + 144x^{2} - 160x^{3} + 100x^{4} - 12x^{5} - 12x^{6} + 4x^{7}}{1 - 7x + 21x^{2} - 35x^{3} + 35x^{4} - 21x^{5} + 7x^{6} - x^{7}}$.\end{theorem}
...
\begin{theorem} For A207401, the recurrence $a(n) = 7\,a(n-1) - 21\,a(n-2) + 35\,a(n-3) - 35\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 16, 256, 1296, 4356, 11664, 26896, 55696, 106276,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207401}{A207401}, revision 8, last modified Jun 22 2018.
