...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207363}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207363}{A207363} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 0 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 2*a(n-2) - 6*a(n-3) + 6*a(n-5) - 2*a(n-6) - 2*a(n-7) + a(n-8).\end{quote}

\begin{quote}\ttfamily G.f.: x*(6 + 24*x + 6*x\textasciicircum{}2 + 9*x\textasciicircum{}3 + 6*x\textasciicircum{}4 - 2*x\textasciicircum{}5 - 2*x\textasciicircum{}6 + x\textasciicircum{}7)/ ((1 - x)\textasciicircum{}5*(1 + x)\textasciicircum{}3).\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 24x^{2} + 6x^{3} + 9x^{4} + 6x^{5} - 2x^{6} - 2x^{7} + x^{8}}{1 - 2x - 2x^{2} + 6x^{3} - 6x^{5} + 2x^{6} + 2x^{7} - x^{8}}.\]
...
\begin{theorem} For A207363, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) - 6\,a(n-3) + 6\,a(n-5) - 2\,a(n-6) - 2\,a(n-7) + a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A207363, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 24x^{2} + 6x^{3} + 9x^{4} + 6x^{5} - 2x^{6} - 2x^{7} + x^{8}}{1 - 2x - 2x^{2} + 6x^{3} - 6x^{5} + 2x^{6} + 2x^{7} - x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 36 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 90, 225, 420, 784, 1260, 2025,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207363}{A207363}, revision 8, last modified Feb 21 2018.
