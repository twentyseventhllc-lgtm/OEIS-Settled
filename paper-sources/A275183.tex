...
\title{Proofs of the conjectured column and row recurrences for OEIS A275183}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A275183}{A275183} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (-2,-1) (-1,0) or (-1,1) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 2*a(n-1) for n>2\\k=2: a(n) = 2*a(n-1) -a(n-2) +a(n-3)\\k=3: a(n) = 4*a(n-1) -6*a(n-2) +5*a(n-3) -a(n-4) -a(n-5) for n>8\\k=4: [order 9] for n>12\\k=5: [order 16] for n>21\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 19 2016). As of revision 4, last modified Jul 19 2016, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A275183, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n > 2$, and fails at $n = 2$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A275183, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $28$ states, $28$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A275183, the recurrence $a(n) = 4\,a(n-1) - 6\,a(n-2) + 5\,a(n-3) - a(n-4) - a(n-5)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $76$ states, $76$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 8$; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 8$; the proved threshold is $n > 8$.

\begin{theorem} For column $k = 4$ of A275183, the recurrence $a(n) = 0$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $220$ states, $220$ after trimming, and $S = 18$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 12$; after that point more than $S = 18$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 12$; the proved threshold is $n > 12$.

\begin{theorem} For column $k = 5$ of A275183, the recurrence $a(n) = 0$ holds for every $n > 21$, and fails at $n = 21$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $678$ states, $678$ after trimming, and $S = 33$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 21$; after that point more than $S = 33$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A275183}{A275183}, revision 4, last modified Jul 19 2016.
