...
\title{Proofs of the conjectured column and row recurrences for OEIS A255091}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A255091}{A255091} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 2 and no row sum 0 and no column sum 0\end{quote}
...
k=1: [linear recurrence of order 20] for n>23\\k=2: [order 80] for n>84\\k=3: [order 34] for n>39\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 14 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A255091, the recurrence $a(n) = 0$ holds for every $n > 23$, and fails at $n = 23$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 23$; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 23$; the proved threshold is $n > 23$.

\begin{theorem} For column $k = 2$ of A255091, the recurrence $a(n) = 0$ holds for every $n > 84$, and fails at $n = 84$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 96$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 84$; after that point more than $S = 96$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 84$; the proved threshold is $n > 84$.

\begin{theorem} For column $k = 3$ of A255091, the recurrence $a(n) = 0$ holds for every $n > 39$, and fails at $n = 39$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 85$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 39$; after that point more than $S = 85$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A255091}{A255091}, revision 6, last modified Jun 02 2025.
