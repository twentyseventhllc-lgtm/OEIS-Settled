...
\title{Proofs of the conjectured column and row recurrences for OEIS A318016}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318016}{A318016} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 2 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +a(n-3) -a(n-4)\\k=3: [order 11] for n>14\\k=4: [order 26] for n>29\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Aug 12 2018). As of revision 4, last modified Aug 12 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A318016, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A318016, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A318016, the recurrence $a(n) = 0$ holds for every $n > 14$, and fails at $n = 14$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $59$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 14$; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 14$; the proved threshold is $n > 14$.

\begin{theorem} For column $k = 4$ of A318016, the recurrence $a(n) = 0$ holds for every $n > 29$, and fails at $n = 29$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $189$ after trimming, and $S = 33$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 29$; after that point more than $S = 33$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318016}{A318016}, revision 4, last modified Aug 12 2018.
