...
\title{Proofs of the conjectured column and row recurrences for OEIS A234898}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234898}{A234898} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 30*a(n-2) -194*a(n-4) +428*a(n-6) -288*a(n-8) +48*a(n-10).\\k=2: [order 51].\end{quote}
...
\begin{theorem} For column $k = 1$ of A234898, the recurrence $a(n) = 30\,a(n-2) - 194\,a(n-4) + 428\,a(n-6) - 288\,a(n-8) + 48\,a(n-10)$ holds for every $n \ge 11$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A234898, the recurrence $a(n) = 0$ holds for every $n \ge 52$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $343$ states, $252$ after trimming, and $S = 61$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 61$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234898}{A234898}, revision 6, last modified Jun 20 2022.
