...
\title{Proofs of the conjectured column and row recurrences for OEIS A234911}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234911}{A234911} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 55*a(n-2) -725*a(n-4) +3663*a(n-6) -7525*a(n-8) +6335*a(n-10) -1805*a(n-12).\\k=2: [order 87].\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 01 2014). As of revision 6, last modified Jun 20 2022, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A234911, the recurrence $a(n) = 55\,a(n-2) - 725\,a(n-4) + 3663\,a(n-6) - 7525\,a(n-8) + 6335\,a(n-10) - 1805\,a(n-12)$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $64$ states, $56$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A234911, the recurrence $a(n) = 0$ holds for every $n \ge 88$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $512$ states, $392$ after trimming, and $S = 100$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 100$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234911}{A234911}, revision 6, last modified Jun 20 2022.
