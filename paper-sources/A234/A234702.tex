...
\title{Proofs of the conjectured column and row recurrences for OEIS A234702}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234702}{A234702} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5 (constant-stress 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 6*a(n-1) +90*a(n-2) -540*a(n-3) -2016*a(n-4) +12096*a(n-5).\\k=2: [order 15].\end{quote}
...
\begin{theorem} For column $k = 1$ of A234702, the recurrence $a(n) = 6\,a(n-1) + 90\,a(n-2) - 540\,a(n-3) - 2016\,a(n-4) + 12096\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $64$ states, $64$ after trimming, and $S = 5$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 5$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A234702, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $512$ states, $512$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234702}{A234702}, revision 6, last modified Jun 20 2022.
