...
\title{Proofs of the conjectured column and row recurrences for OEIS A234333}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234333}{A234333} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 32*a(n-2) -48*a(n-4).\\k=2: [order 11].\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 23 2013). As of revision 6, last modified Jun 20 2022, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A234333, the recurrence $a(n) = 32\,a(n-2) - 48\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A234333, the recurrence $a(n) = 0$ holds for every $n \ge 12$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $64$ states, $64$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234333}{A234333}, revision 6, last modified Jun 20 2022.
