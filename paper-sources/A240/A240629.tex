...
\title{Proofs of the conjectured column and row recurrences for OEIS A240629}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A240629}{A240629} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..3 arrays with no element equal to exactly two horizontal and vertical neighbors, with new values 0..3 introduced in row major order\end{quote}
...
k=1: a(n) = 4*a(n-1) +a(n-2) -6*a(n-3) -3*a(n-4) for n>6\\k=2: [order 10] for n>11\\k=3: [order 46]\end{quote}
...
\begin{theorem} For column $k = 1$ of A240629, the recurrence $a(n) = 4\,a(n-1) + a(n-2) - 6\,a(n-3) - 3\,a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $62$ states, $32$ after trimming, and $S = 7$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 6$; after that point more than $S = 7$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 6$; the proved threshold is $n > 6$.

\begin{theorem} For column $k = 2$ of A240629, the recurrence $a(n) = 0$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $700$ states, $347$ after trimming, and $S = 12$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 12$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 11$; the proved threshold is $n > 11$.

\begin{theorem} For column $k = 3$ of A240629, the recurrence $a(n) = 0$ holds for every $n \ge 47$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $9784$ states, $4895$ after trimming, and $S = 48$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 48$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A240629}{A240629}, revision 6, last modified Jul 23 2025.
