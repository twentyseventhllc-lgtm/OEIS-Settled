...
\title{Proofs of the conjectured column and row recurrences for OEIS A240774}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A240774}{A240774} as

\begin{quote}\itshape T(n,k) = Number of n X k 0..3 arrays with no element equal to exactly one horizontal or vertical neighbor, with new values 0..3 introduced in row major order\end{quote}
...
k=1: a(n) = 4*a(n-1) -3*a(n-2) for n>3,\\k=3: [order 34] for n>35.\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 12 2014). As of revision 6, last modified Jan 02 2022, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A240774, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A240774, the recurrence $a(n) = 0$ holds for every $n > 35$, and fails at $n = 35$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $9735$ states, $4846$ after trimming, and $S = 37$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 35$; after that point more than $S = 37$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A240774}{A240774}, revision 6, last modified Jan 02 2022.
