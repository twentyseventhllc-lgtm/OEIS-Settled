...
\title{Proofs of the conjectured column and row recurrences for OEIS A252605}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A252605}{A252605} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 1 2 or 5\end{quote}
...
k=1: [linear recurrence of order 43] for n>47\\k=2: [order 66] for n>69\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 19 2014). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A252605, the recurrence $a(n) = 0$ holds for every $n > 47$, and fails at $n = 47$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $756$ states, $756$ after trimming, and $S = 58$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 47$; after that point more than $S = 58$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 47$; the proved threshold is $n > 47$.

\begin{theorem} For column $k = 2$ of A252605, the recurrence $a(n) = 0$ holds for every $n > 69$, and fails at $n = 69$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $6642$ states, $6642$ after trimming, and $S = 87$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 69$; after that point more than $S = 87$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A252605}{A252605}, revision 6, last modified Jun 02 2025.
