...
\title{Proofs of the conjectured column and row recurrences for OEIS A260001}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260001}{A260001} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000001 00000011 or 00000101\end{quote}
...
k=1: a(n) = a(n-1) +5*a(n-3) +7*a(n-4) -7*a(n-5) -5*a(n-7)\\k=2: [order 14]\\k=3: [order 19] for n>22\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 12 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A260001, the recurrence $a(n) = a(n-1) + 5\,a(n-3) + 7\,a(n-4) - 7\,a(n-5) - 5\,a(n-7)$ holds for every $n \ge 8$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A260001, the recurrence $a(n) = 0$ holds for every $n \ge 15$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A260001, the recurrence $a(n) = 0$ holds for every $n > 22$, and fails at $n = 22$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 38$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 22$; after that point more than $S = 38$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260001}{A260001}, revision 6, last modified Jun 02 2025.
