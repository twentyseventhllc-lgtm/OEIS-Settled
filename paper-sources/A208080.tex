...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208080}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208080}{A208080} as

\begin{quote}\itshape Number of 5 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 0 1 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) +40*a(n-2) +24*a(n-3) -70*a(n-4) -28*a(n-5) +36*a(n-6) -a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 15*x*(1 - x)*(1 + 12*x + 31*x\textasciicircum{}2 + 2*x\textasciicircum{}3 - 25*x\textasciicircum{}4 + 2*x\textasciicircum{}5 + x\textasciicircum{}6) / (1 - 4*x - 40*x\textasciicircum{}2 - 24*x\textasciicircum{}3 + 70*x\textasciicircum{}4 + 28*x\textasciicircum{}5 - 36*x\textasciicircum{}6 + x\textasciicircum{}8). - Colin Barker, Jan 20 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 10, last modified Jan 20 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 4\,a(n-1) + 40\,a(n-2) + 24\,a(n-3) - 70\,a(n-4) - 28\,a(n-5) + 36\,a(n-6) - \\ a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{15x + 165x^{2} + 285x^{3} - 435x^{4} - 405x^{5} + 405x^{6} - 15x^{7} - 15x^{8}}{1 - 4x - 40x^{2} - 24x^{3} + 70x^{4} + 28x^{5} - 36x^{6} + x^{8}}.\]
...
\begin{theorem} For A208080, the recurrence $a(n) = 4\,a(n-1) + 40\,a(n-2) + 24\,a(n-3) - 70\,a(n-4) - 28\,a(n-5) + 36\,a(n-6) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,41$. Every one of them vanished. After it, more than $S = 9$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A208080, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{15x + 165x^{2} + 285x^{3} - 435x^{4} - 405x^{5} + 405x^{6} - 15x^{7} - 15x^{8}}{1 - 4x - 40x^{2} - 24x^{3} + 70x^{4} + 28x^{5} - 36x^{6} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 18 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 15, 225, 1785, 16065, 139605, 1228095, 10751415, 94313535,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208080}{A208080}, revision 10, last modified Jan 20 2018.
