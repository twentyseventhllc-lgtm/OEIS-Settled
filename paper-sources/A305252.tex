...
\title{Proofs of the conjectured column and row recurrences for OEIS A305252}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305252}{A305252} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 3, 4 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3) -4*a(n-4) for n>5\\k=3: [order 12]\\k=4: [order 26] for n>30\\k=5: [order 54] for n>59\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 28 2018). As of revision 4, last modified May 28 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A305252, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A305252, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 4\,a(n-3) - 4\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A305252, the recurrence $a(n) = 0$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $45$ after trimming, and $S = 14$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 14$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A305252, the recurrence $a(n) = 0$ holds for every $n > 30$, and fails at $n = 30$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $118$ after trimming, and $S = 36$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 30$; after that point more than $S = 36$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 30$; the proved threshold is $n > 30$.

\begin{theorem} For column $k = 5$ of A305252, the recurrence $a(n) = 0$ holds for every $n > 59$, and fails at $n = 59$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $309$ after trimming, and $S = 67$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 59$; after that point more than $S = 67$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305252}{A305252}, revision 4, last modified May 28 2018.
