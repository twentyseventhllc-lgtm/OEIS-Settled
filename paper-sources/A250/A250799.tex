...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250799}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250799}{A250799} as

\begin{quote}\itshape Number of (2+1) X (n+1) 0..1 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 4*a(n-2) - a(n-3) + 2*a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(12 - 15*x - 3*x\textasciicircum{}2 + 8*x\textasciicircum{}3) / ((1 - x)*(1 - 2*x)*(1 - x - x\textasciicircum{}2)). - Colin Barker, Nov 20 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{24x - 30x^{2} - 6x^{3} + 16x^{4}}{1 - 4x + 4x^{2} + x^{3} - 2x^{4}}.\]
...
\begin{theorem} For A250799, the recurrence $a(n) = 4\,a(n-1) - 4\,a(n-2) - a(n-3) + 2\,a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250799, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{24x - 30x^{2} - 6x^{3} + 16x^{4}}{1 - 4x + 4x^{2} + x^{3} - 2x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 24, 66, 162, 376, 838, 1818, 3868, 8114,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250799}{A250799}, revision 7, last modified Nov 20 2018.
