...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250578}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250578}{A250578} as

\begin{quote}\itshape Number of (n+1) X (3+1) 0..1 arrays with nondecreasing max(x(i,j),x(i,j-1)) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) - 6*a(n-2) - 7*a(n-3) + 21*a(n-4) - 14*a(n-5) - a(n-6) + 4*a(n-7) - a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(55 - 133*x - 44*x\textasciicircum{}2 + 335*x\textasciicircum{}3 - 256*x\textasciicircum{}4 - 10*x\textasciicircum{}5 + 68*x\textasciicircum{}6 - 17*x\textasciicircum{}7) / ((1 - x)\textasciicircum{}3*(1 - 2*x - 3*x\textasciicircum{}2 + 5*x\textasciicircum{}3 + x\textasciicircum{}4 - x\textasciicircum{}5)). - Colin Barker, Nov 15 2018\end{quote}
...
\begin{multline*} a(n) = 5\,a(n-1) - 6\,a(n-2) - 7\,a(n-3) + 21\,a(n-4) - 14\,a(n-5) - a(n-6) + \\ 4\,a(n-7) - a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{55x - 133x^{2} - 44x^{3} + 335x^{4} - 256x^{5} - 10x^{6} + 68x^{7} - 17x^{8}}{1 - 5x + 6x^{2} + 7x^{3} - 21x^{4} + 14x^{5} + x^{6} - 4x^{7} + x^{8}}.\]
...
\begin{theorem} For A250578, the recurrence $a(n) = 5\,a(n-1) - 6\,a(n-2) - 7\,a(n-3) + 21\,a(n-4) - 14\,a(n-5) - a(n-6) + 4\,a(n-7) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250578, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{55x - 133x^{2} - 44x^{3} + 335x^{4} - 256x^{5} - 10x^{6} + 68x^{7} - 17x^{8}}{1 - 5x + 6x^{2} + 7x^{3} - 21x^{4} + 14x^{5} + x^{6} - 4x^{7} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 55, 142, 336, 778, 1779, 4077, 9346, 21510,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250578}{A250578}, revision 7, last modified Nov 15 2018.
