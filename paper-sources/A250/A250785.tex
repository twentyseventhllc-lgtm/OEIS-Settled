...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250785}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250785}{A250785} as

\begin{quote}\itshape Number of (3+1) X (n+1) 0..1 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) - 8*a(n-2) + 3*a(n-3) + 3*a(n-4) - 2*a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(18 - 42*x + 19*x\textasciicircum{}2 + 22*x\textasciicircum{}3 - 16*x\textasciicircum{}4) / ((1 - x)\textasciicircum{}2*(1 - 2*x)*(1 - x - x\textasciicircum{}2)). - Colin Barker, Nov 19 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Nov 27 2014).

The line quoted ending in that signature is contributed by Colin Barker on Nov 19 2018.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{36x - 84x^{2} + 38x^{3} + 44x^{4} - 32x^{5}}{1 - 5x + 8x^{2} - 3x^{3} - 3x^{4} + 2x^{5}}.\]
...
\begin{theorem} For A250785, the recurrence $a(n) = 5\,a(n-1) - 8\,a(n-2) + 3\,a(n-3) + 3\,a(n-4) - 2\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250785, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{36x - 84x^{2} + 38x^{3} + 44x^{4} - 32x^{5}}{1 - 5x + 8x^{2} - 3x^{3} - 3x^{4} + 2x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 36, 96, 230, 534, 1194, 2604, 5568, 11732,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250785}{A250785}, revision 8, last modified Nov 19 2018.
