...
\title{Proofs of the conjectured column and row recurrences for OEIS A250536}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250536}{A250536} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with nondecreasing max(x(i,j),x(i,j-1)) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}
...
k=1: [linear recurrence of order 16]\\k=2: [order 29]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 24 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A250536, the recurrence $a(n) = 0$ holds for every $n \ge 17$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A250536, the recurrence $a(n) = 0$ holds for every $n \ge 30$, at every index at which it can be stated.\end{theorem}
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250536}{A250536}, revision 6, last modified Jul 23 2025.
