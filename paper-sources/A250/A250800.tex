...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250800}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250800}{A250800} as

\begin{quote}\itshape Number of (3+1) X (n+1) 0..1 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: 2*x*(29 - 55*x + 23*x\textasciicircum{}2 + 23*x\textasciicircum{}3 - 16*x\textasciicircum{}4) / ((1 - x)\textasciicircum{}2*(1 - 2*x)*(1 - x - x\textasciicircum{}2)). - Colin Barker, Nov 20 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{58x - 110x^{2} + 46x^{3} + 46x^{4} - 32x^{5}}{1 - 5x + 8x^{2} - 3x^{3} - 3x^{4} + 2x^{5}}.\]
...
That refinement leaves $12$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 5$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A250800, the recurrence $a(n) = 5\,a(n-1) - 8\,a(n-2) + 3\,a(n-3) + 3\,a(n-4) - 2\,a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250800, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{58x - 110x^{2} + 46x^{3} + 46x^{4} - 32x^{5}}{1 - 5x + 8x^{2} - 3x^{3} - 3x^{4} + 2x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 58, 180, 482, 1190, 2776, 6230, 13598, 29084,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250800}{A250800}, revision 7, last modified Nov 20 2018.
