...
\title{Proofs of the conjectured column and row recurrences for OEIS A250783}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250783}{A250783} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction\end{quote}
...
k=3: a(n) = 4*a(n-1) -5*a(n-2) +2*a(n-3)\\k=4: a(n) = 6*a(n-1) -14*a(n-2) +16*a(n-3) -9*a(n-4) +2*a(n-5)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 27 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 3$ of A250783, the recurrence $a(n) = 4\,a(n-1) - 5\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A250783, the recurrence $a(n) = 6\,a(n-1) - 14\,a(n-2) + 16\,a(n-3) - 9\,a(n-4) + 2\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $32$ states, $32$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250783}{A250783}, revision 6, last modified Jul 23 2025.
