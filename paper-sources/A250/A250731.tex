...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250731}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250731}{A250731} as

\begin{quote}\itshape Number of (2+1) X (n+1) 0..1 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 2*a(n-2) - 3*a(n-3) for n>4.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(18 + 10*x - 18*x\textasciicircum{}2 - x\textasciicircum{}3) / ((1 - x)*(1 - x - 3*x\textasciicircum{}2)). - Colin Barker, Nov 16 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{18x + 10x^{2} - 18x^{3} - x^{4}}{1 - 2x - 2x^{2} + 3x^{3}}.\]
...
\begin{theorem} For A250731, the recurrence $a(n) = 2\,a(n-1) + 2\,a(n-2) - 3\,a(n-3)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,44$. The last nonvanishing one is at $m = 1$, that is at $n = 4$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 4$. The theorem is at least as strong.

\begin{theorem} For A250731, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{18x + 10x^{2} - 18x^{3} - x^{4}}{1 - 2x - 2x^{2} + 3x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 18, 46, 110, 257, 596, 1376, 3173, 7310,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250731}{A250731}, revision 7, last modified Nov 16 2018.
