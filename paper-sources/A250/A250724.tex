...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250724}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250724}{A250724} as

\begin{quote}\itshape Number of (n+1) X (3+1) 0..1 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - 5*a(n-2) + 5*a(n-4) - 4*a(n-5) + a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(50 - 90*x + 18*x\textasciicircum{}2 + 83*x\textasciicircum{}3 - 70*x\textasciicircum{}4 + 17*x\textasciicircum{}5) / ((1 - x)\textasciicircum{}5*(1 + x)). - Colin Barker, Nov 16 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{50x - 90x^{2} + 18x^{3} + 83x^{4} - 70x^{5} + 17x^{6}}{1 - 4x + 5x^{2} - 5x^{4} + 4x^{5} - x^{6}}.\]
...
\begin{theorem} For A250724, the recurrence $a(n) = 4\,a(n-1) - 5\,a(n-2) + 5\,a(n-4) - 4\,a(n-5) + a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250724, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{50x - 90x^{2} + 18x^{3} + 83x^{4} - 70x^{5} + 17x^{6}}{1 - 4x + 5x^{2} - 5x^{4} + 4x^{5} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 36 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 50, 110, 208, 365, 600, 942, 1418, 2065,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250724}{A250724}, revision 7, last modified Nov 16 2018.
