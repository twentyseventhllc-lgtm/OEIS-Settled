...
\title{Proofs of the conjectured column and row recurrences for OEIS A256810}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A256810}{A256810} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 0 or 1 and no column sum 0 or 1\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2) +7*a(n-3) +7*a(n-4) +3*a(n-5) -a(n-6) -6*a(n-7) -2*a(n-8)\\k=2: [order 14]\\k=3: [order 28]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 10 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A256810, the recurrence $a(n) = a(n-1) + a(n-2) + 7\,a(n-3) + 7\,a(n-4) + 3\,a(n-5) - a(n-6) - 6\,a(n-7) - 2\,a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A256810, the recurrence $a(n) = 0$ holds for every $n \ge 15$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A256810, the recurrence $a(n) = 0$ holds for every $n \ge 29$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A256810}{A256810}, revision 6, last modified Jun 02 2025.
