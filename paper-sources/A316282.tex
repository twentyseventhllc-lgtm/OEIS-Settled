...
\title{Proofs of the conjectured column and row recurrences for OEIS A316282}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A316282}{A316282} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3, 4, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +3*a(n-2) +2*a(n-3)\\k=3: [order 9] for n>10\\k=4: [order 27] for n>28\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jun 28 2018). As of revision 4, last modified Jun 28 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A316282, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A316282, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) + 2\,a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A316282, the recurrence $a(n) = 0$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A316282, the recurrence $a(n) = 0$ holds for every $n > 28$, and fails at $n = 28$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 28$; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A316282}{A316282}, revision 4, last modified Jun 28 2018.
