...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207249}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207249}{A207249} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 1 and 1 0 1 horizontally and 0 1 0 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 4*a(n-2) + 7*a(n-4) + 8*a(n-5) - a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(3 + 12*x + 3*x\textasciicircum{}2 + 11*x\textasciicircum{}3 + 21*x\textasciicircum{}4 - 3*x\textasciicircum{}5 - 3*x\textasciicircum{}7) / (1 - 2*x - 4*x\textasciicircum{}2 - 7*x\textasciicircum{}4 - 8*x\textasciicircum{}5 + x\textasciicircum{}8). - Colin Barker, Feb 20 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 16 2012).

The line quoted ending in that signature is contributed by Colin Barker on Feb 20 2018.
...
\[a(n) = 2\,a(n-1) + 4\,a(n-2) + 7\,a(n-4) + 8\,a(n-5) - a(n-8).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 24x^{2} + 6x^{3} + 22x^{4} + 42x^{5} - 6x^{6} - 6x^{8}}{1 - 2x - 4x^{2} - 7x^{4} - 8x^{5} + x^{8}}.\]
...
\begin{theorem} For A207249, the recurrence $a(n) = 2\,a(n-1) + 4\,a(n-2) + 7\,a(n-4) + 8\,a(n-5) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A207249, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 24x^{2} + 6x^{3} + 22x^{4} + 42x^{5} - 6x^{6} - 6x^{8}}{1 - 2x - 4x^{2} - 7x^{4} - 8x^{5} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 102, 370, 1232, 4238, 14406, 49164,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207249}{A207249}, revision 8, last modified Feb 20 2018.
