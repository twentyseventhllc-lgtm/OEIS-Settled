...
\title{A proof of the conjectured linear recurrence for OEIS A281059}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281059}{A281059} as

\begin{quote}\itshape Number of 4Xn 0..1 arrays with no element equal to more than one of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 6*a(n-1) -12*a(n-2) +5*a(n-3) +12*a(n-4) -12*a(n-5) -3*a(n-6) +6*a(n-7) -a(n-9) for n>17\end{quote}
...
The reachable part of the digraph has $793$ states, $375$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 71$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of that $71\times71$ matrix; the bound is computed, not assumed.
...
\begin{theorem} For A281059, the recurrence $a(n) = 6\,a(n-1) - 12\,a(n-2) + 5\,a(n-3) + 12\,a(n-4) - 12\,a(n-5) - 3\,a(n-6) + 6\,a(n-7) - a(n-9)$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,114$. The last nonvanishing one is at $m = 8$, that is at $n = 17$. After it, more than $S = 71$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 29, 376, 1022, 2452, 5400, 10570, 19892,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281059}{A281059}, revision 4, last modified Jan 13 2017.
