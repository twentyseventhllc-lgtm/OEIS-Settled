...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196917}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196917}{A196917} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,3,2,4,0 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +a(n-3) -a(n-4) for n>5.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + x + x\textasciicircum{}2 - 3*x\textasciicircum{}3 + x\textasciicircum{}4) / (1 - 2*x - x\textasciicircum{}3 + x\textasciicircum{}4). - Colin Barker, May 09 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + x^{2} + x^{3} - 3x^{4} + x^{5}}{1 - 2x - x^{3} + x^{4}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 1, \tau(1) = 3, \tau(2) = 2, \tau(3) = 4, \tau(4) = 0. The condition is that for every cell,
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $18$; neither removal changes any count.
...
\begin{theorem} For A196917, the recurrence $a(n) = 2\,a(n-1) + a(n-3) - a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,47$. The last nonvanishing one is at $m = 1$, that is at $n = 5$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 5$. The theorem is at least as strong.

\begin{theorem} For A196917, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + x^{2} + x^{3} - 3x^{4} + x^{5}}{1 - 2x - x^{3} + x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 7, 12, 27, 58, 121, 257,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196917}{A196917}, revision 9, last modified May 09 2018.
