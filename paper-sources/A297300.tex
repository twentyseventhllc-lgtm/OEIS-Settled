...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297300}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297300}{A297300} as

\begin{quote}\itshape Number of 3 X n 0..1 arrays with every 1 horizontally or antidiagonally adjacent to 2 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) - a(n-2) + 2*a(n-3) - a(n-4) for n>6.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 - x)*(1 + x\textasciicircum{}2)\textasciicircum{}2 / (1 - 2*x + x\textasciicircum{}2 - 2*x\textasciicircum{}3 + x\textasciicircum{}4). - Colin Barker, Feb 26 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x - x^{2} + 2x^{3} - 2x^{4} + x^{5} - x^{6}}{1 - 2x + x^{2} - 2x^{3} + x^{4}}.\]
...
The construction gives $72$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $20$; neither removal changes any count.
...
\begin{theorem} For A297300, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 2\,a(n-3) - a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 2$, that is at $n = 6$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 6$. The theorem is at least as strong.

\begin{theorem} For A297300, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x - x^{2} + 2x^{3} - 2x^{4} + x^{5} - x^{6}}{1 - 2x + x^{2} - 2x^{3} + x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 1, 3, 5, 9, 17, 32, 60,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297300}{A297300}, revision 7, last modified Feb 26 2019.
