...
\title{A proof of the conjectured linear recurrence for OEIS A318540}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A318540}{A318540} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 4 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-3) +a(n-4) +3*a(n-5) +a(n-7) for n>8\end{quote}
...
\[a(n) = a(n-1) + 3\,a(n-3) + a(n-4) + 3\,a(n-5) + a(n-7).\]
...
\begin{theorem} For A318540, the recurrence $a(n) = a(n-1) + 3\,a(n-3) + a(n-4) + 3\,a(n-5) + a(n-7)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,54$. The last nonvanishing one is at $m = 1$, that is at $n = 8$. After it, more than $S = 9$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 32 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 3, 6, 13, 26, 52, 107, 218,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A318540}{A318540}, revision 4, last modified Aug 28 2018.
