...
\title{Proofs of the conjectured column and row recurrences for OEIS A303690}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303690}{A303690} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 2, 3 or 4 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 3*a(n-1) +a(n-2) +2*a(n-3) -2*a(n-4) -4*a(n-5) for n>6\\k=3: [order 16] for n>18\\k=4: [order 31] for n>35\\k=5: [order 93] for n>97\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 28 2018). As of revision 4, last modified Apr 28 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} In A303690, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A303690, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 2\,a(n-3) - 2\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A303690, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $64$ after trimming, and $S = 18$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 18$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 18$; the proved threshold is $n > 18$.

\begin{theorem} For column $k = 4$ of A303690, the recurrence $a(n) = 0$ holds for every $n > 35$, and fails at $n = 35$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $153$ after trimming, and $S = 39$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 35$; after that point more than $S = 39$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 35$; the proved threshold is $n > 35$.

\begin{theorem} For column $k = 5$ of A303690, the recurrence $a(n) = 0$ holds for every $n > 97$, and fails at $n = 97$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $518$ after trimming, and $S = 110$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 97$; after that point more than $S = 110$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303690}{A303690}, revision 4, last modified Apr 28 2018.
