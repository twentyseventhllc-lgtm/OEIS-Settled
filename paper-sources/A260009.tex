...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A260009}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260009}{A260009} as

\begin{quote}\itshape Number of (n+2) X (2+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00000101 00010101 or 01010101\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + a(n-3) + 3*a(n-4) - 5*a(n-5) - a(n-6) - a(n-8) + a(n-9) for n>10.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(49 + 56*x\textasciicircum{}2 + 25*x\textasciicircum{}3 - 169*x\textasciicircum{}4 - 27*x\textasciicircum{}5 - 9*x\textasciicircum{}6 - 20*x\textasciicircum{}7 + 33*x\textasciicircum{}8 - x\textasciicircum{}9) / ((1 - 2*x - x\textasciicircum{}2 + x\textasciicircum{}3)*(1 + x\textasciicircum{}2 - 2*x\textasciicircum{}4 - x\textasciicircum{}6)). - Colin Barker, Dec 27 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Dec 27 2018, the entry records no proof and links to none, so the statement is open. This note settles it.

Written out, the claim is

\begin{multline*} a(n) = 2\,a(n-1) + a(n-3) + 3\,a(n-4) - 5\,a(n-5) - a(n-6) - a(n-8) + \\ a(n-9). \end{multline*}

stated for $n > 10$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{49x + 56x^{3} + 25x^{4} - 169x^{5} - 27x^{6} - 9x^{7} - 20x^{8} + 33x^{9} - x^{10}}{1 - 2x - x^{3} - 3x^{4} + 5x^{5} + x^{6} + x^{8} - x^{9}}.\]
...
\begin{theorem} For A260009, the recurrence $a(n) = 2\,a(n-1) + a(n-3) + 3\,a(n-4) - 5\,a(n-5) - a(n-6) - a(n-8) + a(n-9)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}
...
\begin{theorem} For A260009, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{49x + 56x^{3} + 25x^{4} - 169x^{5} - 27x^{6} - 9x^{7} - 20x^{8} + 33x^{9} - x^{10}}{1 - 2x - x^{3} - 3x^{4} + 5x^{5} + x^{6} + x^{8} - x^{9}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 49, 98, 252, 578, 1232, 2738, 6262, 14112,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260009}{A260009}, revision 9, last modified Dec 27 2018.
