...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A243601}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A243601}{A243601} as

\begin{quote}\itshape Number of length n+2 0..4 arrays with no three elements in a row with pattern aba (with a!=b) and new values 0..4 introduced in 0..4 order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 8*a(n-1) - 18*a(n-2) + 5*a(n-3) + 17*a(n-4) - 4*a(n-5) - 6*a(n-6) - a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(4 - 22*x + 21*x\textasciicircum{}2 + 24*x\textasciicircum{}3 - 15*x\textasciicircum{}4 - 13*x\textasciicircum{}5 - 2*x\textasciicircum{}6) / ((1 - x)*(1 - x - x\textasciicircum{}2)*(1 - 2*x - x\textasciicircum{}2)*(1 - 4*x - x\textasciicircum{}2)). - Colin Barker, Nov 02 2018\end{quote}
...
\begin{multline*} a(n) = 8\,a(n-1) - 18\,a(n-2) + 5\,a(n-3) + 17\,a(n-4) - 4\,a(n-5) - 6\,a(n-6) - \\ a(n-7). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x - 22x^{2} + 21x^{3} + 24x^{4} - 15x^{5} - 13x^{6} - 2x^{7}}{1 - 8x + 18x^{2} - 5x^{3} - 17x^{4} + 4x^{5} + 6x^{6} + x^{7}}.\]
...
\begin{theorem} For A243601, the recurrence $a(n) = 8\,a(n-1) - 18\,a(n-2) + 5\,a(n-3) + 17\,a(n-4) - 4\,a(n-5) - 6\,a(n-6) - a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A243601, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x - 22x^{2} + 21x^{3} + 24x^{4} - 15x^{5} - 13x^{6} - 2x^{7}}{1 - 8x + 18x^{2} - 5x^{3} - 17x^{4} + 4x^{5} + 6x^{6} + x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 10, 29, 96, 349, 1350, 5425, 22297,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A243601}{A243601}, revision 8, last modified Nov 02 2018.
