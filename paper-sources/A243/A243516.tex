...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A243516}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A243516}{A243516} as

\begin{quote}\itshape Number of length n+2 0..7 arrays with no three elements in a row with pattern aba or abb (with a!=b) and new values 0..7 introduced in 0..7 order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 17*a(n-1) - 111*a(n-2) + 355*a(n-3) - 584*a(n-4) + 468*a(n-5) - 144*a(n-6).\end{quote}

\begin{quote}\ttfamily G.f.: x*(3 - 46*x + 258*x\textasciicircum{}2 - 655*x\textasciicircum{}3 + 739*x\textasciicircum{}4 - 288*x\textasciicircum{}5) / ((1 - x)\textasciicircum{}2*(1 - 2*x)*(1 - 3*x)*(1 - 4*x)*(1 - 6*x)).\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{3x - 46x^{2} + 258x^{3} - 655x^{4} + 739x^{5} - 288x^{6}}{1 - 17x + 111x^{2} - 355x^{3} + 584x^{4} - 468x^{5} + 144x^{6}}.\]
...
The reachable part of that digraph has $167$ states, $167$ after trimming. Identifying states with the same number of completions of every length, and then the mirror identification on the edges coming in, leaves $S = 8$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of the $8\times8$ quotient matrix; that degree bound is computed here, not assumed.
...
\begin{theorem} For A243516, the recurrence $a(n) = 17\,a(n-1) - 111\,a(n-2) + 355\,a(n-3) - 584\,a(n-4) + 468\,a(n-5) - 144\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A243516, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{3x - 46x^{2} + 258x^{3} - 655x^{4} + 739x^{5} - 288x^{6}}{1 - 17x + 111x^{2} - 355x^{3} + 584x^{4} - 468x^{5} + 144x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 5, 10, 25, 77, 280, 1156, 5267,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A243516}{A243516}, revision 8, last modified Nov 02 2018.
