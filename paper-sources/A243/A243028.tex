...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A243028}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A243028}{A243028} as

\begin{quote}\itshape Number of length n+3 0..3 arrays with no four elements in a row with pattern abba (possibly a=b) and new values 0..3 introduced in 0..3 order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 4*a(n-1) - a(n-2) + 2*a(n-3) - 6*a(n-4) - 2*a(n-5) - 3*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(13 - 10*x - 8*x\textasciicircum{}2 - 36*x\textasciicircum{}3 - 16*x\textasciicircum{}4 - 14*x\textasciicircum{}5) / ((1 - x - x\textasciicircum{}3)*(1 - 3*x - 2*x\textasciicircum{}2 - 3*x\textasciicircum{}3)). - Colin Barker, Nov 01 2018\end{quote}
...
\[a(n) = 4\,a(n-1) - a(n-2) + 2\,a(n-3) - 6\,a(n-4) - 2\,a(n-5) - 3\,a(n-6).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{13x - 10x^{2} - 8x^{3} - 36x^{4} - 16x^{5} - 14x^{6}}{1 - 4x + x^{2} - 2x^{3} + 6x^{4} + 2x^{5} + 3x^{6}}.\]
...
The reachable part of that digraph has $104$ states, $104$ after trimming. Identifying states with the same number of completions of every length, and then the mirror identification on the edges coming in, leaves $S = 10$ blocks. The count satisfies the linear recurrence given by the characteristic polynomial of the $10\times10$ quotient matrix; that degree bound is computed here, not assumed.
...
\begin{theorem} For A243028, the recurrence $a(n) = 4\,a(n-1) - a(n-2) + 2\,a(n-3) - 6\,a(n-4) - 2\,a(n-5) - 3\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A243028, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{13x - 10x^{2} - 8x^{3} - 36x^{4} - 16x^{5} - 14x^{6}}{1 - 4x + x^{2} - 2x^{3} + 6x^{4} + 2x^{5} + 3x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 13, 42, 147, 536, 1987, 7414, 27736, 103868,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A243028}{A243028}, revision 8, last modified Nov 01 2018.
