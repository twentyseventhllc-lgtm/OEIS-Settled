...
\title{Proofs of the conjectured column and row recurrences for OEIS A298635}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298635}{A298635} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 2, 3, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 3*a(n-2) +2*a(n-3) -2*a(n-4) for n>5\\k=3: [order 11] for n>13\\k=4: [order 23] for n>29\\k=5: [order 58] for n>64\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 23 2018). As of revision 4, last modified Jan 23 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A298635, the recurrence $a(n) = a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A298635, the recurrence $a(n) = 3\,a(n-2) + 2\,a(n-3) - 2\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A298635, the recurrence $a(n) = 0$ holds for every $n > 13$, and fails at $n = 13$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A298635, the recurrence $a(n) = 0$ holds for every $n > 29$, and fails at $n = 29$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $124$ after trimming, and $S = 35$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 29$; after that point more than $S = 35$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 29$; the proved threshold is $n > 29$.

\begin{theorem} For column $k = 5$ of A298635, the recurrence $a(n) = 0$ holds for every $n > 64$, and fails at $n = 64$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $364$ after trimming, and $S = 75$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 64$; after that point more than $S = 75$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 64$; the proved threshold is $n > 64$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298635}{A298635}, revision 4, last modified Jan 23 2018.
