...
\title{Proofs of the conjectured column and row recurrences for OEIS A296952}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296952}{A296952} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with each 1 adjacent to 1, 3 or 5 king-move neighboring 1s\end{quote}
...
k=1: a(n) = a(n-1) +a(n-3)\\k=2: a(n) = 2*a(n-1) +8*a(n-3) -12*a(n-4)\\k=3: [order 19]\\k=4: [order 46]\end{quote}
...
\begin{theorem} For column $k = 1$ of A296952, the recurrence $a(n) = a(n-1) + a(n-3)$ holds for every $n \ge 4$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A296952, the recurrence $a(n) = 2\,a(n-1) + 8\,a(n-3) - 12\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A296952, the recurrence $a(n) = 0$ holds for every $n \ge 20$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 19$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 19$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A296952, the recurrence $a(n) = 0$ holds for every $n \ge 47$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $272$ after trimming, and $S = 49$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 49$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296952}{A296952}, revision 4, last modified Dec 22 2017.
