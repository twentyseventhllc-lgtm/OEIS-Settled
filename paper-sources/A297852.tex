...
\title{A proof of the conjectured recurrence and generating function for OEIS A297852}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297852}{A297852} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with every element equal to 1, 2 or 4 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) - 2*a(n-2) + 4*a(n-3) - 10*a(n-4) + 4*a(n-5) for n>6.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 - 6*x\textasciicircum{}3 - 4*x\textasciicircum{}4 + 4*x\textasciicircum{}5) / ((1 - 2*x)*(1 - x - 4*x\textasciicircum{}3 + 2*x\textasciicircum{}4)). - Colin Barker, Feb 19 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 13, last modified Feb 20 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x - 6x^{4} - 4x^{5} + 4x^{6}}{1 - 3x + 2x^{2} - 4x^{3} + 10x^{4} - 4x^{5}}.\]
...
\begin{theorem} For A297852, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) + 4\,a(n-3) - 10\,a(n-4) + 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 1$, that is at $n = 6$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 6$. The theorem is at least as strong.

\begin{theorem} For A297852, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x - 6x^{4} - 4x^{5} + 4x^{6}}{1 - 3x + 2x^{2} - 4x^{3} + 10x^{4} - 4x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 33 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 7, 13, 23, 49, 95, 177,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297852}{A297852}, revision 13, last modified Feb 20 2018.
