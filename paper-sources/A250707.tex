...
\title{Proofs of the conjectured column and row recurrences for OEIS A250707}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250707}{A250707} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction\end{quote}
...
k=1: [linear recurrence of order 16]\\k=2: [order 30]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Nov 26 2014). As of revision 6, last modified Jul 23 2025, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A250707, the recurrence $a(n) = 0$ holds for every $n \ge 17$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $16$ states, $16$ after trimming, and $S = 16$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 16$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A250707, the recurrence $a(n) = 0$ holds for every $n \ge 31$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $64$ states, $64$ after trimming, and $S = 41$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 41$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250707}{A250707}, revision 6, last modified Jul 23 2025.
