...
\title{A proof of the conjectured order of the recurrence for OEIS A320405}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A320405}{A320405} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 4 or 5 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
That refinement leaves $115$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 95$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\item {\bfseries The published terms.} The walk count reproduces all 21 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 35, 156, 786, 3761, 18725, 92578, 463045,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A320405}{A320405}, revision 4, last modified Oct 12 2018.
