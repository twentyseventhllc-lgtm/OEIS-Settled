...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208504}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208504}{A208504} as

\begin{quote}\itshape Number of 6 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 1 0 and 1 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 7*a(n-2) - 11*a(n-3) - 4*a(n-4) + 4*a(n-5) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 19*x*(1 + 17*x - 21*x\textasciicircum{}2 - 96*x\textasciicircum{}3 + 113*x\textasciicircum{}4 + 52*x\textasciicircum{}5 - 40*x\textasciicircum{}6) / (1 - 2*x - 7*x\textasciicircum{}2 + 11*x\textasciicircum{}3 + 4*x\textasciicircum{}4 - 4*x\textasciicircum{}5). - Colin Barker, Jul 03 2018\end{quote}
...
The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 9, last modified Jul 03 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{19x + 323x^{2} - 399x^{3} - 1824x^{4} + 2147x^{5} + 988x^{6} - 760x^{7}}{1 - 2x - 7x^{2} + 11x^{3} + 4x^{4} - 4x^{5}}.\]
...
\begin{theorem} For A208504, the recurrence $a(n) = 2\,a(n-1) + 7\,a(n-2) - 11\,a(n-3) - 4\,a(n-4) + 4\,a(n-5)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,43$. The last nonvanishing one is at $m = 2$, that is at $n = 7$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A208504, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{19x + 323x^{2} - 399x^{3} - 1824x^{4} + 2147x^{5} + 988x^{6} - 760x^{7}}{1 - 2x - 7x^{2} + 11x^{3} + 4x^{4} - 4x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 19, 361, 456, 1406, 4104, 12654, 37430, 114494,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208504}{A208504}, revision 9, last modified Jul 03 2018.
