...
\title{A proof of the conjectured linear recurrence for OEIS A218593}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A218593}{A218593} as

\begin{quote}\itshape Hilltop maps: number of 2Xn binary arrays indicating the locations of corresponding elements not exceeded by any horizontal or antidiagonal neighbor in a random 0..2 2Xn array\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) +3*a(n-2) +3*a(n-3) +a(n-4) +5*a(n-5) +2*a(n-6) +2*a(n-7) +a(n-8) +a(n-9) -a(n-10) -a(n-11) -a(n-12) -a(n-13) for n>14\end{quote}
...
\begin{multline*} a(n) = 3\,a(n-1) + 3\,a(n-2) + 3\,a(n-3) + a(n-4) + 5\,a(n-5) + 2\,a(n-6) + \\ 2\,a(n-7) + a(n-8) + a(n-9) - a(n-10) - a(n-11) - a(n-12) - \\ a(n-13). \end{multline*}
...
The reachable part of the subset automaton has $44$ states, $44$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 15$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of that $15\times15$ matrix. The bound is computed, not assumed.
...
\begin{theorem} For A218593, the recurrence $a(n) = 3\,a(n-1) + 3\,a(n-2) + 3\,a(n-3) + a(n-4) + 5\,a(n-5) + 2\,a(n-6) + 2\,a(n-7) + a(n-8) + a(n-9) - a(n-10) - a(n-11) - a(n-12) - a(n-13)$ holds for every $n > 14$, and fails at $n = 14$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,57$. The last nonvanishing one is at $m = 1$, that is at $n = 14$. After it, more than $S = 15$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 13, 57, 225, 895, 3565, 14189, 56489,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A218593}{A218593}, revision 8, last modified Oct 03 2025.
