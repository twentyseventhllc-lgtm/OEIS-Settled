...
\title{A proof of the conjectured recurrence and generating function for OEIS A298282}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298282}{A298282} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element equal to 0, 1, 2, 3 or 6 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) - a(n-2) + 2*a(n-3) + 4*a(n-4) - a(n-5) + 2*a(n-6) - 2*a(n-7) - 3*a(n-8) - 2*a(n-9) - 2*a(n-10) - 2*a(n-11) for n>12.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(4 + 13*x - x\textasciicircum{}2 + 12*x\textasciicircum{}3 + 3*x\textasciicircum{}4 - 13*x\textasciicircum{}5 - 8*x\textasciicircum{}6 - 19*x\textasciicircum{}7 - 13*x\textasciicircum{}8 - 7*x\textasciicircum{}9 - 2*x\textasciicircum{}10 - 4*x\textasciicircum{}11) / (1 - 3*x + x\textasciicircum{}2 - 2*x\textasciicircum{}3 - 4*x\textasciicircum{}4 + x\textasciicircum{}5 - 2*x\textasciicircum{}6 + 2*x\textasciicircum{}7 + 3*x\textasciicircum{}8 + 2*x\textasciicircum{}9 + 2*x\textasciicircum{}10 + 2*x\textasciicircum{}11). - Colin Barker, Feb 26 2018\end{quote}
...
\begin{multline*} a(n) = 3\,a(n-1) - a(n-2) + 2\,a(n-3) + 4\,a(n-4) - a(n-5) + 2\,a(n-6) - \\ 2\,a(n-7) - 3\,a(n-8) - 2\,a(n-9) - 2\,a(n-10) - 2\,a(n-11). \end{multline*}

stated for $n > 12$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x + 13x^{2} - x^{3} + 12x^{4} + 3x^{5} - 13x^{6} - 8x^{7} - 19x^{8} - 13x^{9} - 7x^{10} - 2x^{11} - 4x^{12}}{1 - 3x + x^{2} - 2x^{3} - 4x^{4} + x^{5} - 2x^{6} + 2x^{7} + 3x^{8} + 2x^{9} + 2x^{10} + 2x^{11}}.\]
...
Here the refinement leaves $S = 14$ blocks. Writing $L$ for the $S\times S$ quotient matrix, $\tilde w$ for the start weights summed over each block and $\tilde f$ for the acceptance indicator,
...
\begin{theorem} For A298282, the recurrence $a(n) = 3\,a(n-1) - a(n-2) + 2\,a(n-3) + 4\,a(n-4) - a(n-5) + 2\,a(n-6) - 2\,a(n-7) - 3\,a(n-8) - 2\,a(n-9) - 2\,a(n-10) - 2\,a(n-11)$ holds for every $n > 12$, and fails at $n = 12$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,56$. The last nonvanishing one is at $m = 1$, that is at $n = 12$. After it, more than $S = 14$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 12$. The theorem is at least as strong.

\begin{theorem} For A298282, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x + 13x^{2} - x^{3} + 12x^{4} + 3x^{5} - 13x^{6} - 8x^{7} - 19x^{8} - 13x^{9} - 7x^{10} - 2x^{11} - 4x^{12}}{1 - 3x + x^{2} - 2x^{3} - 4x^{4} + x^{5} - 2x^{6} + 2x^{7} + 3x^{8} + 2x^{9} + 2x^{10} + 2x^{11}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 25, 70, 205, 614, 1860, 5631, 17034,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298282}{A298282}, revision 8, last modified Feb 26 2018.
