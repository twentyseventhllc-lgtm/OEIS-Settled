...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A295914}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A295914}{A295914} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with each 1 adjacent to 0 or 3 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 7*a(n-2) + 6*a(n-3) - 3*a(n-4) - 4*a(n-5) + a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(8 + 12*x + 3*x\textasciicircum{}2 - 7*x\textasciicircum{}3 - 3*x\textasciicircum{}4 + x\textasciicircum{}5) / ((1 + x)*(1 - 3*x - 4*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + 5*x\textasciicircum{}4 - x\textasciicircum{}5)). - Colin Barker, Feb 22 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{8x + 12x^{2} + 3x^{3} - 7x^{4} - 3x^{5} + x^{6}}{1 - 2x - 7x^{2} - 6x^{3} + 3x^{4} + 4x^{5} - x^{6}}.\]
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $55$; neither removal changes any count.
...
\begin{theorem} For A295914, the recurrence $a(n) = 2\,a(n-1) + 7\,a(n-2) + 6\,a(n-3) - 3\,a(n-4) - 4\,a(n-5) + a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A295914, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{8x + 12x^{2} + 3x^{3} - 7x^{4} - 3x^{5} + x^{6}}{1 - 2x - 7x^{2} - 6x^{3} + 3x^{4} + 4x^{5} - x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 28, 115, 467, 1880, 7604, 30721, 124117,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A295914}{A295914}, revision 8, last modified Feb 22 2019.
