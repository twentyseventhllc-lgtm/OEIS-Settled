...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A250606}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A250606}{A250606} as

\begin{quote}\itshape Number of (n+1) X (3+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)-x(i-1,j) in the j direction\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 5*a(n-1) - 9*a(n-2) + 7*a(n-3) - 2*a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(57 - 145*x + 110*x\textasciicircum{}2 - 32*x\textasciicircum{}3) / ((1 - x)\textasciicircum{}3*(1 - 2*x)). - Colin Barker, Nov 15 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{57x - 145x^{2} + 110x^{3} - 32x^{4}}{1 - 5x + 9x^{2} - 7x^{3} + 2x^{4}}.\]
...
\begin{theorem} For A250606, the recurrence $a(n) = 5\,a(n-1) - 9\,a(n-2) + 7\,a(n-3) - 2\,a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A250606, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{57x - 145x^{2} + 110x^{3} - 32x^{4}}{1 - 5x + 9x^{2} - 7x^{3} + 2x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 57, 140, 297, 592, 1153, 2236, 4353, 8528,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A250606}{A250606}, revision 7, last modified Nov 15 2018.
