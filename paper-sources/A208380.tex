...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A208380}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A208380}{A208380} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 1 0 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 4*a(n-2) + 9*a(n-3) + 5*a(n-4) - 2*a(n-5) + 4*a(n-6) for n>8.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 4*x*(2 + 14*x + 3*x\textasciicircum{}2 - 4*x\textasciicircum{}3 - 31*x\textasciicircum{}4 - 22*x\textasciicircum{}5 + 12*x\textasciicircum{}6 - 16*x\textasciicircum{}7) / ((1 - 2*x - 4*x\textasciicircum{}2)*(1 + x + 2*x\textasciicircum{}2 - x\textasciicircum{}3 + x\textasciicircum{}4)). - Colin Barker, Jul 02 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{8x + 56x^{2} + 12x^{3} - 16x^{4} - 124x^{5} - 88x^{6} + 48x^{7} - 64x^{8}}{1 - x - 4x^{2} - 9x^{3} - 5x^{4} + 2x^{5} - 4x^{6}}.\]
...
\begin{theorem} For A208380, the recurrence $a(n) = a(n-1) + 4\,a(n-2) + 9\,a(n-3) + 5\,a(n-4) - 2\,a(n-5) + 4\,a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,44$. The last nonvanishing one is at $m = 2$, that is at $n = 8$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 8$. The theorem is at least as strong.

\begin{theorem} For A208380, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{8x + 56x^{2} + 12x^{3} - 16x^{4} - 124x^{5} - 88x^{6} + 48x^{7} - 64x^{8}}{1 - x - 4x^{2} - 9x^{3} - 5x^{4} + 2x^{5} - 4x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 64, 108, 420, 1344, 4212, 13860, 44880,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A208380}{A208380}, revision 8, last modified Jul 02 2018.
