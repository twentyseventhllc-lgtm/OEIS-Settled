...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A299592}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299592}{A299592} as

\begin{quote}\itshape Number of n X 5 0..1 arrays with every element unequal to 0, 1 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +3*a(n-3) +a(n-4) -a(n-5) -a(n-6) for n>9.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(8 + 3*x\textasciicircum{}2 + 2*x\textasciicircum{}3 + 9*x\textasciicircum{}4 + 33*x\textasciicircum{}5 + 10*x\textasciicircum{}6 - 14*x\textasciicircum{}7 - 10*x\textasciicircum{}8) / ((1 + x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - x - 2*x\textasciicircum{}2 - x\textasciicircum{}3)). - Colin Barker, Feb 15 2018\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Feb 13 2018).

The line quoted ending in that signature is contributed by Colin Barker on Feb 15 2018.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 11, last modified Feb 15 2018, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{8x + 3x^{3} + 2x^{4} + 9x^{5} + 33x^{6} + 10x^{7} - 14x^{8} - 10x^{9}}{1 - x - x^{2} - 3x^{3} - x^{4} + x^{5} + x^{6}}.\]
...
The construction gives $1056$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $33$; neither removal changes any count.
...
For scale: at $n = 8$ there are $2^{40}$ arrays, about $10^{12}$, against $13$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A299592, the recurrence $a(n) = a(n-1) + a(n-2) + 3\,a(n-3) + a(n-4) - a(n-5) - a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,54$. The last nonvanishing one is at $m = 3$, that is at $n = 9$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A299592, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{8x + 3x^{3} + 2x^{4} + 9x^{5} + 33x^{6} + 10x^{7} - 14x^{8} - 10x^{9}}{1 - x - x^{2} - 3x^{3} - x^{4} + x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 8, 19, 53, 113, 256, 541, 1148,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299592}{A299592}, revision 11, last modified Feb 15 2018.
