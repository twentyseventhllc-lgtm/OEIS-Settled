...
\title{A proof of the conjectured linear recurrence for OEIS A303678}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303678}{A303678} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-4) +a(n-5) for n>8\end{quote}
...
\begin{theorem} For A303678, the recurrence $a(n) = a(n-1) + a(n-4) + a(n-5)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,58$. The last nonvanishing one is at $m = 3$, that is at $n = 8$. After it, more than $S = 10$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 17, 15, 28, 44, 64, 90, 132,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303678}{A303678}, revision 4, last modified Apr 28 2018.
