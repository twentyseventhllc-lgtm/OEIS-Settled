...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196479}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196479}{A196479} as

\begin{quote}\itshape Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,0,2,1,4 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 4*a(n-3) - 3*a(n-4) - 3*a(n-5) - a(n-6) + a(n-7) -a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(3 + 2*x\textasciicircum{}2 - 4*x\textasciicircum{}3 - 4*x\textasciicircum{}4 - 3*x\textasciicircum{}5 + 3*x\textasciicircum{}6 - 3*x\textasciicircum{}7) / (1 - 2*x - 4*x\textasciicircum{}3 + 3*x\textasciicircum{}4 + 3*x\textasciicircum{}5 + x\textasciicircum{}6 - x\textasciicircum{}7 + x\textasciicircum{}8). - Colin Barker, Mar 01 2018\end{quote}
...
\begin{multline*} a(n) = 2\,a(n-1) + 4\,a(n-3) - 3\,a(n-4) - 3\,a(n-5) - a(n-6) + a(n-7) - \\ a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{3x + 2x^{3} - 4x^{4} - 4x^{5} - 3x^{6} + 3x^{7} - 3x^{8}}{1 - 2x - 4x^{3} + 3x^{4} + 3x^{5} + x^{6} - x^{7} + x^{8}}.\]
...
The construction gives $650$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $34$; neither removal changes any count.
...
\begin{theorem} For A196479, the recurrence $a(n) = 2\,a(n-1) + 4\,a(n-3) - 3\,a(n-4) - 3\,a(n-5) - a(n-6) + a(n-7) - a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,51$. Every one of them vanished. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A196479, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{3x + 2x^{3} - 4x^{4} - 4x^{5} - 3x^{6} + 3x^{7} - 3x^{8}}{1 - 2x - 4x^{3} + 3x^{4} + 3x^{5} + x^{6} - x^{7} + x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 6, 14, 36, 83, 192, 468, 1112,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196479}{A196479}, revision 9, last modified Mar 01 2018.
