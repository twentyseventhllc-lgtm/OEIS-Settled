...
\title{Proofs of the conjectured column and row recurrences for OEIS A262472}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A262472}{A262472} as

\begin{quote}\itshape T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each row divisible by 3 and each column divisible by 5, read as a binary number with top and left being the most significant bits\end{quote}
...
k=1: a(n) = 3*a(n-1) -3*a(n-2) +3*a(n-3) -2*a(n-4)\\k=2: [order 8]\\k=3: [order 15]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Sep 23 2015). As of revision 4, last modified Sep 23 2015, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A262472, the recurrence $a(n) = 3\,a(n-1) - 3\,a(n-2) + 3\,a(n-3) - 2\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A262472, the recurrence $a(n) = 0$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $25$ states, $25$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A262472, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $125$ states, $125$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item 1 line: degree bound too large for the minimal-order search
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A262472}{A262472}, revision 4, last modified Sep 23 2015.
