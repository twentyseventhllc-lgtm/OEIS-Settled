...
\title{Proofs of the conjectured column and row recurrences for OEIS A255228}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A255228}{A255228} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 2 and no column sum 2\end{quote}
...
k=1: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4) for n>5\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 17 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
\begin{theorem} For column $k = 1$ of A255228, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + a(n-3) - a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 10$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 10$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A255228}{A255228}, revision 6, last modified Jun 02 2025.
