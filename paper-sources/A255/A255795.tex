...
\title{A proof of the conjectured linear recurrence for OEIS A255795}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A255795}{A255795} as

\begin{quote}\itshape Number of (n+2)X(2+2) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 2 and no row sum 0 and no column sum 3\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-2) +a(n-4) +5*a(n-6) -2*a(n-8) -2*a(n-10) -7*a(n-12) +a(n-14) +a(n-16) +3*a(n-18) for n>22\end{quote}
...
\begin{multline*} a(n) = a(n-2) + a(n-4) + 5\,a(n-6) - 2\,a(n-8) - 2\,a(n-10) - 7\,a(n-12) + \\ a(n-14) + a(n-16) + 3\,a(n-18). \end{multline*}
...
That refinement leaves $58$ blocks. The mirror reduction is then applied: two blocks receiving the same weight from the start, for every walk length, also contribute identically, and the refinement that finds them looks at edges coming in rather than going out. Writing $L$ for the resulting $S \times S$ matrix with $S = 36$, $\tilde w$ for the start weights and $\tilde f$ for the acceptance weights,
...
\begin{theorem} For A255795, the recurrence $a(n) = a(n-2) + a(n-4) + 5\,a(n-6) - 2\,a(n-8) - 2\,a(n-10) - 7\,a(n-12) + a(n-14) + a(n-16) + 3\,a(n-18)$ holds for every $n > 22$, and fails at $n = 22$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,94$. The last nonvanishing one is at $m = 4$, that is at $n = 22$. After it, more than $S = 36$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 134, 166, 178, 262, 340, 472, 600, 856,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A255795}{A255795}, revision 6, last modified Jun 02 2025.
