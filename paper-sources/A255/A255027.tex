...
\title{Proofs of the conjectured column and row recurrences for OEIS A255027}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A255027}{A255027} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 or 1 and no antidiagonal sum 0 or 1 and no row sum 0 or 1 and no column sum 0 or 1\end{quote}
...
k=1: a(n) = a(n-1) +3*a(n-3) +4*a(n-4) +4*a(n-5) +2*a(n-6) for n>7\\k=2: [order 8] for n>9\\k=3: [order 15] for n>17\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 13 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A255027, the recurrence $a(n) = a(n-1) + 3\,a(n-3) + 4\,a(n-4) + 4\,a(n-5) + 2\,a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $72$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 7$; the proved threshold is $n > 7$.

\begin{theorem} For column $k = 2$ of A255027, the recurrence $a(n) = 0$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A255027, the recurrence $a(n) = 0$ holds for every $n > 17$, and fails at $n = 17$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 21$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 17$; after that point more than $S = 21$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A255027}{A255027}, revision 6, last modified Jun 02 2025.
