...
\title{A proof of the conjectured linear recurrence for OEIS A304216}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304216}{A304216} as

\begin{quote}\itshape Number of n X 3 0..1 arrays with every element unequal to 0, 1, 3 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-3) +2*a(n-4) for n>5.\end{quote}
...
\begin{theorem} For A304216, the recurrence $a(n) = a(n-1) + a(n-3) + 2\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 1$, that is at $n = 5$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 34 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 7, 10, 17, 32, 56, 93, 159,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304216}{A304216}, revision 7, last modified Sep 30 2023.
