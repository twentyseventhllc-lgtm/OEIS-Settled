...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1)\\k=3: a(n) = 8*a(n-1) -2*a(n-2) +5*a(n-3) -13*a(n-4) -6*a(n-5)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 28 2018). As of revision 4, last modified Feb 28 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
