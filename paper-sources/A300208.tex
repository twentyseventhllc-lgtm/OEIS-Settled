...
\title{Proofs of the conjectured column and row recurrences for OEIS A300208}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A300208}{A300208} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 3, 4, 5, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1)\\k=3: a(n) = 8*a(n-1) -2*a(n-2) +5*a(n-3) -13*a(n-4) -6*a(n-5)\\k=4: [order 15]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 28 2018). As of revision 4, last modified Feb 28 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A300208, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A300208, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A300208, the recurrence $a(n) = 8\,a(n-1) - 2\,a(n-2) + 5\,a(n-3) - 13\,a(n-4) - 6\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A300208, the recurrence $a(n) = 0$ holds for every $n \ge 16$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $264$ after trimming, and $S = 15$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 15$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A300208}{A300208}, revision 4, last modified Feb 28 2018.
