...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A206931}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A206931}{A206931} as

\begin{quote}\itshape Number of n X 3 0..1 arrays avoiding 0 0 0 horizontally and 0 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 6*a(n-1) - 9*a(n-2) - 6*a(n-3) + 18*a(n-4) - 9*a(n-6) + a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(7 + 7*x - 20*x\textasciicircum{}2 + 28*x\textasciicircum{}3 - 8*x\textasciicircum{}4 - 8*x\textasciicircum{}5 + x\textasciicircum{}6 + x\textasciicircum{}7) / ((1 - x)*(1 + x)*(1 - 3*x + x\textasciicircum{}2)*(1 - x - x\textasciicircum{}2)*(1 - 2*x - x\textasciicircum{}2)). - Colin Barker, Jun 17 2018\end{quote}
...
\[a(n) = 6\,a(n-1) - 9\,a(n-2) - 6\,a(n-3) + 18\,a(n-4) - 9\,a(n-6) + a(n-8).\]
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{7x + 7x^{2} - 20x^{3} + 28x^{4} - 8x^{5} - 8x^{6} + x^{7} + x^{8}}{1 - 6x + 9x^{2} + 6x^{3} - 18x^{4} + 9x^{6} - x^{8}}.\]
...
\begin{theorem} For A206931, the recurrence $a(n) = 6\,a(n-1) - 9\,a(n-2) - 6\,a(n-3) + 18\,a(n-4) - 9\,a(n-6) + a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A206931, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{7x + 7x^{2} - 20x^{3} + 28x^{4} - 8x^{5} - 8x^{6} + x^{7} + x^{8}}{1 - 6x + 9x^{2} + 6x^{3} - 18x^{4} + 9x^{6} - x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 7, 49, 211, 811, 2791, 9055, 28081, 84403,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A206931}{A206931}, revision 11, last modified Jun 17 2018.
