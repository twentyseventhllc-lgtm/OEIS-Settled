...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A260241}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260241}{A260241} as

\begin{quote}\itshape Number of (n+2) X (1+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00000001 00000101 or 00001001\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) - 2*a(n-2) + 9*a(n-3) - 4*a(n-4) + 9*a(n-5) - 9*a(n-6) for n>7.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(48 + 3*x + 154*x\textasciicircum{}2 - 111*x\textasciicircum{}3 + 141*x\textasciicircum{}4 - 239*x\textasciicircum{}5 + 102*x\textasciicircum{}6) / (1 - 2*x + 2*x\textasciicircum{}2 - 9*x\textasciicircum{}3 + 4*x\textasciicircum{}4 - 9*x\textasciicircum{}5 + 9*x\textasciicircum{}6). - Colin Barker, Dec 28 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{48x + 3x^{2} + 154x^{3} - 111x^{4} + 141x^{5} - 239x^{6} + 102x^{7}}{1 - 2x + 2x^{2} - 9x^{3} + 4x^{4} - 9x^{5} + 9x^{6}}.\]
...
\begin{theorem} For A260241, the recurrence $a(n) = 2\,a(n-1) - 2\,a(n-2) + 9\,a(n-3) - 4\,a(n-4) + 9\,a(n-5) - 9\,a(n-6)$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,52$. The last nonvanishing one is at $m = 1$, that is at $n = 7$. After it, more than $S = 15$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 7$. The theorem is at least as strong.

\begin{theorem} For A260241, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{48x + 3x^{2} + 154x^{3} - 111x^{4} + 141x^{5} - 239x^{6} + 102x^{7}}{1 - 2x + 2x^{2} - 9x^{3} + 4x^{4} - 9x^{5} + 9x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 25 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 48, 99, 256, 635, 1598, 4027, 10110, 25421,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260241}{A260241}, revision 7, last modified Dec 28 2018.
