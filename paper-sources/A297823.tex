...
\title{Proofs of the conjectured column and row recurrences for OEIS A297823}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297823}{A297823} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 1, 2 or 3 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 3*a(n-1) +a(n-2) +2*a(n-3) -2*a(n-4) -4*a(n-5) for n>6\\k=3: [order 9] for n>11\\k=4: [order 18] for n>21\\k=5: [order 49] for n>55\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 06 2018). As of revision 4, last modified Jan 06 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A297823, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A297823, the recurrence $a(n) = 3\,a(n-1) + a(n-2) + 2\,a(n-3) - 2\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A297823, the recurrence $a(n) = 0$ holds for every $n > 11$, and fails at $n = 11$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $50$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 11$; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 11$; the proved threshold is $n > 11$.

\begin{theorem} For column $k = 4$ of A297823, the recurrence $a(n) = 0$ holds for every $n > 21$, and fails at $n = 21$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $138$ after trimming, and $S = 32$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 21$; after that point more than $S = 32$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 21$; the proved threshold is $n > 21$.

\begin{theorem} For column $k = 5$ of A297823, the recurrence $a(n) = 0$ holds for every $n > 55$, and fails at $n = 55$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $399$ after trimming, and $S = 64$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 55$; after that point more than $S = 64$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 55$; the proved threshold is $n > 55$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297823}{A297823}, revision 4, last modified Jan 06 2018.
