...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A260277}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A260277}{A260277} as

\begin{quote}\itshape Number of (n+2) X (1+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00000000 00000001 or 00001011\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 3*a(n-3) + a(n-4) + a(n-8) for n>10.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(34 + 12*x + 53*x\textasciicircum{}2 + x\textasciicircum{}3 - 8*x\textasciicircum{}4 + 5*x\textasciicircum{}5 + 9*x\textasciicircum{}6 + 22*x\textasciicircum{}7 - x\textasciicircum{}8 - x\textasciicircum{}9) / (1 - x - 3*x\textasciicircum{}3 - x\textasciicircum{}4 - x\textasciicircum{}8). - Colin Barker, Dec 28 2018\end{quote}
...
\[a(n) = a(n-1) + 3\,a(n-3) + a(n-4) + a(n-8).\]

stated for $n > 10$.

Written out, the claim is

\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{34x + 12x^{2} + 53x^{3} + x^{4} - 8x^{5} + 5x^{6} + 9x^{7} + 22x^{8} - x^{9} - x^{10}}{1 - x - 3x^{3} - x^{4} - x^{8}}.\]
...
\begin{theorem} For A260277, the recurrence $a(n) = a(n-1) + 3\,a(n-3) + a(n-4) + a(n-8)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,56$. The last nonvanishing one is at $m = 2$, that is at $n = 10$. After it, more than $S = 13$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 10$. The theorem is at least as strong.

\begin{theorem} For A260277, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{34x + 12x^{2} + 53x^{3} + x^{4} - 8x^{5} + 5x^{6} + 9x^{7} + 22x^{8} - x^{9} - x^{10}}{1 - x - 3x^{3} - x^{4} - x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 29 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 34, 46, 99, 202, 366, 714, 1428, 2750,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A260277}{A260277}, revision 7, last modified Dec 28 2018.
