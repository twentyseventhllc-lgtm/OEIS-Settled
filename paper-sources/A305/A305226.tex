...
\title{A proof of the conjectured linear recurrence for OEIS A305226}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305226}{A305226} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 3 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-2) +a(n-3) -a(n-4) -3*a(n-5) -2*a(n-6) +2*a(n-7) for n>9\end{quote}
...
\begin{multline*} a(n) = a(n-1) + 3\,a(n-2) + a(n-3) - a(n-4) - 3\,a(n-5) - 2\,a(n-6) + \\ 2\,a(n-7). \end{multline*}
...
The construction gives $272$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $88$; neither removal changes any count.
...
\begin{theorem} For A305226, the recurrence $a(n) = a(n-1) + 3\,a(n-2) + a(n-3) - a(n-4) - 3\,a(n-5) - 2\,a(n-6) + 2\,a(n-7)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,53$. The last nonvanishing one is at $m = 2$, that is at $n = 9$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 49, 54, 116, 253, 541, 1248, 2831,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305226}{A305226}, revision 4, last modified May 27 2018.
