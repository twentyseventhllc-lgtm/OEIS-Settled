...
\title{Proofs of the conjectured column and row recurrences for OEIS A305089}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305089}{A305089} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 1, 2, 3, 6 or 7 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 2*a(n-1) +3*a(n-2) -2*a(n-3) -6*a(n-4) -4*a(n-5) for n>6\\k=3: [order 14] for n>16\\k=4: [order 19] for n>24\\k=5: [order 97] for n>101\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (May 25 2018). As of revision 4, last modified May 25 2018, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A305089, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A305089, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A305089, the recurrence $a(n) = 0$ holds for every $n > 16$, and fails at $n = 16$.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A305089, the recurrence $a(n) = 0$ holds for every $n > 24$, and fails at $n = 24$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $162$ after trimming, and $S = 30$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 24$; after that point more than $S = 30$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 24$; the proved threshold is $n > 24$.

\begin{theorem} For column $k = 5$ of A305089, the recurrence $a(n) = 0$ holds for every $n > 101$, and fails at $n = 101$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $600$ after trimming, and $S = 118$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 101$; after that point more than $S = 118$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 101$; the proved threshold is $n > 101$.
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305089}{A305089}, revision 4, last modified May 25 2018.
