...
\title{Proofs of the conjectured column and row recurrences for OEIS A305347}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305347}{A305347} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element unequal to 0, 1, 3, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = a(n-1) +8*a(n-3) -4*a(n-4)\\k=3: [order 12]\\k=4: [order 36] for n>37\\k=5: [order 37] for n>48\end{quote}
...
The condition is $c(i,j) \in \{0, 1, 3, 5, 8\}$ for every cell of the array, together with $A[0][0]=0$.
...
\begin{theorem} For column $k = 1$ of A305347, the recurrence $a(n) = a(n-1) + a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A305347, the recurrence $a(n) = a(n-1) + 8\,a(n-3) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A305347, the recurrence $a(n) = 0$ holds for every $n \ge 13$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 4$ of A305347, the recurrence $a(n) = 0$ holds for every $n > 37$, and fails at $n = 37$.\end{theorem}
...
\begin{theorem} For column $k = 5$ of A305347, the recurrence $a(n) = 0$ holds for every $n > 48$, and fails at $n = 48$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $223$ after trimming, and $S = 52$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 48$; after that point more than $S = 52$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305347}{A305347}, revision 4, last modified May 30 2018.
