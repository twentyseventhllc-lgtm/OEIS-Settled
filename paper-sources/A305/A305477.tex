...
\title{A proof of the conjectured linear recurrence for OEIS A305477}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A305477}{A305477} as

\begin{quote}\itshape Number of nX3 0..1 arrays with every element unequal to 0, 1, 3, 6 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +3*a(n-3) -2*a(n-6) for n>9.\end{quote}
...
\begin{theorem} For A305477, the recurrence $a(n) = a(n-1) + 3\,a(n-3) - 2\,a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,54$. The last nonvanishing one is at $m = 3$, that is at $n = 9$. After it, more than $S = 9$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 33 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 3, 7, 10, 21, 40, 70, 125, 231,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A305477}{A305477}, revision 16, last modified Jun 29 2018.
