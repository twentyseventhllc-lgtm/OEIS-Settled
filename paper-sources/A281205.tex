...
\title{Proofs of the conjectured column and row recurrences for OEIS A281205}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A281205}{A281205} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with no element equal to more than one of its horizontal and antidiagonal neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards\end{quote}
...
k=1: a(n) = a(n-1)\\k=2: a(n) = 6*a(n-1) -11*a(n-2) +6*a(n-3) -a(n-4)\\k=3: a(n) = 4*a(n-1) -4*a(n-2) for n>3\\k=4: a(n) = 2*a(n-1) +3*a(n-2) -4*a(n-3) -6*a(n-4) +2*a(n-5) +4*a(n-6) -a(n-8)\\k=5: a(n) = 4*a(n-1) -4*a(n-2) -2*a(n-4) +4*a(n-5) -a(n-8)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 17 2017). As of revision 4, last modified Jan 17 2017, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} In A281205, column $k = 1$ is identically zero, and the claimed recurrence therefore holds at every index.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A281205, the recurrence $a(n) = 6\,a(n-1) - 11\,a(n-2) + 6\,a(n-3) - a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $48$ states, $27$ after trimming, and $S = 5$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 5$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A281205, the recurrence $a(n) = 4\,a(n-1) - 4\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $150$ states, $57$ after trimming, and $S = 5$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 3$; after that point more than $S = 5$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 3$; the proved threshold is $n > 3$.

\begin{theorem} For column $k = 4$ of A281205, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) - 4\,a(n-3) - 6\,a(n-4) + 2\,a(n-5) + 4\,a(n-6) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $457$ states, $100$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 5$ of A281205, the recurrence $a(n) = 4\,a(n-1) - 4\,a(n-2) - 2\,a(n-4) + 4\,a(n-5) - a(n-8)$ holds for every $n \ge 9$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1365$ states, $174$ after trimming, and $S = 13$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 13$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A281205}{A281205}, revision 4, last modified Jan 17 2017.
