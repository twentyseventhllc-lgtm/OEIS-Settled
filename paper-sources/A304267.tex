...
\title{A proof of the conjectured linear recurrence for OEIS A304267}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A304267}{A304267} as

\begin{quote}\itshape Number of nX5 0..1 arrays with every element unequal to 0, 1, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
The construction gives $1056$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $34$; neither removal changes any count.
...
\begin{theorem} For A304267, the recurrence $a(n) = a(n-1) + a(n-2) + 10\,a(n-3) + 4\,a(n-4) - 2\,a(n-5) - 12\,a(n-6) - 5\,a(n-7) - 4\,a(n-8) + 4\,a(n-9)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,53$. The last nonvanishing one is at $m = 1$, that is at $n = 10$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 8, 7, 19, 50, 146, 404, 1074, 2990,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A304267}{A304267}, revision 4, last modified May 09 2018.
