...
k=1: a(n) = a(n-1)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Dec 11 2017). As of revision 4, last modified Dec 11 2017, the entry records no proof of any of them and links to none. This note proves the 1 column recurrence listed above.
...
Let $q = 2$ and let $A$ be an $n \times 1$ array with entries in $\{0,1,\dots,1\}$.
...
\[c(i,j) \;=\; \#\bigl\{(\delta,\varepsilon)\in\mathcal{N} \;:\; 0\le i+\delta<n,\ 0\le j+\varepsilon<1,\ A[i+\delta][j+\varepsilon] = 1\bigr\}.\]
