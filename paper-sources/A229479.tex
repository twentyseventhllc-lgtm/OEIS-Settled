...
\title{Proofs of the conjectured column and row recurrences for OEIS A229479}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A229479}{A229479} as

\begin{quote}\itshape T(n,k) = number of defective 4-colorings of an n X k 0..3 array connected horizontally, antidiagonally and vertically with exactly one mistake, and colors introduced in row-major 0..3 order\end{quote}
...
k=1: a(n) = 8*a(n-1) - 22*a(n-2) + 24*a(n-3) - 9*a(n-4) for n > 6.\\k=2: a(n) = 8*a(n-1) - 16*a(n-2) for n > 4.\\k=3: a(n) = 14*a(n-1) - 65*a(n-2) + 112*a(n-3) - 64*a(n-4) for n > 5.\\k=4: [order 8] for n > 9.\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Sep 24 2013). As of revision 7, last modified Apr 27 2021, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
Let $A$ be an $n' \times 4$ array of colours from $\{0,\dots,3\}$ with $n' = n$ rows.
...
\begin{theorem} For column $k = 1$ of A229479, the recurrence $a(n) = 8\,a(n-1) - 22\,a(n-2) + 24\,a(n-3) - 9\,a(n-4)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A229479, the recurrence $a(n) = 8\,a(n-1) - 16\,a(n-2)$ holds for every $n > 4$, and fails at $n = 4$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $73$ states, $44$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 4$; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 4$; the proved threshold is $n > 4$.

\begin{theorem} For column $k = 3$ of A229479, the recurrence $a(n) = 14\,a(n-1) - 65\,a(n-2) + 112\,a(n-3) - 64\,a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $210$ states, $119$ after trimming, and $S = 8$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 5$; after that point more than $S = 8$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 5$; the proved threshold is $n > 5$.

\begin{theorem} For column $k = 4$ of A229479, the recurrence $a(n) = 0$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $672$ states, $357$ after trimming, and $S = 18$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 9$; after that point more than $S = 18$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A229479}{A229479}, revision 7, last modified Apr 27 2021.
