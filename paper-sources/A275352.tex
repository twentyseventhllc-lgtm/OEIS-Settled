...
\title{Proofs of the conjectured column and row recurrences for OEIS A275352}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A275352}{A275352} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with no element equal to any value at offset (-2,-1) (-2,1) or (-1,-2) and new values introduced in order 0..2\end{quote}
...
k=1: a(n) = 4*a(n-1) -3*a(n-2)\\k=2: a(n) = 4*a(n-1) for n>3\\k=3: [order 17] for n>20\end{quote}

\begin{quote}\ttfamily Empirical for row n:\\
n=1: a(n) = 4*a(n-1) -3*a(n-2)\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jul 24 2016). As of revision 4, last modified Jul 24 2016, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences and the 1 row recurrence listed above.
...
An offset and its negative say the same thing, so the offsets are normalised here to point upwards or, in the same row, leftwards: (-2,-1), (-2,1), (-1,-2).
...
\begin{theorem} For column $k = 1$ of A275352, the recurrence $a(n) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A275352, the recurrence $a(n) = 4\,a(n-1)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $100$ states, $100$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 3$; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 3$; the proved threshold is $n > 3$.

\begin{theorem} For column $k = 3$ of A275352, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $510$ states, $510$ after trimming, and $S = 66$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 20$; after that point more than $S = 66$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 20$; the proved threshold is $n > 20$.

\begin{theorem} For row $n = 1$ of A275352, the recurrence $a(k) = 4\,a(n-1) - 3\,a(n-2)$ holds for every $k \ge 3$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $1$; the digraph has $16$ states, $16$ after trimming, and $S = 3$ blocks after refinement. The model reproduces every published value of row $n = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 3$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item 3 lines: model does not reproduce the published table
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A275352}{A275352}, revision 4, last modified Jul 24 2016.
