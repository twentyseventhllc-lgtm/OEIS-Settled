...
\title{Proofs of the conjectured column and row recurrences for OEIS A303800}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A303800}{A303800} as

\begin{quote}\itshape T(n,k) = Number of n X k 0..1 arrays with every element unequal to 0, 1, 2 or 4 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +8*a(n-3) -8*a(n-4) -8*a(n-5) for n>6\\k=3: a(n) = a(n-1) +a(n-2) +2*a(n-3) -a(n-5) -2*a(n-6) -2*a(n-7) for n>10\\k=4: [order 28] for n>32\\k=5: [order 18] for n>24\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 30 2018). As of revision 6, last modified May 31 2023, the entry records no proof of any of them and links to none. This note proves the 5 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A303800, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A303800, the recurrence $a(n) = 2\,a(n-1) + 8\,a(n-3) - 8\,a(n-4) - 8\,a(n-5)$ holds for every $n > 6$, and fails at $n = 6$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A303800, the recurrence $a(n) = a(n-1) + a(n-2) + 2\,a(n-3) - a(n-5) - 2\,a(n-6) - 2\,a(n-7)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $40$ after trimming, and $S = 11$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 10$; after that point more than $S = 11$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 10$; the proved threshold is $n > 10$.

\begin{theorem} For column $k = 4$ of A303800, the recurrence $a(n) = 0$ holds for every $n > 32$, and fails at $n = 32$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $141$ after trimming, and $S = 33$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 32$; after that point more than $S = 33$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 32$; the proved threshold is $n > 32$.

\begin{theorem} For column $k = 5$ of A303800, the recurrence $a(n) = 0$ holds for every $n > 24$, and fails at $n = 24$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $232$ after trimming, and $S = 34$ blocks after refinement. The model reproduces every published value of column $k = 5$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 24$; after that point more than $S = 34$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A303800}{A303800}, revision 6, last modified May 31 2023.
