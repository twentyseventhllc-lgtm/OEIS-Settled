...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 2*a(n-1) +8*a(n-3) -8*a(n-4) -8*a(n-5) for n>6\\k=3: a(n) = a(n-1) +a(n-2) +2*a(n-3) -a(n-5) -2*a(n-6) -2*a(n-7) for n>10\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Apr 30 2018). As of revision 6, last modified May 31 2023, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
