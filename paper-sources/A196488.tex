...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A196488}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A196488}{A196488} as

\begin{quote}\itshape Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,3,0,1,4 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +a(n-2) +6*a(n-3) +10*a(n-4) +15*a(n-5) +9*a(n-6) +4*a(n-7) +a(n-8).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 2*x + 5*x\textasciicircum{}2 + 5*x\textasciicircum{}3 + 9*x\textasciicircum{}4 + 8*x\textasciicircum{}5 + 3*x\textasciicircum{}6 + x\textasciicircum{}7) / (1 - x - x\textasciicircum{}2 - 6*x\textasciicircum{}3 - 10*x\textasciicircum{}4 - 15*x\textasciicircum{}5 - 9*x\textasciicircum{}6 - 4*x\textasciicircum{}7 - x\textasciicircum{}8). - Colin Barker, May 09 2018\end{quote}
...
\begin{multline*} a(n) = a(n-1) + a(n-2) + 6\,a(n-3) + 10\,a(n-4) + 15\,a(n-5) + 9\,a(n-6) + \\ 4\,a(n-7) + a(n-8). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 2x^{2} + 5x^{3} + 5x^{4} + 9x^{5} + 8x^{6} + 3x^{7} + x^{8}}{1 - x - x^{2} - 6x^{3} - 10x^{4} - 15x^{5} - 9x^{6} - 4x^{7} - x^{8}}.\]
...
The entry lists, for each value $x$, a target value $\tau(x)$: \tau(0) = 2, \tau(1) = 3, \tau(2) = 0, \tau(3) = 1, \tau(4) = 4. The condition is that for every cell,
...
The construction gives $15750$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $72$; neither removal changes any count.
...
\begin{theorem} For A196488, the recurrence $a(n) = a(n-1) + a(n-2) + 6\,a(n-3) + 10\,a(n-4) + 15\,a(n-5) + 9\,a(n-6) + 4\,a(n-7) + a(n-8)$ holds for every $n \ge 9$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A196488, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 2x^{2} + 5x^{3} + 5x^{4} + 9x^{5} + 8x^{6} + 3x^{7} + x^{8}}{1 - x - x^{2} - 6x^{3} - 10x^{4} - 15x^{5} - 9x^{6} - 4x^{7} - x^{8}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 3, 9, 23, 69, 199, 553, 1563,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A196488}{A196488}, revision 8, last modified May 09 2018.
