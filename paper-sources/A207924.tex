...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207924}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207924}{A207924} as

\begin{quote}\itshape Number of n X 4 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 0 and 1 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 3*a(n-2) + 6*a(n-3) - a(n-4) - a(n-6) for n>8.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(9 + 63*x - 45*x\textasciicircum{}2 - 9*x\textasciicircum{}3 - 125*x\textasciicircum{}4 + 17*x\textasciicircum{}5 - 7*x\textasciicircum{}6 + 17*x\textasciicircum{}7) / ((1 + x + x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - 3*x - x\textasciicircum{}2 - x\textasciicircum{}3)). - Colin Barker, Mar 06 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x + 63x^{2} - 45x^{3} - 9x^{4} - 125x^{5} + 17x^{6} - 7x^{7} + 17x^{8}}{1 - 2x - 3x^{2} - 6x^{3} + x^{4} + x^{6}}.\]
...
\begin{theorem} For A207924, the recurrence $a(n) = 2\,a(n-1) + 3\,a(n-2) + 6\,a(n-3) - a(n-4) - a(n-6)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,44$. The last nonvanishing one is at $m = 2$, that is at $n = 8$. After it, more than $S = 9$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 8$. The theorem is at least as strong.

\begin{theorem} For A207924, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x + 63x^{2} - 45x^{3} - 9x^{4} - 125x^{5} + 17x^{6} - 7x^{7} + 17x^{8}}{1 - 2x - 3x^{2} - 6x^{3} + x^{4} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 81, 144, 576, 1936, 6400, 21904, 73984,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207924}{A207924}, revision 8, last modified Mar 06 2018.
