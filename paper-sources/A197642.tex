...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A197642}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A197642}{A197642} as

\begin{quote}\itshape Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 4,2,2,0,2 for x=0,1,2,3,4\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) +a(n-2) +a(n-3) +14*a(n-4) -9*a(n-5) -8*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 + 6*x\textasciicircum{}2 + 6*x\textasciicircum{}3 - 23*x\textasciicircum{}4 - 8*x\textasciicircum{}5) / ((1 - x)*(1 - x - 2*x\textasciicircum{}2 - 3*x\textasciicircum{}3 - 17*x\textasciicircum{}4 - 8*x\textasciicircum{}5)). - Colin Barker, May 14 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x + 6x^{3} + 6x^{4} - 23x^{5} - 8x^{6}}{1 - 2x - x^{2} - x^{3} - 14x^{4} + 9x^{5} + 8x^{6}}.\]
...
\begin{theorem} For A197642, the recurrence $a(n) = 2\,a(n-1) + a(n-2) + a(n-3) + 14\,a(n-4) - 9\,a(n-5) - 8\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A197642, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x + 6x^{3} + 6x^{4} - 23x^{5} - 8x^{6}}{1 - 2x - x^{2} - x^{3} - 14x^{4} + 9x^{5} + 8x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 27 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 2, 11, 31, 66, 185, 595, 1760,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A197642}{A197642}, revision 8, last modified May 14 2018.
