...
\title{Proofs of the conjectured column and row recurrences for OEIS A299345}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299345}{A299345} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..1 arrays with every element equal to 0, 1, 2, 3, 5 or 8 king-move adjacent elements, with upper left element zero\end{quote}
...
k=1: a(n) = 2*a(n-1)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +2*a(n-3) -6*a(n-4) -4*a(n-5)\\k=3: [order 16] for n>18\\k=4: [order 64] for n>65\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 07 2018). As of revision 4, last modified Feb 07 2018, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A299345, the recurrence $a(n) = 2\,a(n-1)$ holds for every $n \ge 2$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A299345, the recurrence $a(n) = 4\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - 6\,a(n-4) - 4\,a(n-5)$ holds for every $n \ge 6$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A299345, the recurrence $a(n) = 0$ holds for every $n > 18$, and fails at $n = 18$.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $72$ states, $68$ after trimming, and $S = 20$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 18$; after that point more than $S = 20$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

The entry states this line for $n > 18$; the proved threshold is $n > 18$.

\begin{theorem} For column $k = 4$ of A299345, the recurrence $a(n) = 0$ holds for every $n > 65$, and fails at $n = 65$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $272$ states, $263$ after trimming, and $S = 67$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 65$; after that point more than $S = 67$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299345}{A299345}, revision 4, last modified Feb 07 2018.
