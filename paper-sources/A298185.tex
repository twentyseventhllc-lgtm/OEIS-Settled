...
\title{A proof of the conjectured linear recurrence for OEIS A298185}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A298185}{A298185} as

\begin{quote}\itshape Number of nX6 0..1 arrays with every element equal to 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) -2*a(n-2) -a(n-3) +2*a(n-4) -2*a(n-5) +a(n-7) for n>8\end{quote}
...
The construction gives $4160$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $20$; neither removal changes any count.
...
\begin{theorem} For A298185, the recurrence $a(n) = 3\,a(n-1) - 2\,a(n-2) - a(n-3) + 2\,a(n-4) - 2\,a(n-5) + a(n-7)$ holds for every $n > 8$, and fails at $n = 8$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,60$. The last nonvanishing one is at $m = 1$, that is at $n = 8$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 39 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 0, 5, 5, 6, 7, 10, 14, 20,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A298185}{A298185}, revision 4, last modified Jan 14 2018.
