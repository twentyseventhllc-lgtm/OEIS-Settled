...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207847}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207847}{A207847} as

\begin{quote}\itshape Number of 4 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 1 1 1 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 6*a(n-2) + a(n-3) - a(n-4) for n>5.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 10*x*(1 + 9*x + 8*x\textasciicircum{}2 - 7*x\textasciicircum{}3 + x\textasciicircum{}4) / (1 - x - 6*x\textasciicircum{}2 - x\textasciicircum{}3 + x\textasciicircum{}4). - Colin Barker, Jun 25 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{10x + 90x^{2} + 80x^{3} - 70x^{4} + 10x^{5}}{1 - x - 6x^{2} - x^{3} + x^{4}}.\]
...
\begin{theorem} For A207847, the recurrence $a(n) = a(n-1) + 6\,a(n-2) + a(n-3) - a(n-4)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,40$. The last nonvanishing one is at $m = 1$, that is at $n = 5$. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 5$. The theorem is at least as strong.

\begin{theorem} For A207847, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{10x + 90x^{2} + 80x^{3} - 70x^{4} + 10x^{5}}{1 - x - 6x^{2} - x^{3} + x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 10, 100, 240, 780, 2320, 7140, 21600, 65980,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207847}{A207847}, revision 8, last modified Jun 25 2018.
