...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A207718}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A207718}{A207718} as

\begin{quote}\itshape Number of 3 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 0 vertically\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + 7*a(n-2) - a(n-3) - 4*a(n-4) + a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 6*x*(1 + 4*x - 4*x\textasciicircum{}3 + x\textasciicircum{}4) / ((1 + x)*(1 - 4*x + x\textasciicircum{}2)*(1 + x - x\textasciicircum{}2)). - Colin Barker, Mar 05 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{6x + 24x^{2} - 24x^{4} + 6x^{5}}{1 - 2x - 7x^{2} + x^{3} + 4x^{4} - x^{5}}.\]
...
\begin{theorem} For A207718, the recurrence $a(n) = 2\,a(n-1) + 7\,a(n-2) - a(n-3) - 4\,a(n-4) + a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A207718, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{6x + 24x^{2} - 24x^{4} + 6x^{5}}{1 - 2x - 7x^{2} + x^{3} + 4x^{4} - x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 23 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 6, 36, 114, 450, 1644, 6186, 23010, 85992,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A207718}{A207718}, revision 8, last modified Mar 05 2018.
