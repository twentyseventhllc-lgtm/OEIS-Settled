...
\title{Proofs of the conjectured column and row recurrences for OEIS A263040}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A263040}{A263040} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each row and column not divisible by 5, read as a binary number with top and left being the most significant bits\end{quote}
...
k=1: [linear recurrence of order 30]\\k=2: [order 31]\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Oct 08 2015). As of revision 4, last modified Oct 08 2015, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A263040, the recurrence $a(n) = 0$ holds for every $n \ge 31$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $125$ states, $125$ after trimming, and $S = 75$ blocks after refinement. The model reproduces every published value of column $k = 1$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 75$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 2$ of A263040, the recurrence $a(n) = 0$ holds for every $n \ge 32$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $625$ states, $625$ after trimming, and $S = 120$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 120$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A263040}{A263040}, revision 4, last modified Oct 08 2015.
