...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A231390}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A231390}{A231390} as

\begin{quote}\itshape Number of (n+1) X (2+1) 0..2 arrays with no element unequal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 2*a(n-1) + a(n-2) - 2*a(n-3) for n>5.\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(7 - 6*x - 8*x\textasciicircum{}2 - 4*x\textasciicircum{}3 - 8*x\textasciicircum{}4) / ((1 - x)*(1 + x)*(1 - 2*x)). - Colin Barker, Sep 28 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{7x - 6x^{2} - 8x^{3} - 4x^{4} - 8x^{5}}{1 - 2x - x^{2} + 2x^{3}}.\]
...
The reachable part of the digraph has $660$ states, $31$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 8$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of that $8\times8$ matrix. The bound is computed, not assumed.
...
\begin{theorem} For A231390, the recurrence $a(n) = 2\,a(n-1) + a(n-2) - 2\,a(n-3)$ holds for every $n > 5$, and fails at $n = 5$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,50$. The last nonvanishing one is at $m = 2$, that is at $n = 5$. After it, more than $S = 8$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 5$. The theorem is at least as strong.

\begin{theorem} For A231390, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{7x - 6x^{2} - 8x^{3} - 4x^{4} - 8x^{5}}{1 - 2x - x^{2} + 2x^{3}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 33 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 7, 8, 15, 20, 31, 52, 95, 180,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A231390}{A231390}, revision 8, last modified Sep 28 2018.
