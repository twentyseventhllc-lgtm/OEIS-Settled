...
\title{A proof of the conjectured linear recurrence for OEIS A302511}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A302511}{A302511} as

\begin{quote}\itshape Number of nX4 0..1 arrays with every element equal to 0, 1, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the line:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +2*a(n-3) +2*a(n-4) -a(n-6) -a(n-7) for n>10\end{quote}
...
\begin{theorem} For A302511, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + 2\,a(n-4) - a(n-6) - a(n-7)$ holds for every $n > 10$, and fails at $n = 10$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,58$. The last nonvanishing one is at $m = 3$, that is at $n = 10$. After it, more than $S = 12$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 33 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 5, 5, 15, 28, 48, 89, 163, 297,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A302511}{A302511}, revision 4, last modified Apr 09 2018.
