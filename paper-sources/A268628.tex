...
\title{Proofs of the conjectured column and row recurrences for OEIS A268628}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A268628}{A268628} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..2 arrays with some element plus some horizontally or vertically adjacent neighbor totalling two no more than once\end{quote}
...
k=1: a(n) = 4*a(n-1) -4*a(n-2)\\k=2: a(n) = 6*a(n-1) -9*a(n-2) for n>3\\k=3: a(n) = 10*a(n-1) -29*a(n-2) +20*a(n-3) -4*a(n-4)\\k=4: [order 6] for n>7\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 09 2016). As of revision 4, last modified Feb 09 2016, the entry records no proof of any of them and links to none. This note proves the 4 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A268628, the recurrence $a(n) = 4\,a(n-1) - 4\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A268628, the recurrence $a(n) = 6\,a(n-1) - 9\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A268628, the recurrence $a(n) = 10\,a(n-1) - 29\,a(n-2) + 20\,a(n-3) - 4\,a(n-4)$ holds for every $n \ge 5$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $64$ states, $37$ after trimming, and $S = 6$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 6$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 4$ of A268628, the recurrence $a(n) = 0$ holds for every $n > 7$, and fails at $n = 7$.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $166$ states, $85$ after trimming, and $S = 9$ blocks after refinement. The model reproduces every published value of column $k = 4$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 7$; after that point more than $S = 9$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A268628}{A268628}, revision 4, last modified Feb 09 2016.
