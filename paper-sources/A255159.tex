...
\title{Proofs of the conjectured column and row recurrences for OEIS A255159}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A255159}{A255159} as

\begin{quote}\itshape T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 1 and no row sum 0 or 1 and no column sum 0 or 1\end{quote}
...
k=1: a(n) = 2*a(n-1) -a(n-2) +3*a(n-3) +a(n-4) +2*a(n-5) +2*a(n-6)\\k=2: [order 11]\\k=3: [order 18] for n>20\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 15 2015). As of revision 6, last modified Jun 02 2025, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A255159, the recurrence $a(n) = 2\,a(n-1) - a(n-2) + 3\,a(n-3) + a(n-4) + 2\,a(n-5) + 2\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A255159, the recurrence $a(n) = 0$ holds for every $n \ge 12$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 3$ of A255159, the recurrence $a(n) = 0$ holds for every $n > 20$, and fails at $n = 20$.\end{theorem}

{\itshape Proof.} The width is $5$; the digraph has $1056$ states, $1056$ after trimming, and $S = 25$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 20$; after that point more than $S = 25$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A255159}{A255159}, revision 6, last modified Jun 02 2025.
