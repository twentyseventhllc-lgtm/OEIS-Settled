...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A125445}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A125445}{A125445} as

\begin{quote}\itshape Number of base 10 circular n-digit numbers with adjacent digits differing by 8 or less\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily G.f.: (1 - x\textasciicircum{}2 - 16*x\textasciicircum{}3) / ((1 - x)*(1 - 9*x - 8*x\textasciicircum{}2)).\end{quote}

\begin{quote}\ttfamily a(n) = 10*a(n-1) - a(n-2) - 8*a(n-3) for n>3.\end{quote}
...
\[\sum_{n \ge 0} a(n)\,x^n \;=\; \frac{1 - x^{2} - 16x^{3}}{1 - 10x + x^{2} + 8x^{3}}.\]

Written out, the claim is

\[a(n) = 10\,a(n-1) - a(n-2) - 8\,a(n-3).\]
...
\[|d_i - d_{i+1}| \le 8\quad (1 \le i < n), \qquad |d_n - d_1| \le 8.\]
...
Let $G$ be the digraph on the vertex set $\{0,\dots,9\}$ with an edge from $i$ to $j$ exactly when $|i-j| \le 8$, and let $M$ be its adjacency matrix, a $10 \times 10$ symmetric 0--1 band matrix with $M_{ij} = 1$ iff $|i-j| \le 8$.
...
\begin{theorem} For A125445, the generating function is exactly $\sum_{n\ge 0} a(n)x^n = \frac{1 - x^{2} - 16x^{3}}{1 - 10x + x^{2} + 8x^{3}}$.\end{theorem}
...
\begin{theorem} For A125445, the recurrence $a(n) = 10\,a(n-1) - a(n-2) - 8\,a(n-3)$ holds for every $n \ge 4$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,38$. Every one of them vanished. After it, more than $S = 10$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$
...
\item {\bfseries The published terms.} The walk count reproduces all 19 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (0) rather than fitted. The first of them are 1, 10, 98, 946, 9282, 91090, 894050, 8775154,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A125445}{A125445}, revision 17, last modified Aug 12 2026.
