...
\title{Proofs of the conjectured column and row recurrences for OEIS A234890}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A234890}{A234890} as

\begin{quote}\itshape T(n,k) is the number of (n+1) X (k+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings)\end{quote}
...
k=1: a(n) = 14*a(n-2) -37*a(n-4) +21*a(n-6).\\k=2: [order 27].\\k=3: [order 46].\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Jan 01 2014). As of revision 6, last modified Jun 20 2022, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A234890, the recurrence $a(n) = 14\,a(n-2) - 37\,a(n-4) + 21\,a(n-6)$ holds for every $n \ge 7$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A234890, the recurrence $a(n) = 0$ holds for every $n \ge 28$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $3$; the digraph has $216$ states, $150$ after trimming, and $S = 31$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 31$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$

\begin{theorem} For column $k = 3$ of A234890, the recurrence $a(n) = 0$ holds for every $n \ge 47$, at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The width is $4$; the digraph has $1296$ states, $750$ after trimming, and $S = 57$ blocks after refinement. The model reproduces every published value of column $k = 3$ --- and of the rest of the triangle. The residuals were computed exactly and all of them vanish; after that point more than $S = 57$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A234890}{A234890}, revision 6, last modified Jun 20 2022.
