...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A259736}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A259736}{A259736} as

\begin{quote}\itshape Number of (n+2) X (2+2) 0..1 arrays with each 3 X 3 subblock having clockwise perimeter pattern 00010101 or 01010101\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 2*a(n-3) + a(n-4).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: 2*x*(11 + 7*x + 19*x\textasciicircum{}2 + 9*x\textasciicircum{}3) / (1 - x - 2*x\textasciicircum{}3 - x\textasciicircum{}4). - Colin Barker, Dec 26 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{22x + 14x^{2} + 38x^{3} + 18x^{4}}{1 - x - 2x^{3} - x^{4}}.\]
...
\begin{theorem} For A259736, the recurrence $a(n) = a(n-1) + 2\,a(n-3) + a(n-4)$ holds for every $n \ge 5$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A259736, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{22x + 14x^{2} + 38x^{3} + 18x^{4}}{1 - x - 2x^{3} - x^{4}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 31 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 22, 36, 74, 136, 230, 414, 760, 1356,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A259736}{A259736}, revision 8, last modified Dec 26 2018.
