...
\title{A proof of the conjectured recurrence and generating function for OEIS A317767}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A317767}{A317767} as

\begin{quote}\itshape Number of nX2 0..1 arrays with every element unequal to 0, 1 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) +2*a(n-3) -a(n-6)\end{quote}

\begin{quote}\ttfamily Empirical: G.f.: -x*(x-1)*(x\textasciicircum{}4+3*x\textasciicircum{}3+4*x\textasciicircum{}2+3*x+2)/(1-x-2*x\textasciicircum{}3+x\textasciicircum{}6) . - R. J. Mathar, May 09 2023\end{quote}

That line carries no separate signature; the entry itself is by R. H. Hardin (Aug 06 2018).

The line quoted ending in that signature is contributed by R. J. Mathar on May 09 2023.

The word {\itshape Empirical} --- equivalently {\itshape Conjecture} --- is the entry's own: the statement is recorded as fitted to the published terms, and no proof is given there. As of revision 6, last modified May 09 2023, the entry records no proof and links to none, so the statement is open. This note settles it.
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x + x^{2} + x^{3} - x^{4} - 2x^{5} - x^{6}}{1 - x - 2x^{3} + x^{6}}.\]
...
\begin{theorem} For A317767, the recurrence $a(n) = a(n-1) + 2\,a(n-3) - a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,55$. Every one of them vanished. After it, more than $S = 6$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

\begin{theorem} For A317767, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x + x^{2} + x^{3} - x^{4} - 2x^{5} - x^{6}}{1 - x - 2x^{3} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 37 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 3, 4, 7, 11, 18, 30, 49,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A317767}{A317767}, revision 6, last modified May 09 2023.
