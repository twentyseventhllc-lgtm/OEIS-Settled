...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A296733}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A296733}{A296733} as

\begin{quote}\itshape Number of n X 2 0..1 arrays with each 1 adjacent to 1, 2 or 4 king-move neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) - a(n-2) + 8*a(n-3) - 12*a(n-4) + 2*a(n-5) - 12*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(2 + x + 2*x\textasciicircum{}2)*(1 + 2*x - 2*x\textasciicircum{}2 - 6*x\textasciicircum{}3) / (1 - 3*x + x\textasciicircum{}2 - 8*x\textasciicircum{}3 + 12*x\textasciicircum{}4 - 2*x\textasciicircum{}5 + 12*x\textasciicircum{}6). - Colin Barker, Feb 25 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{2x + 5x^{2} - 10x^{4} - 10x^{5} - 12x^{6}}{1 - 3x + x^{2} - 8x^{3} + 12x^{4} - 2x^{5} + 12x^{6}}.\]
...
\begin{theorem} For A296733, the recurrence $a(n) = 3\,a(n-1) - a(n-2) + 8\,a(n-3) - 12\,a(n-4) + 2\,a(n-5) - 12\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A296733, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{2x + 5x^{2} - 10x^{4} - 10x^{5} - 12x^{6}}{1 - 3x + x^{2} - 8x^{3} + 12x^{4} - 2x^{5} + 12x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 2, 11, 31, 88, 287, 881, 2686, 8347,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A296733}{A296733}, revision 7, last modified Feb 25 2019.
