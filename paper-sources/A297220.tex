...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A297220}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A297220}{A297220} as

\begin{quote}\itshape Number of n X 4 0..1 arrays with every 1 horizontally or antidiagonally adjacent to 1 neighboring 1s\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = a(n-1) + 6*a(n-2) + 11*a(n-3) + 6*a(n-4) + a(n-5).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(2 + x)\textasciicircum{}2*(1 + 2*x + 2*x\textasciicircum{}2) / (1 - x - 6*x\textasciicircum{}2 - 11*x\textasciicircum{}3 - 6*x\textasciicircum{}4 - x\textasciicircum{}5). - Colin Barker, Feb 26 2019\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{4x + 12x^{2} + 17x^{3} + 10x^{4} + 2x^{5}}{1 - x - 6x^{2} - 11x^{3} - 6x^{4} - x^{5}}.\]
...
\begin{theorem} For A297220, the recurrence $a(n) = a(n-1) + 6\,a(n-2) + 11\,a(n-3) + 6\,a(n-4) + a(n-5)$ holds for every $n \ge 6$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A297220, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{4x + 12x^{2} + 17x^{3} + 10x^{4} + 2x^{5}}{1 - x - 6x^{2} - 11x^{3} - 6x^{4} - x^{5}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 24 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 4, 16, 57, 207, 751, 2720, 9861, 35741,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A297220}{A297220}, revision 8, last modified Feb 26 2019.
