...
\title{A proof of the conjectured recurrence and generating function for OEIS A299593}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A299593}{A299593} as

\begin{quote}\itshape Number of n X 6 0..1 arrays with every element unequal to 0, 1 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero\end{quote}
...
\begin{quote}\ttfamily Empirical g.f.: x*(13 + 2*x + 7*x\textasciicircum{}2 + 32*x\textasciicircum{}3 + 42*x\textasciicircum{}4 + 92*x\textasciicircum{}5 + 23*x\textasciicircum{}6 - 39*x\textasciicircum{}7 - 29*x\textasciicircum{}8) / ((1 + x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - x - 2*x\textasciicircum{}2 - x\textasciicircum{}3)). - Colin Barker, Feb 15 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{13x + 2x^{2} + 7x^{3} + 32x^{4} + 42x^{5} + 92x^{6} + 23x^{7} - 39x^{8} - 29x^{9}}{1 - x - x^{2} - 3x^{3} - x^{4} + x^{5} + x^{6}}.\]
...
The construction gives $4160$ states. Removing states unreachable from a start state, and states from which no accepting state can be reached, leaves $53$; neither removal changes any count.
...
Here the refinement leaves $S = 25$ blocks. Writing $L$ for the $S\times S$ quotient matrix, $\tilde w$ for the start weights summed over each block and $\tilde f$ for the acceptance indicator,
...
For scale: at $n = 8$ there are $2^{48}$ arrays, about $10^{14}$, against $25$ blocks here. Direct enumeration settles nothing at that size; the walk count does.
...
\begin{theorem} For A299593, the recurrence $a(n) = a(n-1) + a(n-2) + 3\,a(n-3) + a(n-4) - a(n-5) - a(n-6)$ holds for every $n > 9$, and fails at $n = 9$.\end{theorem}

{\itshape Proof.} The residuals $u_m$ were computed exactly for $m = 1,\dots,65$. The last nonvanishing one is at $m = 3$, that is at $n = 9$. After it, more than $S = 25$ consecutive residuals vanish, so by Section 5 every later residual vanishes as well. $\square$

The entry claims the recurrence for $n > 9$. The theorem is at least as strong.

\begin{theorem} For A299593, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{13x + 2x^{2} + 7x^{3} + 32x^{4} + 42x^{5} + 92x^{6} + 23x^{7} - 39x^{8} - 29x^{9}}{1 - x - x^{2} - 3x^{3} - x^{4} + x^{5} + x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 28 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 13, 15, 35, 121, 256, 576, 1225, 2601,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A299593}{A299593}, revision 8, last modified Feb 15 2018.
