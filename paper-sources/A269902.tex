...
\title{Proofs of the conjectured column and row recurrences for OEIS A269902}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A269902}{A269902} as

\begin{quote}\itshape T(n,k)=Number of nXk 0..6 arrays with some element plus some horizontally or vertically adjacent neighbor totalling six exactly once\end{quote}
...
k=1: a(n) = 12*a(n-1) -36*a(n-2)\\k=2: a(n) = 62*a(n-1) -961*a(n-2) for n>3\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Mar 07 2016). As of revision 4, last modified Mar 07 2016, the entry records no proof of any of them and links to none. This note proves the 2 column recurrences listed above.
...
\begin{theorem} For column $k = 1$ of A269902, the recurrence $a(n) = 12\,a(n-1) - 36\,a(n-2)$ holds for every $n \ge 3$, at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For column $k = 2$ of A269902, the recurrence $a(n) = 62\,a(n-1) - 961\,a(n-2)$ holds for every $n > 3$, and fails at $n = 3$.\end{theorem}

{\itshape Proof.} The width is $2$; the digraph has $141$ states, $92$ after trimming, and $S = 4$ blocks after refinement. The model reproduces every published value of column $k = 2$ --- and of the rest of the triangle. The residuals were computed exactly and the last nonvanishing one is at $n = 3$; after that point more than $S = 4$ consecutive residuals vanish, so by Section 3 every later one vanishes too. $\square$
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A269902}{A269902}, revision 4, last modified Mar 07 2016.
