...
k=1: a(n) = a(n-1) +a(n-2)\\k=2: a(n) = 4*a(n-1) -2*a(n-2) +4*a(n-3) for n>4\\k=3: a(n) = 6*a(n-1) +8*a(n-2) +20*a(n-3) +25*a(n-4) +11*a(n-5) +2*a(n-6) for n>7\end{quote}

The lines carry no separate signature; the entry itself is by R. H. Hardin (Feb 17 2018). As of revision 4, last modified Feb 17 2018, the entry records no proof of any of them and links to none. This note proves the 3 column recurrences listed above.
