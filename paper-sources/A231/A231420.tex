...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A231420}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A231420}{A231420} as

\begin{quote}\itshape Number of (1+1) X (n+1) 0..2 arrays with no element equal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 8*a(n-1) + 4*a(n-2) - 58*a(n-3) - 24*a(n-4) + 40*a(n-5) - 16*a(n-6).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(9 - x - 90*x\textasciicircum{}2 - 36*x\textasciicircum{}3 + 60*x\textasciicircum{}4 - 24*x\textasciicircum{}5) / (1 - 8*x - 4*x\textasciicircum{}2 + 58*x\textasciicircum{}3 + 24*x\textasciicircum{}4 - 40*x\textasciicircum{}5 + 16*x\textasciicircum{}6). - Colin Barker, Sep 28 2018\end{quote}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{9x - x^{2} - 90x^{3} - 36x^{4} + 60x^{5} - 24x^{6}}{1 - 8x - 4x^{2} + 58x^{3} + 24x^{4} - 40x^{5} + 16x^{6}}.\]
...
The reachable part of the digraph has $197$ states, $100$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 9$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of that $9\times9$ matrix. The bound is computed, not assumed.
...
\begin{theorem} For A231420, the recurrence $a(n) = 8\,a(n-1) + 4\,a(n-2) - 58\,a(n-3) - 24\,a(n-4) + 40\,a(n-5) - 16\,a(n-6)$ holds for every $n \ge 7$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A231420, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{9x - x^{2} - 90x^{3} - 36x^{4} + 60x^{5} - 24x^{6}}{1 - 8x - 4x^{2} + 58x^{3} + 24x^{4} - 40x^{5} + 16x^{6}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 19 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 9, 71, 514, 3838, 28486, 212060, 1578180, 11748804,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A231420}{A231420}, revision 8, last modified Sep 28 2018.
