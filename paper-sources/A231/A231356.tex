...
\title{A proof of the conjectured linear recurrence and generating function for OEIS A231356}
...
The Online Encyclopedia of Integer Sequences records \href{https://oeis.org/A231356}{A231356} as

\begin{quote}\itshape Number of (n+1) X (1+1) 0..3 arrays with no element unequal to a strict majority of its horizontal, vertical and antidiagonal neighbors, with values 0..3 introduced in row major order\end{quote}

and carries in its Formula section the lines:

\begin{quote}\ttfamily Empirical: a(n) = 3*a(n-1) + 5*a(n-2) - 5*a(n-3) - 24*a(n-4) - 2*a(n-5) + 15*a(n-6) + 9*a(n-7).\end{quote}

\begin{quote}\ttfamily Empirical g.f.: x*(1 - x - 7*x\textasciicircum{}2 - 5*x\textasciicircum{}3 + 11*x\textasciicircum{}4 + 13*x\textasciicircum{}5 + 3*x\textasciicircum{}6) / ((1 - x)*(1 - x - 2*x\textasciicircum{}2 - x\textasciicircum{}3)*(1 - x - 6*x\textasciicircum{}2 - 9*x\textasciicircum{}3)). - Colin Barker, Sep 28 2018\end{quote}
...
\begin{multline*} a(n) = 3\,a(n-1) + 5\,a(n-2) - 5\,a(n-3) - 24\,a(n-4) - 2\,a(n-5) + 15\,a(n-6) + \\ 9\,a(n-7). \end{multline*}
...
\[\sum_{n \ge 1} a(n)\,x^n \;=\; \frac{x - x^{2} - 7x^{3} - 5x^{4} + 11x^{5} + 13x^{6} + 3x^{7}}{1 - 3x - 5x^{2} + 5x^{3} + 24x^{4} + 2x^{5} - 15x^{6} - 9x^{7}}.\]
...
The reachable part of the digraph has $476$ states, $72$ after trimming; the forward identification of states with equal numbers of completions of every length, then the mirror identification on the edges coming in, leaves $S = 9$ blocks, and the count satisfies the linear recurrence given by the characteristic polynomial of that $9\times9$ matrix. The bound is computed, not assumed.
...
\begin{theorem} For A231356, the recurrence $a(n) = 3\,a(n-1) + 5\,a(n-2) - 5\,a(n-3) - 24\,a(n-4) - 2\,a(n-5) + 15\,a(n-6) + 9\,a(n-7)$ holds for every $n \ge 8$ --- at every index at which it can be stated.\end{theorem}
...
\begin{theorem} For A231356, the generating function is exactly $\sum_{n\ge 1} a(n)x^n = \frac{x - x^{2} - 7x^{3} - 5x^{4} + 11x^{5} + 13x^{6} + 3x^{7}}{1 - 3x - 5x^{2} + 5x^{3} + 24x^{4} + 2x^{5} - 15x^{6} - 9x^{7}}$.\end{theorem}
...
\item {\bfseries The published terms.} The walk count reproduces all 26 terms the entry publishes, exactly, in integer arithmetic, with the index taken from the entry's stated offset (1) rather than fitted. The first of them are 1, 2, 4, 12, 33, 102, 329, 1075,~\dots
...
\item OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences}, entry \href{https://oeis.org/A231356}{A231356}, revision 8, last modified Sep 28 2018.
